import { simulateThermalModel, StepObserver, ThermalConnection, ThermalNode, OutdoorBoundary, SimulationOptions } from "../thermal/thermalModel";
import { runMorrisAnalysis, MorrisParameterDefinition, MorrisResult } from "../sensitivity/morris";
...
export function runMonteCarlo(options: MonteCarloOptions): MonteCarloResult {
  ...
  for (let run = 0; run < options.runs; run++) {
    ...
    const metrics = evaluateScenario(scenario);
...
function sampleFromDistribution(distribution: DistributionDefinition, rng: Mulberry32): number {
  switch (distribution.kind) {
    ...
    default:
      throw new Error(Unsupported distribution kind: );
  }
}
...
function runMorris(options: MonteCarloOptions, config: MonteCarloMorrisConfig): MorrisResult {
  ...
  const evaluator = (inputs: Record<string, number>): number => {
    const scenario = options.scenarioBuilder(inputs);
    const metrics = evaluateScenario(scenario);
