...
let cachedBase = null;
function buildDemoReportBaseData() {
    if (cachedBase) {
        return cachedBase;
    }
...
function assertNumericFilled(html, snippet, label) {
...
test("QA · demo · Раздел 5 ОВ/ТС — ключевые поля заполнены", () => {
    ...
    assertContains(html, "6 Проектные решения по отоплению", "Раздел 5 ОВ/ТС");
    assertContains(html, "7 Проектные решения по вентиляции", "Раздел 5 ОВ/ТС");
    assertContains(html, "8 Расчётные тепловые нагрузки", "Раздел 5 ОВ/ТС");
    assertContains(html, "9 Теплотехнические характеристики оболочки", "Раздел 5 ОВ/ТС");
    assertContains(html, "10 Энергетические показатели", "Раздел 5 ОВ/ТС");
    assertContains(html, "12 Вывод", "Раздел 5 ОВ/ТС");
    ...
    assertContains(html, "Полный перечень элементов наружной оболочки приведён в приложении А.", "Раздел 5 ОВ/ТС");
...
test("QA · demo · Расчёт тепловой защиты — таблица kоб с колонкой nt·A/Rприв", () => {
    ...
    assertContains(html, "4 Проверка сопротивления теплопередаче", "Тепловая защита");
...
test("QA · demo · Энергетический паспорт — строгая 9-секционная таблица", () => {
    ...
    if (html.includes("<th class=\"rx-th rx-align-right\">Фактическое значение</th>")) {
        throw new Error("Колонка «Фактическое значение» не должна показываться на проектной стадии.");
    }
    assertContains(html, "не применяется на стадии проектной документации", "Энергопаспорт");
    ...
    if (kobRow && kobRow.factValue !== "—") {
        throw new Error(`Фактическое значение kоб должно быть "—" пока нет обследования, получено: ${kobRow.factValue}`);
    }
});
test("QA · demo · Эксплуатационный паспорт — баннер ЧЕРНОВИК при отсутствии экспл. данных", () => {
    ...
    // Разделы
    assertContains(html, "Паспорт проектных теплотехнических характеристик", "Экспл. паспорт");
    assertContains(html, "1 Общие сведения об объекте", "Экспл. паспорт");
    assertContains(html, "2 Геометрические показатели", "Экспл. паспорт");
    assertContains(html, "3 Конструктивные показатели оболочки", "Экспл. паспорт");
    assertContains(html, "4 Инженерные системы", "Экспл. паспорт");
    assertContains(html, "5 Проектные нагрузки", "Экспл. паспорт");
    assertContains(html, "6 Теплотехнические характеристики", "Экспл. паспорт");
    assertContains(html, "7 Энергетические показатели", "Экспл. паспорт");
    assertContains(html, "8 Рекомендации по эксплуатации", "Экспл. паспорт");
    ...
    if (data.isDraft) {
        throw new Error("Эксплуатационный паспорт ошибочно помечен как ЧЕРНОВИК, хотя сценарий с уставками задан.");
    }
});
test("QA · demo · Краткое инженерное заключение — короткий документ", () => {
    ...
    assertContains(html, "4 Заключение", "Инж. заключение");
    assertContains(html, "5 Риски и замечания", "Инж. заключение");
    assertContains(html, "6 Рекомендации", "Инж. заключение");
    // Документ должен быть компактным: не более ~6 таблиц
    const tableCount = (html.match(/<table /g) ?? []).length;
    if (tableCount > 6) {
        throw new Error(`Краткое инженерное заключение слишком тяжёлое (таблиц ${tableCount} > 6). Должно быть компактным.`);
    }
    assertContains(html, "Статус документа", "Инж. заключение");
});
test("QA · demo · отсутствие '0' как реального расчётного значения", () => {
    ...
    for (const section of calculatedSections) {
        for (const row of section.rows) {
            ...
            if (row.label.includes("Коэффициент рекуперации")) {
                continue;
            }
            if (zeroPattern.test(designTrim)) {
                throw new Error(`${section.title}: поле "${row.label}" выводит "${row.designValue}" как расчётное значение — должно быть "недостаточно данных".`);
            }
        }
    }
});
    ...
    for (const message of data.missingData) {
        for (const forbidden of FORBIDDEN_STRINGS) {
            if (message.includes(forbidden)) {
                throw new Error(`Сообщение «${message}» содержит техническую переменную «${forbidden}» — должно быть человеческое описание.`);
            }
        }
    }
});
