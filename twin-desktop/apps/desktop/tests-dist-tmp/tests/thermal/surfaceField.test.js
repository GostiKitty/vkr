...
function createSurfaceFieldFixture() {
    return {
        levels: [
            {
                id: "level-1",
...
test("upper-level wall patches are warmer near the floor when the level below is heated", () => {
    ...
    const upperWallPatches = result.patches.filter((patch) => patch.levelId === "level-2" && patch.surfaceKind === "wall");
    ...
    const highestRowPatches = upperWallPatches.filter((patch) => patch.row === Math.max(...upperWallPatches.map((entry) => entry.row)));
    const avgLow = lowestRowPatches.reduce((sum, patch) => sum + patch.patchTemperatureC, 0) / lowestRowPatches.length;
    const avgHigh = highestRowPatches.reduce((sum, patch) => sum + patch.patchTemperatureC, 0) / highestRowPatches.length;
