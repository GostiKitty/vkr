...
import { buildDefaultEconomicScenario, calculateDiscountedPayback_years, calculateNpv_RUB, calculateProfitabilityIndex, calculateRetrofitCost, calculateSimplePayback_years, economicDefaults, estimateAnnualEnergySaving_kWh, runEconomicAssessment, } from "../../src/core/economics/index.js";
import { test } from "../testHarness.js";
function cloneValue(value) {
    return JSON.parse(JSON.stringify(value));
}
function collectInvalidNumbers(value, path = "root") {
    ...
    if (value && typeof value === "object") {
        return Object.entries(value).flatMap(([key, entry]) => collectInvalidNumbers(entry, `${path}.${key}`));
    }
    return [];
}
...
test("economics: absent zone does not become scenario leader only due to low cost", () => {
    const report = cloneValue(buildDemoSp50RunResult(sampleBuildingSP50, "economic-test").report);
    report.constructions = report.constructions.map((entry) => entry.constructionType === "window" || entry.constructionType === "lantern"
