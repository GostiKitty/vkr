...
const BRICK_THREE_LAYER_LAYERS = [
...
export const ENVELOPE_PRESETS = {
    ...
    "window-pvc-double-glazed": {
        ...
        sourceNote: "ПВХ 3-кам., СП 4-16Ar-4-14Ar-4И; Uw=1,2 Вт/(м²·К) — типовой паспорт энергоэффективного блока (ГОСТ 30674-99, R≥0,83 м²·K/Вт; базовый 4-10-4-10-4 ≈1,96 Вт/(м²·К)).",
...
export const DEFAULT_ENVELOPE_PRESET_IDS = {
...
export function listEnvelopePresets(kind) {
    return Object.values(ENVELOPE_PRESETS).filter((preset) => preset.kind === kind);
}
export function resolveDefaultPresetId(kind) {
    return DEFAULT_ENVELOPE_PRESET_IDS[kind];
}
export function resolvePresetLayers(preset) {
    return cloneLayers(preset.layers ?? preset.reportLayers ?? []);
}
export function applyEnvelopePresetToWall(wall, presetId) {
...
export function applyEnvelopePresetToWindow(opening, presetId) {
    const preset = getEnvelopePreset(presetId);
...
export function applyEnvelopePresetToDoor(opening, presetId) {
    const preset = getEnvelopePreset(presetId);
...
export function applyEnvelopePresetToRoof(roof, presetId) {
    const preset = getEnvelopePreset(presetId);
...
export function applyEnvelopePresetToFloorSlab(slab, presetId) {
    const preset = getEnvelopePreset(presetId);
...
export function describeEnvelopePreset(presetId) {
    const preset = getEnvelopePreset(presetId);
