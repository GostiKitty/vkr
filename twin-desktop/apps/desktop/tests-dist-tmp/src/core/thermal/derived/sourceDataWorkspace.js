...
function buildEngineeringSection(model, scenarioConfig, scenario, thermalResult) {
    ...
    const diameters = Array.from(new Set(model.pipes
    ...
    const requiredMassFlowKgS = requiredPowerW != null && hydronic.deltaT != null && hydronic.deltaT > 0
        ? calculateRequiredHydronicMassFlow(requiredPowerW, hydronic.deltaT, fluid.cp)
        : null;
    const requiredVolumeFlowM3H = requiredPowerW != null && hydronic.deltaT != null && hydronic.deltaT > 0
    ...
    return {
        ...
        computedFields: [
            ...
            field("engineering.pipe-insulation", "Теплоизоляция труб", model.pipes.length ? `${insulatedPipeCount}/${model.pipes.length}` : null, null, model.pipes.length ? "model" : "missing"),
