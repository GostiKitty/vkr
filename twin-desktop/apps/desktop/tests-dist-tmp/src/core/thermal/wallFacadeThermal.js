...
export function computeWallOpeningAreasM2(openings) {
    let windowAreaM2 = 0;
    let doorAreaM2 = 0;
...
function finitePositive(x, fallback) {
 ...
 */
export function computeWallFacadeConductances(wall, openings, fallbackAssemblyId, conductanceMultiplier) {
    const warnings = [];
    ...
    if (openingRaw > wallAreaM2 + 1e-6) {
        warnings.push(`Площадь проёмов (${openingRaw.toFixed(2)} м²) больше площади стены (${wallAreaM2.toFixed(2)} м²) ` +
    ...
    const weightedU = (opaqueAreaM2 * opaqueU + windowConductance + doorConductance) / denom;
...
export function computeFallbackFacadeConductance_W_K(areaM2, fallbackAssemblyId = DEFAULT_WALL_ASSEMBLY_ID) {
