...
export function generateThermalProtectionHtml(data, options = {}) {
...
${showRcDisclaimer ? `<section><p class="rx-note">${escapeHtml(REPORT_EXPORT_RC_DISCLAIMER)}</p></section>` : ""}
...
${renderBibliography([
        ...
        { id: "3", label: "ГОСТ Р 21.101. Система проектной документации для строительства." },
    ])}
${renderSignatureBlock(data.meta)}
${renderGostStamp(data.meta, DOCUMENT_LABEL)}
`;
    return wrapHtmlDocument(REPORT_EXPORT_TEMPLATE_MARKER["thermal-protection"], REPORT_EXPORT_TITLE["thermal-protection"], body);
}
...
function renderResistanceSection(data) {
    return `
<section id="tp-4">
  <h3 class="rx-section-title">4 Проверка сопротивления теплопередаче</h3>
  ${renderDataTable(null, [
        { label: "Обозначение", width: "8%" },
        { label: "Наименование", width: "16%" },
        { label: "Тип", width: "12%" },
        ...
        { label: "Статус", width: "10%" },
        { label: "Основание Rнорм", width: "13%" },
        { label: "Источник данных", width: "13%" },
        ...
        cells: [
            ...
            row.nt,
            row.status,
            row.requiredResistanceBasis,
            row.sourceData,
        ],
    })))}
</section>`.trim();
}
function renderReducedResistanceSection(data) {
    return `
<section id="tp-5">
  <h3 class="rx-section-title">5 Проверка приведённого сопротивления теплопередаче</h3>
  <p>Проверка приведённого сопротивления выполняется по каждому элементу наружной оболочки и контролируется по значениям Rприв, Rнорм, nt и статусу соответствия.</p>
  ${renderDataTable("Сводная проверка по документу", [
        { label: "Показатель", width: "60%" },
        { label: "Значение", align: "right" },
    ], data.complianceRows.map((row) => ({ cells: [row.label, row.value] })))}
</section>`.trim();
}
function renderKobSection(data) {
    return `
<section id="tp-6">
  <h3 class="rx-section-title">6 Расчёт удельной теплозащитной характеристики kоб</h3>
  ${renderDataTable("Фрагменты оболочки и их вклад в kоб", [
...
function renderEnergyCharacteristicSection(data) {
    return `
<section id="tp-7">
...
function renderConclusionsSection(data) {
    return `
<section id="tp-9">
...
function renderCompositionAppendix(data) {
    return `
<section id="tp-app-a">
...
function renderEnvelopeAppendix(data) {
    return `
<section id="tp-app-b">
  ...
  ${renderDataTable(null, [
        ...
        { label: "Статус", width: "10%" },
        { label: "Основание Rнорм", width: "12%" },
        { label: "Источник данных", width: "12%" },
        { label: "ID модели", width: "10%" },
        { label: "Примечание", width: "12%" },
...
function renderKobAppendix(data) {
    return `
<section id="tp-app-v">
