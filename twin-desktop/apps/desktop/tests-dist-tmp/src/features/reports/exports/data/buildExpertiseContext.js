import { formatDisplayEnum, REPORT_EXPORT_DASH, REPORT_EXPORT_NOT_SET, } from "../helpers";
import { DEFAULT_EXPERTISE_REPORT_INPUTS, } from "../store/expertiseInputs.store";
...
export function buildExpertiseContext(input) {
    ...
    const fields = [
        ...
        resolveField({
            ...
            scenarioValue: passport.operation.mechanicalVentilationEnabled === null
                ? ""
        ...
        resolveField({
            ...
            scenarioValue: passport.operation.heatRecoveryFactor === null
                ? ""
        ...
        resolveField({
            ...
            scenarioValue: input.scenarioConfig ? "сценарий расчёта проекта" : "",
        ...
        resolveField({
            ...
            scenarioValue: passport.operation.internalGainDay_W_m2 !== null
        ...
        resolveField({
            ...
            calculatedValue: energy?.solarGainCharacteristic_W_m3K !== null &&
    ...
    const normalizedFields = fields.map((field) => finalizeField(field, inputs.exportMode));
    const fieldMap = Object.fromEntries(normalizedFields.map((field) => [field.key, field]));
    ...
    const clarificationLines = buildClarificationLines(normalizedFields, inputs.handleMissingDataMode);
...
function buildCompletenessReport(fields) {
    ...
    return {
        ...
        percent: tracked.length > 0 ? Math.round((filledTotal / tracked.length) * 100) : 100,
        criticalMissing: required.filter((field) => !field.isFilled),
        recommendedMissing: recommended.filter((field) => !field.isFilled),
    };
}
function buildClarificationLines(fields, mode) {
    const missing = fields.filter((field) => !field.isFilled && field.importance !== "optional");
    if (!missing.length) {
...
function buildLimitations(inputs) {
    ...
    if (inputs.solarGainsMode !== "manual") {
        lines.push("Солнечные теплопоступления требуют уточнения методики расчёта; значение qrad не подменяется фиктивным числом.");
    }
    return lines;
}
function resolveField(config) {
    const resolved = firstMeaningful([
        { source: "user-input", value: normalizeText(config.userValue) },
        { source: "model", value: normalizeText(config.modelValue) },
        { source: "scenario", value: normalizeText(config.scenarioValue) },
        { source: "calculated", value: normalizeText(config.calculatedValue) },
        { source: "demo-default", value: normalizeText(config.demoValue) },
...
function finalizeField(field, exportMode) {
    const importance = exportMode === "strict-expertise" && STRICT_REQUIRED_KEYS.has(field.key)
        ? "required"
        : field.importance;
    const isFilled = computeFieldFilled(field, exportMode);
    const statusLabel = buildFieldStatusLabel(field, exportMode, isFilled);
    const sourceLabel = SIGNATURE_FIELD_KEYS.has(field.key) &&
...
function computeFieldFilled(field, exportMode) {
    if (field.source === "not-provided") {
        return false;
    }
    if (SIGNATURE_FIELD_KEYS.has(field.key) &&
        field.rawValue === REPORT_EXPORT_DASH) {
        return exportMode === "demo" || exportMode === "vkr-brief";
    }
    return true;
}
function buildFieldStatusLabel(field, exportMode, isFilled) {
    if (SIGNATURE_FIELD_KEYS.has(field.key) &&
        field.rawValue === REPORT_EXPORT_DASH &&
        exportMode === "demo") {
...
function buildInputRegisterRow(field, exportMode) {
    if (SIGNATURE_FIELD_KEYS.has(field.key) &&
        field.rawValue === REPORT_EXPORT_DASH &&
        exportMode === "demo") {
        return {
...
function formatDocumentSignature(field) {
    if (!field) {
...
function buildStatusLabel(source) {
    switch (source) {
        case "demo-default":
...
function exportModeLabel(value) {
    switch (value) {
...
function missingModeLabel(value) {
    switch (value) {
        case "hide":
...
function missingValueForContext(context) {
    switch (context) {
        case "requisite":
...
function expandStageLabel(value) {
