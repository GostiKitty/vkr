...
import { formatDynamicMetricValue, REPORT_EXPORT_NO_DATA, } from "../helpers";
export function buildProjectOvTsData(base) {
    ...
    const envelopeGroupRows = envelopeView.groupedElements.map((entry) => ({
        key: entry.key,
    ...
    return {
        ...
        expertise,
        preflight,
        sectionTitle: "Раздел 5. Сведения об инженерном оборудовании, сетях и системах инженерно-технического обеспечения",
        ...
        missingData,
        appendixEnvelopeRows: envelopeView.appendixElements.map((entry) => mapEnvelopeAppendixRow(entry, expertise.showTechnicalIdsInAppendix)),
        appendixDetailedRows,
        rcDynamicRows: buildRcDynamicRows(base),
        rcDynamicAvailable: dynamics.available,
    };
}
function buildRcDynamicRows(base) {
...
function buildConclusions(base) {
    ...
    const lines = [];
    const integralPass = metrics.kobStatus === "соответствует" && metrics.qHeatingStatus === "соответствует";
    if (integralPass && metrics.envelopeElementFailures.length > 0) {
        lines.push(`Соответствует по интегральным показателям. Требуется устранить несоответствие поэлементной проверки: ${metrics.envelopeElementFailures
...
function mapEnvelopeAppendixRow(entry, showTechnicalIdsInAppendix) {
    return {
        key: entry.key,
...
function metric(key, label, unit, value, note) {
    return { key, label, unit, value, note };
}
function buildDoorClarificationLines(elements) {
    return elements
        .filter((entry) => entry.category === "external-door" &&
        entry.includeInEnvelope &&
        entry.normalizedResistance === null)
        .map((entry) => `Для наружной двери ${entry.designation || entry.name} требуется задать нормативное сопротивление теплопередаче или подтвердить проектное значение.`);
}
function uniqueHumanMessages(model, values) {
    const seen = new Set();
    const result = [];
...
function humanizeMessage(message, model) {
