...
const REPORT_TITLE = "Расчет теплотехнических характеристик ограждающих конструкций и удельного расхода тепловой энергии на отопление и вентиляцию";
const REPORT_SECTION_TITLE = "Раздел 5. Сведения об инженерном оборудовании, сетях и системах инженерно-технического обеспечения";
const REPORT_SUBSECTION_TITLE = "Отопление, вентиляция и кондиционирование воздуха, тепловые сети";
export function buildThermalProtectionReportData(input) {
    ...
    const suspiciousSurfaceTemperatureCount = constructionDiagnostics.filter((item) => item.suspiciousSurfaceTemperature).length;
    const failedConstructions = (sp50?.constructions ?? []).filter((item) => item.status === "fail");
    const insufficientConstructions = (sp50?.constructions ?? []).filter((item) => item.status === "insufficient_data");
...
function buildSourceDataRows(base, sp50, generatedAtLabel) {
    return [
        ...
        row("heatingAverage", "Средняя температура отопительного периода", "°C", formatMetricNumber(base.climate.outdoorHeatingAverageC)),
        row("heatingDuration", "Продолжительность отопительного периода", "сут.", formatMetricNumber(base.climate.heatingDurationDays, 0)),
        ...
        row("humidity", "Относительная влажность внутреннего воздуха", "%", formatMetricNumber(sp50?.sourceData.indoorRelativeHumidityPercent, 0)),
        row("operationMode", "Режим эксплуатации", "", buildOperationModeLabel(base)),
    ];
}
function buildBuildingSummaryRows(base, sp50, groups) {
    return [
        row("purpose", "Назначение здания", "", mapBuildingCategory(sp50?.sourceData.buildingCategory ?? null)),
        row("storeys", "Этажность", "эт.", formatMetricNumber(sp50?.sourceData.storeys ?? base.summary.levelCount, 0)),
        ...
        row("groups", "Основные группы конструкций", "", groups.length ? groups.map((group) => group.typeLabel).join(", ") : NO_DATA),
        row("compactness", "Показатель компактности", "1/м", formatMetricNumber(sp50?.building.compactness_1_m, 3)),
    ];
}
function buildClimateRows(base, sp50, scenarioConfig) {
    return [
        row("outdoorDesign", "Расчетная наружная температура", "°C", formatMetricNumber(base.climate.outdoorDesignTemperatureC), "По климатической базе и данным модели"),
        row("heatingAverage", "Средняя температура отопительного периода", "°C", formatMetricNumber(base.climate.outdoorHeatingAverageC), "По климатической базе"),
        row("heatingDuration", "Продолжительность отопительного периода", "сут.", formatMetricNumber(base.climate.heatingDurationDays, 0), "По климатической базе"),
        row("gsop", "ГСОП", "°C·сут.", formatMetricNumber(sp50?.sourceData.gsop), "Расчет по СП 131"),
        row("indoor", "Внутренняя температура", "°C", formatMetricNumber(base.climate.indoorTemperatureC ?? base.operation.daySetpointC), "Сценарий расчета"),
        row("humidity", "Относительная влажность", "%", formatMetricNumber(sp50?.sourceData.indoorRelativeHumidityPercent, 0), "Данные модели / допущение"),
        row("ventilation", "Кратность вентиляции", "1/ч", formatMetricNumber(base.operation.ventilationACH, 2), scenarioConfig ? "Сценарий расчета" : "Требует уточнения"),
        row("infiltration", "Кратность инфильтрации", "1/ч", formatMetricNumber(base.operation.infiltrationACH, 2), scenarioConfig ? "Сценарий расчета" : "Требует уточнения"),
        row("heatRecovery", "Коэффициент рекуперации", "доля", formatMetricNumber(base.operation.heatRecoveryFactor, 2), "Сценарий расчета"),
    ];
}
...
function buildThermalProtectionSummaryRows(sp50) {
    return [
        row("kobActual", "kоб расчетное", "Вт/(м³·К)", formatMetricNumber(sp50?.building.kob_W_m3K, 3)),
        row("kobNorm", "kоб нормативное", "Вт/(м³·К)", formatMetricNumber(sp50?.building.kobNorm_W_m3K, 3)),
        row("kobStatus", "Статус", "", sp50 ? mapBuildingCompliance(sp50.building.complies, sp50.building.status) : NO_DATA),
    ];
}
function buildThermalProtectionConclusion(sp50, failedCount) {
    if (!sp50) {
...
function buildEnergyRows(energy) {
    if (!energy) {
        return [
    ...
    return [
        row("qHeatingCharacteristic", "Удельная характеристика расхода тепловой энергии", "Вт/(м³·К)", formatMetricNumber(energy.qHeatingCharacteristic_W_m3K, 3)),
        row("qNorm", "Нормативный удельный расход", "кВт·ч/м²", formatMetricNumber(energy.qHeatingNorm_kWh_m2, 2)),
        row("annualHeatingEnergy", "Годовой расход тепловой энергии", "кВт·ч", formatMetricNumber(energy.annualHeatingEnergy_kWh, 1)),
        ...
        row("energyStatus", "Статус", "", mapBuildingCompliance(energy.complies, energy.status)),
    ];
}
function buildEnergyNote(energy) {
    if (!energy) {
...
function buildDynamicRows(base) {
    return [
        ...
        row("avgTemp", "Средняя температура воздуха по зонам", "°C", formatMetricNumber(base.thermalResults.averageRoomTemperatureC, 1)),
        row("minTemp", "Минимальная температура", "°C", formatMetricNumber(base.thermalResults.minRoomTemperatureC, 1)),
        row("maxTemp", "Максимальная температура", "°C", formatMetricNumber(base.thermalResults.maxRoomTemperatureC, 1)),
        row("discomfort", "Часы дискомфорта", "ч", formatMetricNumber(base.thermalResults.discomfortHours, 1)),
    ];
}
function buildDynamicLossRows(base) {
    const diagnostics = base.thermalResults.available ? base.thermalResults : null;
    const building = base.sp50Report?.building ?? null;
    void building;
    return [
        lossRow("opaque", "Непрозрачные конструкции", formatMetricNumber(diagnostics?.totalOpaqueLossW, 0), "Суммарные расчетные потери через непрозрачные ограждения"),
        lossRow("window", "Окна", formatMetricNumber(diagnostics?.totalWindowLossW, 0), "Суммарные расчетные потери через оконные заполнения"),
        lossRow("door", "Наружные двери", formatMetricNumber(diagnostics?.totalDoorLossW, 0), "Суммарные расчетные потери через наружные двери"),
        lossRow("infiltration", "Инфильтрация", formatMetricNumber(diagnostics?.totalInfiltrationLossW, 0), "Потери, обусловленные неорганизованным воздухообменом"),
        lossRow("ventilation", "Вентиляция", formatMetricNumber(diagnostics?.totalVentilationLossW, 0), "Потери, обусловленные механической вентиляцией"),
    ];
}
function buildConclusions(input) {
    ...
    const normative = !sp50
        ? "Нормативная проверка теплозащиты не сформирована."
        : failedConstructions.length > 0 || sp50.building.complies === false
            ? `Нормативная проверка теплозащиты выявила ${formatInteger(failedConstructions.length)} конструкций(ю), не соответствующих расчетным требованиям, и требует корректировки решений по оболочке здания.`
    ...
    const dynamic = !base.thermalResults.available
    ...
    const missing = humanMissingData.length > 0
...
function buildEnergyPassportRows(energy) {
    if (!energy) {
        return [];
    }
    return [
        ...
        row("ventilationCharacteristic", "Вентиляционная составляющая", "Вт/(м³·К)", formatMetricNumber(energy.ventilationCharacteristic_W_m3K, 3)),
        row("internalGainCharacteristic", "Внутренние теплопоступления", "Вт/(м³·К)", formatMetricNumber(energy.internalGainCharacteristic_W_m3K, 3)),
        row("solarGainCharacteristic", "Солнечные теплопоступления", "Вт/(м³·К)", formatMetricNumber(energy.solarGainCharacteristic_W_m3K, 3)),
        row("beta", "Коэффициент использования теплопоступлений", "доля", formatMetricNumber(energy.betaGainUseFactor, 2)),
...
function buildEnvelopeAppendixRows(diagnostics) {
...
function buildRcSummaryRows(base) {
    return [
        row("peakLoad", "Пиковая тепловая нагрузка", "кВт", formatMetricNumber(base.thermalResults.peakLoadKW, 2)),
        row("energy", "Тепловая энергия за период", "кВт·ч", formatMetricNumber(base.thermalResults.totalEnergyKWh, 1)),
...
function buildRcZoneRows(thermalResult) {
...
function groupConstructions(constructions) {
    const map = new Map();
    constructions.forEach((construction) => {
        ...
        current.weightedActualResistance = mergeWeightedAverage(current.weightedActualResistance, current.area - construction.areaM2, construction.actualResistance_m2K_W, construction.areaM2);
        current.weightedRequiredResistance = mergeWeightedAverage(current.weightedRequiredResistance, current.area - construction.areaM2, construction.normalizedResistance_m2K_W, construction.areaM2);
        current.weightedUValue = mergeWeightedAverage(current.weightedUValue, current.area - construction.areaM2, construction.heatTransferCoefficient_W_m2K, construction.areaM2);
...
function buildConstructionDiagnostics(constructions, sp50) {
    ...
    return constructions
        ...
        const suspiciousSurfaceTemperature = hasSuspiciousSurfaceTemperature(construction, indoorTemperature, outdoorTemperature);
...
function buildCriticalEnvelopeRows(diagnostics) {
    return diagnostics
...
function buildHumanMissingData(base, sp50) {
...
function buildHumanWarnings(base, sp50) {
    return uniqueHumanMessages([
...
function evaluateDynamicComfort(base) {
    ...
    const averageOk = typeof average === "number" && typeof setpoint === "number"
...
function buildOperationModeLabel(base) {
...
function resolveDocumentCode(model, reportMeta, projectId) {
...
function row(key, label, unit, value, note) {
    return { key, label, unit, value, note };
}
function lossRow(key, label, value, note) {
...
function mergeWeightedAverage(currentAverage, currentWeight, nextValue, nextWeight) {
...
function resolveConstructionGroupLabel(type) {
    switch (type) {
        case "wall":
...
function hasClassificationConflict(construction) {
    const openingType = construction.constructionType === "window" ||
    ...
    const sourcePrefixMismatch = (id.startsWith("window-") && construction.constructionType !== "window") ||
...
function hasSuspiciousSurfaceTemperature(construction, indoorTemperature, outdoorTemperature) {
    if (!Number.isFinite(construction.internalSurfaceTemperatureC) ||
...
function buildConstructionNote(construction, classificationConflict, suspiciousSurfaceTemperature) {
...
function resolveConstructionSeverity(construction, classificationConflict) {
    if (construction.status === "fail") {
        return -3;
    }
    if (classificationConflict) {
...
function resolveConstructionStatusLabel(item) {
...
function mapBuildingCompliance(complies, status) {
    if (complies === true) {
        return "соответствует";
    }
    if (complies === false) {
...
function mapBuildingCategory(category) {
    switch (category) {
        case "residential":
            return "жилое";
        case "public":
...
function uniqueHumanMessages(values) {
    const seen = new Set();
    return values
