...
import { createPipeDefaults, defaultEquipmentParams, DOOR_DEFAULTS, DUCT_DEFAULTS, FLOOR_SLAB_DEFAULTS, ROOF_DEFAULTS, SENSOR_DEFAULTS, WALL_DEFAULTS, WINDOW_DEFAULTS, } from "../defaults";
...
const Canvas2D = React.forwardRef(({ model, activeLevelId, tool, selection, gridStep, snapEnabled, orthogonalMode, adjacencyOverlay, neighborEdges, centroids, loops, loopDebug, smartSnapshot, showSmartOverlay = false, thermalFrame = null, transientFrame = null, preparedThermalField = null, showHeatmap = false, heatmapIsPreview = false, engineeringVisualizationMode = "plan", engineeringSchematicStyle = "gost", showEngineeringOverlay = false, exportMode = false, thermalBuildOptions, multiSelection = [], onViewportPointerDown, onToolChange, onSelectionChange, onMultiSelectionChange, onAddRoom, onUpdateRoom, onSetWalls, onUpdateWall, onAddRoof, onAddFloorSlab, onAddDoor, onUpdateDoor, onAddWindow, onUpdateWindow, onAddPipe, onUpdatePipe, onAddDuct, onUpdateDuct, pipeType, heatingDisplayMode, showNetworkFlowArrows = true, equipmentType, engineeringEquipmentType, engineeringEquipmentVariant, wallPreset = resolveDefaultPresetId("wall"), windowPreset = resolveDefaultPresetId("window"), doorPreset = resolveDefaultPresetId("door"), roofPreset = resolveDefaultPresetId("roof"), slabPreset = resolveDefaultPresetId("slab"), onAddEquipment, onAddSensor, onUpdateEquipment, onUpdateSensor, onAddEngineeringEquipment, onUpdateEngineeringEquipment, onAddEngineeringPipe, onRemoveSelection, layoutFitEpoch = 0, }, ref) => {
    ...
    const floorSlabs = useMemo(() => (model.floorSlabs ?? []).filter((slab) => slab.levelId === activeLevelId), [activeLevelId, model.floorSlabs]);
    ...
    useEffect(() => {
        const handleKeyDown = (event) => {
            ...
            const rectTypingActive = rectStart != null && (tool === "roomRect" || tool === "roof" || tool === "slab");
    ...
    return (_jsxs("div", { className: "flex h-full min-h-0 min-w-0 max-w-full w-full flex-col overflow-hidden bg-[color:var(--surface-elevated)]", children: [_jsxs("div", { ref: containerRef, className: "ui-build-canvas-cursor relative h-full min-h-0 min-w-0 w-full max-w-full flex-1 touch-none overflow-hidden overscroll-contain", onPointerDown: handleCanvasPointerDown, onPointerMove: handlePointerMove, onPointerUp: handlePointerUp, onPointerLeave: handlePointerLeave, onMouseDown: handleMouseDownFallback, onMouseMove: handleMouseMoveFallback, onMouseUp: handleMouseUpFallback, onDoubleClick: handleDoubleClick, onContextMenu: (event) => {
                    ...
                    backgroundSize: gridStyle.size,
                }, children: [_jsxs("div", { className: "pointer-events-none absolute inset-0 z-20 flex min-h-0 flex-col justify-between gap-3 p-3", children: [_jsx("div", { className: "flex min-w-0 flex-wrap items-start justify-between gap-3", children: _jsxs("div", { className: "flex min-w-0 flex-1 flex-wrap items-start gap-3", children: [tool === "roomRect" && rectStart ? (_jsx("div", { className: "pointer-events-auto w-full max-w-[320px] sm:max-w-[280px]", children: _jsxs("div", { className: "rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--surface-overlay)] p-3 shadow-lg backdrop-blur", children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-wide text-[color:var(--text-soft)]", children: "\u0420\u0430\u0437\u043C\u0435\u0440\u044B \u043F\u043E\u043C\u0435\u0449\u0435\u043D\u0438\u044F" }), _jsx("p", { className: "mt-1 text-[11px] leading-snug text-[color:var(--text-soft)]", children: "\u0412\u0432\u043E\u0434\u0438\u0442\u0435 \u0446\u0438\u0444\u0440\u044B \u043D\u0430 \u043A\u043B\u0430\u0432\u0438\u0430\u0442\u0443\u0440\u0435 \u0438\u043B\u0438 \u0432 \u043F\u043E\u043B\u044F \u043D\u0438\u0436\u0435. Enter \u2014 \u0441\u043B\u0435\u0434\u0443\u044E\u0449\u0435\u0435 \u043F\u043E\u043B\u0435, Enter \u043D\u0430 \u0434\u043B\u0438\u043D\u0435 \u2014 \u0441\u043E\u0437\u0434\u0430\u0442\u044C." }), _jsxs("div", { className: `mt-2 grid gap-2 ${narrowOverlay ? "grid-cols-1" : "grid-cols-1"}`, children: [_jsxs("label", { className: "grid gap-1 text-xs text-[color:var(--text-soft)]", children: ["\u0428\u0438\u0440\u0438\u043D\u0430, \u043C", _jsx("input", { type: "text", value: rectInput.width, onFocus: () => {
                                                                            setRectActiveField("width");
                                                                            setRectInputLocked(true);
                                                                        }, onBlur: () => {
                                    ...
                                    }), floorSlabs.map((slab) => (_jsx(PlanSurfacePolygon, { points: slab.boundary, worldToScreen: worldToScreen, selected: isSelected("slab", slab.id), label: slab.name, fill: transientSourceMatches(transientFrame, "slab", slab.id) && transientBadge
                                        ? transientBadge.color
                                        : "rgba(59,130,246,0.08)", stroke: transientSourceMatches(transientFrame, "slab", slab.id) ? "#0f172a" : "#2563eb", strokeDasharray: slab.kind === "ground" ? "10 4" : slab.kind === "attic" ? "4 4" : slab.kind === "basement" ? "2 6" : undefined, badgeText: transientSourceMatches(transientFrame, "slab", slab.id) ? transientBadge?.label : undefined, unstable: transientSourceMatches(transientFrame, "slab", slab.id) ? !transientFrame?.stable : false }, slab.id))), roofs.map((roof) => (_jsx(PlanSurfacePolygon, { points: roof.boundary, worldToScreen: worldToScreen, selected: isSelected("roof", roof.id), label: roof.name, fill: transientSourceMatches(transientFrame, "roof", roof.id) && transientBadge
                                        ? transientBadge.color
                                        : roof.kind === "flat"
                                            ? "rgba(245,158,11,0.12)"
                                            : "rgba(245,158,11,0.04)", stroke: transientSourceMatches(transientFrame, "roof", roof.id) ? "#0f172a" : "#d97706", hatch: roof.kind === "flat", arrow: roof.kind === "pitched" ? roof.slope : undefined, badgeText: transientSourceMatches(transientFrame, "roof", roof.id) ? transientBadge?.label : undefined, unstable: transientSourceMatches(transientFrame, "roof", roof.id) ? !transientFrame?.stable : false }, roof.id))), rooms.map((room) => (_jsx(RoomPolygon, { room: room, worldToScreen: worldToScreen, screenToWorld: screenToWorld, scale: view.zoom, selected: isSelected("room", room.id), compactLabels: showSmartOverlay, suppressPlanLabel: showSmartOverlay, obstacles: roomLabelObstacles, schematicMode: engineeringOverlayActive, exportMode: exportMode }, room.id))), levelLoops.map((loop) => (_jsx(LoopOverlay, { loop: loop, worldToScreen: worldToScreen, selected: isSelected("loop", loop.id), hovered: hoverLoopId === loop.id }, loop.id))), loopDebug &&
                                    ...
                                    })() })), walls.map((wall) => (_jsx(WallSegment, { wall: wall, walls: walls, worldToScreen: worldToScreen, selected: isSelected("wall", wall.id), schematicMode: engineeringOverlayActive, exportMode: exportMode, transientHighlight: transientSourceMatches(transientFrame, "wall", wall.id) && transientBadge
                                        ? {
                                            color: transientBadge.color,
                                            badgeText: transientBadge.label,
                                            unstable: !(transientFrame?.stable ?? true),
                                        }
                                        : null }, wall.id))), openingPreview && (_jsx("g", { pointerEvents: "none", opacity: 0.65, children: (() => {
                                    ...
                                    return (_jsxs("g", { children: [_jsx("polyline", { points: screenPoints.map((point) => `${point.x},${point.y}`).join(" "), fill: "none", stroke: style.outline, strokeWidth: style.width + (selectedPipe ? 5 : 3), strokeLinecap: "round", strokeLinejoin: "round" }), _jsx("polyline", { points: screenPoints.map((point) => `${point.x},${point.y}`).join(" "), fill: "none", stroke: selectedPipe ? "var(--accent-base)" : style.stroke, strokeWidth: style.width + (selectedPipe ? 0.8 : 0), strokeDasharray: style.dashArray, strokeLinecap: "round", strokeLinejoin: "round" }), !exportMode && label ? (_jsxs("g", { pointerEvents: "none", opacity: selectedPipe || view.zoom > 34 ? 1 : 0.82, children: [_jsx("rect", { x: label.x - 38, y: label.y - 20, width: 76, height: 16, rx: 8, fill: "color-mix(in srgb, var(--surface-base) 92%, transparent)", stroke: "color-mix(in srgb, var(--border-soft) 72%, transparent)" }), _jsx("text", { x: label.x, y: label.y - 8, textAnchor: "middle", className: "fill-[color:var(--text-muted)] text-[10px] font-semibold", children: `${ENGINEERING_MEDIUM_LABELS[pipe.medium]} • ${Math.round(pipe.diameter)}` })] })) : null] }, pipe.id));
                                        ...
                                        return (_jsx("polyline", { points: engineeringPipePreview.points.map((point) => {
                                                const screen = worldToScreen(point);
                                                return `${screen.x},${screen.y}`;
                                            }).join(" "), fill: "none", stroke: stroke, strokeWidth: 3, strokeDasharray: dashArray, strokeLinecap: "round", strokeLinejoin: "round" }));
                                    ...
                                    const pumpFlowRotation = item.type === "pump"
                                        ? (() => {
                                            const outgoing = engineeringPipes.find((pipe) => pipe.fromEquipmentId === item.id && pipe.points.length >= 2);
                                            if (outgoing) {
                                            ...
                                            const incoming = engineeringPipes.find((pipe) => pipe.toEquipmentId === item.id && pipe.points.length >= 2);
                                            if (incoming) {
                                    ...
                                    return (_jsxs("g", { children: [renderEngineeringEquipmentSymbol({
                                            ...
                                            }, center, { selected: selectedEquipment, hovered: hoveredEquipment }), !exportMode && item.name ? (_jsxs("g", { pointerEvents: "none", children: [_jsx("rect", { x: center.x - 52, y: center.y + Math.max(16, (item.height * view.zoom) / 2 + 10), width: 104, height: 18, rx: 9, fill: "color-mix(in srgb, var(--surface-base) 92%, transparent)", stroke: "color-mix(in srgb, var(--border-soft) 72%, transparent)" }), _jsx("text", { x: center.x, y: center.y + Math.max(28, (item.height * view.zoom) / 2 + 22), textAnchor: "middle", className: "fill-[color:var(--text-base)] text-[10px] font-semibold", children: item.name })] })) : null, showPorts
                                                ? item.ports.map((port) => {
                                            ...
                                            }, worldToScreen(pointerWorld), { preview: true });
...
const PlanSurfacePolygon = ({ points, worldToScreen, selected, label, fill, stroke, strokeDasharray, hatch = false, arrow, badgeText, unstable = false, }) => {
    if (points.length < 3) {
        return null;
    }
    ...
    const arrowEnd = arrow
        ...
        : null;
    return (_jsxs("g", { children: [hatch ? (_jsx("defs", { children: _jsx("pattern", { id: patternId, width: "8", height: "8", patternUnits: "userSpaceOnUse", patternTransform: "rotate(35)", children: _jsx("line", { x1: "0", y1: "0", x2: "0", y2: "8", stroke: stroke, strokeWidth: "1", opacity: "0.24" }) }) })) : null, _jsx("polygon", { points: pointsAttr.join(" "), fill: hatch ? `url(#${patternId})` : fill, stroke: selected ? "#0f172a" : stroke, strokeWidth: selected ? 2.4 : 1.6, strokeDasharray: strokeDasharray, opacity: selected ? 0.95 : 0.9 }), !hatch ? _jsx("polygon", { points: pointsAttr.join(" "), fill: fill, opacity: selected ? 0.9 : 0.72, stroke: "none" }) : null, arrow && arrowEnd ? (_jsxs(_Fragment, { children: [_jsx("line", { x1: centroid.x, y1: centroid.y, x2: arrowEnd.x, y2: arrowEnd.y, stroke: selected ? "#0f172a" : stroke, strokeWidth: 1.8 }), _jsx("polygon", { points: `${arrowEnd.x},${arrowEnd.y} ${arrowEnd.x - 8 * Math.cos(angleRad - 0.45)},${arrowEnd.y - 8 * Math.sin(angleRad - 0.45)} ${arrowEnd.x - 8 * Math.cos(angleRad + 0.45)},${arrowEnd.y - 8 * Math.sin(angleRad + 0.45)}`, fill: selected ? "#0f172a" : stroke })] })) : null, badgeText ? (_jsxs("g", { transform: `translate(${centroid.x},${centroid.y - 18})`, children: [_jsx("rect", { x: -72, y: -10, width: 144, height: 20, rx: 10, fill: unstable ? "rgba(254,243,199,0.96)" : "rgba(255,255,255,0.96)", stroke: unstable ? "#d97706" : "#cbd5e1" }), _jsx("text", { textAnchor: "middle", fill: unstable ? "#92400e" : "#0f172a", fontSize: 10, fontWeight: 700, children: unstable ? `${badgeText} · неуст.` : badgeText })] })) : null] }));
};
...
const WallSegment = ({ wall, walls, worldToScreen, selected, transientHighlight, schematicMode = false, exportMode = false, }) => {
...
const getWallPolygon = (wall, walls) => {
