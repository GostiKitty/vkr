import { EQUIPMENT_TYPE_LABELS, NETWORK_SYSTEM_TYPE_LABELS, PIPE_CIRCUIT_ROLE_LABELS, PIPE_FLOW_ROLE_LABELS, PIPE_TYPE_LABELS, resolveNetworkSystemType, resolvePipeCircuitRole, SENSOR_TYPE_LABELS, } from "../../../entities/networks/types";
import { getEquipmentDisplayName, getFloorDisplayLabel, getRoomDisplayLabel, } from "../utils/entityLabels";
const PIPE_MATERIAL_LABELS = {
    steel: "Сталь",
    pex: "PEX",
    copper: "Медь",
...
const EQUIPMENT_PURPOSE_LABELS = {
...
export function formatHeatingCircuitLabel(systemId) {
...
export function formatPipeMaterialLabel(material) {
    if (!material) {
        return "Не задан";
    }
    return PIPE_MATERIAL_LABELS[material] ?? "Не задан";
}
export function formatSensorStatusLabel(status) {
    if (!status) {
        return "Нет данных";
    }
    return SENSOR_STATUS_LABELS[status] ?? "Нет данных";
}
export function formatEquipmentStateLabel(state) {
    if (!state) {
        return "Не задано";
    }
    return EQUIPMENT_STATE_LABELS[state] ?? "Не задано";
}
export function describeSupplyReturnRole(pipe) {
    if (!pipe) {
        return "Не задано";
    }
...
export function describeEquipmentConnectionStatus(equipment, pipes) {
...
export function getEquipmentPurposeLabel(type) {
    return EQUIPMENT_PURPOSE_LABELS[type] ?? EQUIPMENT_TYPE_LABELS[type] ?? "Инженерное оборудование";
}
export function buildEquipmentHoverDetails(model, equipment) {
...
export function buildPipeHoverDetails(pipe) {
    return [
...
export function buildDuctHoverDetails(duct) {
...
export function buildSensorHoverDetails(model, sensor) {
    const value = sensor.value != null && Number.isFinite(sensor.value)
...
export function getEquipmentInspectorTitle(equipment, allEquipment) {
