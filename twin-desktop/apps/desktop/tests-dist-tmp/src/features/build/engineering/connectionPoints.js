...
export function connectionPointSupportsPipe(point) {
    return point.medium === "pipe";
}
export function connectionPointSupportsDuct(point) {
    return point.medium === "duct";
}
export function getNetworkEndpointConnectionIds(network, equipment, medium, tolerance = 0.42) {
    ...
    equipment.forEach((item) => {
        ...
        if (endpoints.some((endpoint) => points.some((point) => Math.hypot(endpoint.x - point.position.x, endpoint.y - point.position.y) <= tolerance))) {
