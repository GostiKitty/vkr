...
import { buildSmartModelSnapshot } from "../../../core/networks/index";
const LIVE_SYNC_OPTIONS = {
    trackHistory: false,
    persist: false,
};
export function useSmartBuilding({ model, thermalResult, updateEquipment, updatePipe, updateDuct, updateSensor, addEvent, updateEvent, removeEvent, }) {
    ...
    useEffect(() => {
        ...
        snapshot.events.forEach((event) => {
            ...
            if (existing.message !== event.message ||
    ...
    useEffect(() => {
        snapshot.sensorStates.forEach((state) => {
            ...
            const sameHistory = (!currentHistory && !nextHistory) ||
