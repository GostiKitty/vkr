...
function visitPolygon(bounds, polygon, elevation) {
    polygon.forEach((point) => {
        expandBounds(bounds, point.x, elevation, point.y);
    });
}
export function resolveSimplePreviewLevelId(model, activeLevelId) {
    if (!activeLevelId) {
        return null;
    }
    const hasShellOnLevel = model.rooms.some((room) => room.levelId === activeLevelId) ||
...
export function buildSimplePreviewBounds(model, activeLevelId) {
...
export function calculateSimplePreviewCamera(bounds, mode = "iso") {
    if (bounds.empty) {
        return {
...
export function calculateSimplePreviewGrid(bounds) {
    const maxDim = Math.max(bounds.size.x, bounds.size.y, bounds.size.z, MIN_DIMENSION_M);
    return {
        size: maxDim * 2,
