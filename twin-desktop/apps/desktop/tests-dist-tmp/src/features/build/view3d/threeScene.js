...
import { getPipeVisualStyle, normalizeFlowDirection, resolvePipeColor, SENSOR_TYPE_LABELS, } from "../../../entities/networks/types";
...
import { createThermalFieldModel, sampleSmoothedThermalFieldAtPoint, sampleThermalFieldAtPoint, sampleWallSurfaceTemperatures, } from "../../../core/thermal/field";
...
import { createPipeDefaults, defaultEquipmentParams, DOOR_DEFAULTS, DUCT_DEFAULTS, SENSOR_DEFAULTS, WINDOW_DEFAULTS, } from "../defaults";
import { createEquipmentVisual, createSensorVisual, getEquipmentBaseY, getEquipmentWorldConnectionPoint, getSensorWorldPosition, } from "./equipmentMeshes";
...
export function createBuildScene(canvas, callbacksRef) {
    ...
    const interactionPlane = new THREE.Mesh(new THREE.BoxGeometry(1, 0.02, 1), new THREE.MeshStandardMaterial({ color: 0xffffff, transparent: true, opacity: 0 }));
    ...
    let dragState = null;
    let draft = null;
    let draftPreviewPoint = null;
    let placementPreview = null;
    let hoveredObject = null;
    let holdDragTimeoutId = null;
    let pendingHoldDrag = null;
    ...
    let currentHeatmapState = null;
    let heatmapCache = null;
    let currentSelection = null;
    let currentTransientFrame = null;
    let currentContext = {
        tool: "select",
        activeLevelId: null,
    ...
    const resolveSelectionLevelId = (selection) => {
        if (!selection) {
    ...
    const getLevelElevation = (levelId) => {
        if (!levelId) {
    ...
    const renderDraft = () => {
        ...
        if (draft && draft.points.length > 0) {
            ...
            previewPoints.slice(1).forEach((point, index) => {
                ...
                const end = point;
                const segment = activeDraft.tool === "pipe"
                    ? buildCylinderBetween(start, end, 0.035, 0xf97316)
                    : buildBoxBetween(start, end, activeDraft.tool === "duct" ? 0.3 : 0.18, activeDraft.tool === "duct" ? 0.18 : 3, 0x0f172a);
                if (!segment) {
                    return;
                }
                segment.position.y =
        ...
        const previewObject = previewPlacement.tool === "equipment"
            ? buildEquipmentMesh({
                ...
                params: {},
                connectedNetworkIds: [],
            }, level.elevation_m, level.height_m, null)
            : buildSensorMesh({
                id: "preview-sensor",
                type: "temperature",
                position: { ...previewPlacement.point },
    ...
    const appendDraftPoint = (point) => {
        const activeLevel = getActiveLevel();
        if (!activeLevel) {
            return;
        }
        if (!draft || draft.tool !== currentContext.tool || draft.levelId !== activeLevel.id) {
            draft = { tool: currentContext.tool, points: [{ ...point }], levelId: activeLevel.id };
            draftPreviewPoint = null;
            renderDraft();
            return;
        }
    ...
    const startDrag = (object, selection, point) => {
        if (!point) {
            return;
        }
    ...
    const applyDragPreview = (point) => {
        ...
        dragState.object.position.set(dragState.rootStartPosition.x + dx, dragState.rootStartPosition.y, dragState.rootStartPosition.z + dz);
    };
    const findRoomIdAtPoint = (levelId, point) => {
        if (!levelId) {
            return null;
        }
        return (currentModel.rooms.find((room) => room.levelId === levelId && polygonContainsPoint(point, room.polygon))?.id ?? null);
    };
    const commitDrag = () => {
        if (!dragState) {
            return;
        ...
        dragState.object.position.set(dragState.rootStartPosition.x, dragState.rootStartPosition.y, dragState.rootStartPosition.z);
        dragState = null;
    ...
    const placeOpeningOnWall = (kind, wallId, point) => {
        ...
        const defaults = kind === "door"
            ? { width_m: DOOR_DEFAULTS.width, height_m: DOOR_DEFAULTS.height, sill_m: undefined }
    ...
    const resolveOpeningPlacement = (wall, centerOffsetM, widthM, ignoreOpeningId) => {
    ...
    const placeNetworkObject = (kind, point) => {
        const activeLevel = getActiveLevel();
        if (!activeLevel) {
            return;
        }
    ...
    const handlePointerDown = (event) => {
        ...
        const hovered = objectHits
            .map((hit) => findSelectableObject(hit.object))
    ...
    const handlePointerMove = (event) => {
        ...
        const intersects = raycaster.intersectObjects(scene.children, true);
        const hoveredEntry = intersects
            .map((hit) => ({
            object: findSelectableObject(hit.object),
            point: { x: hit.point.x, y: hit.point.z },
        }))
            .find((entry) => Boolean(entry.object)) ?? null;
        ...
        callbacksRef.current.onHoverInfo?.(currentContext.showTooltip === false || !hoveredSelection
            ? null
            :
                hoveredSelection
                    ? buildHoverInfo(hoveredSelection, event.clientX - rect.left, event.clientY - rect.top, currentModel, currentHeatmapState, hoveredEntry?.point ?? null)
                    : null);
        const canMoveHoveredSelection = (currentContext.tool === "move" || currentContext.tool === "select") &&
            hoveredSelection &&
            currentSelection &&
    ...
    const update = (model, context, options, selection, frame, transientFrame, thermalField, sectionHeight, sectionMode, sectionOffset) => {
        resize();
        currentModel = model;
        ...
        if (options.showRooms) {
            ...
            renderGeometry.roomVolumes.forEach((room) => {
                ...
                const mesh = buildRoomMesh(room, levelElevation, levelHeight, clampHeight, selection, currentHeatmapState, currentContext.showVolumeTint ?? false);
                if (mesh) {
                    roomGroup.add(mesh);
                }
            });
        ...
        if (options.showWalls) {
            ...
            renderGeometry.walls.forEach(({ wall, openings }) => {
                const mesh = buildWallMesh(wall, getLevelBaseElevation(levelMap, wall.levelId), selection, clampHeight, openings, extensions.get(wall.id), options.transparentWalls ?? false, options.presentationMode ?? "default", !currentContext.safeMode && currentContext.showWallSurfaces ? currentHeatmapState : null, currentContext.safeMode ? null : currentTransientFrame);
                if (mesh) {
                    wallGroup.add(mesh);
                }
            });
        ...
        if (options.showWalls) {
            ...
            renderGeometry.roofs.forEach((roof) => {
                const mesh = buildRoofMesh(roof, selection, options.transparentWalls ?? false, options.presentationMode ?? "default", currentContext.safeMode ? null : currentTransientFrame);
                if (mesh) {
                    roofGroup.add(mesh);
                }
            });
            ...
            renderGeometry.floorSlabs.forEach((slab) => {
                const mesh = buildFloorSlabMesh(slab, selection, options.transparentWalls ?? false, options.presentationMode ?? "default", currentContext.safeMode ? null : currentTransientFrame);
                if (mesh) {
                    slabGroup.add(mesh);
                }
            });
        ...
        if (options.showNetworks || options.showEquipment) {
            ...
            if (options.showNetworks) {
                ...
                model.ducts.forEach((duct) => {
                    const mesh = buildDuctMesh(duct, getLevelBaseElevation(levelMap, duct.levelId), getLevelHeight(levelMap, duct.levelId, 3), selection);
                    if (mesh) {
                        networkGroup.add(mesh);
                    }
                });
            ...
            if (options.showEquipment) {
                model.equipment.forEach((item) => {
                    const mesh = buildEquipmentMesh(item, getLevelBaseElevation(levelMap, item.levelId), getLevelHeight(levelMap, item.levelId, 3), selection, currentContext.useSimplifiedEquipment ?? false);
                    if (mesh) {
                        networkGroup.add(mesh);
                    }
                });
                ...
                model.sensors.forEach((sensor) => {
                    const mesh = buildSensorMesh(sensor, getLevelBaseElevation(levelMap, sensor.levelId), getLevelHeight(levelMap, sensor.levelId, 3), selection);
                    if (mesh) {
                        networkGroup.add(mesh);
                    }
                });
        ...
        if (currentContext.debug) {
            buildDebugObjects(model, renderGeometry, levelMap, currentHeatmapState, currentContext.debug).forEach((object) => debugGroup.add(object));
        }
...
function disposeSceneNode(object) {
...
function setHoverState(object, hovered) {
    if (!object) {
        return;
    }
    object.traverse((entry) => {
...
function applyPreviewStyle(object) {
    object.traverse((entry) => {
        const mesh = entry;
        if (!mesh.material) {
            return;
...
const buildRoomMesh = (room, levelElevation, roomHeight, sectionHeight, selection, heatmapState, showVolumeTint) => {
    ...
    const tintedColor = showVolumeTint && temp !== null ? new THREE.Color(temperatureToColor(temp, heatmapState?.minTemperatureC ?? 16, heatmapState?.maxTemperatureC ?? 26)).getHex() : 0xe2e8f0;
...
const buildWallMesh = (wall, levelElevation, selection, sectionHeight, openings, extension, transparent = false, presentationMode = "default", heatmapState, transientFrame) => {
    ...
    const transientColor = transientMatch
        ? new THREE.Color(temperatureToColor(transientFrame.innerSurfaceTemperature_C, transientFrame.minTemperature_C, transientFrame.maxTemperature_C)).getHex()
        : null;
    ...
    const wallMesh = new THREE.Mesh(geometry, new THREE.MeshStandardMaterial({
        color: wallColor,
        transparent,
...
const buildRoofMesh = (roof, selection, transparent = false, presentationMode = "default", transientFrame) => {
    ...
    const engineeringOverview = presentationMode === "engineering-overview";
    const mesh = buildPolygonPrismMesh(roof.boundary, (point) => {
...
const buildFloorSlabMesh = (slab, selection, transparent = false, presentationMode = "default", transientFrame) => {
    ...
    const transientColor = transientMatch
        ...
        : null;
    const engineeringOverview = presentationMode === "engineering-overview";
    const mesh = buildPolygonPrismMesh(slab.boundary, () => ({
...
function buildPolygonPrismMesh(boundary, heightResolver, options) {
...
const buildOpeningMesh = (opening, wall, levelElevation, selection, kind) => {
    ...
    if (kind === "door") {
        ...
        const leftJamb = new THREE.Mesh(new THREE.BoxGeometry(jambWidth, opening.height_m + topHeight, depth), jambMaterial.clone());
        ...
        const rightJamb = new THREE.Mesh(new THREE.BoxGeometry(jambWidth, opening.height_m + topHeight, depth), jambMaterial.clone());
        ...
        const topJamb = new THREE.Mesh(new THREE.BoxGeometry(opening.width_m, topHeight, depth), jambMaterial.clone());
        ...
        const leafGeometry = new THREE.BoxGeometry(Math.max(0.12, opening.width_m - jambWidth * 2), Math.max(0.12, opening.height_m - 0.08), panelDepth);
        ...
        const leftFrame = new THREE.Mesh(new THREE.BoxGeometry(jambWidth, opening.height_m, depth), frameMaterial.clone());
        leftFrame.position.set(-opening.width_m / 2 + jambWidth / 2, sill + opening.height_m / 2, 0);
        decorateSolidMesh(leftFrame, 0xffffff, 0.2);
        const rightFrame = new THREE.Mesh(new THREE.BoxGeometry(jambWidth, opening.height_m, depth), frameMaterial.clone());
        rightFrame.position.set(opening.width_m / 2 - jambWidth / 2, sill + opening.height_m / 2, 0);
        decorateSolidMesh(rightFrame, 0xffffff, 0.2);
        const topFrame = new THREE.Mesh(new THREE.BoxGeometry(opening.width_m, jambWidth, depth), frameMaterial.clone());
        topFrame.position.set(0, sill + opening.height_m - jambWidth / 2, 0);
        decorateSolidMesh(topFrame, 0xffffff, 0.2);
        const bottomFrame = new THREE.Mesh(new THREE.BoxGeometry(opening.width_m, jambWidth, depth), frameMaterial.clone());
        bottomFrame.position.set(0, sill + jambWidth / 2, 0);
        decorateSolidMesh(bottomFrame, 0xffffff, 0.2);
        const glassGeometry = new THREE.BoxGeometry(Math.max(0.08, opening.width_m - jambWidth * 2.8), Math.max(0.08, opening.height_m - jambWidth * 2.8), Math.max(0.03, depth * 0.08));
...
function resolveWallBaseColor(angle, transparent) {
...
const buildCornerPatch = (corner, sectionHeight) => {
...
const buildPipeMesh = (pipe, levelElevation, selection) => {
    ...
    displayPath.slice(1).forEach((point, index) => {
        ...
        if (sleeve) {
            ...
            const sleeveMaterial = sleeve.material;
    ...
    displayPath.forEach((point) => {
        const joint = new THREE.Mesh(new THREE.SphereGeometry(Math.max(radius * 1.16, 0.035), 14, 14), new THREE.MeshStandardMaterial({
...
const buildDuctMesh = (duct, levelElevation, levelHeight, selection) => {
    ...
    duct.path.slice(1).forEach((point, index) => {
        ...
        const end = point;
        const segment = buildBoxBetween(start, end, width, height, selection?.kind === "duct" && selection.id === duct.id ? 0xfacc15 : 0x64748b);
        if (segment) {
...
const buildEquipmentMesh = (item, levelElevation, levelHeight, selection, simplified = false) => {
...
const buildSensorMesh = (sensor, levelElevation, levelHeight, selection) => {
...
function buildNetworkConnectionMeshes(model, levelMap) {
    const objects = [];
    ...
    model.pipes.forEach((pipe) => {
        ...
        pipe.connectedEquipmentIds.forEach((equipmentId) => {
            ...
            const line = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: new THREE.Color(resolvePipeColor(pipe)).getHex(), transparent: true, opacity: 0.28, depthWrite: false }));
            objects.push(line);
        });
    });
    model.ducts.forEach((duct) => {
        ...
        duct.connectedEquipmentIds.forEach((equipmentId) => {
            ...
            const geometry = new THREE.BufferGeometry().setFromPoints(points);
            const line = new THREE.Line(geometry, new THREE.LineDashedMaterial({ color: 0x7b8a9a, dashSize: 0.18, gapSize: 0.12, transparent: true, opacity: 0.24, depthWrite: false }));
...
function buildWallProfileShape(localLength, visibleHeight, openings, usableStart, usableEnd) {
    ...
    openings.forEach((opening) => {
        ...
        const openingBottom = opening.type === "window" ? clamp(opening.sill ?? 0, 0, visibleHeight - 0.05) : 0;
...
const attachOpeningToWall = (opening, wall) => {
    const wallLength = Math.hypot(wall.b.x - wall.a.x, wall.b.y - wall.a.y);
...
function buildWallSurfaceProfile(field, wall, openings, surfaceTemperatures) {
    ...
    return Array.from({ length: sampleCount }, (_, index) => {
        ...
        const sampleRoomSurface = (roomId, sign, averageSurfaceC, fallbackTemperatureC) => {
            if (!roomId) {
            ...
            const localSourceBoostC = (field.sourcesByLevel.get(wall.levelId) ?? []).reduce((sum, source) => {
                ...
                const sourceFactor = source.kind === "radiator" ? 0.36 : source.kind === "pipe" ? 0.16 : source.kind === "equipment" ? 0.12 : 0.06;
        ...
        const openingModifier = openings.reduce((sum, opening) => {
        ...
        const positiveBase = sampleRoomSurface(boundary.positiveRoomId, positiveSign, surfaceTemperatures.positiveSideC, surfaceTemperatures.positiveSideC);
        const negativeBase = sampleRoomSurface(boundary.negativeRoomId, -positiveSign, surfaceTemperatures.negativeSideC, surfaceTemperatures.negativeSideC);
        const positiveSideC = clamp(positiveBase + openingModifier - bridgePenalty, field.minTemperatureC - 1, field.maxTemperatureC + 1);
        const negativeSideC = clamp(negativeBase + openingModifier * 0.72 - bridgePenalty * 0.55, field.minTemperatureC - 1, field.maxTemperatureC + 1);
        return {
            offsetRatio,
            positiveSideC,
            negativeSideC,
...
function buildWallTemperatureSurfaces(shape, thickness, surfaceTemperatures, minTemperatureC, maxTemperatureC) {
    ...
    return surfaces.map(({ fallbackTemperatureC, translateZ, profile }) => {
        ...
        const geometry = new THREE.ExtrudeGeometry(shape, { depth: overlayDepth, bevelEnabled: false });
...
function interpolateWallProfileTemperature(profile, offsetRatio, fallbackTemperatureC) {
    if (!profile.length) {
        return fallbackTemperatureC;
    }
...
const decorateSolidMesh = (mesh, edgeColor, edgeOpacity) => {
    mesh.castShadow = true;
    mesh.receiveShadow = true;
    const edgeLines = new THREE.LineSegments(new THREE.EdgesGeometry(mesh.geometry), new THREE.LineBasicMaterial({
        color: edgeColor,
        transparent: true,
        opacity: edgeOpacity,
        depthWrite: false,
...
const addSoftSelectionHalo = (mesh, color, opacity, scale) => {
    const halo = new THREE.Mesh(mesh.geometry.clone(), new THREE.MeshBasicMaterial({
        color,
        transparent: true,
        opacity,
        side: THREE.BackSide,
...
const getLevelBaseElevation = (levelMap, levelId) => levelMap.get(levelId)?.elevation_m ?? 0;
const formatFloorSlabKind = (kind) => {
    switch (kind) {
        case "interfloor":
...
const getLevelHeight = (levelMap, levelId, fallback) => levelMap.get(levelId)?.height_m ?? fallback;
const getModelLevelElevation = (model, levelId) => model.levels.find((level) => level.id === levelId)?.elevation_m ?? 0;
const computeModelBounds = (model, options) => {
    ...
    const matchesLevel = (levelId) => !activeLevelId || levelId === activeLevelId;
    ...
    if (includeWalls) {
        ...
        (model.roofs ?? []).forEach((roof) => {
            ...
            roof.boundary.forEach((point) => {
                const rise = roof.kind === "pitched" && roof.slope
...
function buildHeatmapState(model, frame, renderGeometry = buildGeometryRenderModel(model), thermalBuildOptions, precomputedField, cache) {
    if (!frame) {
        return null;
    }
...
function buildHoverInfo(selection, screenX, screenY, model, heatmapState, worldPoint) {
    const formatHoverTemperature = (value) => typeof value === "number" ? `${value.toFixed(1)} °C` : "—";
    switch (selection.kind) {
        case "room": {
            ...
            return room
                ? {
                    ...
                    temperatureC: heatmapState && worldPoint
        ...
        case "wall":
            {
                ...
                const details = surface && boundary
        ...
        case "duct": {
            ...
            const sectionLabel = duct?.section.shape === "round"
        ...
        case "sensor": {
            ...
            return {
                ...
                subtitle: sensor?.value !== null && sensor?.value !== undefined
...
function buildHeatmapOverlay(model, heatmapState, options) {
        ...
        for (let x = bounds.minX; x <= bounds.maxX; x += step) {
            for (let y = bounds.minY; y <= bounds.maxY; y += step) {
                ...
                if (options.showHeatmap) {
                    overlays.push(buildHeatmapCell(cellSamples, sample.elevation + 0.03, heatmapState.minTemperatureC, heatmapState.maxTemperatureC));
                }
...
function buildDebugObjects(model, renderGeometry, levelMap, heatmapState, debug) {
    const objects = [];
    if (debug.showRoomContours) {
        renderGeometry.roomVolumes.forEach((room) => {
            const points = [...room.polygon, room.polygon[0]].map((point) => new THREE.Vector3(point.x, (levelMap.get(room.levelId)?.elevation_m ?? 0) + 0.06, point.y));
            const geometry = new THREE.BufferGeometry().setFromPoints(points);
            const line = new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0x6366f1, transparent: true, opacity: 0.92 }));
            objects.push(line);
        });
    }
    ...
    if (debug.showRadiatorInfluence && heatmapState) {
        heatmapState.field.sources.forEach((source) => {
            const ring = new THREE.EllipseCurve(0, 0, source.decayM, source.decayM, 0, Math.PI * 2, false, 0)
                .getPoints(64)
                .map((point) => new THREE.Vector3(point.x + source.position.x, (levelMap.get(source.levelId)?.elevation_m ?? 0) + 0.12, point.y + source.position.y));
            const geometry = new THREE.BufferGeometry().setFromPoints([...ring, ring[0]]);
            objects.push(new THREE.Line(geometry, new THREE.LineBasicMaterial({ color: 0xdc2626, transparent: true, opacity: 0.78 })));
        });
    }
    if (debug.showCoolingZones && heatmapState) {
        heatmapState.field.boundaries.forEach((boundary) => {
            ...
            objects.push(new THREE.LineSegments(geometry, new THREE.LineBasicMaterial({ color: 0x2563eb, transparent: true, opacity: 0.72 })));
        });
    }
    if (debug.showOpeningHosts) {
        ...
        [...model.doors, ...model.windows].forEach((opening) => {
            ...
            const y = (levelMap.get(wall.levelId)?.elevation_m ?? 0) +
            ...
            objects.push(new THREE.LineSegments(geometry, new THREE.LineBasicMaterial({ color: opening.id.startsWith("win") ? 0x0ea5e9 : 0xf59e0b, transparent: true, opacity: 0.7 })));
        });
    }
    return objects;
}
function buildIsoValues(minTemperatureC, maxTemperatureC, count) {
    ...
    return Array.from({ length: count }, (_, index) => roundedMin + normalizedStep * index).filter((value) => value >= minTemperatureC - normalizedStep * 0.5 && value <= maxTemperatureC + normalizedStep * 0.5);
}
function buildHeatmapCell(cellSamples, elevation, minTemperatureC, maxTemperatureC) {
    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute("position", new THREE.Float32BufferAttribute(cellSamples.flatMap(({ point }) => [point.x, elevation, point.y]), 3));
    geometry.setAttribute("color", new THREE.Float32BufferAttribute(cellSamples.flatMap(({ value }) => {
...
function buildCellContours(cellSamples, isoValues, elevation) {
    const contours = [];
    const edges = [
        [0, 1],
        [1, 2],
    ...
    isoValues.forEach((iso) => {
        const intersections = edges
            ...
            return new THREE.Vector3(THREE.MathUtils.lerp(from.point.x, to.point.x, normalized), elevation, THREE.MathUtils.lerp(from.point.y, to.point.y, normalized));
        })
            .filter((point) => Boolean(point));
        if (intersections.length < 2) {
            return;
        }
        const segmentPairs = intersections.length === 4
            ? [
...
function fitCameraToModel(camera, controls, bounds) {
...
function setTopDownView(camera, controls, bounds, model, selection) {
    const focus = selection && selection.kind !== "loop" ? resolveSelectionFocus(model, selection) : null;
    const target = focus
...
function focusSelection(camera, controls, model, selection) {
...
function resolveSelectionFocus(model, selection) {
    switch (selection.kind) {
        case "room": {
            const room = model.rooms.find((entry) => entry.id === selection.id);
            if (!room) {
...
function closestPointOnPolyline(point, path) {
    if (path.length < 2) {
        return null;
    }
    let bestPoint = null;
...
const buildCylinderBetween = (start, end, radius, color) => {
...
const buildBoxBetween = (start, end, width, height, color) => {
    const dx = end.x - start.x;
