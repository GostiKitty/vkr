export function computeEnvelopeMetrics(model, adjacency) {
    ...
    const windowAreaByWall = new Map();
    const doorAreaByWall = new Map();
    model.windows.forEach((window) => {
        if (!window.anchor.wallId) {
            return;
        }
        windowAreaByWall.set(window.anchor.wallId, (windowAreaByWall.get(window.anchor.wallId) ?? 0) + window.width_m * window.height_m);
    });
    model.doors.forEach((door) => {
        if (!door.anchor.wallId) {
            return;
        }
        doorAreaByWall.set(door.anchor.wallId, (doorAreaByWall.get(door.anchor.wallId) ?? 0) + door.width_m * door.height_m);
    });
    const roomMetrics = {};
