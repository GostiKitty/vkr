...
import Build3DCanonicalPreview from "./view3d/Build3DCanonicalPreview";
import { DEFAULT_STABLE_VIEWER_OPTIONS } from "./view3d/viewerOptions";
...
import { EngineeringCallout, ThermalFieldLegend, } from "../../shared/ui/EngineeringUi";
...
import { BuildToolIcon } from "./components/buildToolIcons";
...
import { buildLevelName, getLevelDisplayLabel, getSelectionDisplayLabel, } from "./utils/entityLabels";
...
import { buildThermalFieldOptions, DEFAULT_THERMAL_DISPLAY_OPTIONS, } from "./thermal/displayOptions";
import { buildSurfaceFieldResult, getSurfaceFieldValueRange, SURFACE_FIELD_MODE_LABELS, SURFACE_FIELD_MODE_UNITS, } from "../../core/thermal/surfaceField";
...
import { engineeringModeUsesHeatmap, engineeringModeUsesOverlay, } from "./engineering/viewMode";
import { buildTransientVisualizationFrame } from "./thermal/transientVisualization";
import { applyEngineeringOverviewPreset, clearEngineeringOverviewPreset, isEngineeringOverviewPresetActive, shouldEnableEngineeringOverviewForDemoStep, } from "./view3d/engineeringOverview";
...
import { buildVideoDemoProjectModel, VIDEO_DEMO_PROJECT_ID, } from "./demoVideoProject";
...
import { createTypicalCtpEngineeringSystems, createItpParallelDhwDependentHeating, } from "./engineering2d/catalog";
import { DEFAULT_ENVELOPE_PRESET_IDS } from "../../entities/envelope/envelopePresets";
import { computeSolarPosition } from "../../core/solar/solarPosition";
import SolarTimeOverlay from "./components/SolarTimeOverlay";
...
const WORKSPACE_SIDEBAR_TAB_META = {
    browser: { title: "Проводник", icon: "levels" },
    properties: { title: "Свойства", icon: "sliders" },
    project: { title: "Проект", icon: "cube" },
};
function QuickToolButton({ icon, label, title, active, disabled, compact, onClick, }) {
    const tooltipLabel = compact ? label : title || label;
    return (_jsx(ToolbarTooltip, { label: tooltipLabel, children: _jsxs("button", { type: "button", onClick: onClick, disabled: disabled, "aria-label": label, "aria-pressed": active, className: `ui-control inline-flex h-9 items-center gap-1.5 rounded-full border transition ${compact ? "w-9 justify-center px-0" : "px-3"} text-xs font-semibold ${disabled
                ...
                : active
                    ...
                    : "border-[color:var(--border-base)] bg-[color:var(--surface-elevated)] text-[color:var(--text-muted)] hover:border-[color:var(--border-strong)] hover:bg-[color:var(--surface-muted)] hover:text-[color:var(--text-base)]"}`, children: [_jsx(BuildToolIcon, { name: icon, className: "h-4 w-4 shrink-0" }), !compact ? _jsx("span", { className: "hidden xl:inline", children: label }) : null] }) }));
}
function BuildToolbarDivider() {
    return _jsx("div", { className: "mx-0.5 hidden h-7 w-px shrink-0 bg-[color:var(--border-soft)] md:block", "aria-hidden": "true" });
}
function BuildToolbarGroup({ label, children, }) {
    return (_jsx("div", { className: "flex min-w-0 items-center gap-1.5", role: "group", "aria-label": label, children: _jsx("div", { className: "flex flex-wrap items-center gap-1", children: children }) }));
}
function ViewModeTab({ icon, label, active, onClick, }) {
    return (_jsx(ToolbarTooltip, { label: label, children: _jsxs("button", { type: "button", role: "tab", "aria-selected": active, "aria-label": label, onClick: onClick, className: `ui-segmented-control__item inline-flex items-center gap-1.5 px-2.5 sm:px-3 ${active ? "ui-segmented-control__item--active" : ""}`, children: [_jsx(BuildToolIcon, { name: icon, className: "h-4 w-4 shrink-0" }), _jsx("span", { className: "hidden sm:inline", children: label })] }) }));
}
...
function BuildPageContent() {
    ...
    const view3dCanvasRef = useRef(null);
    const view3dViewportRef = useRef(null);
    const projectValidationSectionRef = useRef(null);
    const engineeringOverviewSnapshotRef = useRef(null);
    const engineeringOverviewZoomRafRef = useRef(null);
    const canvasWorkspaceFitRafRef = useRef(null);
    const hasFittedOnThis3DEntryRef = useRef(false);
    const [showHelp, setShowHelp] = useState(false);
    const [equipmentPreset, setEquipmentPreset] = useState("radiator");
    const [engineeringEquipmentPreset, setEngineeringEquipmentPreset] = useState("heatExchanger");
    const [engineeringEquipmentVariant, setEngineeringEquipmentVariant] = useState(undefined);
    const [pipePreset, setPipePreset] = useState("heating_supply");
    ...
    const [heatingDisplayMode, setHeatingDisplayMode] = useState("lineRole");
    const [showNetworkFlowArrows, setShowNetworkFlowArrows] = useState(true);
    const [visualizationMode, setVisualizationMode] = useState("plan");
    const [schematicStyle, setSchematicStyle] = useState("gost");
    const [viewer, setViewer] = useState(DEFAULT_STABLE_VIEWER_OPTIONS);
    ...
    const [workspaceSidebarTab, setWorkspaceSidebarTab] = useState("browser");
    ...
    const setActiveViewport = useCallback((mode) => {
        setWorkspaceMode(mode);
    }, [setWorkspaceMode]);
    const resolvedViewport = activeViewport === "results" ? "plan" : activeViewport;
    ...
    const [activeTransientResult, setActiveTransientResult] = useState(null);
    const [selectedTransientTimeIndex, setSelectedTransientTimeIndex] = useState(0);
    const [activeTransientSourceId, setActiveTransientSourceId] = useState(null);
    const [activeTransientSourceType, setActiveTransientSourceType] = useState(null);
    ...
    const [thermalDisplay, setThermalDisplay] = useState(() => ({
    ...
    const [threeDHoverInfo, setThreeDHoverInfo] = useState(null);
    const [view3dCameraState, setView3dCameraState] = useState(null);
    const [surfaceFieldOverlayDebug, setSurfaceFieldOverlayDebug] = useState(null);
    const [sceneDebug, setSceneDebug] = useState(DEFAULT_BUILD_SCENE_DEBUG_OPTIONS);
    ...
    const downloadBlob = useCallback((file, filename) => {
    ...
    const scheduleEngineeringOverviewFit = useCallback((action) => {
    ...
    const { viewSnapshots, projectSnapshots, addViewSnapshot, removeViewSnapshot, addProjectSnapshot, removeProjectSnapshot, getProjectSnapshot, } = useSnapshots(projectKey);
    ...
    const solarPosition = useMemo(() => computeSolarPosition({
    ...
    const validationIssues = useMemo(() => [...validateModel(model), ...validateEngineeringModel(model, smartSnapshot)], [adjacency, model, smartSnapshot]);
    const roomNames = useMemo(() => Object.fromEntries(model.rooms.map((room, index) => [
        room.id,
        getVisibleRoomName(room, index),
    ])), [model.rooms]);
    const loopMap = useMemo(() => Object.fromEntries(roomLoops.map((loop) => [loop.id, loop])), [roomLoops]);
    const importedTwinLoadedRef = useRef(null);
    const centeredProjectRef = useRef(null);
    const centeredNetworksRef = useRef(null);
    ...
    const isWorkspaceSidebarMobileViewport = useCallback(() => typeof window !== "undefined" && !window.matchMedia("(min-width: 768px)").matches, []);
    const openWorkspaceSidebar = useCallback((tab) => {
    ...
    const handleWorkspaceSidebarToggle = useCallback((tab) => {
    ...
    const closeAllTransientPanels = useCallback((options) => {
    ...
    function syncWallsFromRooms(rooms) {
    ...
    const handleAddTypicalCtp = useCallback(() => {
        ...
        const current = model.engineeringSystems ?? { equipment: [], pipes: [] };
    ...
    const removeSelection = useCallback((target) => {
    ...
    function handleUpdateRoom(roomId, patch) {
        const nextPatch = "name" in patch ? { ...patch, source: "manual" } : patch;
    ...
    const handleExport = useCallback(() => {
        const twin = buildModelToTwin(model, { projectName: model.meta?.name });
    ...
    const handleOpenVideoDemo = useCallback(() => {
        const hasEditableModel = model.rooms.length > 0 ||
            model.walls.length > 0 ||
            (model.roofs?.length ?? 0) > 0 ||
            (model.floorSlabs?.length ?? 0) > 0;
        const currentSourceProjectId = typeof model.meta?.sourceProjectId === "string" ? model.meta.sourceProjectId : null;
        const alreadyDemo = projectId === VIDEO_DEMO_PROJECT_ID || currentSourceProjectId === VIDEO_DEMO_PROJECT_ID;
        if (typeof window !== "undefined" && hasEditableModel && !alreadyDemo) {
            const confirmed = window.confirm("Открыть прежний демонстрационный дом? Текущая локальная модель будет заменена.");
            if (!confirmed) {
                return;
            }
        }
        ...
        notifyInfo("Демонстрационный дом загружен.");
    }, [loadModelSnapshot, model.floorSlabs, model.meta, model.roofs, model.rooms.length, model.walls.length, projectId, setProjectId, setProjectKey, setWorkflowStep]);
    ...
    const handlePlanExport = useCallback(async (format) => {
    ...
    const handleAddLevel = useCallback((payload) => {
        addLevel({
            ...payload,
    ...
    const handleKeyShortcuts = useCallback((event) => {
        const key = event.key.toLowerCase();
        const editingViewport = activeViewport === "plan" || activeViewport === "networks" || activeViewport === "view3d";
    ...
    const handleValidationFix = useCallback((action) => {
        let changes = 0;
    ...
    const helpContent = (_jsxs("div", { className: "rounded-3xl border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] p-4 shadow-sm", children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-wide text-[color:var(--text-soft)]", children: "\u0413\u043E\u0440\u044F\u0447\u0438\u0435 \u043A\u043B\u0430\u0432\u0438\u0448\u0438" }), _jsxs("ul", { className: "mt-2 space-y-1 text-sm text-[color:var(--text-muted)]", children: [_jsx("li", { children: "`1` \u2014 \u0432\u044B\u0431\u043E\u0440, `2` \u2014 \u043F\u0440\u044F\u043C\u043E\u0443\u0433\u043E\u043B\u044C\u043D\u0430\u044F \u043A\u043E\u043C\u043D\u0430\u0442\u0430, `3` \u2014 \u043A\u043E\u043C\u043D\u0430\u0442\u0430 \u043F\u043E \u043A\u043E\u043D\u0442\u0443\u0440\u0443, `4` \u2014 \u0441\u0442\u0435\u043D\u0430" }), _jsx("li", { children: "`5` \u2014 \u043A\u0440\u044B\u0448\u0430, `6` \u2014 \u043F\u0435\u0440\u0435\u043A\u0440\u044B\u0442\u0438\u0435, `7` \u2014 \u0442\u0440\u0443\u0431\u043E\u043F\u0440\u043E\u0432\u043E\u0434, `8` \u2014 \u0432\u043E\u0437\u0434\u0443\u0445\u043E\u0432\u043E\u0434, `9` \u2014 \u043E\u0431\u043E\u0440\u0443\u0434\u043E\u0432\u0430\u043D\u0438\u0435, `A` \u2014 \u0434\u0430\u0442\u0447\u0438\u043A" }), _jsx("li", { children: "`Del` / `Backspace` \u2014 \u0443\u0434\u0430\u043B\u0438\u0442\u044C \u0432\u044B\u0431\u0440\u0430\u043D\u043D\u044B\u0435 \u043E\u0431\u044A\u0435\u043A\u0442\u044B" }), _jsx("li", { children: "`Ctrl+Z / Ctrl+Y` \u2014 \u043E\u0442\u043C\u0435\u043D\u0430 \u0438 \u043F\u043E\u0432\u0442\u043E\u0440" }), _jsx("li", { children: "`Esc` \u2014 \u043E\u0442\u043C\u0435\u043D\u0430 \u0442\u0435\u043A\u0443\u0449\u0435\u0439 \u043E\u043F\u0435\u0440\u0430\u0446\u0438\u0438 \u0438 \u0432\u043E\u0437\u0432\u0440\u0430\u0442 \u043A \u0431\u0430\u0437\u043E\u0432\u043E\u043C\u0443 \u0440\u0435\u0436\u0438\u043C\u0443 \u043A\u0443\u0440\u0441\u043E\u0440\u0430" }), _jsx("li", { children: "\u041A\u043E\u043B\u0435\u0441\u043E + \u043C\u043E\u0434\u0438\u0444\u0438\u043A\u0430\u0442\u043E\u0440\u044B \u0434\u043B\u044F \u043C\u0430\u0441\u0448\u0442\u0430\u0431\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F, \u043F\u0440\u0430\u0432\u0430\u044F \u043A\u043D\u043E\u043F\u043A\u0430 \u0434\u043B\u044F \u043F\u0430\u043D\u043E\u0440\u0430\u043C\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u044F" })] })] }));
    ...
    const transientVisualizationFrame = useMemo(() => transientVisualizationEnabled
    ...
    const thermalFieldBuildOptions = useMemo(() => buildThermalFieldOptions(thermalDisplay, thermalOptions, threeDThermalSourceFrame), [thermalDisplay, thermalOptions, threeDThermalSourceFrame]);
    const legacy3DThermalVisualizationActive = thermalDisplay.showFloorField || thermalDisplay.showWallSurfaces || thermalDisplay.showContours;
    const surfaceThermalVisualizationActive = thermalDisplay.showSurfaceField || thermalDisplay.showHeatSources || thermalDisplay.showThermalBridges;
    const thermalVisualizationActive = legacy3DThermalVisualizationActive ||
        surfaceThermalVisualizationActive ||
        thermalDisplay.showVolumeTint ||
        thermalDisplay.showTooltip;
    const thermalAnalysis = usePreparedThermalAnalysis(model, thermalFieldBuildOptions, threeDThermalSourceFrame, thermalVisualizationActive);
    ...
    const threeDTemperatureRange = useMemo(() => {
        ...
        return buildTemperatureLegendRange(threeDThermalField.minTemperatureC, threeDThermalField.maxTemperatureC, !thermalFrame);
    }, [effective3DThermalFrame, thermalFrame, threeDThermalField]);
    const planThermalFrame = thermalFrame ?? previewThermalFrame;
    const planHeatmapActive = thermalDisplay.showFloorField && engineeringModeUsesHeatmap(visualizationMode) && Boolean(planThermalFrame);
    ...
    const getPlanImageDataUrl = useCallback(async () => {
        const svg = canvasRef.current?.getSvgElement() ?? null;
        if (!svg) {
    ...
    const getView3DImageDataUrl = useCallback(() => {
    ...
    const handleCaptureViewSnapshot = useCallback(async (kind) => {
        const imageData = kind === "plan" ? await getPlanImageDataUrl() : getView3DImageDataUrl();
        if (!imageData) {
            return;
        }
        const defaultTitle = kind === "plan" ? `План ${new Date().toLocaleTimeString()}` : `3D ${new Date().toLocaleTimeString()}`;
    ...
    const handleRestoreProjectSnapshot = useCallback((id) => {
    ...
    const handleDeleteViewSnapshot = useCallback((id) => {
        removeViewSnapshot(id);
    }, [removeViewSnapshot]);
    const handleDeleteProjectSnapshot = useCallback((id) => {
        removeProjectSnapshot(id);
    }, [removeProjectSnapshot]);
    const handleCreateRoomFromLoop = useCallback((loopId) => {
    ...
    const handleSetWalls = useCallback((walls) => {
    ...
    const handleCreateNewProject = useCallback(() => {
        const hasGeometry = model.rooms.length > 0 ||
            model.walls.length > 0 ||
            (model.roofs?.length ?? 0) > 0 ||
            (model.floorSlabs?.length ?? 0) > 0;
    ...
    const handleViewportChange = useCallback((mode) => {
        if (mode === "results") {
            navigate("/results");
            return;
        }
    ...
    const handleViewerPatch = useCallback((patch) => {
    ...
    const handleVideoDemoStepChange = useCallback((stepId) => {
        const focusPlan = () => {
            window.requestAnimationFrame(() => {
                canvasRef.current?.zoomToFit();
            });
        };
        const focus3D = (mode = "fit") => {
            window.requestAnimationFrame(() => {
                if (mode === "top") {
    ...
    const applyEngineeringOverview = useCallback(() => {
        ...
        const alreadyApplied = activeViewport === "view3d" &&
    ...
    const handleSceneDebugPatch = useCallback((patch) => {
        setSceneDebug((prev) => ({ ...prev, ...patch }));
    }, []);
    const focusViewportTarget = useCallback((target) => {
        const desiredViewport = target?.kind === "pipe" || target?.kind === "duct" || target?.kind === "equipment" || target?.kind === "sensor"
            ? "networks"
            : "plan";
    ...
    const buildEditingActive = resolvedViewport === "plan" || resolvedViewport === "networks" || resolvedViewport === "view3d";
    const showBuildToolsPanel = buildEditingActive;
    const showHeaderQuickTools = buildEditingActive && toolsPlacement !== "left" && resolvedViewport !== "view3d";
    ...
    const workspaceGridColumns = panelSide === "left"
    ...
    const has3DGeometry = model.rooms.length > 0 ||
        model.walls.length > 0 ||
        (model.roofs?.length ?? 0) > 0 ||
        (model.floorSlabs?.length ?? 0) > 0 ||
    ...
    const orderedLevels = useMemo(() => [...model.levels].sort((left, right) => left.elevation_m - right.elevation_m ||
        left.height_m - right.height_m ||
        left.id.localeCompare(right.id)), [model.levels]);
    ...
    const handleWorkspaceContextMenu = useCallback((event) => {
    ...
    const handleWorkspaceLayoutTransitionEnd = useCallback((event) => {
    ...
    const handleThermalResult = useCallback((nextResult) => {
    ...
    const workspaceSidebarContent = buildEditingActive ? (_jsxs(_Fragment, { children: [showSidebarToolPalette ? (_jsx("div", { className: "ui-build-sidebar-block", children: _jsx(BuildToolPalette, { variant: "sidebar", ...toolPaletteProps }) })) : null, activeViewport === "networks" ? (_jsx("div", { className: "ui-build-sidebar-block", children: _jsx(EngineeringLibraryPanel, { currentTool: tool, selectedType: engineeringEquipmentPreset, selectedVariant: engineeringEquipmentVariant, onPickPipe: () => {
                    ...
                    }, onAddItpParallelDhw: handleAddItpParallelDhw }) })) : null, workspaceSidebarTab === "browser" ? (_jsxs(_Fragment, { children: [_jsx("div", { className: "ui-build-sidebar-block", children: _jsx(ProjectBrowser, { embedded: true, model: model, activeLevelId: activeLevelId, selection: selection, onSelect: (nextSelection) => {
                                setSelection(nextSelection);
                                if (nextSelection) {
                                    openWorkspaceSidebar("properties");
                                }
                            }, onLevelSelect: setActiveLevel, onFocusViewport: focusViewportTarget }) }), _jsxs("div", { className: "ui-build-sidebar-block", children: [_jsx(LevelsPanel, { levels: model.levels, activeLevelId: activeLevelId, onSelectLevel: (levelId) => {
                                    setActiveLevel(levelId);
                                    handleViewportChange("plan");
                                }, onAddLevel: handleAddLevel, onUpdateLevel: updateLevel }), engineeringModeUsesOverlay(visualizationMode) ? (_jsx(EngineeringLegendPanel, { styleMode: schematicStyle, compact: true })) : null] })] })) : workspaceSidebarTab === "properties" ? (_jsx(RightInspector, { model: model, selection: selection, tool: tool, equipmentPreset: equipmentPreset, engineeringEquipmentPreset: engineeringEquipmentPreset, pipePreset: pipePreset, wallPreset: wallPreset, windowPreset: windowPreset, doorPreset: doorPreset, roofPreset: roofPreset, slabPreset: slabPreset, activeLevelLabel: activeLevelName, neighbors: adjacency.neighbors, roomNames: roomNames, loops: loopMap, roomEnvelopes: roomEnvelopeMap, onUpdateRoom: handleUpdateRoom, onUpdateWall: updateWall, onUpdateRoof: updateRoof, onAddFloorSlab: addFloorSlab, onUpdateFloorSlab: updateFloorSlab, onUpdateDoor: updateDoor, onUpdateWindow: updateWindow, onUpdatePipe: updatePipe, onUpdateDuct: updateDuct, onUpdateEquipment: updateEquipment, onUpdateSensor: updateSensor, onUpdateEngineeringEquipment: updateEngineeringEquipment, onUpdateEngineeringPipe: updateEngineeringPipe, onRemoveSelection: removeAllSelected, onCreateRoomFromLoop: handleCreateRoomFromLoop })) : (_jsxs(_Fragment, { children: [_jsxs("section", { className: "ui-panel p-4 sm:p-5", children: [_jsxs("div", { className: "flex items-start justify-between gap-3", children: [_jsxs("div", { children: [_jsx("p", { className: "text-xs font-semibold uppercase tracking-wide text-[color:var(--text-soft)]", children: "\u041F\u0440\u043E\u0435\u043A\u0442" }), _jsx("p", { className: "mt-1 text-sm text-[color:var(--text-muted)]", children: "\u0424\u0430\u0439\u043B \u043F\u0440\u043E\u0435\u043A\u0442\u0430, \u0441\u043D\u0438\u043C\u043A\u0438 \u0438 \u0438\u043D\u0436\u0435\u043D\u0435\u0440\u043D\u0430\u044F \u0441\u0432\u043E\u0434\u043A\u0430 \u0431\u0435\u0437 \u043E\u0442\u0434\u0435\u043B\u044C\u043D\u043E\u0433\u043E \u0432\u0441\u043F\u043B\u044B\u0432\u0430\u044E\u0449\u0435\u0433\u043E \u0441\u043B\u043E\u044F." })] }), _jsx("button", { type: "button", onClick: handleNewModel, className: "ui-control rounded-[12px] border border-rose-200/80 bg-rose-50/85 px-3 py-2 text-xs font-semibold text-rose-700 transition hover:border-rose-300", children: "\u041D\u043E\u0432\u044B\u0439" })] }), _jsxs("div", { className: "mt-4 grid gap-2 sm:grid-cols-2", children: [_jsx("button", { type: "button", onClick: handleSaveSnapshot, className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)]", children: "\u0421\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C \u0441\u043D\u0438\u043C\u043E\u043A" }), _jsx("button", { type: "button", onClick: handleRestoreSnapshot, disabled: !hasSnapshot, className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)] disabled:cursor-not-allowed disabled:opacity-50", children: "\u0412\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u0438\u0442\u044C \u0441\u043D\u0438\u043C\u043E\u043A" }), _jsx("button", { type: "button", onClick: handleExport, className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)]", children: "\u042D\u043A\u0441\u043F\u043E\u0440\u0442 Twin JSON" }), _jsx("button", { type: "button", onClick: handleUseAsTwin, className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)]", children: "\u0418\u0441\u043F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u044C \u043A\u0430\u043A Twin" }), _jsx("button", { type: "button", onClick: () => handlePlanExport("png"), className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)]", children: "\u042D\u043A\u0441\u043F\u043E\u0440\u0442 PNG" }), _jsx("button", { type: "button", onClick: () => handlePlanExport("pdf"), className: "ui-control rounded-[14px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-2 text-left text-sm font-semibold text-[color:var(--text-muted)] transition hover:border-[color:var(--border-strong)]", children: "\u0421\u043A\u0430\u0447\u0430\u0442\u044C \u043F\u043B\u0430\u043D PDF" })] })] }), _jsxs("section", { className: "ui-panel p-4 sm:p-5", children: [_jsxs("div", { className: "mb-3", children: [_jsx("p", { className: "ui-build-section-title", children: "\u0412\u0435\u0440\u0441\u0438\u0438 \u0438 \u043A\u0430\u0434\u0440\u044B" }), _jsx("p", { className: "mt-1 text-sm text-[color:var(--text-muted)]", children: "\u0421\u043D\u0438\u043C\u043A\u0438 \u0432\u0438\u0434\u0430 \u0438 \u043F\u0440\u043E\u0435\u043A\u0442\u0430 \u0442\u0435\u043F\u0435\u0440\u044C \u043D\u0430\u0445\u043E\u0434\u044F\u0442\u0441\u044F \u0432 \u0441\u0442\u0430\u0431\u0438\u043B\u044C\u043D\u043E\u0439 \u0431\u043E\u043A\u043E\u0432\u043E\u0439 \u043E\u0431\u043B\u0430\u0441\u0442\u0438, \u0430 \u043D\u0435 \u0432 \u043F\u043B\u0430\u0432\u0430\u044E\u0449\u0435\u043C dropdown." })] }), _jsx(SnapshotsPanel, { viewSnapshots: viewSnapshots, projectSnapshots: projectSnapshots, onCapturePlan: () => handleCaptureViewSnapshot("plan"), onCapture3d: () => handleCaptureViewSnapshot("view3d"), onCreateProjectSnapshot: handleSaveProjectSnapshot, onDeleteViewSnapshot: handleDeleteViewSnapshot, onDeleteProjectSnapshot: handleDeleteProjectSnapshot, onRestoreProjectSnapshot: handleRestoreProjectSnapshot })] }), showHelp ? helpContent : null, _jsxs("section", { className: "ui-panel p-4 sm:p-5", children: [_jsx("div", { className: "mb-3", children: _jsx("p", { className: "ui-build-section-title", children: editorMode === "networks" ? "Сети и оборудование" : "Сводка модели" }) }), editorMode === "networks" ? (_jsx(NetworkSystemsPanel, { model: model, snapshot: smartSnapshot, onSetActiveScenario: setActiveScenario })) : (_jsx(ModelSummaryPanel, { model: model, snapshot: smartSnapshot, issuesCount: validationIssues.length, compact: true }))] }), editorMode === "architecture" ? (_jsxs("div", { ref: projectValidationSectionRef, className: "space-y-3", children: [_jsx(ValidationPanel, { issues: validationIssues, onFocus: (target) => {
                                    if (target) {
                                        setSelection(target);
                                        focusViewportTarget(target);
                                    }
                                }, onFix: handleValidationFix }), _jsx(RoomProblemsPanel, { problems: roomProblems, levels: model.levels })] })) : null] }))] })) : null;
    return (_jsxs("section", { className: "flex h-full min-h-0 min-w-0 flex-col gap-3 overflow-hidden bg-transparent px-2 pb-3 pt-2 text-[color:var(--text-base)]", children: [_jsxs("div", { className: "flex flex-wrap items-start justify-between gap-3 px-1", children: [_jsx("div", { className: "min-w-0", children: _jsx("h1", { className: "text-xl font-semibold tracking-tight text-[color:var(--text-base)]", children: "\u041C\u043E\u0434\u0435\u043B\u044C \u0437\u0434\u0430\u043D\u0438\u044F" }) }), _jsxs("div", { className: "flex flex-wrap gap-2", children: [_jsx("button", { type: "button", onClick: handleNewModel, className: "ui-btn-secondary px-4 py-2 text-sm", children: "\u041D\u043E\u0432\u044B\u0439" }), _jsx("button", { type: "button", onClick: handleImportIfc, className: "ui-btn-secondary px-4 py-2 text-sm", children: "\u0418\u043C\u043F\u043E\u0440\u0442 IFC" }), _jsx("button", { type: "button", onClick: handleOpenVideoDemo, className: "ui-btn-secondary px-4 py-2 text-sm", children: "\u041E\u0442\u043A\u0440\u044B\u0442\u044C \u0434\u0435\u043C\u043E-\u0434\u043E\u043C" }), _jsx("button", { type: "button", onClick: handleSaveProject, className: "ui-btn-secondary px-4 py-2 text-sm", children: "\u0421\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C" }), _jsx("button", { type: "button", onClick: handleOpenReportExport, className: "ui-btn-primary px-4 py-2 text-sm", children: "\u042D\u043A\u0441\u043F\u043E\u0440\u0442" })] })] }), _jsxs("header", { className: "ui-toolbar sticky top-0 z-30 flex min-h-[48px] shrink-0 flex-wrap items-center gap-2 px-3 py-2 sm:gap-3 sm:px-4", children: [_jsxs("div", { className: "flex min-w-0 flex-1 flex-wrap items-center gap-2 sm:gap-2.5", children: [_jsx(BuildToolbarGroup, { label: "\u0412\u0438\u0434", children: _jsxs("div", { className: "ui-segmented-control", role: "tablist", "aria-label": "\u0420\u0435\u0436\u0438\u043C \u043F\u0440\u043E\u0441\u043C\u043E\u0442\u0440\u0430", children: [_jsx(ViewModeTab, { icon: "plan2d", label: "\u041F\u043B\u0430\u043D", active: resolvedViewport === "plan", onClick: () => handleViewportChange("plan") }), _jsx(ViewModeTab, { icon: "networks", label: "\u0421\u0435\u0442\u0438", active: resolvedViewport === "networks", onClick: () => handleViewportChange("networks") }), _jsx(ViewModeTab, { icon: "view3d", label: "3D", active: resolvedViewport === "view3d", onClick: () => handleViewportChange("view3d") })] }) }), showHeaderQuickTools ? (_jsxs(_Fragment, { children: [_jsx(BuildToolbarDivider, {}), networksViewportActive ? (_jsxs(_Fragment, { children: [_jsxs(BuildToolbarGroup, { label: "\u0421\u0435\u0442\u0438", children: [_jsx(QuickToolButton, { icon: "cursor", label: "\u0412\u044B\u0431\u043E\u0440", title: "\u0412\u044B\u0434\u0435\u043B\u0435\u043D\u0438\u0435 \u0438 \u0440\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u0435", active: tool === "select", compact: true, onClick: () => handleToolChange("select") }), _jsx(QuickToolButton, { icon: "engineeringPipe", label: "\u0422\u0440\u0443\u0431\u0430", title: "\u0421\u043E\u0435\u0434\u0438\u043D\u0438\u0442\u044C \u043E\u0431\u043E\u0440\u0443\u0434\u043E\u0432\u0430\u043D\u0438\u0435 \u0442\u0440\u0443\u0431\u043E\u0439", active: tool === "engineeringPipe", compact: true, onClick: () => {
                                                            handleViewportChange("networks");
                                                            handleToolChange("engineeringPipe");
                                                        } })] }), _jsx(BuildToolbarDivider, {}), _jsxs(BuildToolbarGroup, { label: "\u042D\u043A\u0440\u0430\u043D", children: [_jsx(QuickToolButton, { icon: "fit", label: "\u0412\u043F\u0438\u0441\u0430\u0442\u044C", title: "\u0412\u043F\u0438\u0441\u0430\u0442\u044C \u043C\u043E\u0434\u0435\u043B\u044C \u0432 \u044D\u043A\u0440\u0430\u043D", compact: true, onClick: handlePaletteZoomToFit }), _jsx(QuickToolButton, { icon: "grid", label: "\u0421\u0435\u0442\u043A\u0430", title: "\u041F\u0440\u0438\u0432\u044F\u0437\u043A\u0430 \u043A \u0441\u0435\u0442\u043A\u0435 \u0438 \u0448\u0430\u0433 \u043F\u043E\u0441\u0442\u0440\u043E\u0435\u043D\u0438\u044F", active: snapEnabled, compact: true, onClick: toggleSnap }), _jsx(QuickToolButton, { icon: "orthogonal", label: "90\u00B0", title: "\u041E\u0440\u0442\u043E\u0433\u043E\u043D\u0430\u043B\u044C\u043D\u044B\u0435 \u0443\u0433\u043B\u044B 90\u00B0", active: orthogonalMode, compact: true, onClick: toggleOrthogonalMode })] })] })) : (_jsxs(_Fragment, { children: [_jsxs(BuildToolbarGroup, { label: "\u0420\u0438\u0441\u043E\u0432\u0430\u043D\u0438\u0435", children: [_jsx(QuickToolButton, { icon: "cursor", label: "\u0412\u044B\u0431\u043E\u0440", title: "\u0412\u044B\u0434\u0435\u043B\u0435\u043D\u0438\u0435 \u0438 \u0440\u0435\u0434\u0430\u043A\u0442\u0438\u0440\u043E\u0432\u0430\u043D\u0438\u0435", active: tool === "select", compact: true, onClick: () => handleToolChange("select") }), _jsx(QuickToolButton, { icon: "wall", label: "\u0421\u0442\u0435\u043D\u044B", title: "\u041D\u0430\u0440\u0438\u0441\u043E\u0432\u0430\u0442\u044C \u0441\u0442\u0435\u043D\u044B \u0438 \u043E\u0433\u0440\u0430\u0436\u0434\u0435\u043D\u0438\u044F", active: tool === "wall", compact: true, onClick: () => {
                                                            handleViewportChange("plan");
                                                            handleToolChange("wall");
                                                        } }), _jsx(QuickToolButton, { icon: "roomRect", label: "\u041F\u043E\u043C\u0435\u0449\u0435\u043D\u0438\u044F", title: "\u041F\u0440\u044F\u043C\u043E\u0443\u0433\u043E\u043B\u044C\u043D\u043E\u0435 \u043F\u043E\u043C\u0435\u0449\u0435\u043D\u0438\u0435", active: tool === "roomRect" || tool === "room", compact: true, onClick: () => {
                                                            handleViewportChange("plan");
                                                            handleToolChange("roomRect");
                                                        } }), _jsx(QuickToolButton, { icon: "window", label: "\u041E\u043A\u043D\u0430", title: "\u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u043E\u043A\u043D\u0430 \u0432 \u0432\u044B\u0431\u0440\u0430\u043D\u043D\u044B\u0435 \u0441\u0442\u0435\u043D\u044B", active: tool === "window", compact: true, onClick: () => {
                                                            handleViewportChange("plan");
                                                            handleToolChange("window");
                                                        } }), _jsx(QuickToolButton, { icon: "door", label: "\u0414\u0432\u0435\u0440\u0438", title: "\u0414\u043E\u0431\u0430\u0432\u0438\u0442\u044C \u0434\u0432\u0435\u0440\u0438 \u0432 \u0432\u044B\u0431\u0440\u0430\u043D\u043D\u044B\u0435 \u0441\u0442\u0435\u043D\u044B", active: tool === "door", compact: true, onClick: () => {
                                                            handleViewportChange("plan");
                                                            handleToolChange("door");
                                                        } }), _jsx(QuickToolButton, { icon: "roof", label: "\u041A\u0440\u044B\u0448\u0430", title: "\u041D\u0430\u0440\u0438\u0441\u043E\u0432\u0430\u0442\u044C \u043A\u0440\u043E\u0432\u043B\u044E", active: tool === "roof", compact: true, onClick: () => {
                                                            handleViewportChange("plan");
                                                            handleToolChange("roof");
                                                        } })] }), _jsx(BuildToolbarDivider, {}), _jsxs(BuildToolbarGroup, { label: "\u042D\u043A\u0440\u0430\u043D", children: [_jsx(QuickToolButton, { icon: "fit", label: "\u0412\u043F\u0438\u0441\u0430\u0442\u044C", title: "\u0412\u043F\u0438\u0441\u0430\u0442\u044C \u043C\u043E\u0434\u0435\u043B\u044C \u0432 \u044D\u043A\u0440\u0430\u043D", compact: true, onClick: handlePaletteZoomToFit }), _jsx(QuickToolButton, { icon: "grid", label: "\u0421\u0435\u0442\u043A\u0430", title: "\u041F\u0440\u0438\u0432\u044F\u0437\u043A\u0430 \u043A \u0441\u0435\u0442\u043A\u0435 \u0438 \u0448\u0430\u0433 \u043F\u043E\u0441\u0442\u0440\u043E\u0435\u043D\u0438\u044F", active: snapEnabled, compact: true, onClick: toggleSnap }), _jsx(QuickToolButton, { icon: "orthogonal", label: "90\u00B0", title: "\u041E\u0440\u0442\u043E\u0433\u043E\u043D\u0430\u043B\u044C\u043D\u044B\u0435 \u0443\u0433\u043B\u044B 90\u00B0", active: orthogonalMode, compact: true, onClick: toggleOrthogonalMode })] }), !planViewportActive ? (_jsxs(_Fragment, { children: [_jsx(BuildToolbarDivider, {}), _jsxs(BuildToolbarGroup, { label: "\u0418\u043D\u0436\u0435\u043D\u0435\u0440\u0438\u044F", children: [_jsx(QuickToolButton, { icon: "pipeSupply", label: "\u0422\u0440\u0430\u0441\u0441\u044B", title: "\u0420\u0438\u0441\u043E\u0432\u0430\u0442\u044C \u0438\u043D\u0436\u0435\u043D\u0435\u0440\u043D\u044B\u0435 \u0442\u0440\u0430\u0441\u0441\u044B", active: tool === "pipe" || tool === "duct" || tool === "engineeringPipe", compact: true, onClick: () => {
                                                                    handleViewportChange("networks");
                                                                    handleToolChange("pipe");
                                                                } }), _jsx(QuickToolButton, { icon: "radiator", label: "\u041E\u0431\u043E\u0440\u0443\u0434\u043E\u0432\u0430\u043D\u0438\u0435", title: "\u0420\u0430\u0434\u0438\u0430\u0442\u043E\u0440\u044B \u0438 \u0438\u043D\u0436\u0435\u043D\u0435\u0440\u043D\u043E\u0435 \u043E\u0431\u043E\u0440\u0443\u0434\u043E\u0432\u0430\u043D\u0438\u0435", active: tool === "equipment" || tool === "engineeringEquipment", compact: true, onClick: () => {
                                                                    handleViewportChange("networks");
                                                                    handleToolChange("equipment");
                                                                } })] })] })) : null, _jsx(BuildToolbarDivider, {}), _jsx(BuildToolbarGroup, { label: "\u0423\u0440\u043E\u0432\u043D\u0438", children: _jsx(QuickToolButton, { icon: "levels", label: "\u0423\u0440\u043E\u0432\u043D\u0438", title: "\u0423\u0440\u043E\u0432\u043D\u0438 \u0438 \u043F\u0440\u043E\u0432\u043E\u0434\u043D\u0438\u043A", active: isWorkspaceSidebarOpen && workspaceSidebarTab === "browser", compact: compactHeaderTools, onClick: () => handleWorkspaceSidebarToggle("browser") }) })] }))] })) : null] }), _jsxs("div", { className: "flex shrink-0 items-center justify-end gap-2", children: [levelQuickNav.length ? (_jsxs("div", { className: "ui-tabs-track flex items-center gap-1.5 p-1", children: [resolvedViewport === "view3d" && orderedLevels.length > 1 && (_jsxs("button", { type: "button", onClick: () => setShowAllLevels3D((prev) => !prev), className: `rounded-[var(--radius-control)] px-2.5 py-1.5 text-left text-[11px] font-semibold transition ${showAllLevels3D
                                            ...
                                            : "text-[color:var(--text-muted)] hover:bg-[color:var(--surface-elevated)] hover:text-[color:var(--text-base)]"}`, children: [_jsx("span", { className: "block leading-tight", children: "\u0412\u0441\u0435" }), _jsx("span", { className: `block text-[10px] ${showAllLevels3D ? "text-[color:var(--text-muted)]" : "text-[color:var(--text-soft)]"}`, children: "\u0443\u0440\u043E\u0432\u043D\u0438" })] })), levelQuickNav.map((level) => (_jsxs("button", { type: "button", onClick: () => {
                                            setActiveLevel(level.id);
                                            setShowAllLevels3D(false);
                                        }, className: `rounded-[var(--radius-control)] px-2.5 py-1.5 text-left text-[11px] font-semibold transition ${level.active && !showAllLevels3D
                                            ? "ui-control-active border border-[color:var(--accent-muted)] bg-[color:var(--accent-soft)] text-[color:var(--text-base)] shadow-[var(--shadow-control)]"
                                            : "text-[color:var(--text-muted)] hover:bg-[color:var(--surface-elevated)] hover:text-[color:var(--text-base)]"}`, children: [_jsx("span", { className: "block leading-tight", children: level.label }), _jsx("span", { className: `block text-[10px] ${level.active && !showAllLevels3D ? "text-[color:var(--text-muted)]" : "text-[color:var(--text-soft)]"}`, children: level.elevationLabel })] }, level.id)))] })) : null, buildEditingActive && resolvedViewport !== "view3d" ? (_jsx(_Fragment, { children: _jsx("button", { type: "button", onClick: () => handleWorkspaceSidebarToggle("properties"), className: `ui-control rounded-[12px] border px-3 py-1.5 text-xs font-semibold transition-all duration-200 ease-out active:scale-95 ${showWorkspaceSidebar && workspaceSidebarTab === "properties"
                                        ? "ui-control-active border-[color:var(--border-base)] bg-[color:var(--surface-elevated)] text-[color:var(--text-base)]"
                                        : "border-[color:var(--border-base)] bg-[color:var(--surface-elevated)] text-[color:var(--text-muted)] hover:border-[color:var(--border-strong)]"}`, children: "\u0421\u0432\u043E\u0439\u0441\u0442\u0432\u0430" }) })) : null, _jsx("button", { type: "button", onClick: handleSaveSnapshot, className: "ui-control hidden rounded-[12px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-1.5 text-sm font-semibold text-[color:var(--text-muted)] transition-all duration-200 hover:border-[color:var(--border-strong)]", children: "\u0421\u043E\u0445\u0440\u0430\u043D\u0438\u0442\u044C" }), _jsx("button", { type: "button", onClick: handleRestoreSnapshot, disabled: !hasSnapshot, className: "ui-control hidden rounded-[12px] border border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] px-3 py-1.5 text-sm font-semibold text-[color:var(--text-muted)] transition-all duration-200 hover:border-[color:var(--border-strong)] disabled:cursor-not-allowed disabled:opacity-50", children: "\u0412\u043E\u0441\u0441\u0442\u0430\u043D\u043E\u0432\u0438\u0442\u044C" })] })] }), _jsxs("div", { onContextMenu: handleWorkspaceContextMenu, onTransitionEnd: handleWorkspaceLayoutTransitionEnd, className: `relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden md:transition-[grid-template-columns,gap] md:duration-300 md:ease-[cubic-bezier(0.22,1,0.36,1)] ${showBuildToolsPanel
                    ? panelSide === "left"
                        ? `md:grid md:h-full md:max-h-full md:grid-rows-[minmax(0,1fr)] md:items-stretch ${showWorkspaceSidebar ? "md:gap-3" : "md:gap-0"}`
                        : `md:grid md:h-full md:max-h-full md:grid-rows-[minmax(0,1fr)] md:items-stretch ${showWorkspaceSidebar ? "md:gap-3" : "md:gap-0"}`
                    : ""}`, style: showBuildToolsPanel ? { gridTemplateColumns: workspaceGridColumns } : undefined, children: [showBuildToolsPanel ? (_jsxs("aside", { style: { ["--build-sidebar-width"]: `${workspaceSidebarWidth}px` }, className: `ui-panel max-md:absolute max-md:inset-y-0 max-md:z-20 relative flex h-full max-h-full min-h-0 min-w-0 max-md:w-[min(92vw,var(--build-sidebar-width))] max-md:max-w-[560px] flex-col overflow-hidden rounded-[var(--radius-panel)] border-[color:var(--border-soft)] bg-[color:var(--surface-muted)]/30 shadow-[inset_-1px_0_0_var(--border-soft)] transition-[opacity,transform,border-color] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] md:relative md:inset-auto md:z-0 md:w-full md:max-w-none ${panelSide === "left" ? "max-md:left-0 md:col-start-1" : "max-md:right-0 max-md:left-auto md:col-start-2 md:shadow-[inset_1px_0_0_var(--border-soft)]"} ${showWorkspaceSidebar ? "opacity-100 md:pointer-events-auto" : "pointer-events-none opacity-0 md:border-transparent"}`, children: [_jsx(WorkspaceSidebarResizeHandle, { panelSide: panelSide, enabled: showWorkspaceSidebar }), isWorkspaceSidebarContentVisible ? (_jsxs("div", { className: "absolute inset-0 flex min-h-0 min-w-0 flex-col overflow-hidden", children: [_jsxs("div", { className: "ui-build-sidebar-head", children: [_jsxs("div", { className: "ui-build-sidebar-head__main", children: [_jsx("span", { className: "ui-build-sidebar-head__icon", "aria-hidden": "true", children: _jsx(BuildToolIcon, { name: WORKSPACE_SIDEBAR_TAB_META[workspaceSidebarTab].icon, className: "h-4 w-4" }) }), _jsx("div", { className: "min-w-0", children: _jsx("p", { className: "ui-build-sidebar-head__title truncate", children: WORKSPACE_SIDEBAR_TAB_META[workspaceSidebarTab].title }) })] }), _jsx("div", { className: "ui-build-sidebar-head__actions", children: _jsx("button", { type: "button", onClick: () => {
                                                        setIsWorkspaceSidebarContentVisible(false);
                                                        setIsWorkspaceSidebarOpen(false);
                                                    }, className: "ui-build-sidebar-head__action", title: "\u0421\u043A\u0440\u044B\u0442\u044C \u043F\u0430\u043D\u0435\u043B\u044C (Esc)", "aria-label": "\u0421\u043A\u0440\u044B\u0442\u044C \u043F\u0430\u043D\u0435\u043B\u044C", children: _jsx("svg", { viewBox: "0 0 24 24", className: "h-4 w-4", "aria-hidden": "true", children: _jsx("path", { fill: "none", stroke: "currentColor", strokeWidth: "1.8", strokeLinecap: "round", strokeLinejoin: "round", d: "M15 6l-6 6 6 6" }) }) }) })] }), _jsx("div", { className: "ui-build-sidebar-scroll ui-scroll flex min-h-0 min-w-0 flex-1 basis-0 flex-col gap-3 overflow-x-hidden overflow-y-auto", children: workspaceSidebarContent })] })) : null] })) : null, _jsx("main", { className: `ui-panel relative z-10 grid min-h-0 min-w-0 grid-rows-[minmax(0,1fr)] overflow-hidden rounded-[var(--radius-panel)] ${showBuildToolsPanel && panelSide === "right" ? "md:col-start-1 md:row-start-1" : ""}`, children: _jsxs("div", { className: `min-h-0 min-w-0 max-w-full overflow-hidden ${resolvedViewport === "view3d" ? "bg-[color:var(--surface-muted)]" : "bg-[color:var(--surface-base)]"}`, children: [(resolvedViewport === "plan" || resolvedViewport === "networks") && (_jsxs("div", { className: "relative h-full min-h-0 min-w-0 max-w-full", children: [resolvedViewport === "networks" && (_jsxs("div", { className: "hidden rounded-2xl border border-[color:var(--info-border)] bg-[color:var(--info-bg)] px-4 py-3 text-sm text-[color:var(--text-muted)]", children: [_jsx("p", { className: "font-semibold text-[color:var(--text-base)]", children: "\u0420\u0435\u0436\u0438\u043C \u0438\u043D\u0436\u0435\u043D\u0435\u0440\u043D\u044B\u0445 \u0441\u0435\u0442\u0435\u0439" }), _jsx("p", { className: "mt-1", children: "\u0420\u0438\u0441\u0443\u0439\u0442\u0435 \u0442\u0440\u0443\u0431\u044B, \u0432\u043E\u0437\u0434\u0443\u0445\u043E\u0432\u043E\u0434\u044B, \u043E\u0431\u043E\u0440\u0443\u0434\u043E\u0432\u0430\u043D\u0438\u0435 \u0438 \u0434\u0430\u0442\u0447\u0438\u043A\u0438. \u0412\u0441\u0435 \u044D\u043B\u0435\u043C\u0435\u043D\u0442\u044B \u0432\u0438\u0434\u043D\u044B \u043D\u0430 \u043F\u043B\u0430\u043D\u0435 \u0438 \u0432 3D." })] })), _jsx(Canvas2D, { ref: canvasRef, layoutFitEpoch: canvasLayoutFitEpoch, model: model, activeLevelId: activeLevelId, tool: tool, selection: selection, multiSelection: multiSelection, gridStep: gridStep || DEFAULT_GRID_STEP, snapEnabled: snapEnabled, orthogonalMode: orthogonalMode, adjacencyOverlay: adjacencyOverlay, neighborEdges: adjacency.edges, centroids: adjacency.centroids, loops: roomLoops, loopDebug: loopDebugOverlay, smartSnapshot: smartSnapshot, showSmartOverlay: resolvedViewport === "networks", showNetworkFlowArrows: showNetworkFlowArrows, thermalFrame: planThermalFrame, transientFrame: transientVisualizationFrame, preparedThermalField: threeDThermalField, showHeatmap: showPlanHeatmap, heatmapIsPreview: planHeatmapIsPreview, engineeringVisualizationMode: visualizationMode, engineeringSchematicStyle: schematicStyle, showEngineeringOverlay: showEngineeringOverlay, exportMode: planExportMode, thermalBuildOptions: thermalFieldBuildOptions, onViewportPointerDown: handleCanvasWorkspacePointerDown, onToolChange: handleToolChange, onMultiSelectionChange: setMultiSelection, onSelectionChange: (sel) => {
                                                setMultiSelection([]);
                                                setSelection(sel);
                                                if (!sel) {
                                                    closeAllTransientPanels({ keepPanels: true });
                                                ...
                                                openWorkspaceSidebar("properties");
                                                if (sel) {
                                                    handleViewportChange(sel.kind === "pipe" ||
                                                        sel.kind === "duct" ||
                                                        sel.kind === "equipment" ||
                                                        sel.kind === "sensor" ||
                                            ...
                                            }, onAddRoom: handleAddRoom, onUpdateRoom: handleUpdateRoom, onSetWalls: handleSetWalls, onUpdateWall: updateWall, onAddRoof: addRoof, onUpdateRoof: updateRoof, onAddFloorSlab: addFloorSlab, onUpdateFloorSlab: updateFloorSlab, onAddDoor: addDoor, onUpdateDoor: updateDoor, onAddWindow: addWindow, onUpdateWindow: updateWindow, onAddPipe: addPipe, onUpdatePipe: updatePipe, onAddDuct: addDuct, onUpdateDuct: updateDuct, pipeType: pipePreset, heatingDisplayMode: heatingDisplayMode, equipmentType: equipmentPreset, engineeringEquipmentType: engineeringEquipmentPreset, engineeringEquipmentVariant: engineeringEquipmentVariant, wallPreset: wallPreset, windowPreset: windowPreset, doorPreset: doorPreset, roofPreset: roofPreset, slabPreset: slabPreset, onAddEquipment: addEquipment, onAddSensor: addSensor, onUpdateEquipment: updateEquipment, onUpdateSensor: updateSensor, onAddEngineeringEquipment: addEngineeringEquipment, onUpdateEngineeringEquipment: updateEngineeringEquipment, onAddEngineeringPipe: addEngineeringPipe, onUpdateEngineeringPipe: updateEngineeringPipe, onRemoveSelection: handleRemove }), _jsx("div", { className: "pointer-events-none absolute right-3 top-3 z-10 flex flex-col items-end gap-2", children: showSolarSim ? (_jsx("div", { className: "pointer-events-auto", children: _jsx(SolarTimeOverlay, { state: solarTimeState, onChange: (patch) => setSolarTimeState(patch), onClose: () => setShowSolarSim(false) }) })) : (_jsx("button", { type: "button", onClick: () => setShowSolarSim(true), title: "\u041F\u043E\u043A\u0430\u0437\u0430\u0442\u044C \u043F\u043E\u043B\u043E\u0436\u0435\u043D\u0438\u0435 \u0441\u043E\u043B\u043D\u0446\u0430", className: "pointer-events-auto flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white/90 text-base text-slate-500 shadow-sm hover:bg-slate-50", children: "\u2600\uFE0F" })) })] })), resolvedViewport === "view3d" && (_jsxs("div", { ref: view3dViewportRef, onContextMenu: (event) => event.preventDefault(), className: "relative h-full min-h-0 min-w-0 max-w-full overflow-hidden overscroll-contain touch-none", children: [_jsx(Build3DCanonicalPreview, { ref: view3dCanvasRef, model: model, activeLevelId: effective3DLevelId, selection: selection, viewer: viewer, thermalField: threeDThermalField, surfaceField: surfaceFieldResult, surfaceFieldMode: thermalDisplay.surfaceFieldMode, showSurfaceField: thermalDisplay.showSurfaceField, showHeatSources: thermalDisplay.showHeatSources, showThermalBridges: thermalDisplay.showThermalBridges, surfaceFieldOpacity: thermalDisplay.surfaceFieldOpacity, showTemperature: show3DFloorThermalField, showWallTemperature: show3DFloorThermalField, solarPosition: solarPosition, onSelect: (sel) => {
                                                setSelection(sel);
                                                setThreeDHoverInfo(null);
                                                if (!sel) {
                                                    return;
                                                ...
                                                openWorkspaceSidebar("properties");
                                            }, onHoverInfo: setThreeDHoverInfo, onCameraStateChange: setView3dCameraState, onSurfaceFieldDebug: setSurfaceFieldOverlayDebug }), _jsxs("div", { className: "pointer-events-none absolute inset-0 z-20 flex flex-col justify-between gap-3 p-3", children: [has3DGeometry || (thermalVisualizationActive && !threeDTimelineActive) ? (_jsxs("div", { className: "absolute left-3 top-3 z-10 flex max-w-[min(calc(100%-6rem),40rem)] items-start gap-2", children: [has3DGeometry ? (_jsx(OrientationHelper3D, { cameraState: view3dCameraState, levelName: showAllLevels3D ? "Все уровни" : (activeLevelName || "Все уровни") })) : null, thermalVisualizationActive && !threeDTimelineActive ? (_jsx("div", { className: "ui-overlay pointer-events-auto min-w-[18rem] flex-1 animate-fade-scale rounded-xl px-3 py-2", children: _jsxs("div", { className: "flex flex-wrap items-center gap-x-3 gap-y-2", children: [_jsx("p", { className: "shrink-0 text-[10px] font-semibold uppercase tracking-wide text-[color:var(--text-soft)]", children: "\u041D\u0430\u0440\u0443\u0436\u043D\u0430\u044F \u0442\u0435\u043C\u043F\u0435\u0440\u0430\u0442\u0443\u0440\u0430" }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx("button", { type: "button", onClick: () => setThermalDisplay((prev) => ({
                                                                                    ...prev,
                                                                                    outdoorTemperatureC: Math.max(prev.outdoorTemperatureC - 1, -50),
                                                                                })), className: "ui-control flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[color:var(--border-base)] bg-[color:var(--surface-elevated)] text-sm font-bold text-[color:var(--text-muted)] hover:bg-[color:var(--surface-muted)] hover:text-[color:var(--text-base)]", title: "\u22121\u00B0C", children: "\u2212" }), _jsxs("div", { className: "min-w-[3.25rem] text-center", children: [_jsxs("span", { className: "text-lg font-bold tabular-nums text-[color:var(--text-base)]", children: [thermalDisplay.outdoorTemperatureC > 0 ? "+" : "", thermalDisplay.outdoorTemperatureC.toFixed(0)] }), _jsx("span", { className: "ml-0.5 text-xs text-[color:var(--text-muted)]", children: "\u00B0C" })] }), _jsx("button", { type: "button", onClick: () => setThermalDisplay((prev) => ({
                                                                                    ...prev,
                                                                                    outdoorTemperatureC: Math.min(prev.outdoorTemperatureC + 1, 50),
                                                                                })), className: "ui-control flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-[color:var(--border-base)] bg-[color:var(--surface-elevated)] text-sm font-bold text-[color:var(--text-muted)] hover:bg-[color:var(--surface-muted)] hover:text-[color:var(--text-base)]", title: "+1\u00B0C", children: "+" })] }), _jsx("div", { className: "flex min-w-0 flex-1 flex-wrap items-center justify-end gap-1", children: [-25, -15, -5, 0, 5].map((preset) => (_jsxs("button", { type: "button", onClick: () => setThermalDisplay((prev) => ({ ...prev, outdoorTemperatureC: preset })), className: `ui-control rounded-full border px-2.5 py-0.5 text-[10px] font-semibold transition ${thermalDisplay.outdoorTemperatureC === preset
                                                                                ? "border-[color:var(--accent-muted)] bg-[color:var(--accent-soft)] text-[color:var(--text-base)]"
                                                                                : "border-[color:var(--border-soft)] bg-[color:var(--surface-elevated)] text-[color:var(--text-muted)] hover:border-[color:var(--border-base)] hover:text-[color:var(--text-base)]"}`, children: [preset > 0 ? "+" : "", preset, "\u00B0"] }, preset))) })] }) })) : null] })) : null, _jsx("div", { className: "pointer-events-auto absolute right-3 top-3 z-10", children: _jsx("button", { type: "button", onClick: () => setShowSolarSim((v) => !v), title: showSolarSim ? "Отключить симуляцию солнца" : "Показать положение солнца", className: `flex h-8 w-8 items-center justify-center rounded-lg border text-base shadow-sm transition-colors ${showSolarSim
                                                            ? "border-amber-400 bg-amber-50 text-amber-600"
                                                            : "border-slate-200 bg-white/90 text-slate-500 hover:bg-slate-50"}`, children: "\u2600\uFE0F" }) }), showSolarSim && (_jsx("div", { className: "pointer-events-auto absolute bottom-4 right-4 z-10", children: _jsx(SolarTimeOverlay, { state: solarTimeState, onChange: (patch) => setSolarTimeState(patch), onClose: () => setShowSolarSim(false) }) })), thermalDisplay.showLegend &&
                                                    !engineeringOverviewActive &&
                                                    ((thermalDisplay.showSurfaceField && threeDSurfaceLegend) ||
                                                        (!THREE_D_SAFE_MODE && !thermalDisplay.showSurfaceField && threeDTemperatureRange)) ? (_jsx("div", { className: "ui-overlay pointer-events-none absolute bottom-4 left-4 z-10 w-72 animate-fade-scale", children: _jsx(ThermalFieldLegend, { minC: thermalDisplay.showSurfaceField ? (threeDSurfaceLegend?.min ?? 0) : (threeDTemperatureRange?.min ?? 15), maxC: thermalDisplay.showSurfaceField ? (threeDSurfaceLegend?.max ?? 1) : (threeDTemperatureRange?.max ?? 30), avgC: thermalDisplay.showSurfaceField ? threeDSurfaceLegend?.average : undefined, title: thermalDisplay.showSurfaceField ? (threeDSurfaceLegend?.title ?? "Thermal Field") : "Temperature Scale", unitLabel: thermalDisplay.showSurfaceField ? (threeDSurfaceLegend?.unit ?? "°C") : "°C", condensationMode: thermalDisplay.showSurfaceField && thermalDisplay.surfaceFieldMode === "condensationRisk", source: thermalDisplay.showSurfaceField
                                                            ? hasVisibleSurfaceFieldData
                                                                ? undefined
                                                                : `Поле недоступно: ${surfaceFieldUiState.reason}`
                                                            : threeDTemperatureRange?.synthetic
                                                                ? "Предварительная оценка"
                                                                : "Расчётное поле", warnings: thermalDisplay.showSurfaceField && surfaceFieldResult?.warnings.length
                                                            ? surfaceFieldResult.warnings
                                                            : undefined }) })) : null] }), thermalDisplay.showTooltip && threeDHoverInfo ? (_jsxs("div", { className: "ui-overlay pointer-events-none absolute z-20 max-w-[280px] px-3 py-2 animate-fade-scale", style: threeDTooltipStyle ?? undefined, children: [_jsx("p", { className: "text-sm font-semibold text-[color:var(--text-base)]", children: threeDHoverInfo?.title ?? "" }), threeDHoverInfo?.subtitle ? _jsx("p", { className: "mt-0.5 text-xs text-[color:var(--text-soft)]", children: threeDHoverInfo?.subtitle }) : null, typeof threeDHoverInfo?.temperatureC === "number" ? (_jsxs("p", { className: "mt-2 text-xs font-semibold text-[color:var(--text-muted)]", children: [(threeDHoverInfo?.temperatureC ?? 0).toFixed(1), " \u00B0C"] })) : null, threeDHoverInfo?.details?.length ? (_jsx("div", { className: "mt-2 space-y-1 border-t border-[color:var(--border-soft)] pt-2 text-[11px] text-[color:var(--text-muted)]", children: (threeDHoverInfo?.details ?? []).map((detail) => (_jsxs("div", { className: "flex items-center justify-between gap-3", children: [_jsx("span", { children: detail.label }), _jsx("span", { className: "font-semibold text-[color:var(--text-muted)]", children: detail.value })] }, detail.label))) })) : null] })) : null] })), activeViewport === "results" && pathname === "/results" && (_jsx("div", { "data-testid": "workspace-results-viewport", className: "ui-scroll h-full max-h-full min-h-0 min-w-0 overflow-x-hidden overflow-y-auto overscroll-contain p-3 pb-6 xl:p-4 xl:pb-6", children: _jsxs("div", { className: "mx-auto w-full max-w-[1840px] min-w-0 space-y-[var(--space-section)]", children: [_jsx("section", { id: "thermal-results-section", className: "space-y-4", children: _jsx(ThermalSimulationPanel, { projectKey: projectKey, model: model, adjacency: adjacency, options: thermalOptions, onOptionsChange: setThermalOptions, onResult: handleThermalResult, onLoadDemoModel: loadModelSnapshot, onVideoDemoStepChange: handleVideoDemoStepChange, onDemoStepChange: (stepId) => {
                                                        if (THREE_D_SAFE_MODE) {
                                                            return;
                                                        }
                                                    ...
                                                    }, onTransientTimeIndexChange: setSelectedTransientTimeIndex, onTransientVisualizationEnabledChange: setTransientVisualizationEnabled, onRequestThermalModelView: handleOpenThermalModelView }) }), _jsx(ModelSummaryPanel, { model: model, snapshot: smartSnapshot, issuesCount: validationIssues.length }), _jsxs("section", { className: "space-y-4", children: [_jsx("p", { className: "ui-build-section-title", children: "\u041E\u0431\u043E\u043B\u043E\u0447\u043A\u0430" }), _jsx(EnvelopeDashboard, { metrics: envelopeMetrics })] }), _jsxs("section", { className: "space-y-4", children: [_jsx("p", { className: "ui-build-section-title", children: "\u0418\u043D\u0436\u0435\u043D\u0435\u0440\u043D\u044B\u0435 \u0441\u0435\u0442\u0438" }), _jsx(NetworkSystemsPanel, { model: model, snapshot: smartSnapshot, onSetActiveScenario: setActiveScenario })] }), _jsxs("section", { className: "space-y-4", children: [_jsx("p", { className: "ui-build-section-title", children: "\u0412\u0435\u0440\u043E\u044F\u0442\u043D\u043E\u0441\u0442\u044C \u0438 \u043A\u0430\u043B\u0438\u0431\u0440\u043E\u0432\u043A\u0430" }), _jsxs("div", { className: "grid min-w-0 gap-4 lg:grid-cols-2", children: [_jsx(EngineeringCallout, { variant: "info", title: "\u041A\u0430\u043B\u0438\u0431\u0440\u043E\u0432\u043A\u0430", children: _jsx("p", { children: "\u0411\u0435\u0440\u0451\u0442 12 \u043C\u0435\u0441\u044F\u0447\u043D\u044B\u0445 \u0437\u043D\u0430\u0447\u0435\u043D\u0438\u0439 \u043F\u043E\u0442\u0440\u0435\u0431\u043B\u0435\u043D\u0438\u044F \u0438 \u043F\u043E\u0434\u0431\u0438\u0440\u0430\u0435\u0442 \u043D\u0435\u0441\u043A\u043E\u043B\u044C\u043A\u043E \u043F\u0430\u0440\u0430\u043C\u0435\u0442\u0440\u043E\u0432 \u043C\u043E\u0434\u0435\u043B\u0438 \u0442\u0430\u043A, \u0447\u0442\u043E\u0431\u044B \u0440\u0430\u0441\u0447\u0451\u0442 \u0431\u044B\u043B \u0431\u043B\u0438\u0436\u0435 \u043A \u0444\u0430\u043A\u0442\u0443." }) }), _jsx(EngineeringCallout, { variant: "info", title: "\u0420\u0430\u0437\u0431\u0440\u043E\u0441 \u0440\u0435\u0437\u0443\u043B\u044C\u0442\u0430\u0442\u043E\u0432", children: _jsx("p", { children: "\u041C\u043E\u0434\u0435\u043B\u044C \u043C\u043D\u043E\u0433\u043E \u0440\u0430\u0437 \u043F\u0435\u0440\u0435\u0441\u0447\u0438\u0442\u044B\u0432\u0430\u0435\u0442\u0441\u044F \u0441 \u0440\u0430\u0437\u043D\u043E\u0439 \u043F\u043E\u0433\u043E\u0434\u043E\u0439, \u0438\u043D\u0444\u0438\u043B\u044C\u0442\u0440\u0430\u0446\u0438\u0435\u0439 \u0438 \u0432\u043D\u0443\u0442\u0440\u0435\u043D\u043D\u0438\u043C\u0438 \u043D\u0430\u0433\u0440\u0443\u0437\u043A\u0430\u043C\u0438, \u0447\u0442\u043E\u0431\u044B \u043E\u0446\u0435\u043D\u0438\u0442\u044C \u0440\u0438\u0441\u043A\u0438 \u0438 \u0432\u0435\u0440\u043E\u044F\u0442\u043D\u044B\u0439 \u0434\u0438\u0430\u043F\u0430\u0437\u043E\u043D \u043D\u0430\u0433\u0440\u0443\u0437\u043E\u043A." }) })] }), _jsxs("div", { className: "grid min-w-0 gap-4 2xl:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]", children: [_jsx(ThermalMonteCarloPanel, { model: model, adjacency: adjacency, options: thermalOptions }), _jsx(ThermalCalibrationPanel, { model: model, adjacency: adjacency, options: thermalOptions })] })] })] }) }))] }) })] })] }));
}
export function BuildPage() {
    return _jsx(BuildPageContent, {});
}
