...
import { buildDemoHouseModel, buildDemoScenarioConfig, buildDemoThermalResult, } from "../../demo/demoHouse";
import { DEMO_PROJECT_ID as CANONICAL_DEMO_PROJECT_ID, DEMO_PROJECT_NAME as CANONICAL_DEMO_PROJECT_NAME, DEMO_PROJECT_SOURCE as CANONICAL_DEMO_PROJECT_SOURCE, isCanonicalDemoProjectModel, } from "../../shared/utils/demoProject";
import { resolveModelDoorU_W_m2K, resolveModelWindowU_W_m2K, TYPICAL_PVC_WINDOW_G_VALUE, } from "../../shared/utils/openingThermalData";
...
export function buildDemoProjectModel() {
...
export function buildDemoProjectScenarioConfig(model) {
...
export function buildDemoReportInputs(model = buildDemoProjectModel(), scenarioConfig = buildDemoProjectScenarioConfig()) {
    ...
    const gsopValue = climate?.indoorTemperatureC != null &&
        climate.outdoorHeatingPeriodAverageC != null &&
        climate.heatingPeriodDurationDays != null
        ? gsop(climate.indoorTemperatureC, climate.outdoorHeatingPeriodAverageC, climate.heatingPeriodDurationDays)
        : null;
    return {
        ...
        modelNote: "Демонстрационная инженерно заполненная модель. Все derived-показатели считаются автоматически из BuildingModel и core diagnostics.",
        modelVersion: "demo-house",
        climateCity: climate?.city ?? defaults.city,
        outdoorDesignTemperatureC: climate?.outdoorDesignTemperatureC != null ? String(climate.outdoorDesignTemperatureC) : "",
        outdoorHeatingAverageC: climate?.outdoorHeatingPeriodAverageC != null ? String(climate.outdoorHeatingPeriodAverageC) : "",
        heatingDurationDays: climate?.heatingPeriodDurationDays != null ? String(climate.heatingPeriodDurationDays) : "",
        gsop: gsopValue != null ? gsopValue.toFixed(0) : "",
        humidityZone: climate?.humidityZone === "dry"
            ? "сухая"
            : climate?.humidityZone === "wet"
                ? "влажная"
                : "нормальная",
        operationCondition: model.thermalProtection?.operationCondition ?? defaults.operationCondition,
        indoorDesignTemperatureC: climate?.indoorTemperatureC != null ? String(climate.indoorTemperatureC) : String(defaults.indoorDesignTemperatureC),
        indoorRelativeHumidityPercent: climate?.indoorRelativeHumidityPercent != null
        ...
        operationNotes: "Часть эксплуатационных реквизитов и экономических параметров заданы как demo assumptions и должны уточняться на проектной стадии.",
        exportMode: "demo",
...
export function buildPreparedDemoProject() {
...
export function buildDemoSourceDataPreset() {
    return buildPreparedDemoProject();
}
export function applyDemoSourceDataPreset(model, scenarioConfig) {
    const nextModel = {
...
export function isDemoProjectModel(model) {
