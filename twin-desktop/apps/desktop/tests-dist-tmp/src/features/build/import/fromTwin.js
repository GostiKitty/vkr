...
function estimateRoomHeight(areaM2, volumeM3) {
...
function rectPolygon(x, y, width, depth) {
    return [
        { x, y },
        { x: x + width, y },
        { x: x + width, y: y + depth },
...
function buildWalls(levelId, polygon, heightM) {
...
export function buildModelFromTwin(twin, sourceProjectId) {
    ...
    model.meta = {
        ...
        note: "Редактируемая BIM-модель собрана автоматически по помещениям, потому что исходный IFC-движок не передает точную геометрию стен и проемов.",
    };
    return model;
}
