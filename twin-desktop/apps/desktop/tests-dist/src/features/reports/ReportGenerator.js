...
export function ReportGenerator() {
    ...
    return (<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <button type="button" onClick={handleGenerate} disabled={status === "working" || !twin} className={`rounded-xl px-5 py-2 text-sm font-semibold text-white transition ${status === "working" || !twin
      ...
      {message && (<p className={`mt-3 text-sm ${status === "error" ? "text-red-600" : status === "done" ? "text-emerald-600" : "text-slate-500"}`}>
          {message}
        </p>)}
      {!twin && (<p className="mt-3 text-xs text-slate-500">
