...
export function ResultsPanel({ projectId: _projectId }) {
    ...
    const tabContent = {
        ...
        probabilistic: (_jsx(MonteCarloResultsSection, { baseResult: visibleThermalResult, baseOptions: activeOptions, buildingModel: buildModel, climateLabel: climateLabel, monteCarloResult: visibleMonteCarloResult, onEditUncertainty: () => navigate("/uncertainty") })),
    ...
    return (_jsx(SummaryHighlightGrid, { className: "mt-1", items: [
            {
                ...
                value: (_jsxs("span", { className: "inline-flex items-center gap-1.5", children: [formatEnergy(energy, "кВт·ч"), _jsx(MetricInfoTooltip, { ...resultsMetricInfo.energy })] })),
                hint: "нтеграл нагрузки за период базового расчёта.",
            },
            {
                label: "Пиковая нагрузка",
                value: (_jsxs("span", { className: "inline-flex items-center gap-1.5", children: [peakLoad == null ? "—" : `${formatNumber(peakLoad, { maximumFractionDigits: 2 })} кВт`, _jsx(MetricInfoTooltip, { ...resultsMetricInfo.peakLoad })] })),
                hint: "Максимум суммарной мощности отопления.",
            },
            {
                label: "Часы дискомфорта",
                value: (_jsxs("span", { className: "inline-flex items-center gap-1.5", children: [discomfortHours == null ? "—" : `${formatNumber(discomfortHours, { maximumFractionDigits: 1 })} ч`, _jsx(MetricInfoTooltip, { ...resultsMetricInfo.discomfort })] })),
                hint: "Суммарно по зонам при отклонении ниже уставки.",
            },
            {
                label: "Медиана Monte Carlo",
                value: (_jsxs("span", { className: "inline-flex items-center gap-1.5", children: [formatEnergy(monteCarloP50, "кВт·ч"), _jsx(MetricInfoTooltip, { ...resultsMetricInfo.monteCarloP50 })] })),
                hint: "P50 по энергии при разбросе параметров.",
                tone: monteCarloP50 != null ? "info" : "neutral",
            },
            {
                label: "Риск недогрева",
                value: (_jsxs("span", { className: "inline-flex items-center gap-1.5", children: [formatPercentage(underheatingRisk), _jsx(MetricInfoTooltip, { ...resultsMetricInfo.underheatingRisk })] })),
...
function MapTab({ currentFrame, frames, graph, onOpenCalculation, onSelectRoom, onSliderChange, onTogglePlay, playing, selectedRoomId, timeIndex, timeLabel, }) {
