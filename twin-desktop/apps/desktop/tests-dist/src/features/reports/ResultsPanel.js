...
import { EmptyState, MetricInfoTooltip, WorkspaceInlineNotice, } from "../../shared/ui";
...
import { MISSING_BUILD_GEOMETRY_MESSAGE, runLocalThermalCalculation, } from "../runs/runLocalThermalCalculation";
...
export function ResultsPanel({ projectId: _projectId }) {
    ...
    const tabContent = {
        ...
        economy: _jsx(ResultsEconomyTab, { report: sp50Report }),
    ...
    return (_jsxs("div", { className: "flex min-h-0 flex-col gap-4", children: [showResultAlerts ? (_jsx(ResultsWorkspaceAlerts, { calculationError: calculationError, onDismissCalculationError: () => setCalculationError(null), onRecalculate: handleRunCalculation, showStaleWarning: thermalResultState === "stale" || monteCarloResultState === "stale" })) : null, _jsxs("div", { className: "ui-results-workspace", children: [_jsx(ResultsTabBar, { value: activeTab === "rooms" ? "overview" : activeTab, onChange: (tab) => setActiveTab(tab) }), _jsx("div", { className: "ui-results-workspace__panel ui-results-workspace__panel--flush", children: _jsx("div", { className: `ui-results-workspace__body animate-fade-slide ${isOverviewPanel ? "min-h-0" : "min-h-[20rem]"}`, children: tabContent[activeTab] }, activeTab) })] })] }));
}
function isMissingGeometryCalculationError(message) {
    return message === MISSING_BUILD_GEOMETRY_MESSAGE || message.includes("помещения и стены");
}
function ResultsWorkspaceAlerts({ calculationError, onDismissCalculationError, onRecalculate, showStaleWarning, }) {
    return (_jsxs("div", { className: "space-y-2", children: [calculationError ? (_jsx(WorkspaceInlineNotice, { message: isMissingGeometryCalculationError(calculationError)
                    ? "Нет модели для расчёта."
                    : calculationError, actions: isMissingGeometryCalculationError(calculationError) ? (_jsx("button", { type: "button", onClick: () => {
                        onDismissCalculationError();
                        navigate("/model");
                    }, className: "ui-btn-primary px-4 py-2 text-sm", children: "\u041E\u0442\u043A\u0440\u044B\u0442\u044C \u043C\u043E\u0434\u0435\u043B\u044C" })) : (_jsx("button", { type: "button", onClick: onDismissCalculationError, className: "ui-btn-secondary px-3 py-1.5 text-sm", children: "\u0417\u0430\u043A\u0440\u044B\u0442\u044C" })) })) : null, showStaleWarning ? (_jsx(WorkspaceInlineNotice, { message: "\u041C\u043E\u0434\u0435\u043B\u044C \u0438\u0437\u043C\u0435\u043D\u0438\u043B\u0430\u0441\u044C \u2014 \u043F\u0435\u0440\u0435\u0441\u0447\u0438\u0442\u0430\u0439\u0442\u0435.", actions: _jsx("button", { type: "button", onClick: onRecalculate, className: "ui-btn-primary px-4 py-2 text-sm", children: "\u041F\u0435\u0440\u0435\u0441\u0447\u0438\u0442\u0430\u0442\u044C" }) })) : null] }));
}
