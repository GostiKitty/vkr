...
export function generateMonteCarloAnalytics(twin) {
    ...
    const scenarioBuilder = (samples) => buildScenarioFromTwin(twin, samples, {
        durationHours: 24,
        timestepMinutes: 15,
    });
...
export function generateCalibrationSummary(twin) {
    if (!twin?.spaces?.length) {
        return null;
    }
...
export function buildScientificReportData(args) {
    const sections = [];
    ...
    sections.push({
        ...
        paragraphs: [
            `Building "${twin?.building?.name ?? "Unnamed"}" contains ${spaces.length || 0} thermal zones covering ${formatArea(totalArea)} with a combined volume of ${formatVolume(totalVolume)}.`,
            `Source schema: ${twin?.meta?.schema_version ?? "n/a"} · Imported from ${twin?.meta?.source ?? "unknown"} on ${twin?.meta?.created_at ?? "unspecified date"}.`,
        ],
    });
    const assumptionBullets = twin?.assumptions && Object.keys(twin.assumptions).length
    ...
    if (args.monteCarlo?.result) {
        ...
        sections.push({
            ...
            paragraphs: [
                `The ensemble of ${mc.runs} simulations yields peak heating demand μ=${formatNumber(mc.peakHeatingLoad.mean, { maximumFractionDigits: 1 })} kW with σ=${formatNumber(mc.peakHeatingLoad.stdDev, { maximumFractionDigits: 1 })} kW.`,
...
export function createScientificReportPdf(data) {
...
function buildPdfDocument(pages, width, height) {
    ...
    pages.forEach((content) => {
        ...
        addObject(pageId, `<< /Type /Page /Parent 2 0 R /Resources << /Font << /F1 ${fontId} 0 R >> >> /Contents ${contentId} 0 R >>`);
    });
...
function buildScenarioFromTwin(twin, samples, options) {
    ...
    const nodes = (twin.spaces ?? []).map((space, idx) => {
...
function buildObservedMonthlyEnergy(twin) {
    const spaces = twin.spaces ?? [];
    const totalArea = spaces.reduce((sum, space) => sum + (space.area_m2 ?? 0), 0);
...
function synthesizeMonthlyEnergy(twin, params) {
    const spaces = twin.spaces ?? [];
    const totalArea = spaces.reduce((sum, space) => sum + (space.area_m2 ?? 0), 0);
...
function deriveTemperatureSpan(frames) {
    if (!frames.length) {
        return { min: 0, max: 0 };
    }
    let min = Infinity;
...
function buildConclusions(args, span) {
    const bullets = [];
    if (args.monteCarlo?.result) {
        bullets.push(`Peak load exhibits ${formatNumber(args.monteCarlo.result.peakHeatingLoad.stdDev, {
            maximumFractionDigits: 1,
    ...
    if (args.calibration) {
        ...
        bullets.push(`Calibrated envelope factor = ${formatNumber(envelopeFactor, {
            maximumFractionDigits: 2,
        })} with residual RMSE ${formatNumber(args.calibration.rmse, { maximumFractionDigits: 2 })}.`);
    }
    bullets.push(`Simulated thermal range spans ${formatNumber(span.min, { maximumFractionDigits: 1 })}–${formatNumber(span.max, {
        maximumFractionDigits: 1,
