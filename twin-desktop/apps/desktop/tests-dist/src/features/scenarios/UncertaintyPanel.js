...
export function UncertaintyPanel() {
    ...
    const [evaluationMode, setEvaluationMode] = useState(config?.evaluationMode ?? "full-physics");
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="flex flex-col gap-1 text-sm font-medium text-slate-600">
            Количество прогонов
            <input type="number" min={10} max={10000} value={runs} onChange={(event) => setRuns(Number(event.target.value))} className="rounded-xl border border-slate-200 px-3 py-2 text-slate-900 shadow-inner focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"/>
          </label>

          <label className="flex flex-col gap-1 text-sm font-medium text-slate-600">
            Режим расчёта
            <select value={evaluationMode} onChange={(event) => setEvaluationMode(event.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-slate-900 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200">
        ...
        <button type="submit" className="mt-4 rounded-xl bg-slate-900 px-6 py-2 text-base font-semibold text-white shadow-sm transition hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-400">
          Сохранить настройки
        </button>

        {hasSaved && (<p className="mt-2 text-sm text-emerald-600">Конфигурация сохранена.</p>)}
      </div>
