...
export function SpaceDetails() {
    ...
    return (<div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      {loading ? (<div className="space-y-3">
          {Array.from({ length: 5 }).map((_, idx) => (<div key={idx} className="h-10 rounded-xl bg-slate-100 animate-pulse"/>))}
        </div>) : !space ? (<div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 p-6 text-center text-sm text-slate-500 transition-all duration-300">
          Выберите помещение, чтобы увидеть детали.
        </div>) : (<dl className="divide-y divide-slate-100 text-sm text-slate-700">
          <InfoRow label="ID" value={space.id}/>
          <InfoRow label="Название" value={space.name ?? "—"}/>
          ...
          <InfoRow label="Объем" value={formatVolume(space.volume_m3)}/>
          <InfoRow label="Температура" value={formatTemperature(currentTemp)}/>
        </dl>)}
    </div>);
}
function InfoRow({ label, value }) {
    return (<div className="flex items-center justify-between gap-6 py-3">
