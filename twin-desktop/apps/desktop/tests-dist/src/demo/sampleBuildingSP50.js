...
const DEMO_WINDOW_LAYERS = [{ materialId: "window_block", thickness_m: 0.04 }];
const DEMO_DOOR_LAYERS = [
...
export const sampleBuildingSP50 = {
...
export function demoRunSP50Calculation() {
    return buildDemoSp50RunResult(sampleBuildingSP50, "sampleBuildingSP50");
}
export function buildDemoSp50RunResult(model, scenarioId = "sampleBuildingSP50") {
...
export function exportDemoSp50ReportToJson(result) {
    const payload = {
...
function buildEngineeringConclusion(report, topLosses) {
    const topLabels = topLosses.map((entry) => entry.label.toLowerCase());
    const contributorsText = topLabels.length >= 2 ? `${topLabels.slice(0, 2).join(" и ")}` : topLabels[0] ?? "ограждающие конструкции";
...
function formatMetric(value, maximumFractionDigits) {
