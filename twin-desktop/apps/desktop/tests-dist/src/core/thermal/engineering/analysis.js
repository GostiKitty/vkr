...
export function runEngineeringThermalAnalysis(model, adjacency, options, simulationResult) {
    void adjacency;
    ...
    const { physics, rooms, envelope, gains, balance, targetTemperatureC, outdoorTemperatureC, fieldRoomTemperaturesC, fastFieldModel, } = evaluation;
    const fastField = fastFieldModel ? buildFastFieldSummary(fastFieldModel, engineeringOptions) : null;
    const detailedField = buildDetailedFieldResult(model, physics, engineeringOptions, outdoorTemperatureC, fieldRoomTemperaturesC);
    ...
    if (sp50?.energy.usesPlaceholderInputs) {
        validation.push({
            ...
            recommendation: "Задайте климат (СП 131), ACH инфильтрации/вентиляции в сценарии или energyVentilation в thermalProtection здания.",
        });
    }
...
function resolveFieldRoomTemperatures(roomBalances, simulationResult, analysisTimeHours, fallbackTemperatureC) {
    const fromSimulation = simulationResult?.timeline.reduce((best, frame) => {
        if (!best) {
            return frame;
        }
    ...
    const values = Object.values(temperatures).filter((value) => Number.isFinite(value));
...
function buildSolarGainFactor(options) {
    return (options.solarIrradianceW_m2 * options.solarTransmittance * options.solarShadingFactor) / 115;
}
function evaluateEngineeringState(model, options, engineeringOptions, simulationResult, overrides = {}) {
    ...
    const fastFieldModel = overrides.includeField
        ...
        }, renderGeometry, physics)
        : null;
    const rooms = buildRoomBalances(model, physics);
    const envelope = buildEnvelopeResults(model, physics, resolvedOptions, targetTemperatureC, outdoorTemperatureC, overrides.wallResistanceDelta_m2K_W ?? 0);
    const gains = buildGainResults(model, physics);
    const balance = buildBalanceSummary(model, rooms, envelope, gains, resolvedOptions, targetTemperatureC, outdoorTemperatureC);
    return {
...
function mergeEngineeringOptions(options) {
    return {
...
function resolveTargetTemperature(options, analysisTimeHours, explicitTarget) {
    if (Number.isFinite(explicitTarget)) {
        return explicitTarget;
    }
    const hour = ((analysisTimeHours % 24) + 24) % 24;
    const isDay = options.setpoints.dayStartHour < options.setpoints.nightStartHour
...
function buildRoomBalances(model, physics) {
    return Array.from(physics.roomBalances.values())
        ...
        passiveBalanceW: balance.lightingGainW +
...
function buildEnvelopeResults(model, physics, options, targetTemperatureC, outdoorTemperatureC, wallResistanceDelta_m2K_W = 0) {
    const externalResults = physics.surfaces
        ...
        const totalResistance = options.surfaceResistances.internal_m2K_W +
            layerResistance +
    ...
    const roomBased = Array.from(physics.roomBalances.values()).flatMap((balance) => {
...
function buildGainResults(model, physics) {
    const result = [];
...
function buildBalanceSummary(model, rooms, envelope, gains, options, targetTemperatureC, outdoorTemperatureC) {
    ...
    const effectiveCapacitance_J_K = rooms.reduce((sum, room) => sum + effectiveAirCapacitance(room.volumeM3, options.effectiveMassFactor), 0);
...
*/
function buildScenarioResults(model, options, engineeringOptions, baselineState, simulationResult) {
    const initialTemperatureC = simulationResult?.timeline[0]
        ? averageRoomTemperatures(simulationResult.timeline[0].rooms)
    ...
    const baselineSummary = summarizeScenario(baselinePoints, options.timestepMinutes ?? 10);
    return variants.map((variant) => {
        const state = variant.id === "baseline"
            ? baselineState
        ...
        const outdoorShiftC = (variant.overrides.outdoorTemperatureC ?? baselineState.outdoorTemperatureC) - baselineState.outdoorTemperatureC;
...
function buildEquivalentForecastContext(state, options, initialTemperatureC) {
    return {
...
function simulateEquivalentForecast(context, outdoorShiftC) {
    const timestepMinutes = Math.max(1, context.options.timestepMinutes ?? 10);
    const timestepSeconds = timestepMinutes * 60;
    const timestepHours = timestepMinutes / 60;
    const durationHours = context.options.duration === "7d" ? 24 * 7 : 24;
    ...
    for (let step = 0; step <= totalSteps; step += 1) {
        ...
        const hourOfDay = ((timeHours % 24) + 24) % 24;
        const isDay = context.options.setpoints.dayStartHour < context.options.setpoints.nightStartHour
            ? hourOfDay >= context.options.setpoints.dayStartHour && hourOfDay < context.options.setpoints.nightStartHour
            : hourOfDay >= context.options.setpoints.dayStartHour || hourOfDay < context.options.setpoints.nightStartHour;
        const setpointTemperatureC = isDay ? context.options.setpoints.day : context.options.setpoints.night;
        const passiveGainsW = isDay ? context.passiveDayGainW : context.passiveNightGainW;
        const outdoorTemperatureC = context.options.outdoor.baseC +
            (context.options.outdoor.amplitudeC ?? 0) * Math.sin(((timeHours + (context.options.outdoor.phaseShiftHours ?? 0)) / 24) * Math.PI * 2) +
            (context.options.outdoor.seasonalOffsetC ?? 0) +
            outdoorShiftC;
        const lossPowerW = context.ua_W_K * (indoorTemperatureC - outdoorTemperatureC);
        const requiredHeatingW = Math.max(0, ((setpointTemperatureC - indoorTemperatureC) * context.effectiveCapacitance_J_K) / timestepSeconds + lossPowerW - passiveGainsW);
...
function summarizeScenario(points, timestepMinutes) {
    const temperatures = points.map((point) => point.indoorTemperatureC);
    const warmupPoint = points.find((point) => point.indoorTemperatureC >= point.setpointTemperatureC - 0.2) ?? null;
...
function buildHydronicAssessment(model, requiredPowerW, installedHeatingCapacityW) {
    ...
    const requiredMassFlowKgS = hydronic.deltaT === null
        ? null
        : calculateRequiredHydronicMassFlow(requiredPowerW, hydronic.deltaT, DEFAULT_WATER_HEAT_CAPACITY_J_KG_K, 1);
    const requiredVolumeFlowM3H = hydronic.deltaT === null
        ? null
        : calculateRequiredHydronicVolumeFlowM3H(requiredPowerW, hydronic.deltaT, DEFAULT_WATER_DENSITY_KG_M3, DEFAULT_WATER_HEAT_CAPACITY_J_KG_K, 1);
    const powerDeficitW = hydronic.availablePowerW === null ? null : Math.max(0, requiredPowerW - hydronic.availablePowerW);
    const formatNullable = (value, unit, digits = 2) => value === null ? "не задано" : `${roundNumber(value, digits)} ${unit}`;
    return {
        mode: "derived_only",
        requiredPowerW,
...
function buildSensitivityResults(model, options, engineeringOptions, baselineState) {
    const wallEntries = baselineState.envelope.filter((entry) => entry.kind === "wall");
    const weightedWallResistance = wallEntries.reduce((sum, entry) => sum + entry.totalResistance_m2K_W * entry.areaM2, 0) /
        Math.max(wallEntries.reduce((sum, entry) => sum + entry.areaM2, 0), 1);
    ...
    const maxImpact = Math.max(...items.map((entry) => Math.abs(entry.state.balance.requiredHeatingW - baselineState.balance.requiredHeatingW)), 1);
    return items
        .map((entry) => {
...
function buildInputSources(model, options, envelope) {
    return [
        {
...
function buildConfidenceSummary(inputs, validation) {
