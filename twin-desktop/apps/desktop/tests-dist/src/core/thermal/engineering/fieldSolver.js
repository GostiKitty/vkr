...
import { AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, DEFAULT_EFFECTIVE_AIR_DIFFUSIVITY_M2_S, } from "./constants";
...
export function buildDetailedFieldResult(model, physics, options, outdoorTemperatureC, roomTemperatureAnchorsC = {}) {
    ...
    const roomBalanceById = new Map(Array.from(physics.roomBalances.values())
    ...
    const surfaceTemperatureMap = new Map();
    surfacesByLevel.forEach((surface) => {
        surfaceTemperatureMap.set(surface.wallId, buildSurfaceBoundaryTemperatures(surface, roomBalanceById, outdoorTemperatureC, options));
    });
    const grid = [];
    ...
    for (let row = 0; row < rows; row += 1) {
        ...
        for (let col = 0; col < cols; col += 1) {
            ...
            const baseTemperatureC = roomTemperatureAnchorsC[room.roomId] ??
            ...
            const boundaryCandidates = relatedSurfaces
                ...
                const boundaryTemperatureC = surface.windowAreaM2 > 0 && distance <= WINDOW_BAND_M
                    ? windowTemperature
    ...
    const effectiveConductivity_W_mK = AIR_DENSITY_KG_M3 * AIR_HEAT_CAPACITY_J_KG_K * DEFAULT_EFFECTIVE_AIR_DIFFUSIVITY_M2_S;
    let iterations = 0;
    ...
    for (iterations = 0; iterations < options.grid.maxIterations; iterations += 1) {
        ...
        for (let row = 0; row < rows; row += 1) {
            for (let col = 0; col < cols; col += 1) {
                ...
                const roomAnchorC = roomTemperatureAnchorsC[cell.roomId] ??
    ...
    const cells = grid.flatMap((row, rowIndex) => row.flatMap((cell, colIndex) => cell.valid && cell.roomId
...
export function buildFastFieldSummary(fastField, options) {
    ...
    const cells = fastField.rooms
        ...
        .flatMap((room) => room.samplePoints.map((point, index) => ({
        row: 0,
        col: index,
        x: point.x,
        y: point.y,
...
export function buildComfortSummary(field, targetTemperatureC) {
...
function buildSurfaceBoundaryTemperatures(surface, roomBalanceById, outdoorTemperatureC, options) {
    const positiveRoomTemperatureC = surface.positiveRoomId ? roomBalanceById.get(surface.positiveRoomId)?.airTemperatureC ?? outdoorTemperatureC : outdoorTemperatureC;
    const negativeRoomTemperatureC = surface.negativeRoomId ? roomBalanceById.get(surface.negativeRoomId)?.airTemperatureC ?? outdoorTemperatureC : outdoorTemperatureC;
    ...
    const wallResistanceExternal = options.surfaceResistances.internal_m2K_W + layerResistance + options.surfaceResistances.external_m2K_W;
    const wallResistanceInternal = options.surfaceResistances.internal_m2K_W * 2 + layerResistance;
    const wallByRoomId = {};
    const windowByRoomId = {};
    const doorByRoomId = {};
    if (surface.kind === "external") {
        ...
        const wallSurfaceTemperatureC = computeExternalSurfaceTemperature(roomTemperatureC, outdoorTemperatureC, wallResistanceExternal, options.surfaceResistances.internal_m2K_W);
        const windowSurfaceTemperatureC = computeExternalSurfaceTemperature(roomTemperatureC, outdoorTemperatureC, 1 / Math.max(options.windowU_W_m2K, 1e-6), options.surfaceResistances.internal_m2K_W);
        const doorSurfaceTemperatureC = computeExternalSurfaceTemperature(roomTemperatureC, outdoorTemperatureC, 1 / Math.max(options.doorU_W_m2K, 1e-6), options.surfaceResistances.internal_m2K_W);
        if (roomId) {
    ...
    if (surface.positiveRoomId && surface.negativeRoomId) {
        const [positiveSurfaceTemperatureC, negativeSurfaceTemperatureC] = computeInternalSurfaceTemperatures(positiveRoomTemperatureC, negativeRoomTemperatureC, wallResistanceInternal, options.surfaceResistances.internal_m2K_W);
...
function computeExternalSurfaceTemperature(indoorTemperatureC, outdoorTemperatureC, totalResistance_m2K_W, internalSurfaceResistance_m2K_W) {
...
function computeInternalSurfaceTemperatures(positiveRoomTemperatureC, negativeRoomTemperatureC, totalResistance_m2K_W, internalSurfaceResistance_m2K_W) {
...
function buildDistributedSources(model, roomById, roomBalanceById, targetLevelId) {
    const sources = [];
    ...
    roomBalanceById.forEach((balance, roomId) => {
        ...
        distributeEquipmentPower(sources, hostedHeating, balance.heatingDeliveredW, roomId, targetLevelId, `${roomId}:heating`, 0.7);
        ...
        distributeEquipmentPower(sources, hostedEquipment, balance.equipmentGainW, roomId, targetLevelId, `${roomId}:equipment`, 0.75);
        const solarSurfaces = model.walls
            .filter((wall) => wall.levelId === targetLevelId)
            .map((wall) => ({
            wall,
            roomHosted: polygonContainsPoint({ x: (wall.a.x + wall.b.x) / 2, y: (wall.a.y + wall.b.y) / 2 }, room.polygon),
        }))
...
function distributeEquipmentPower(target, equipment, totalPowerW, roomId, levelId, idPrefix, radiusM) {
    if (totalPowerW <= 0) {
        return;
    }
    if (!equipment.length) {
...
function resolveNominalPower(item) {
...
function addUniformSource(cells, powerW) {
...
function addLocalizedSource(cells, powerW, position, radiusM) {
    ...
    if (!weightedCells.length) {
        ...
        }, { cell: null, distanceM: Number.POSITIVE_INFINITY }).cell;
...
function resolveNeighborTemperature(grid, row, col, cell) {
...
function pickCharacteristicCells(cells, category) {
    const meanValue = average(cells.map((cell) => cell.temperatureC));
    const source = category === "occupied"
...
function mapBand(grid, predicate, category) {
    return grid
        .flatMap((row) => row)
