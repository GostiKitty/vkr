...
export function analyzeEngineeringModel(model, options = {}) {
    ...
    model.pipes.forEach((pipe) => {
        ...
        if (velocity_m_s < config.minPipeVelocity_m_s || velocity_m_s > config.maxPipeVelocity_m_s) {
            ...
            recommendations.push({
                ...
                action: velocity_m_s > config.maxPipeVelocity_m_s
    ...
    return {
        ...
        summary: {
            ...
            averagePipePressurePa: roundValue(model.pipes.length ? model.pipes.reduce((sum, pipe) => sum + pipe.pressurePa, 0) / model.pipes.length : 0),
            warningCount: warnings.length,
        },
    };
}
function buildPressureSensorWarning(sensor, warningId, options) {
    return {
        id: warningId,
        code: "abnormal_pressure",
        severity: "warning",
