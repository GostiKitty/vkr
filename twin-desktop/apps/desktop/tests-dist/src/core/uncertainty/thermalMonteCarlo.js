...
import { runThermalSimulation, } from "../thermal/solver";
export const THERMAL_UNCERTAINTY_DEFINITIONS = [
    {
        id: "outdoorBiasC",
...
const MONTE_CARLO_THERMAL_SCOPE_RU = "Анализ неопределённости: многократный запуск той же зональной RC-модели с вариацией входных параметров (климат, ACH, внутренние тепловыделения, уставки). Результат — распределение показателей RC, а не нормативное заключение по СП 50 и не полевое CFD.";
const DEFAULT_BINS = 24;
const DEFAULT_VAR_LEVEL = 0.95;
const DEFAULT_SEED = 42;
export const THERMAL_MONTE_CARLO_UNDERHEAT_THRESHOLD_C = 20;
export const THERMAL_MONTE_CARLO_SENSITIVITY_METHOD_LABEL_RU = "Нормированная абсолютная корреляция Пирсона с выбранной целевой метрикой";
export const THERMAL_MONTE_CARLO_TARGET_METRICS = [
    {
        id: "totalEnergyKWh",
...
function collectThermalMonteCarloSamplesSync(options, adjacency) {
    ...
    const samples = [];
    const peakLoadsKW = [];
    const totalEnergiesKWh = [];
    const discomfortHours = [];
    const minimumIndoorTemperaturesC = [];
    const roomUnderheatRisk = [];
    const roomRiskAccumulator = {};
...
function finalizeThermalMonteCarloResult(collected, options) {
    ...
    return {
        ...
        underheatingBelow20CProbability: options.runs > 0 ? collected.underheatingBelow20Count / options.runs : undefined,
...
export function runThermalMonteCarlo(options) {
    if (options.runs <= 0) {
...
export async function runThermalMonteCarloAsync(options) {
    if (options.runs <= 0) {
        throw new Error("Количество прогонов Monte Carlo должно быть больше нуля");
    }
    const runs = Math.min(options.runs, THERMAL_MONTE_CARLO_MAX_RUNS);
...
function extractMetrics(result, durationDays) {
...
function sampleUncertainty(definitions, uniforms) {
    const sample = {};
...
function applySampleToOptions(baseOptions, sample) {
...
function summarizeDistribution(values, bins, varLevel) {
    if (!values.length) {
        return {
            mean: 0,
            stdDev: 0,
...
function updateRoomRiskAccumulator(accumulator, roomMetrics) {
...
function buildRoomRiskSummary(accumulator) {
...
export function getThermalMonteCarloTargetMetricValues(result, targetMetric) {
    switch (targetMetric) {
        case "peakLoadKW":
...
export function buildSensitivityFactors(samples, metricValues, baseOptions, targetMetric) {
    if (!samples.length || samples.length !== metricValues.length) {
        return [];
    }
    const normalizedMetricValues = targetMetric === "roomUnderheatRisk" ? metricValues.map((value) => value / 100) : metricValues;
    const occupancy = baseOptions.occupancy ?? {
        dayFraction: 1,
        nightFraction: 0.2,
    };
    const averageInternalGains = (baseOptions.internalGains.dayGain_W_m2 + baseOptions.internalGains.nightGain_W_m2) / 2;
    ...
    const rows = [
        {
            id: "infiltrationACH",
            label: "Инфильтрация",
            score: Math.abs(pearsonCorrelation(samples.map((sample) => Math.max(0.02, infiltrationBase * sample.infiltrationMultiplier)), normalizedMetricValues)),
        },
        {
        ...
        },
        {
            ...
            score: Math.abs(pearsonCorrelation(samples.map((sample) => baseOptions.setpoints.day + sample.setpointOffsetC), normalizedMetricValues)),
        },
        {
            id: "nightSetpoint",
            label: "Ночная уставка",
            score: Math.abs(pearsonCorrelation(samples.map((sample) => baseOptions.setpoints.night + sample.setpointOffsetC), normalizedMetricValues)),
        },
        {
            id: "internalGains",
            label: "Внутренние теплопоступления",
            score: Math.abs(pearsonCorrelation(samples.map((sample) => averageInternalGains * sample.internalGainMultiplier), normalizedMetricValues)),
        },
        {
            id: "occupancy",
            label: "Коэффициент занятости",
            score: Math.abs(pearsonCorrelation(samples.map((sample) => clamp(averageOccupancy * sample.occupancyMultiplier, 0, 1)), normalizedMetricValues)),
        },
    ];
...
function buildCdf(sortedValues, maxPoints) {
...
function sampleFrom(distribution, u) {
    ...
    switch (distribution.kind) {
        ...
        default: {
            ...
            throw new Error(`Unknown distribution ${exhaustive.kind ?? "unknown"}`);
        }
    }
}
function inverseTriangular(u, min, mode, max) {
...
function inverseRegularizedBeta(u, alpha, beta) {
    if (u <= 0) {
        return 0;
    }
    if (u >= 1) {
...
function initialBetaGuess(u, alpha, beta) {
    if (u === 0) {
        return 0;
    }
    if (u === 1) {
...
function buildCorrelator(dim, correlationMatrix) {
    if (!correlationMatrix) {
        return (rng) => Array.from({ length: dim }, () => standardNormalCdf(rng.nextGaussian()));
    }
...
function inverseStandardNormal(p) {
    ...
    return ((((((a[0] * rMid + a[1]) * rMid + a[2]) * rMid + a[3]) * rMid + a[4]) * rMid + a[5]) * qMid /
