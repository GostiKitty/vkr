...
import { SENSOR_TYPE_LABELS, isHeatingPipeType, normalizeFlowDirection, resolvePipeColor, resolvePipeFlowRole, } from "../../entities/networks/types";
...
export function buildSmartModelSnapshot(model, thermalResult, timestamp = Date.now()) {
    ...
    const equipmentConnections = new Map();
    const pipeConnections = new Map();
    const ductConnections = new Map();
    const equipmentById = new Map(model.equipment.map((item) => [item.id, item]));
    model.pipes.forEach((pipe) => pipeConnections.set(pipe.id, uniqueIds(pipe.connectedEquipmentIds.filter((equipmentId) => {
        const equipment = equipmentById.get(equipmentId);
        return equipment ? canConnectEquipmentToNetwork(equipment, pipe).compatible : false;
    }))));
    model.ducts.forEach((duct) => ductConnections.set(duct.id, uniqueIds(duct.connectedEquipmentIds.filter((equipmentId) => {
        const equipment = equipmentById.get(equipmentId);
...
function buildPipeState(pipe, connectedEquipmentIds) {
...
function buildDuctState(duct, connectedEquipmentIds) {
    return {
        networkId: duct.id,
        kind: "duct",
...
function buildSmartEvents(roomStates, equipmentStates, sensorStates, networkStates, timestamp) {
    const events = [];
...
function evaluateSensor(sensor, roomState, model, timestamp) {
...
function findNearestPipePressure(position, levelId, pipes) {
    let nearest = null;
...
function findNearestDuctPressure(position, levelId, ducts) {
    let nearest = null;
...
function findNearestFlow(position, levelId, model) {
...
function buildSensorHistory(history, value, timestamp) {
    if (value === null) {
        return history;
    }
    const next = [...history];
...
function inferNetworkConnections(equipment, model, suggestions) {
...
function selectPipeConnections(equipment, candidates) {
    ...
    switch (equipment.type) {
        ...
        default:
            return [sorted[0]?.id].filter((value) => Boolean(value));
    }
}
function resolveRoomIdForPoint(point, levelId, rooms) {
...
function clampTemperature(value, setpointC) {
