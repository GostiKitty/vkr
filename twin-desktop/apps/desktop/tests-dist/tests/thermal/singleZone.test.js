import { runThermalSimulation, simulateThermalNetwork } from "../../src/core/thermal/solver.js";
...
test("runThermalSimulation keeps old ideal mode behaviour by default", () => {
    ...
    expectApproximatelyEqual(withoutMode.summary.peakLoadKW, withIdealMode.summary.peakLoadKW, 1e-9, "Поведение solver по умолчанию должно совпадать с явным ideal mode.");
    expectApproximatelyEqual(withoutMode.summary.totalEnergyKWh, withIdealMode.summary.totalEnergyKWh, 1e-9, "Энергия solver по умолчанию не должна меняться после добавления heatingMode.");
});
