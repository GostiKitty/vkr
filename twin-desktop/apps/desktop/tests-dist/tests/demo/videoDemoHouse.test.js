...
test("video demo thermal result keeps canonical geometry and now supports dedicated temperature overlays", () => {
    ...
    const source = readFileSync(resolve(process.cwd(), "src/features/build/view3d/Build3DCanonicalPreview.tsx"), "utf8");
    ...
    baseline.rooms.forEach((room, index) => {
        ...
        if (room.boundary.some((point, pointIndex) => {
...
test("demo house keeps passive gains below total heat losses in the cold scenario", () => {
    const result = runEngineeringThermalAnalysis(videoDemoHouse, buildAdjacencyGraph(videoDemoHouse), {
        duration: "24h",
        timestepMinutes: 10,
        outdoor: {
            baseC: -18,
    ...
    if (!(result.balance.passiveGainsW < result.balance.totalLossW)) {
        throw new Error(`Passive gains should stay below total losses for the demo house. Got ${result.balance.passiveGainsW} W vs ${result.balance.totalLossW} W.`);
    }
...
test("user-facing 3D UI does not mention simple or recovered preview labels", () => {
    ...
    const panelSource = readFileSync(resolve(process.cwd(), "src/features/build/components/ThreeDControlPanel.tsx"), "utf8");
