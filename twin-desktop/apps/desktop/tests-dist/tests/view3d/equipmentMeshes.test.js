...
import { createEquipmentVisual, createSensorVisual, getEquipmentBaseY, getEquipmentWorldConnectionPoint, getSensorWorldPosition, } from "../../src/features/build/view3d/equipmentMeshes.js";
import { test } from "../testHarness.js";
const baseEquipment = (type) => ({
    id: `eq-${type}`,
    type,
    position: { x: 1, y: 2 },
    levelId: "lvl-1",
...
test("createEquipmentVisual returns Group for all supported types", () => {
    const types = ["radiator", "boiler", "pump", "ahu", "diffuser", "fancoil", "sensor"];
    types.forEach((type) => {
...
test("simplified equipment mode keeps supported equipment lightweight", () => {
    const types = ["radiator", "boiler", "pump"];
    types.forEach((type) => {
