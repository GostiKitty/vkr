import {
  buildTransientConclusion,
  formatHistogramBinLabel,
  formatSafeNumber,
  formatSafePercent,
  formatSafeTemperature,
  formatTransientSampleViewModeLabel,
  hasUsableScenarioSummarySeries,
  hasUsableSensitivitySeries,
  sanitizeIndoorChartTemperature,
  selectTransientSampleByMode,
  shortenScenarioLabel,
} from "../../src/features/build/components/ThermalSimulationPanel.js";
...
import type { TransientCalculationResult } from "../../src/core/thermal/transient/types.js";
import type { TransientMonteCarloResult } from "../../src/core/thermal/transient/uncertainty.js";
...
test("unstable transient conclusion does not contain physically impossible temperatures", () => {
  const result: TransientCalculationResult = {
    scenarioId: "unstable",
    stable: false,
    valid: false,
    scheme: "explicit",
...
test("sample selector returns worst, median and best samples without undefined", () => {
  const result = {
    ...
    sensitivity: [],
    warnings: [],
  } as unknown as TransientMonteCarloResult;
