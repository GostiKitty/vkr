...
import { createEmptyBuildingModel, type Room, type Wall } from "../../src/entities/geometry/types.js";
...
function rectRoom(id: string, levelId: string, x0: number, y0: number, x1: number, y1: number): Room {
  return {
    id,
    name: "Parent",
    levelId,
...
function segmentWall(id: string, levelId: string, a: { x: number; y: number }, b: { x: number; y: number }): Wall {
  return {
    id,
    levelId,
    a,
