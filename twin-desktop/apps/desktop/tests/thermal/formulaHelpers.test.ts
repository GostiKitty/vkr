import {
  ...
  uValue,
  ventilationLoss,
} from "../../src/core/thermal/formulas.js";
...
test("thermal formulas: ventilation loss uses SI inputs", () => {
  expectApproximatelyEqual(
    ventilationLoss(0.1, 1.204, 1005, 20),
    2420.04,
    1e-6,
    "Потери на вентиляцию должны считаться по rho*cp*L*dT."
  );
});

test("thermal formulas: infiltration loss derives airflow from ACH", () => {
  expectApproximatelyEqual(
    infiltrationLoss(0.5, 300, 1.204, 1005, 20),
    1008.35,
    0.01,
    "Потери на инфильтрацию должны использовать ACH и объем."
  );
});
