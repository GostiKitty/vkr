...
test("thermal Monte Carlo exposes metric series for dynamic report analysis", () => {
  ...
  const peakSensitivity = buildSensitivityFactors(
    result.samples,
    result.scenarioSeries.peakLoadKW,
    baseOptions,
    "peakLoadKW"
  );
  ...
  const missingRoomRiskSensitivity = buildSensitivityFactors(
    result.samples,
    result.samples.map(() => Number.NaN),
    baseOptions,
    "roomUnderheatRisk"
  );
