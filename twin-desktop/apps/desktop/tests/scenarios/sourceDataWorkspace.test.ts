...
} from "../../src/core/thermal/derived/sourceDataWorkspace.js";
...
test("homogeneity coefficient reduces Rred, fallback r=1 warns, and explicit psi/chi does not double-count with Rred", () => {
  ...
  const bridgeModeField = homogeneityReport.sections.materials.computedFields.find(
    (field) => field.key === "materials.bridge-mode"
  );
  ...
  const fallbackWallRow = fallbackReport.constructions
    .filter((row) => row.kind === "wall")
    .sort((left, right) => (right.areaM2 ?? 0) - (left.areaM2 ?? 0))[0];
  const fallbackBridgeMode = fallbackReport.sections.materials.computedFields.find(
    (field) => field.key === "materials.bridge-mode"
  );
  const hasR1FallbackWarning = fallbackWallRow?.warnings.some(
    (warning) => warning.includes("принято r = 1") || warning.includes("принято r = 1.")
  );
  const hasModelDerivedRWarning = fallbackWallRow?.warnings.some((warning) =>
    warning.includes("рассчитан из модели")
  );
...
test("planar heterogeneity alone does not block automatic psi/chi enrichment", () => {
  ...
  const withPlanarOnly = envelope.map((fragment) =>
    fragment.constructionType === "wall"
      ? {
          ...fragment,
          heterogeneity: {
...
test("model without thermalProtection.climate gets outdoor climate from SP 131 in report", () => {
  ...
  const designOutdoor = report.sections.climate.computedFields.find(
    (field) => field.key === "climate.design-outdoor"
  );
...
test("geometry opening areas follow model.windows and sync drops stale envelope windows", () => {
  ...
  const windowAreaLive = reportWithOpenings.sections.geometry.computedFields.find(
    (field) => field.key === "geometry.window-area"
  );
  ...
  const windowAreaStale = reportStripped.sections.geometry.computedFields.find(
    (field) => field.key === "geometry.window-area"
  );
