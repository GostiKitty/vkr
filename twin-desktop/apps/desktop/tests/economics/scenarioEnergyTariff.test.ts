...
import {
  KWH_PER_GCAL,
  REGIONAL_ENERGY_TARIFFS,
  resolveHeatingEnergySource,
  resolveScenarioEnergyTariff,
} from "../../src/core/economics/scenarioEnergyTariff.js";
...
test("scenarioEnergyTariff: centralized heat uses Gcal conversion for moscow", () => {
  ...
  assert.equal(
    resolved.tariffRubPerKWh,
    REGIONAL_ENERGY_TARIFFS.moscow.heatTariffRubPerGcal / KWH_PER_GCAL
  );
});
