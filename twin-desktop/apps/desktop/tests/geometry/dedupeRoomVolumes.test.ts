import {
  ...
  dedupeRoomVolumesForModel,
  finalizeRoomVolumesForModel,
} from "../../src/core/geometry/bimPipeline.js";
...
function buildWallsOnlyLeftModel(): BuildingModel {
  return {
    id: "test",
    name: "test",
...
test("canonical 3D keeps temperature overlay for wall-defined left room with internal partition", () => {
  ...
  const field = createThermalFieldModel(model, {
    ...
    roomTemperaturesC: Object.fromEntries(
      Object.entries(preview.rooms).map(([roomId, payload]) => [roomId, payload.temperatureC])
    ),
  });
