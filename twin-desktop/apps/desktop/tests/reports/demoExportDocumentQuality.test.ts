...
test("demo export quality: defaults закрывают титул и автозапуск RC заполняет динамику", () => {
  const base = buildPreparedDemoBase();
  if (base.source.dynamicResultState !== "auto-demo" && base.source.dynamicResultState !== "provided") {
    throw new Error(`Ожидался автозапуск RC или готовый результат, получено: ${base.source.dynamicResultState}`);
  }
  ...
  if (!Number.isFinite(base.passport.thermalResults.peakLoadKW ?? NaN)) {
    throw new Error("После автозапуска RC не заполнена пиковая нагрузка.");
  }
  if (!Number.isFinite(base.passport.thermalResults.totalEnergyKWh ?? NaN)) {
    throw new Error("После автозапуска RC не заполнена тепловая энергия за период.");
  }
  if (!Number.isFinite(base.passport.thermalResults.averageRoomTemperatureC ?? NaN)) {
    throw new Error("После автозапуска RC не заполнена средняя температура помещений.");
  }
});
