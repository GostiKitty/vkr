...
test("mesh audit finds suspicious temperature plane outside shell bbox", () => {
  ...
  const rogueTemperature = new THREE.Mesh(
    new THREE.BoxGeometry(4, 0.02, 3),
    new THREE.MeshStandardMaterial({ color: 0xf97316, transparent: true, opacity: 0.58 })
  );
...
test("mesh audit preserves room-floor category metadata for large colored planes", () => {
  ...
  const temperatureMesh = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xd946ef, transparent: true, opacity: 0.62 })
  );
...
test("mesh audit flags unnamed large red planes", () => {
  const { scene, shellRoot } = createShellScene();
  const unnamedPlane = new THREE.Mesh(
    new THREE.BoxGeometry(5, 0.02, 4),
    new THREE.MeshStandardMaterial({ color: 0xef4444, transparent: true, opacity: 0.72 })
  );
