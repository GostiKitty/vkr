...
import {
  ...
  buildRecoveredTemperatureSurfaces,
  filterDisplayedRecoveredTemperatureSurfaces,
  filterAnchoredTemperatureSurfaces,
  getRecoveredRoomFloorPolygon,
  getRoomPolygonFor3D,
  getRoomTemperatureMap,
  summarizeRecoveredTemperatureSurfaces,
  summarizeRecoveredRoomTemperatures,
} from "../../src/features/build/view3d/buildRecovered3DModel.js";
import {
  DISABLE_ALL_3D_TEMPERATURE,
  resolveRecoveredTemperatureRuntime,
  USE_ROOM_FLOOR_TEMPERATURE_COLORING,
} from "../../src/features/build/view3d/Build3DRecoveredPreview.js";
...
test("recovered openings stay anchored to host wall without NaN", () => {
  ...
  if (
    openings.some((opening) =>
      [opening.center.x, opening.center.y, opening.center.z, opening.rotationY_rad].some((value) => !Number.isFinite(value))
    )
  ) {
...
test("shifted room polygon does not become recovered room floor geometry", () => {
  const shiftedModel = {
    ...sampleBuildingSP50,
    rooms: sampleBuildingSP50.rooms.map((room, index) =>
      index === 0
        ? {
            ...room,
...
test("shifted thermal polygon does not move recovered room floor geometry", () => {
  ...
  field.rooms = field.rooms.map((room) =>
    room.roomId === targetRoom.id ? { ...room, polygon: room.polygon.map((point) => ({ x: point.x + 5, y: point.y - 4 })) } : room
  );
...
test("Build3DRecoveredPreview does not render wall temperature boxes", () => {
  const source = readFileSync(
    "c:\\Users\\Liza\\vkr\\twin-desktop\\apps\\desktop\\src\\features\\build\\view3d\\Build3DRecoveredPreview.tsx",
    "utf-8"
  );
