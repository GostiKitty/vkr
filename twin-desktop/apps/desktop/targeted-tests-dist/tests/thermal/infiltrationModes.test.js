...
test("manualAch preserves previous solver result and reports manual ACH source", () => {
    ...
    expectApproximatelyEqual(manual.summary.peakLoadKW, legacy.summary.peakLoadKW, 1e-9, "manualAch не должен менять peak load относительно старого ACH-пути.");
    expectApproximatelyEqual(manual.summary.totalEnergyKWh, legacy.summary.totalEnergyKWh, 1e-9, "manualAch не должен менять энергопотребление относительно старого ACH-пути.");
...
test("envelopeLeakage computes ACH from envelope area and G_air", () => {
    const building = buildSimpleBuilding();
    const result = resolveBuildingInfiltration(building, {
    ...
    expectApproximatelyEqual(result.calculatedACH, (result.airflowM3s * 3600) / result.geometry.heatedVolumeM3, 1e-9, "ACH должен быть получен из расчётного расхода и фактического отапливаемого объёма модели.");
...
test("pressureBased increases infiltration when wind speed increases", () => {
    const building = buildSimpleBuilding();
    const lowWind = resolveBuildingInfiltration(building, {
    ...
    const highWind = resolveBuildingInfiltration(building, {
        infiltrationMode: "pressureBased",
        pressureBased: {
            windSpeedMps: 8,
            windPressureCoefficient: 0.6,
...
test("heatRecoveryFactor affects only mechanical ventilation loss, not infiltration", () => {
    const building = buildSimpleBuilding();
    const infiltration = resolveBuildingInfiltration(building, {
        infiltrationMode: "pressureBased",
        pressureBased: {
            windSpeedMps: 5,
            windPressureCoefficient: 0.6,
    ...
    expectApproximatelyEqual(roomWithRecovery.infiltrationLossW, roomNoRecovery.infiltrationLossW, 1e-9, "Рекуперация не должна уменьшать потери на инфильтрацию.");
...
test("engineering air terminals add room ventilation without double counting supply and exhaust", () => {
    ...
    expectApproximatelyEqual(room.mechanicalVentilationLossW, expectedLossW, 2, "Приток и вытяжка в одной комнате должны считаться без двойного счёта.");
});
test("engineering analysis keeps infiltration and ventilation losses separate without double counting", () => {
    ...
    const result = runEngineeringThermalAnalysis(building, adjacency, {
        ...buildBaseOptions(),
        infiltration: {
            infiltrationMode: "pressureBased",
            pressureBased: {
    ...
    expectApproximatelyEqual(room.airExchangeLossW, room.infiltrationLossW + room.mechanicalVentilationLossW, 1e-9, "airExchangeLossW должен быть суммой инфильтрации и механической вентиляции.");
    expectApproximatelyEqual(room.ventilationLossW, room.airExchangeLossW, 1e-9, "ventilationLossW в инженерном балансе должен оставаться алиасом суммарного воздухообмена без повторного счёта.");
    expectApproximatelyEqual(result.balance.totalLossW, result.balance.transmissionLossW + result.balance.infiltrationLossW + result.balance.ventilationLossW, 1e-6, "Суммарные потери не должны повторно учитывать механическую вентиляцию и инфильтрацию.");
});
