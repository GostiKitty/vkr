...
test("thicker insulation layer reduces U_total", () => {
    ...
    ], undefined, { includeSp50AirFilms: true });
...
test("openings larger than wall area yield warning and finite conductances", () => {
    ...
    const f = computeWallFacadeConductances(wall, [
...
test("runThermalSimulation exposes diagnostics with building loss shares", () => {
    ...
    const sumPct = building.lossSharePercent.opaque +
...
test("heat recovery reduces mechanical ventilation without changing infiltration", () => {
    ...
    expectApproximatelyEqual(withDiagnostics.building.totalInfiltrationLossW, withoutDiagnostics.building.totalInfiltrationLossW, 1e-6, "инфильтрация не должна меняться от рекуперации");
