    ...
    for (const [roomId, room] of Object.entries(result.rooms)) {
        for (let i = 0; i < room.timeline.length; i++) {
            ...
            if (!Number.isFinite(pt.temperatureC)) {
                throw new Error(`room ${roomId} timeline[${i}].temperatureC is not finite: ${pt.temperatureC}`);
            }
            if (!Number.isFinite(pt.heatingPowerW)) {
                throw new Error(`room ${roomId} timeline[${i}].heatingPowerW is not finite: ${pt.heatingPowerW}`);
            }
        }
    }
});
    ...
    for (const [roomId, room] of Object.entries(result.rooms)) {
        for (let i = 0; i < room.timeline.length; i++) {
            ...
            if (pt.setpointC === undefined || !Number.isFinite(pt.setpointC)) {
                throw new Error(`room ${roomId} timeline[${i}].setpointC is missing or not finite: ${pt.setpointC}. ` +
    ...
    for (const [roomId, room] of Object.entries(result.rooms)) {
        for (let i = 0; i < room.timeline.length; i++) {
            ...
            if (temperatureC < OUTDOOR_LOWER_BOUND || temperatureC > UPPER_BOUND) {
                throw new Error(`room ${roomId} timeline[${i}].temperatureC = ${temperatureC.toFixed(2)} °C ` +
    ...
    for (const [roomId, room] of Object.entries(result.rooms)) {
        for (let i = 0; i < room.timeline.length; i++) {
            ...
            if (setpointC < lo - 0.01 || setpointC > hi + 0.01) {
                throw new Error(`room ${roomId} timeline[${i}].setpointC = ${setpointC} is outside [${lo}, ${hi}].`);
            }
        }
    }
});
test("temperature series: buildChartPoints produces non-empty temperature payload", () => {
    ...
    const chartPoints = room.timeline.map((pt, index) => {
        ...
        let setpointC = Number.isFinite(pt.setpointC) ? pt.setpointC : null;
        if (setpointC === null) {
            const frame = result.timeline[index];
            const frameSetpoint = frame?.rooms?.[firstRoomId]?.setpointC;
            setpointC = Number.isFinite(frameSetpoint) ? frameSetpoint : null;
        }
    ...
    const tempPlottedData = chartPoints.filter((pt) => (typeof pt.airTemperatureC === "number" && Number.isFinite(pt.airTemperatureC)) ||
        (typeof pt.setpointC === "number" && Number.isFinite(pt.setpointC)));
    if (tempPlottedData.length === 0) {
        throw new Error(`Temperature mode plottedData is empty for room ${firstRoomId}. ` +
    ...
    const withTemp = tempPlottedData.filter((pt) => typeof pt.airTemperatureC === "number" && Number.isFinite(pt.airTemperatureC));
    if (!withTemp.length) {
        throw new Error(`No chart point has a finite airTemperatureC for room ${firstRoomId}. ` +
    ...
    const withSetpoint = tempPlottedData.filter((pt) => typeof pt.setpointC === "number" && Number.isFinite(pt.setpointC));
    if (!withSetpoint.length) {
        throw new Error(`No chart point has a finite setpointC for room ${firstRoomId}. ` +
...
test("heating series: setpoint ramp lowers ideal-mode peak power", () => {
    ...
    if (!(peakSteppedW > peakRampedW * 1.15)) {
        throw new Error(`Expected stepped setpoint peak (${peakSteppedW.toFixed(0)} W) to exceed ramped peak (${peakRampedW.toFixed(0)} W) by >15%.`);
    }
});
test("temperature series: chart points for 'Тепловой пункт' are valid when it exists", () => {
    ...
    const result = runDemo();
    const utilityRoomEntry = Object.entries(result.rooms).find(([id]) => id.toLowerCase().includes("utility"));
    if (!utilityRoomEntry) {
    ...
    if (!withTemp.length) {
        throw new Error(`'Тепловой пункт' room ${utilityRoomId}: no finite temperatureC values in timeline (${room.timeline.length} points).`);
    }
    const withSetpoint = room.timeline.filter((pt) => pt.setpointC !== undefined && Number.isFinite(pt.setpointC));
    if (!withSetpoint.length) {
        throw new Error(`'Тепловой пункт' room ${utilityRoomId}: no finite setpointC values in timeline after solver fix.`);
    }
});
