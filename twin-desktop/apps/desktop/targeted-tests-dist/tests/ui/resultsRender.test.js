...
test("ResultsPanel renders its shell even without simulation data", () => {
    resetStores();
    const markup = renderToStaticMarkup(_jsx(ResultsPanel, { projectId: null }));
...
test("MetricsResultsTab renders a recovery empty state without undefined/null", () => {
    resetStores();
    const markup = renderToStaticMarkup(_jsx(MetricsResultsTab, { onRecalculate: () => { } }));
...
test("ThermalSimulationPanel renders a clear empty state in the results viewport", () => {
    ...
    const markup = renderToStaticMarkup(_jsx(ThermalSimulationPanel, { projectKey: "test-project", model: model, adjacency: adjacency, options: DEFAULT_THERMAL_OPTIONS, onOptionsChange: () => { } }));
...
test("ProjectDocumentationPage renders neutral hidden-section placeholder", () => {
    resetStores();
    const markup = renderToStaticMarkup(_jsx(ProjectDocumentationPage, { projectId: null }));
