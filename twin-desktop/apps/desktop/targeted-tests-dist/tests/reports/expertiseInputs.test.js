...
import { DEFAULT_EXPERTISE_REPORT_INPUTS, loadExpertiseInputs, useExpertiseInputsStore, } from "../../src/features/reports/exports/store/expertiseInputs.store.js";
...
function buildDemoBase(overrides = {}) {
...
test("demo signatures use dash instead of not-provided wording", () => {
    ...
    const register = base.expertise.inputRegisterRows.filter((row) => ["checkedBy", "normControl", "chiefEngineer"].includes(row.key));
...
test("external door without normative resistance requires clarification", () => {
    ...
    const doorRow = data.envelopeResistanceRows.find((row) => row.typeLabel.toLowerCase().includes("двер"));
    if (!doorRow) {
        return;
    }
...
test("export source uses correct grammar and no obsolete phrases", () => {
    const pageSource = readFileSync(resolve(process.cwd(), "src/features/reports/ProjectDocumentationPage.tsx"), "utf8");
    ...
    const exportsSource = readFileSync(resolve(process.cwd(), "src/features/reports/exports/data/exportText.ts"), "utf8");
    ...
    const htmlSource = readFileSync(resolve(process.cwd(), "src/features/reports/exports/generators/thermalProtection.ts"), "utf8");
