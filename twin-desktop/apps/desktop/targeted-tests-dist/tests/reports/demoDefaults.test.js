...
import { applyDemoDesignDefaults, DEMO_DEFAULT_FOOTNOTE, DEMO_HOUSE_DESIGN_DEFAULTS, ASSUMPTIONS_SECTION_TITLE, } from "../../src/features/reports/exports/defaults/demoHouseDesignDefaults.js";
import { renderReportHtml } from "../../src/features/reports/exports/exportReportDocument.js";
import { ALL_REPORT_EXPORT_KINDS, REPORT_EXPORT_BUNDLE_FILENAME, } from "../../src/features/reports/exports/exportReportDocument.js";
...
function assertNotContains(html, snippet, label) {
    if (html.includes(snippet)) {
        throw new Error(`${label}: HTML неожиданно содержит '${snippet}'.`);
    }
}
function assertNoForbiddenStrings(html, label) {
...
test("demo-defaults: пользовательские значения не перезаписываются", () => {
    ...
    if (overwrittenKeys.length > 0) {
        throw new Error(`Defaults записали в уже заполненные поля: ${overwrittenKeys.join(", ")}`);
    }
});
// ──────────────────────────────────────────────────────────────────────────
...
test("demo-defaults: заполняют только пустые поля и регистрируют допущения", () => {
    ...
    if (input.reportMeta.projectCipher !== DEMO_HOUSE_DESIGN_DEFAULTS.projectCode ||
...
test("demo-defaults: блок «Принятые проектные допущения» и сноска * присутствуют", () => {
    ...
    if (!energyHtml.includes("Москва *")) {
        throw new Error("Город в энергопаспорте должен быть помечен сноской «*», когда подставлен из defaults.");
    }
    // Раздел 5 ОВ/ТС
...
test("demo-defaults: блок «Принятые проектные допущения» НЕ выводится, если допущений нет", () => {
    ...
    if (base.appliedAssumptions.length !== 0) {
        throw new Error(`На пустой модели не должно быть авто-допущений, получено: ${base.appliedAssumptions
            .map((e) => e.key)
            .join(", ")}`);
    }
    const ovtsHtml = generateProjectOvTsHtml(buildProjectOvTsData(base), {
...
test("demo-defaults: фактические значения энергопаспорта остаются «—» после применения", () => {
    ...
    for (const section of sections) {
        for (const row of section.rows) {
            if (row.factValue !== "—") {
                throw new Error(`${section.title}: фактическое значение «${row.label}» = «${row.factValue}» — должно быть «—».`);
            }
        }
    }
});
...
test("qrad: не подменяется фейковым числом — выводится текстовый статус (демо-дом)", () => {
    ...
    if (!solarRow.designValue.includes("требует уточнения")) {
        throw new Error(`qrad должен выводить текстовый статус «требует уточнения...», получено: ${solarRow.designValue}`);
    }
    // Авто-допущение «методика qrad требует уточнения» должно попадать в реестр.
    const auto = base.appliedAssumptions.some((entry) => entry.key === "energy.solarGain.methodology");
    if (!auto) {
        throw new Error("Авто-допущение про методику qrad должно автоматически попадать в реестр.");
    }
...
test("download-all: формирует 5 файлов с корректными именами 01-…, 02-…, …", () => {
    ...
    for (let i = 0; i < expected.length; i += 1) {
        if (actual[i] !== expected[i]) {
            throw new Error(`Имя файла ${i + 1} не совпало: ожидалось ${expected[i]}, получено ${actual[i]}.`);
        }
    }
});
// ──────────────────────────────────────────────────────────────────────────
