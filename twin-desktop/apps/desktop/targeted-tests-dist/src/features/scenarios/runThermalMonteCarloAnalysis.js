...
const DEFAULT_MONTE_CARLO_MODE = "full-physics";
...
export function runThermalMonteCarloAnalysis() {
    ...
    const evaluationMode = "full-physics";
    const adjacency = buildAdjacencyGraph(buildModel);
    const result = runThermalMonteCarlo({
        model: buildModel,
        baseOptions: buildThermalOptionsFromWorkflow(workflowState.scenarioConfig, undefined, applyScenarioToBuilding(buildModel, workflowState.scenarioConfig)),
        runs: nextRuns,
        adjacency,
    });
