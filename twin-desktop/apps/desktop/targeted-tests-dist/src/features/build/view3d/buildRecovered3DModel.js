...
import { buildGeometryRenderModel, buildRoomPolygonFromWalls } from "../../../core/geometry/bimPipeline";
import { createEmptyRecoveredBounds, extendRecoveredBounds, extendRecoveredBoundsWithPolygon, finalizeRecoveredBounds, } from "./recoveredPreviewMath";
...
export function planToRecoveredScene(point, elevation_m) {
    return {
        x: point.x,
        y: elevation_m,
        z: point.y,
...
export function getRoomPolygonFor3D(room) {
    return clonePolygon(room.polygon);
}
function getThermalRoomPolygon(roomId, thermalField) {
...
export function getRoomTemperatureValue(roomId, thermalField) {
    return thermalField?.roomMap.get(roomId)?.baseTemperatureC ?? null;
}
export function getRoomTemperatureMap(thermalField, activeLevelId) {
...
function getRoomWallBounds(model, thermalField, roomId, levelId, polygonOverride) {
    ...
    if (roomPolygon.length >= 2) {
        for (let index = 0; index < roomPolygon.length; index += 1) {
            ...
            if (closestWall) {
                const wallMatch = closestWall;
                matchedWalls.set(wallMatch.id, wallMatch);
            }
        }
    }
    if (!matchedWalls.size && thermalField) {
        const related = thermalField.boundaries.filter((boundary) => boundary.levelId === levelId &&
...
function getRoomVolumePolygon(renderGeometry, roomId, levelId) {
...
function evaluateRecoveredRoomFloorAlignment(roomId, roomName, levelId, activeLevelId, polygon, source, model, thermalField) {
    ...
    const insideWallShell = polygonBounds.minX >= wallBox2D.minX - 0.22 &&
    ...
    const centerInsideWallShell = polygonCenter.x >= wallBox2D.minX &&
...
export function getRecoveredRoomFloorPolygon(room, model, renderGeometry, thermalField, activeLevelId) {
    ...
    const wallPolygon = buildRoomPolygonFromWalls(model, room);
    ...
    for (const candidate of candidates) {
        ...
        const alignment = evaluateRecoveredRoomFloorAlignment(room.id, roomName, room.levelId, activeLevelId, candidate.polygon, candidate.source, model, thermalField);
...
function collectRecoveredCoordinateWarnings(model, thermalField, rooms) {
    if (!thermalField) {
        return [];
    }
    const warnings = [];
...
export function resolveRecoveredLevelId(model, activeLevelId) {
    if (!activeLevelId) {
        return null;
    }
    const hasShell = model.rooms.some((room) => room.levelId === activeLevelId) ||
...
function buildRecoveredShellBounds(model, levelId) {
    const bounds = createEmptyRecoveredBounds();
    model.rooms
...
function buildRecoveredBounds(result) {
    const bounds = createEmptyRecoveredBounds();
    ...
    result.roofs.forEach((roof) => extendRecoveredBoundsWithPolygon(bounds, roof.boundary, roof.elevation_m, roof.elevation_m + Math.max(roof.thickness_m, 0.04)));
    result.slabs.forEach((slab) => extendRecoveredBoundsWithPolygon(bounds, slab.boundary, slab.elevation_m, slab.elevation_m + Math.max(slab.thickness_m, 0.04)));
    if (bounds.empty) {
        const allPoints = [
...
function buildRecoveredOpenings(model, levelId, type) {
...
export function filterAnchoredTemperatureSurfaces(surfaces, model, activeLevelId, geometryContext) {
    const levelId = resolveRecoveredLevelId(model, activeLevelId);
    const accepted = [];
    const warnings = [];
    let rejectedCount = 0;
    const shellBounds = buildRecoveredShellBounds(model, levelId);
    const reject = (surface, reason) => {
        rejectedCount += 1;
    ...
    surfaces.forEach((surface) => {
        ...
        if (surface.sourceType === "room") {
            ...
            const geometry = getRecoveredRoomFloorPolygon(room, model, geometryContext?.renderGeometry ?? null, geometryContext?.thermalField ?? null, levelId);
...
export function filterDisplayedRecoveredTemperatureSurfaces(surfaces, rooms) {
...
export function summarizeRecoveredTemperatureSurfaces(surfaces, warnings, rejectedCount) {
    if (!surfaces.length) {
...
export function buildRecoveredTemperatureSurfaces(model, thermalField, activeLevelId, options) {
    const rawSurfaces = [];
    const warnings = [];
    const levelId = resolveRecoveredLevelId(model, activeLevelId);
        ...
        thermalField.rooms.forEach((room, index) => {
            ...
            if (DEBUG_RECOVERED_TEMP_SURFACES || DEBUG_RECOVERED_3D_COORDS) {
                ...
                if (roomPolygon.length >= 3) {
                    ...
                    const wallBounds = room.roomId && existingRoom ? getRoomWallBounds(model, thermalField, existingRoom.id, existingRoom.levelId) : null;
                    ...
                    const debugPayload = {
                        ...
                        roomName: existingRoom?.name ?? room.roomId ?? `room-${index}`,
                        levelId: room.levelId,
    ...
    const roomDisplayFilter = filterDisplayedRecoveredTemperatureSurfaces(filtered.surfaces, model.rooms
        .filter((room) => !levelId || room.levelId === levelId)
        .flatMap((room) => {
    ...
    const summary = summarizeRecoveredTemperatureSurfaces(roomDisplayFilter.surfaces, [...new Set([...warnings, ...filtered.warnings, ...roomDisplayFilter.warnings])], filtered.rejectedCount + roomDisplayFilter.rejectedCount);
    return {
        surfaces: roomDisplayFilter.surfaces,
        summary,
    };
}
export function summarizeRecoveredRoomTemperatures(rooms, warnings) {
    ...
    const temperatures = coloredRooms.map((room) => room.temperature_C);
    let min_C = Math.min(...temperatures);
    let max_C = Math.max(...temperatures);
    if (Math.abs(max_C - min_C) < 0.001) {
        min_C -= 1;
...
export function buildRecovered3DModel(model, activeLevelId, options = {}) {
    const levelId = resolveRecoveredLevelId(model, activeLevelId);
    ...
    const rawRoomTemperatureState = showTemperature ? getRoomTemperatureMap(options.thermalField ?? null, levelId) : { roomTemperatures: new Map(), warnings: [] };
    ...
    const rooms = model.rooms
        ...
        logGeometryAlignmentDebug("room-floor-source-audit", {
            ...
            deltaRoomFloorToWallsCenter: wallBounds && recoveredCenter
                ? {
    ...
    const equipment = showEquipment
        ? model.equipment
            ...
            type: item.type,
            position: planToRecoveredScene(item.position, getLevelElevation(model, item.levelId) +
                (item.type === "diffuser" ? 2.7 : item.type === "radiator" ? 0.28 : item.type === "boiler" ? 0.5 : 0.24)),
        }))
        : [];
    const sensors = showEquipment
        ? model.sensors
    ...
    const temperatureSummary = showTemperature
        ? summarizeRecoveredRoomTemperatures(rooms, [
