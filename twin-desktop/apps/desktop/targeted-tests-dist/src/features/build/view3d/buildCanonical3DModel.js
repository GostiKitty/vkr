...
export function buildCanonical3DModel(model, activeLevelId, options = {}) {
    ...
    const roundRoomBoundary = (poly, levelId) => {
        ...
        const radiusForIndex = (index) => {
            ...
            const match = levelFillets.find((fillet) => Math.hypot(fillet.point.x - vertex.x, fillet.point.y - vertex.y) <= 0.3);
    ...
    const wallFillets = wallJoinData.corners
        ...
        const armTemps = filletWalls
            .filter((w) => Math.hypot(w.a.x - corner.point.x, w.a.y - corner.point.y) <= 0.3 ||
    ...
    const stairs = filterLevel(model.stairs ?? []).map((stair) => ({
        id: stair.id,
