...
import { buildRecovered3DModel, } from "./buildRecovered3DModel";
...
export function resolveRecoveredTemperatureRuntime(showTemperature, showWallTemperature) {
...
function logMeshAudit(scene, shellRoot) {
    ...
    if (audit.suspiciousRows.length) {
        console.table(audit.suspiciousRows.map((row) => ({
            name: row.name,
            category: row.category,
            color: row.materialColorHex,
            opacity: row.materialOpacity,
    ...
    if (audit.outsideShellRows.length) {
        console.table(audit.outsideShellRows.map((row) => ({
            name: row.name,
            category: row.category,
            color: row.materialColorHex,
            centerX: row.bbox?.center.x ?? null,
...
export const Build3DRecoveredPreview = React.forwardRef(({ model, activeLevelId, selection, viewer, thermalField = null, showTemperature = false, showWallTemperature = false, onSelect }, forwardedRef) => {
    ...
    const homeModeRef = useRef("focus");
    ...
    const temperatureRuntime = useMemo(() => resolveRecoveredTemperatureRuntime(showTemperature, showWallTemperature), [showTemperature, showWallTemperature]);
    const simpleModel = useMemo(() => buildRecovered3DModel(model, activeLevelId, {
    ...
    const coloredRoomFloorCount = useMemo(() => temperatureRuntime.showTemperature
        ? simpleModel.rooms.filter((room) => room.boundary.length >= 3 && Number.isFinite(room.temperature_C)).length
        : 0, [simpleModel.rooms, temperatureRuntime.showTemperature]);
    ...
    useEffect(() => {
        ...
        simpleModel.rooms.forEach((room) => {
            ...
            const baseMaterial = temperatureRuntime.showTemperature && Number.isFinite(room.temperature_C) && simpleModel.temperatureSummary
                ? createRoomTemperatureMaterial(room.temperature_C, simpleModel.temperatureSummary)
                : ROOM_MATERIAL;
