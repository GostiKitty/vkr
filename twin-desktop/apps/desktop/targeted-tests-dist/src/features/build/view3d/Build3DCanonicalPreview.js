...
import { buildLevelElevationLabelGroup, buildRoomFloorLabelGroup, createRoomFloorLabelRenderer, disposeRoomFloorLabelGroup, } from "./roomFloorLabels3D";
 ...
 */
function createOpeningTemperatureMaterial(temperature_C, summary, isWindow) {
    const min = summary?.min_C ?? temperature_C;
    const max = summary?.max_C ?? temperature_C;
    const span = Math.max(max - min, 1e-6);
    const color = thermalColor((temperature_C - min) / span);
 ...
 */
function wallVertexTemp(uNorm, vNorm, baseTemp_C, wallLength_m, wallHeight_m, isExterior, cornerCoolingStart_C, cornerCoolingEnd_C, openings, startRoomTemp_C, endRoomTemp_C) {
    ...
    const cornerPenalty = cornerCoolingStart_C * Math.exp(-physU / cornerDecay) +
 ...
 */
function addWallThermalMesh(root, wall, openings, cornerCoolingStart_C, cornerCoolingEnd_C, colorScale, isExterior, startRoomTemp_C, endRoomTemp_C) {
    ...
    for (let iv = 0; iv <= nv; iv++) {
        ...
        for (let iu = 0; iu <= nu; iu++) {
            ...
            const T = wallVertexTemp(uNorm, vNorm, wall.temperature_C, wallLength, wallHeight, isExterior, cornerCoolingStart_C, cornerCoolingEnd_C, openings, startRoomTemp_C, endRoomTemp_C);
 ...
 */
function addRoofThermalMesh(root, roof, baseTemp_C, colorScale) {
    ...
    for (let z = minZ; z <= maxZ + step * 0.5; z += step) {
        for (let x = minX; x <= maxX + step * 0.5; x += step) {
            if (pointInPolygon2D(x, z, roof.boundary))
                verts.push([x, z]);
        }
    }
    if (verts.length < 3)
 ...
 */
function addWindowThermalEdges(root, opening, colorScale) {
    const { min_C, max_C } = colorScale;
    const span = Math.max(max_C - min_C, 1e-6);
    const c = thermalColor((opening.temperature_C - min_C) / span);
    const margin = 0.045;
    const boxGeo = new THREE.BoxGeometry(opening.width_m + margin, opening.height_m + margin, opening.depth_m + margin * 2);
 ...
 */
function addWindowFaceThermalMesh(root, opening, colorScale) {
    const { min_C, max_C } = colorScale;
    const span = Math.max(max_C - min_C, 1e-6);
    const baseTemp = opening.temperature_C;
 ...
 */
function thermalColorToCss(color, darken = 0.72) {
...
function addWallFillet(root, fillet, material, options) {
...
function createOpeningMesh(opening, selection, temperatureEnabled, colorScale) {
    const isWindow = opening.type === "window";
    const hasTemperature = temperatureEnabled && Number.isFinite(opening.temperature_C) && colorScale !== null;
    let baseMaterial;
    let disposeThermalMaterial = false;
    if (hasTemperature) {
        baseMaterial = createOpeningTemperatureMaterial(opening.temperature_C, colorScale, isWindow);
...
export const Build3DCanonicalPreview = React.forwardRef(({ model, activeLevelId, selection, viewer, thermalField = null, surfaceField = null, surfaceFieldMode = "surfaceTemperature", showSurfaceField = false, showHeatSources = false, showThermalBridges = false, showTemperature = false, showWallTemperature = false, suppressTemperatureSummaryOverlay = false, surfaceFieldOpacity = 0.52, solarPosition = null, onSelect, onHoverInfo, onCameraStateChange, onSurfaceFieldDebug, }, forwardedRef) => {
    ...
    const levelElevationLabels = useMemo(() => {
        ...
        const appendPoints = (levelId, points) => {
    ...
    const coloredWindowCount = useMemo(() => temperatureEnabled && viewer.showOpenings
        ? canonicalModel.windows.filter((w) => Number.isFinite(w.temperature_C)).length
        : 0, [canonicalModel.windows, temperatureEnabled, viewer.showOpenings]);
    const coloredDoorCount = useMemo(() => temperatureEnabled && viewer.showOpenings
        ? canonicalModel.doors.filter((d) => Number.isFinite(d.temperature_C)).length
        : 0, [canonicalModel.doors, temperatureEnabled, viewer.showOpenings]);
    ...
    useEffect(() => {
        ...
        const handlePointerMove = (event) => {
            ...
            const thermalHit = hits.find((entry) => {
                const cat = entry.object.userData?.category;
                return (cat === "shell" ||
                    cat === "opening" ||
            ...
            if (thermalHit) {
                ...
                let accurateWallTemp = null;
                if (rawCat === "wall-thermal-gradient") {
                    const wd = ud;
                    const pt = thermalHit.point;
                    const uNorm = Math.max(0, Math.min(1, ((pt.x - wd.wallStart.x) * wd.wallDx + (pt.z - wd.wallStart.y) * wd.wallDz) /
                        (wd.wallLength * wd.wallLength)));
                    const vNorm = Math.max(0, Math.min(1, (pt.y - wd.wallElevation_m) / wd.wallHeight));
                    accurateWallTemp = wallVertexTemp(uNorm, vNorm, wd.wallTemp_C, wd.wallLength, wd.wallHeight, wd.isExteriorWall, wd.cornerCoolingStart_C, wd.cornerCoolingEnd_C, wd.openings, wd.startRoomTemp_C, wd.endRoomTemp_C);
                }
                ...
                if (rawCat === "wall-thermal-gradient" ||
                    ...
                    const parentName = rawCat === "wall-thermal-gradient" ? `wall:${parentId}` : `window:${parentId}`;
                    let parentObj;
                    thermalHit.object.parent?.traverse((o) => {
                        if (o.name === parentName)
                            parentObj = o;
                    });
                    if (parentObj?.userData)
                        resolvedUd = parentObj.userData;
                }
                const baseTempC = resolvedUd.temperature_C;
                const tempC = accurateWallTemp ?? baseTempC;
                const hasTempC = typeof tempC === "number" && Number.isFinite(tempC);
                const cat = resolvedUd.category;
                const srcType = resolvedUd.sourceType;
                ...
                if (cat === "opening") {
                    ...
                    title = "Пол";
                    if (hasTempC) {
                        subtitle = `Температура: ${tempC.toFixed(1)} °C`;
                    }
                }
                ...
                onHoverInfoRef.current?.({
                    title,
                    subtitle: subtitle ?? (hasTempC ? `${tempC.toFixed(1)} °C` : "Нет данных"),
                    temperatureC: hasTempC ? tempC : null,
    ...
    useEffect(() => {
        ...
        if (viewer.showRooms && canonicalModel.rooms.length > 0) {
            const roomsForLabels = colorScale
                ? canonicalModel.rooms.map((room) => {
                    ...
                    const { min_C, max_C } = colorScale;
                    const span = Math.max(max_C - min_C, 1e-6);
                    const c = thermalColor((room.temperature_C - min_C) / span);
        ...
        const roomTempById = new Map(canonicalModel.rooms
            .filter((r) => Number.isFinite(r.temperature_C))
            .map((r) => [r.id, r.temperature_C]));
        canonicalModel.walls.forEach((wall) => {
            ...
            const isExteriorWall = wall.adjacentRoomId === null || wall.temperatureSource === "thermalField.boundaries";
            const adjacentRoomTemp_C = wall.adjacentRoomId != null ? (roomTempById.get(wall.adjacentRoomId) ?? null) : null;
            addBoxBetween(shellRoot, wall.start, wall.end, Math.max(wall.thickness_m, 0.08), wallH, wall.elevation_m + wallH * 0.5, wallMeshMaterial, {
                name: `wall:${wall.id}`,
                userData: {
                    category: "shell",
                    sourceType: "wall",
        ...
        if (viewer.showWalls) {
            canonicalModel.wallFillets.forEach((fillet) => {
                const filletMaterial = wallTemperatureEnabled && Number.isFinite(fillet.temperature_C) && colorScale
                    ? createWallTemperatureMaterial(fillet.temperature_C, colorScale, viewer.transparentWalls)
                    : wallMaterial;
        ...
        if (viewer.showOpenings) {
            canonicalModel.doors.forEach((door) => shellRoot.add(createOpeningMesh(door, selection, temperatureEnabled, colorScale)));
            canonicalModel.windows.forEach((windowItem) => shellRoot.add(createOpeningMesh(windowItem, selection, temperatureEnabled, colorScale)));
        }
        // Thermal coloring for roofs: exterior surface temperature per level
        const roofLevelAvgRoomTemp = new Map();
        if (temperatureEnabled && colorScale) {
            const tempsByLevel = new Map();
            canonicalModel.rooms.forEach((room) => {
                if (!Number.isFinite(room.temperature_C))
                    return;
                const arr = tempsByLevel.get(room.levelId) ?? [];
                arr.push(room.temperature_C);
        ...
        canonicalModel.roofs.forEach((roof) => {
            ...
            let roofThermalMat = null;
            let roofSurfaceTemp = null;
            if (temperatureEnabled && colorScale) {
            ...
            if (roof.kind === "pitched" && roof.slope) {
                ...
                if (!shape) {
                    if (disposeRoofMat && roofThermalMat)
                        roofThermalMat.dispose();
                    return;
                }
        ...
        canonicalModel.stairs.forEach((stair) => {
            if (stair.boundary.length < 4 || stair.stepCount < 1)
                return;
            ...
            for (let i = 0; i < n; i++) {
                ...
                stepMesh.position.set((minX + maxX) / 2, stair.elevation_m + stepH * (i + 1) / 2, minY + stepD * i + stepD / 2);
        ...
        shellRoot.traverse((obj) => {
            ...
            if (!isShellSolid || !mesh.geometry)
                return;
            // Не дублируем рёбра для уже обработанных мешей
            if (mesh.children.some((c) => c.name.startsWith("edges:")))
                return;
    ...
    useEffect(() => {
        ...
        if (wallTemperatureEnabled && viewer.showWalls) {
            ...
            const ptKey = (v) => `${Math.round(v.x * 20)}_${Math.round(v.y * 20)}`;
            ...
            const isWallExterior = (w) => w.adjacentRoomId === null || w.temperatureSource === "thermalField.boundaries";
            const getCornerCooling = (wallId, pt) => {
            ...
            const roomTempByIdOverlay = new Map(canonicalModel.rooms
                .filter((r) => Number.isFinite(r.temperature_C))
                .map((r) => [r.id, r.temperature_C]));
            const getCornerRoomTemp = (pt, excludeWallId) => {
                const neighbors = (endpointWalls.get(ptKey(pt)) ?? []).filter((id) => id !== excludeWallId);
                const temps = [];
                for (const nId of neighbors) {
                    const nb = wallById.get(nId);
                    if (!nb?.adjacentRoomId)
                        continue;
            ...
            canonicalModel.walls.forEach((wall) => {
                if (!Number.isFinite(wall.temperature_C))
                    return;
                ...
                const openingInfos = (openingsByWallId.get(wall.id) ?? [])
                ...
                addWallThermalMesh(shellRoot, {
                    ...
                    temperature_C: wall.temperature_C,
                }, openingInfos, getCornerCooling(wall.id, wall.start), getCornerCooling(wall.id, wall.end), temperatureColorScale, isWallExterior(wall), getCornerRoomTemp(wall.start, wall.id), getCornerRoomTemp(wall.end, wall.id));
            });
        }
        ...
        if (viewer.showOpenings) {
            [...canonicalModel.windows, ...canonicalModel.doors].forEach((opening) => {
                if (!Number.isFinite(opening.temperature_C))
                    return;
        ...
        if (thermalField) {
            const roofTempsByLevel = new Map();
            canonicalModel.rooms.forEach((room) => {
                if (!Number.isFinite(room.temperature_C))
                    return;
    ...
    return (_jsxs("div", { className: "relative h-full w-full overflow-hidden touch-none", children: [_jsx("canvas", { ref: canvasRef, className: "block h-full w-full touch-none overscroll-contain" }), !suppressTemperatureSummaryOverlay &&
                temperatureEnabled &&
                temperatureColorScale &&
                (coloredRoomFloorCount > 0 ||
                    ...
                    coloredDoorCount > 0) ? (_jsx("div", { className: "pointer-events-none absolute bottom-3 left-3 z-10 max-w-[15rem]", children: _jsx(ThermalFieldLegend, { title: "\u0422\u0435\u043C\u043F\u0435\u0440\u0430\u0442\u0443\u0440\u043D\u043E\u0435 \u043F\u043E\u043B\u0435", minC: temperatureColorScale.min_C, maxC: temperatureColorScale.max_C, avgC: temperatureColorScale.average_C, gradientCss: thermalGradientCss(), source: [
                        ...
                        .filter(Boolean)
                        .join(" · "), warnings: partialTemperatureWarning ? [partialTemperatureWarning] : undefined, thresholds: temperatureColorScale.min_C < 0 && temperatureColorScale.max_C > 0
