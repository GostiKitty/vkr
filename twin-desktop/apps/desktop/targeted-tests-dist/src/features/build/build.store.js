...
import { detectRoomsFromWalls } from "./auto/detectRoomsFromWalls";
...
import { normalizeEngineeringEquipment, normalizeEngineeringPipe, normalizeEngineeringSystems, rebuildEngineeringSystemsForEquipment, } from "./engineering2d/catalog";
...
const DEFAULT_TOOL = "roomRect";
export const DEFAULT_SOLAR_TIME = {
    hour: 12,
    dayOfYear: 172,
    latitudeDeg: 55.75,
};
...
const normalizeOpeningAnchor = (anchor) => ({
...
const storageKey = (projectKey) => `${STORAGE_PREFIX}.${projectKey}`;
const snapshotStorageKey = (projectKey) => `${storageKey(projectKey)}${SNAPSHOT_SUFFIX}`;
const saveStateToStorage = (projectKey, payload) => {
...
const loadPersistedState = (projectKey) => {
    if (typeof window === "undefined") {
        return null;
    }
    try {
...
const saveSnapshotPayload = (projectKey, payload) => {
    if (typeof window === "undefined") {
        return;
    }
    try {
...
const loadSnapshotPayload = (projectKey) => {
    if (typeof window === "undefined") {
        return null;
    }
    try {
...
const hasSnapshotStored = (projectKey) => {
    if (typeof window === "undefined") {
        return false;
    }
    ...
    return ensureModelRevision({
        ...
        roofs: Array.isArray(model.roofs)
            ? model.roofs.map((roof) => ({
                ...
                elevationBase_m: typeof roof.elevationBase_m === "number" && Number.isFinite(roof.elevationBase_m) ? roof.elevationBase_m : 0,
                thickness_m: typeof roof.thickness_m === "number" && Number.isFinite(roof.thickness_m) ? roof.thickness_m : 0.24,
                slope: roof.slope && Number.isFinite(roof.slope.directionDeg) && Number.isFinite(roof.slope.risePerMeter)
        ...
        floorSlabs: Array.isArray(model.floorSlabs)
            ? model.floorSlabs.map((slab) => ({
                ...
                kind: slab.kind === "attic" || slab.kind === "basement" || slab.kind === "ground"
                    ? slab.kind
                    : "interfloor",
        ...
        pipes: Array.isArray(model.pipes)
            ? model.pipes.map((pipe) => {
                const pipeType = pipe.type === "heating_return" || pipe.type === "dhw" || pipe.type === "chw"
                    ? pipe.type
                    : "heating_supply";
                const legacySystemType = typeof pipe.systemType === "string" ? pipe.systemType : null;
                ...
                return {
                    ...
                    systemType: pipe.systemType === "heating" || pipe.systemType === "water" || pipe.systemType === "ventilation" || pipe.systemType === "custom"
                        ? pipe.systemType
                        : resolveNetworkSystemType(pipeType),
                    heatingSystemKind: pipe.heatingSystemKind === "single_pipe" || pipe.heatingSystemKind === "collector"
                    ...
                    circuitRole: pipe.circuitRole === "supply" || pipe.circuitRole === "return" || pipe.circuitRole === "mixed" || pipe.circuitRole === "unknown"
                        ? pipe.circuitRole
                        : resolvePipeCircuitRole(pipeType),
                    segmentClass: pipe.segmentClass ?? defaults.segmentClass,
                    flowDirection: normalizeFlowDirection((pipe.flowDirection ?? defaults.flowDirection)),
                    ...
                    innerDiameter_mm: typeof pipe.innerDiameter_mm === "number" && Number.isFinite(pipe.innerDiameter_mm)
                        ? pipe.innerDiameter_mm
                        : defaults.innerDiameter_mm,
                    material: pipe.material ?? defaults.material,
                    insulationThickness_mm: typeof pipe.insulationThickness_mm === "number" && Number.isFinite(pipe.insulationThickness_mm)
                        ? pipe.insulationThickness_mm
                        : defaults.insulationThickness_mm,
                    insulationConductivity_W_mK: typeof pipe.insulationConductivity_W_mK === "number" && Number.isFinite(pipe.insulationConductivity_W_mK)
                        ? pipe.insulationConductivity_W_mK
                        : defaults.insulationConductivity_W_mK,
                    roughness_mm: typeof pipe.roughness_mm === "number" && Number.isFinite(pipe.roughness_mm)
                        ? pipe.roughness_mm
                        : defaults.roughness_mm,
                    fluidTemperatureC: typeof pipe.fluidTemperatureC === "number" && Number.isFinite(pipe.fluidTemperatureC)
                        ? pipe.fluidTemperatureC
                        : defaults.fluidTemperatureC,
                    designIndoorTemperatureC: typeof pipe.designIndoorTemperatureC === "number" && Number.isFinite(pipe.designIndoorTemperatureC)
                        ? pipe.designIndoorTemperatureC
                        : defaults.designIndoorTemperatureC,
                    designOutdoorTemperatureC: typeof pipe.designOutdoorTemperatureC === "number" && Number.isFinite(pipe.designOutdoorTemperatureC)
                        ? pipe.designOutdoorTemperatureC
                        : defaults.designOutdoorTemperatureC,
                    temperatureDropC: typeof pipe.temperatureDropC === "number" && Number.isFinite(pipe.temperatureDropC)
                        ? pipe.temperatureDropC
                        : defaults.temperatureDropC,
                    flowRate_kg_s: typeof pipe.flowRate_kg_s === "number" && Number.isFinite(pipe.flowRate_kg_s) ? pipe.flowRate_kg_s : defaults.flowRate_kg_s,
                    designVelocity_m_s: typeof pipe.designVelocity_m_s === "number" && Number.isFinite(pipe.designVelocity_m_s)
                    ...
                    pressureDropPa: typeof pipe.pressureDropPa === "number" && Number.isFinite(pipe.pressureDropPa)
                        ? pipe.pressureDropPa
                        : defaults.pressureDropPa,
                    heatLossW: typeof pipe.heatLossW === "number" && Number.isFinite(pipe.heatLossW) ? pipe.heatLossW : defaults.heatLossW,
        ...
        ducts: Array.isArray(model.ducts)
            ? model.ducts.map((duct) => ({
                ...
                airflow_m3_s: typeof duct.airflow_m3_s === "number" && Number.isFinite(duct.airflow_m3_s) ? duct.airflow_m3_s : 0.35,
                airVelocity_m_s: typeof duct.airVelocity_m_s === "number" && Number.isFinite(duct.airVelocity_m_s)
        ...
        activeScenarioId: typeof model.activeScenarioId === "string" ? model.activeScenarioId : DEFAULT_OPERATIONAL_SCENARIOS[0]?.id ?? null,
...
const sanitizeSelection = (selection, model) => {
    if (!selection) {
        return null;
    }
...
const reconcileWallModel = (model, nextWalls, openings) => {
...
const mergeAutoRooms = (model) => {
    const manualRooms = model.rooms.filter((room) => room.source !== "auto");
    const baseModel = { ...model, rooms: manualRooms };
...
export const useBuildStore = create((set, get) => {
    ...
    const initialWithAuto = skipHeavyBootstrap
        ? { model: normalizedInitial, problems: [], loops: [] }
    ...
    if (!skipHeavyBootstrap) {
        saveStateToStorage(initialKey, buildSnapshotFromState(initialRevisioned.model, {
    ...
    const applyModelUpdate = (recipe, options) => {
        set((state) => {
            ...
            const history = options?.trackHistory === false ? state.history : [cloneModel(state.model), ...state.history].slice(0, HISTORY_LIMIT);
    ...
    const replaceModel = (payload, projectKey) => {
        ...
        const revisioned = incomingRevision > 0
    ...
    return {
        ...
        loadModelSnapshot: (model) => {
            const state = get();
            const snapshot = buildSnapshotFromState(normalizeModelRoomNames(model), {
        ...
        addLevel: (level) => {
            const withId = { ...level, id: level.id ?? createId("lvl") };
        ...
        updateLevel: (levelId, patch) => applyModelUpdate((model) => ({
            ...model,
        ...
        addRoom: (room) => applyModelUpdate((model) => ({
            ...model,
            rooms: upsert(model.rooms, { ...room, source: room.source ?? "manual" }),
        })),
        updateRoom: (roomId, patch) => applyModelUpdate((model) => ({
            ...model,
            rooms: model.rooms.map((room) => (room.id === roomId ? { ...room, ...patch } : room)),
        })),
        removeRoom: (roomId) => applyModelUpdate((model) => ({
            ...model,
            rooms: model.rooms.filter((room) => room.id !== roomId),
        }), { resetSelection: true }),
        addWall: (wall) => applyModelUpdate((model) => reconcileWallModel(model, upsert(model.walls, wall)), { recalcRooms: true }),
        setWalls: (walls) => applyModelUpdate((model) => reconcileWallModel(model, walls), { resetSelection: true, recalcRooms: true }),
        updateWall: (wallId, patch) => applyModelUpdate((model) => {
            const nextWalls = model.walls.map((wall) => (wall.id === wallId ? { ...wall, ...patch } : wall));
            return reconcileWallModel(model, nextWalls);
        }, { recalcRooms: true }),
        removeWall: (wallId) => applyModelUpdate((model) => {
            const nextDoors = model.doors.map((door) => door.anchor.wallId === wallId
                ? {
                    ...door,
                ...
                : door);
            const nextWindows = model.windows.map((window) => window.anchor.wallId === wallId
                ? {
                    ...window,
        ...
        setWallFilletAtPoint: (levelId, point, radius) => applyModelUpdate((model) => {
            const existing = model.wallFillets ?? [];
            const index = existing.findIndex((fillet) => fillet.levelId === levelId &&
        ...
        addDuct: (duct) => applyModelUpdate((model) => ({
            ...model,
            ducts: upsert(model.ducts, duct),
        })),
        updateDuct: (ductId, patch, options) => applyModelUpdate((model) => ({
            ...model,
            ducts: model.ducts.map((duct) => (duct.id === ductId ? { ...duct, ...patch } : duct)),
        }), options),
        removeDuct: (ductId) => applyModelUpdate((model) => ({
            ...model,
        ...
        addEquipment: (equipment) => applyModelUpdate((model) => ({
            ...model,
            equipment: upsert(model.equipment, equipment),
        })),
        updateEquipment: (equipmentId, patch, options) => applyModelUpdate((model) => ({
            ...model,
            equipment: model.equipment.map((equipment) => equipment.id === equipmentId ? { ...equipment, ...patch } : equipment),
        }), options),
        removeEquipment: (equipmentId) => applyModelUpdate((model) => ({
            ...model,
            pipes: model.pipes.map((pipe) => ({
                ...pipe,
        ...
        addSensor: (sensor) => applyModelUpdate((model) => ({
            ...model,
            sensors: upsert(model.sensors, sensor),
        })),
        updateSensor: (sensorId, patch, options) => applyModelUpdate((model) => ({
            ...model,
            sensors: model.sensors.map((sensor) => (sensor.id === sensorId ? { ...sensor, ...patch } : sensor)),
        }), options),
        removeSensor: (sensorId) => applyModelUpdate((model) => ({
            ...model,
            sensors: model.sensors.filter((sensor) => sensor.id !== sensorId),
        }), { resetSelection: true }),
        addEngineeringEquipment: (equipment) => applyModelUpdate((model) => {
        ...
        updateEngineeringEquipment: (equipmentId, patch, options) => applyModelUpdate((model) => {
            const current = normalizeEngineeringSystems(model.engineeringSystems ?? createEmptyEngineeringSystems());
            const nextEquipment = current.equipment.map((equipment) => equipment.id === equipmentId ? normalizeEngineeringEquipment({ ...equipment, ...patch, id: equipmentId, type: patch.type ?? equipment.type }) : equipment);
        ...
        removeEngineeringEquipment: (equipmentId) => applyModelUpdate((model) => {
            const current = normalizeEngineeringSystems(model.engineeringSystems ?? createEmptyEngineeringSystems());
            return {
                ...model,
                engineeringSystems: {
        ...
        addEngineeringPipe: (pipe) => applyModelUpdate((model) => {
            const current = normalizeEngineeringSystems(model.engineeringSystems ?? createEmptyEngineeringSystems());
            return {
                ...model,
                engineeringSystems: {
        ...
        updateEngineeringPipe: (pipeId, patch, options) => applyModelUpdate((model) => {
            const current = normalizeEngineeringSystems(model.engineeringSystems ?? createEmptyEngineeringSystems());
            return {
                ...model,
                engineeringSystems: {
                    ...current,
                    pipes: current.pipes.map((pipe) => pipe.id === pipeId ? normalizeEngineeringPipe({ ...pipe, ...patch, id: pipeId, medium: patch.medium ?? pipe.medium }) : pipe),
                },
            };
        }, options),
        removeEngineeringPipe: (pipeId) => applyModelUpdate((model) => {
            const current = normalizeEngineeringSystems(model.engineeringSystems ?? createEmptyEngineeringSystems());
            return {
                ...model,
                engineeringSystems: {
        ...
        setEngineeringSystems: (systems, options) => applyModelUpdate((model) => ({
            ...model,
            engineeringSystems: normalizeEngineeringSystems(systems),
        }), options),
        setActiveScenario: (scenarioId) => applyModelUpdate((model) => ({
            ...model,
            activeScenarioId: scenarioId,
        })),
        upsertScenario: (scenario) => applyModelUpdate((model) => ({
            ...model,
            scenarios: upsert(model.scenarios, scenario),
        })),
        removeScenario: (scenarioId) => applyModelUpdate((model) => ({
            ...model,
        ...
        addEvent: (event, options) => applyModelUpdate((model) => ({
            ...model,
            events: upsert(model.events, event),
        }), options),
        updateEvent: (eventId, patch, options) => applyModelUpdate((model) => ({
            ...model,
            events: model.events.map((event) => (event.id === eventId ? { ...event, ...patch } : event)),
        }), options),
        removeEvent: (eventId, options) => applyModelUpdate((model) => ({
            ...model,
        ...
        createRoomFromLoop: (loopId) => {
            ...
            set((current) => {
                const nextState = {
                    ...current,
                    selection: { kind: "room", id: newRoom.id },
                    roomLoops: current.roomLoops.map((entry) => entry.id === loopId ? { ...entry, roomId: newRoom.id, roomSource: "manual" } : entry),
                };
                persistViewState(nextState);
                return nextState;
            });
...
const buildRoomName = (index) => `Помещение ${index}`;
const upsert = (items, entry) => {
