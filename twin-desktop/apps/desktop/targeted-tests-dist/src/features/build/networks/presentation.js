...
import { resolveNetworkSystemType, } from "../../../entities/networks/types";
import { buildHeatingModelSnapshot, summarizeDucts } from "../../../core/networks/index";
...
function isEngineeringAirPipe(pipe) {
    return ENGINEERING_AIR_MEDIA.has(pipe.medium);
}
function readEngineeringAirPipeFlowM3s(pipe) {
    return typeof pipe.flowRate === "number" && Number.isFinite(pipe.flowRate) ? Math.max(0, pipe.flowRate) / 3600 : 0;
}
function readEngineeringPipeMetadataNumber(metadata, key) {
    const value = metadata?.[key];
    return typeof value === "number" && Number.isFinite(value) ? value : null;
}
function resolveEngineeringAirSection(pipe) {
...
function resolveEngineeringAirSectionFromMetadata(metadata, prefix = "") {
...
function getEngineeringAirSectionTransitionRatio(pipe) {
...
function estimateEngineeringAirVelocityM3s(pipe) {
...
function describeEngineeringAirSection(pipe) {
    return resolveEngineeringAirSection(pipe).label;
}
function getEngineeringEquipmentLabel(equipment) {
    if (!equipment) {
...
function readEngineeringEquipmentAirflowM3s(parameters) {
...
function getEngineeringAirPipeFrictionLossPa(pipe) {
    const airflow = readEngineeringAirPipeFlowM3s(pipe);
...
function getEngineeringAirEquipmentPressureLossPa(equipment) {
    if (!equipment) {
        return 0;
    }
    const explicitPressureDropPa = readEngineeringParameterNumber(equipment.parameters, "pressureDropPa") ??
    ...
    switch (equipment.type) {
        ...
        case "airFilter": {
            ...
            const contaminationPercent = Math.min(100, Math.max(0, readEngineeringParameterNumber(equipment.parameters, "contaminationPercent") ?? 0));
        ...
        case "airFlowRegulatorVar": {
            ...
            const damperPositionPercent = Math.min(100, Math.max(0, readEngineeringParameterNumber(equipment.parameters, "damperPositionPercent") ?? 100));
...
function getEngineeringAirEquipmentAvailablePressurePa(equipment) {
    if (!equipment) {
        return 0;
    }
    ...
    return Math.max(0, readEngineeringParameterNumber(equipment.parameters, "pressurePa") ??
        readEngineeringParameterNumber(equipment.parameters, "availablePressurePa") ??
        0);
}
function pickPreferredEngineeringAirRoute(current, candidate) {
    if (!current) {
        return candidate;
    }
...
function buildEngineeringAirRouteHydraulics(airPipes, engineeringEquipmentById) {
    const routeByPipeId = new Map();
    const outgoingSupplyPipesByEquipmentId = new Map();
    const incomingExhaustPipesByEquipmentId = new Map();
    const frictionLossByPipeId = new Map(airPipes.map((pipe) => [pipe.id, getEngineeringAirPipeFrictionLossPa(pipe)]));
    ...
    const registerRoute = (pipeId, candidate) => {
    ...
    engineeringEquipmentById.forEach((equipment) => {
        ...
        const bestSupplyLossByEquipmentId = new Map([[equipment.id, 0]]);
        const traverseSupply = (equipmentId, routePressureDropPa) => {
            ...
            outgoingPipes.forEach((pipe) => {
                ...
                const segmentPressureDropPa = (frictionLossByPipeId.get(pipe.id) ?? 0) + getEngineeringAirEquipmentPressureLossPa(downstreamEquipment);
        ...
        const bestExhaustLossByEquipmentId = new Map([[equipment.id, 0]]);
        const traverseExhaust = (equipmentId, routePressureDropPa) => {
            ...
            incomingPipes.forEach((pipe) => {
                ...
                const segmentPressureDropPa = (frictionLossByPipeId.get(pipe.id) ?? 0) + getEngineeringAirEquipmentPressureLossPa(upstreamEquipment);
                const nextRoutePressureDropPa = routePressureDropPa + segmentPressureDropPa;
                registerRoute(pipe.id, {
                    sourceEquipmentId: equipment.id,
                    availablePressurePa,
...
function buildEngineeringAirSummary(model) {
    ...
    const branches = airPipes.map((pipe) => {
        ...
        const fallbackSegmentPressureDropPa = frictionPressureDropPa +
        ...
        const availablePressurePa = hydraulics?.availablePressurePa ??
            Math.max(getEngineeringAirEquipmentAvailablePressurePa(fromEquipment), getEngineeringAirEquipmentAvailablePressurePa(toEquipment));
    ...
    const connectedEquipmentIds = new Set();
    airPipes.forEach((pipe) => {
    ...
    const averageAirVelocity_m_s = branches.length > 0 ? branches.reduce((sum, branch) => sum + branch.airVelocity_m_s, 0) / branches.length : 0;
...
function buildEngineeringRoomAirBalances(model) {
    const balances = new Map();
    const ensureBalance = (roomId, roomName) => {
    ...
    (model.engineeringSystems?.equipment ?? []).forEach((equipment) => {
        ...
        const room = model.rooms.find((entry) => entry.levelId === equipment.levelId && polygonContainsPoint({ x: equipment.x, y: equipment.y }, entry.polygon));
        if (!room) {
            return;
        }
...
export function buildNetworkSystemsPresentation(model, snapshot) {
    ...
    const equipmentById = new Map(model.equipment.map((entry) => [entry.id, entry]));
    const sensorById = new Map(model.sensors.map((entry) => [entry.id, entry]));
    ...
    const suggestions = snapshot.suggestedConnections.slice(0, 6).map((entry) => {
        ...
        const networkTitle = entry.networkKind === "pipe"
        ...
        return {
            ...
            detail: entry.status === "compatible"
    ...
    const connectedDuctCount = model.ducts.filter((duct) => (networkStateById.get(duct.id)?.connectedEquipmentIds.length ?? 0) > 0).length +
    ...
    const averageAirVelocityM3s = totalAirBranchCount > 0
    ...
    return {
        ...
        pipe: {
            ...
            estimatedPressureDropPa: pipePressureDropPa > 0
                ? pipePressureDropPa
                : heatingSnapshot.systems.reduce((sum, system) => sum + system.branches.reduce((branchSum, branch) => branchSum + branch.pressureDropPa, 0), 0),
...
function buildPipeFamilies(pipes, networkStateById) {
...
function buildWarnings(model, snapshot, heatingSnapshot, equipmentById, engineeringAirSummary) {
    const warnings = [];
    const networkStateById = new Map(snapshot.networkStates.map((entry) => [entry.networkId, entry]));
    const engineeringAirEquipment = (model.engineeringSystems?.equipment ?? []).filter((item) => item.ports.some((port) => ENGINEERING_AIR_MEDIA.has(port.medium)));
    const engineeringAirPipes = (model.engineeringSystems?.pipes ?? []).filter(isEngineeringAirPipe);
    const engineeringAirPipeById = new Map(engineeringAirPipes.map((pipe) => [pipe.id, pipe]));
    const engineeringAirPipeIdsByEquipmentId = new Map();
    const seen = new Set();
    const systemById = new Map(heatingSnapshot.systems.map((entry) => [entry.id, entry]));
    engineeringAirPipes.forEach((pipe) => {
        const fromIds = engineeringAirPipeIdsByEquipmentId.get(pipe.fromEquipmentId) ?? new Set();
        fromIds.add(pipe.id);
        engineeringAirPipeIdsByEquipmentId.set(pipe.fromEquipmentId, fromIds);
        const toIds = engineeringAirPipeIdsByEquipmentId.get(pipe.toEquipmentId) ?? new Set();
        toIds.add(pipe.id);
...
function formatHeatingIssue(issue, systemName, equipmentById, model) {
    switch (issue.code) {
...
function getEquipmentLabel(equipment, equipmentList) {
    if (!equipment) {
        return "Оборудование";
    }
...
function getPipeLabel(pipe, pipes) {
    if (!pipe) {
        return "Труба";
    }
...
function getDuctLabel(duct, ducts) {
    if (!duct) {
        return "Воздуховод";
    }
...
function describeDuctSection(duct) {
