...
function SelectionForm({ model, selection, neighbors, roomNames, loops, roomEnvelopes, slabPreset, entity, onUpdateRoom, onUpdateWall, onUpdateRoof, onAddFloorSlab, onUpdateFloorSlab, onUpdateDoor, onUpdateWindow, onUpdatePipe, onUpdateDuct, onUpdateEquipment, onUpdateSensor, onUpdateStair, onUpdateEngineeringEquipment, onUpdateEngineeringPipe, onCreateRoomFromLoop, }) {
    ...
    switch (selection.kind) {
        ...
        case "stair":
            return _jsx(StairForm, { stair: entity, levels: model.levels, onUpdateStair: (id, patch) => onUpdateStair?.(id, patch) });
        case "door":
...
function WallForm({ wall, levels, onUpdateWall, }) {
    ...
    return (_jsxs("div", { className: "space-y-3", children: [_jsx(EnvelopePresetField, { kind: "wall", value: presetId, onChange: (value) => onUpdateWall(wall.id, applyEnvelopePresetToWall(wall, value)) }), _jsx(InfoCard, { rows: [
                ...
                ] }), _jsxs("div", { className: "grid grid-cols-2 gap-3", children: [_jsx(NumberField, { label: hasCustomLayers ? "Толщина (авто), м" : "Толщина, м", value: wall.thickness_m, step: 0.01, onChange: (value) => onUpdateWall(wall.id, { thickness_m: value }) }), _jsx(NumberField, { label: "\u0412\u044B\u0441\u043E\u0442\u0430, \u043C", value: wall.height_m, step: 0.1, onChange: (value) => onUpdateWall(wall.id, { height_m: value }) })] }), properties ? (_jsx(CollapsibleSection, { title: "\u0422\u0435\u043F\u043B\u043E\u0442\u0435\u0445\u043D\u0438\u043A\u0430", children: _jsx("div", { className: "pt-3", children: _jsxs("div", { className: "grid grid-cols-2 gap-2 rounded-2xl border border-[color:var(--border-soft)] bg-[color:var(--surface-base)] p-3", children: [_jsx(ValueTile, { label: "R, \u043C\u00B2\u041A/\u0412\u0442", value: properties.rValue.toFixed(3) }), _jsx(ValueTile, { label: "U, \u0412\u0442/(\u043C\u00B2\u041A)", value: properties.uValue.toFixed(3) }), _jsx("div", { className: "col-span-2", children: _jsx(ValueTile, { label: "\u0422\u0435\u043F\u043B\u043E\u0451\u043C\u043A\u043E\u0441\u0442\u044C, \u0414\u0436/(\u043C\u00B2\u041A)", value: properties.heatCapacity_J_m2K.toFixed(0) }) })] }) }) })) : null, _jsx(CollapsibleSection, { title: "\u0421\u043B\u043E\u0438", children: _jsx("div", { className: "pt-3", children: _jsx(ConstructionLayerEditor, { title: "\u0421\u043E\u0441\u0442\u0430\u0432 \u0441\u0442\u0435\u043D\u044B", layers: layers, hasCustomLayers: hasCustomLayers, onUpdateLayer: updateLayer, onAddLayer: addLayer, onRemoveLayer: removeLayer, onReset: resetToAssembly, resetLabel: "\u0412\u0435\u0440\u043D\u0443\u0442\u044C \u043F\u043E \u0441\u0431\u043E\u0440\u043A\u0435" }) }) })] }));
}
function StairForm({ stair, levels, onUpdateStair, }) {
    const area = stair.boundary.length >= 4
        ? Math.abs(stair.boundary.reduce((sum, p, i, arr) => {
            const next = arr[(i + 1) % arr.length];
            return sum + p.x * next.y - next.x * p.y;
        }, 0) / 2)
        : 0;
    return (_jsxs("div", { className: "space-y-3", children: [_jsx(TextField, { label: "\u041D\u0430\u0437\u0432\u0430\u043D\u0438\u0435", value: stair.name, onChange: (value) => onUpdateStair(stair.id, { name: value }) }), _jsx(InfoCard, { rows: [
                    { label: "Этаж", value: getLevelDisplayLabel({ levels }, stair.levelId) },
                    { label: "Площадь", value: `${area.toFixed(2)} м²` },
                ] }), _jsx(NumberField, { label: "\u0427\u0438\u0441\u043B\u043E \u0441\u0442\u0443\u043F\u0435\u043D\u0435\u0439", value: stair.stepCount, min: 2, max: 40, step: 1, onChange: (value) => onUpdateStair(stair.id, { stepCount: Math.round(Math.max(2, value)) }) }), _jsx(NumberField, { label: "\u0412\u044B\u0441\u043E\u0442\u0430 \u043C\u0430\u0440\u0448\u0430, \u043C", value: stair.totalRise_m, min: 0.3, max: 10, step: 0.1, onChange: (value) => onUpdateStair(stair.id, { totalRise_m: value }) })] }));
}
