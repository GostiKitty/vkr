...
import { describeEnvelopePreset, getEnvelopePreset, resolveDefaultPresetId, resolvePresetLayers, } from "../../../entities/envelope/envelopePresets";
import { getDoorEnvelopeLabel, getRoofEnvelopeLabel, getSlabEnvelopeLabel, getWallEnvelopeLabel, getWindowEnvelopeLabel, } from "../utils/entityLabels";
...
function buildFragmentMetadata(presetId, runtimeU) {
...
function resolveWallLayers(wall) {
...
function resolveRoofLayers(roof) {
...
function resolveSlabLayers(slab) {
...
function resolveOpeningLayers(opening, kind) {
...
function mapSlabConstructionType(kind) {
    switch (kind) {
        case "attic":
            return "atticFloor";
        case "basement":
        ...
        case "ground":
            return "floorOnGround";
        default:
            return "atticFloor";
    }
}
export function syncThermalProtectionEnvelope(model) {
    const previousEnvelopeById = new Map((model.thermalProtection?.envelope ?? []).map((fragment) => [fragment.id, fragment]));
    const envelope = [];
    const wallFillets = model.wallFillets ?? [];
    const roundedRoomArea = (room) => {
        if (!wallFillets.length) {
            return Math.abs(polygonArea(room.polygon));
        }
        const radiusForIndex = (index) => {
            const vertex = room.polygon[index];
            const match = wallFillets.find((fillet) => fillet.levelId === room.levelId &&
