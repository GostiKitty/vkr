...
export function formatDisplayEnum(value, fallback = REPORT_EXPORT_NOT_SET) {
...
export function formatDynamicMetricValue(value, dynamicResultState, options = {}) {
    if (value !== null && value !== undefined && Number.isFinite(value)) {
 ...
 */
export function formatStatus(complies, hasData = true) {
    if (!hasData) {
 ...
 */
export function textOrNotSet(value) {
    if (typeof value !== "string") {
 ...
 */
export function textOrNoData(value) {
    if (typeof value !== "string") {
        return REPORT_EXPORT_NO_DATA;
    }
    const trimmed = value.trim();
 ...
 */
export function renderDataTable(title, columns, rows, options = {}) {
 ...
 */
export function renderGostTitlePage(meta, documentLabel, documentSubtitle) {
    return `
 ...
 */
export function renderGostStamp(meta, documentLabel) {
    return `
...
export function renderToc(items) {
    ...
    const li = items
        .map((item) => `<li class="rx-toc-item"><a href="#${escapeHtml(item.id)}" class="rx-toc-link">${escapeHtml(item.label)}</a></li>`)
        .join("");
    return `
...
export function renderDocumentStatusBlock(preflight, options = {}) {
    const title = options.title ?? "Статус документа";
    const issueRows = [];
    if (options.includeIssues !== false) {
        issueRows.push(...preflight.blockingIssues.map((issue) => `<li>${escapeHtml(issue.message)}</li>`));
    }
    if (options.includeWarnings) {
        issueRows.push(...preflight.warningIssues.map((issue) => `<li>${escapeHtml(issue.message)}</li>`));
    }
    return `
 ...
 */
export function renderSignatureBlock(meta) {
    return `
 ...
 */
export const REPORT_EXPORT_RC_DISCLAIMER = "Справочная динамическая оценка не заменяет нормативную проверку по СП 50.";
/**
 ...
 */
export function renderAssumptionsBlock(entries, options = {}) {
    ...
    const rows = list
        .map((entry) => `<tr><td class="rx-td rx-align-left">${escapeHtml(entry.label)}</td><td class="rx-td rx-align-left">${escapeHtml(entry.value)}</td></tr>`)
        .join("");
    return `
 ...
 */
export function wrapHtmlDocument(templateMarker, documentTitle, body) {
    return `<!DOCTYPE html>
${templateMarker}
<html lang="ru">
<head>
