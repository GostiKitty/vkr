...
export const DEFAULT_EXPERTISE_REPORT_INPUTS = {
    projectName: "",
    objectAddress: "",
    projectCipher: "",
    documentStage: "",
...
export const EXPERTISE_INPUT_SECTIONS = [
    {
        id: "project",
...
function sanitizeLoadedInputs(candidate) {
    return {
        ...DEFAULT_EXPERTISE_REPORT_INPUTS,
        ...candidate,
        solarGainsMode: candidate?.solarGainsMode === "manual" || candidate?.solarGainsMode === "omit"
            ? candidate.solarGainsMode
            : "auto-if-available",
        mechanicalVentilation: candidate?.mechanicalVentilation === "yes" || candidate?.mechanicalVentilation === "no"
            ? candidate.mechanicalVentilation
            : "",
        exportMode: candidate?.exportMode === "strict-expertise" ||
            candidate?.exportMode === "vkr-brief"
            ? candidate.exportMode
            : "demo",
        handleMissingDataMode: candidate?.handleMissingDataMode === "hide" ||
...
export function loadExpertiseInputs(projectKey) {
    ...
    try {
        ...
        return sanitizeLoadedInputs(JSON.parse(raw));
    }
    catch {
        return { ...DEFAULT_EXPERTISE_REPORT_INPUTS };
    }
}
export function saveExpertiseInputs(projectKey, inputs) {
    if (typeof window === "undefined" || !projectKey) {
        return;
    }
    try {
        window.localStorage.setItem(`${STORAGE_KEY}:${projectKey}`, JSON.stringify(inputs));
    }
    catch {
        // ignore quota/privacy failures
    }
}
export const useExpertiseInputsStore = create((set, get) => ({
    isPanelOpen: false,
    ...
    replaceInputs: (projectKey, next) => set((state) => {
        saveExpertiseInputs(projectKey, next);
        return {
            inputsByProject: {
                ...state.inputsByProject,
    ...
    resetProject: (projectKey) => set((state) => {
