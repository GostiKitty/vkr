...
export function generateOperationTechnicalPassportHtml(data, options = {}) {
    const titlePage = renderGostTitlePage(data.meta, DOCUMENT_LABEL, "Проектная стадия");
    const sections = [
        renderSection(data.documentInfo),
        renderInputRegisterSection("op-registry", "Ведомость исходных данных", data.sourceRegisterRows, data.expertise.showIncompleteFields),
    ...
    const appendices = data.appendices
        .map((appendix) => `<section id="${escapeHtml(appendix.id)}"><h3 class="rx-section-title">${escapeHtml(appendix.title)}</h3>${renderDataTable(null, [
        { label: "Показатель", width: "55%" },
        { label: "Значение", width: "30%" },
        { label: "Примечание", width: "15%" },
    ], appendix.rows.map((row) => ({ cells: [row.label, row.value, row.note || "—"] })))}</section>`)
        .join("\n");
    const body = `
${titlePage}
...
`;
    return wrapHtmlDocument(REPORT_EXPORT_TEMPLATE_MARKER["operation-technical-passport"], REPORT_EXPORT_TITLE["operation-technical-passport"], body);
}
