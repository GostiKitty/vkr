...
import { REPORT_EXPORT_NEEDS_CLARIFICATION, REPORT_EXPORT_NO_DATA, } from "../helpers";
export function buildThermalProtectionData(base) {
    const { meta, expertise, preflight, reportMetrics, envelopeView, sp50Report } = base;
    const totalEnvelopeArea = envelopeView.includedElements.reduce((sum, element) => sum + (element.areaM2 ?? 0), 0);
    const climateRows = [
    ...
    const envelopeResistanceRows = envelopeView.includedElements.map((entry) => mapResistanceRow(entry));
    ...
    const energyCharacteristicRows = [
        metric("qHeating", "Удельная характеристика расхода тепловой энергии qот", "Вт/(м³·К)", formatValue(reportMetrics.qHeatingCharacteristic_W_m3K, 3)),
        metric("qNorm", "Нормируемое значение qот", "кВт·ч/м²", formatValue(reportMetrics.qHeatingNorm_kWh_m2, 2)),
        ...
        metric("qByArea", "Удельный расход по площади", "кВт·ч/м²", formatValue(reportMetrics.qByArea_kWh_m2, 2)),
        metric("qByVolume", "Удельный расход по объёму", "кВт·ч/м³", formatValue(reportMetrics.qByVolume_kWh_m3, 3)),
    ];
    const complianceRows = [
        ...
        metric("elementStatus", "Поэлементная проверка оболочки", "", summarizeEnvelopeStatus(base)),
    ...
    return {
        ...
        appendixComposition: buildCompositionAppendix(sp50Report?.constructions ?? [], envelopeView.includedElements, expertise.showTechnicalIdsInAppendix),
...
function buildCompositionSummary(elements) {
    if (!elements.length) {
    ...
    return Array.from(groups.entries()).map(([label, group], index) => metric(`composition-${index}`, label, "шт. / м²", `${group.count} / ${formatValue(group.area, 2)}`));
}
function buildCompositionAppendix(constructions, includedElements, showTechnicalIdsInAppendix) {
    ...
    constructions.forEach((construction) => {
        ...
        construction.layers.forEach((layer, index) => {
            rows.push({
                ...
                thicknessMm: Number.isFinite(layer.thicknessM) && layer.thicknessM > 0
...
function mapResistanceRow(entry) {
    return {
        key: entry.key,
...
function buildKobFragmentRows(elements, totalEnvelopeArea) {
    const totalContribution = elements.reduce((sum, element) => sum + (element.contribution ?? 0), 0);
    return elements.map((entry) => ({
        ...
        share: totalContribution > 0 && totalEnvelopeArea > 0
            ? formatValue(((entry.contribution ?? 0) / totalContribution) * 100, 1)
            : REPORT_EXPORT_NO_DATA,
    }));
}
function buildConclusions(base) {
    const lines = [];
    const metrics = base.reportMetrics;
    const failedElements = metrics.envelopeElementFailures;
    const integralPass = metrics.kobStatus === "соответствует" &&
        metrics.qHeatingStatus === "соответствует";
    if (integralPass && failedElements.length > 0) {
        lines.push(`Соответствует по интегральным показателям. Требуется устранить несоответствие поэлементной проверки: ${failedElements
            .map((entry) => `${entry.designation} (${entry.typeLabel})`)
            .join(", ")}.`);
    }
    else if (integralPass) {
        lines.push("Расчётные интегральные показатели тепловой защиты и энергоэффективности соответствуют нормативным требованиям.");
    }
    else if (metrics.kobStatus === "не соответствует" ||
        metrics.qHeatingStatus === "не соответствует") {
        lines.push("Требуется корректировка проектных решений по тепловой оболочке и/или инженерным параметрам для достижения нормативных показателей.");
    }
    else {
        lines.push("Окончательный нормативный вывод не может быть сделан до устранения замечаний по исходным данным и поэлементной проверке.");
    }
    if (metrics.usesPlaceholderInputs) {
        lines.push("Энергетические показатели сформированы на неполной исходной базе и подлежат уточнению после задания проектных параметров вентиляции и инфильтрации.");
    }
    if (base.preflight.status === "not-ready") {
        lines.push("Финальный статус комплекта: НЕ ГОТОВО К ВЫПУСКУ ДО УСТРАНЕНИЯ ЗАМЕЧАНИЙ.");
    }
...
function summarizeEnvelopeStatus(base) {
...
function buildRequiredResistanceBasis(entry) {
...
function buildSourceDataLabel(entry) {
    switch (entry.category) {
        case "window":
        case "external-door":
...
function formatRequiredResistance(entry) {
...
function metric(key, label, unit, value) {
    return { key, label, unit, value };
}
function uniqueMessages(model, values) {
    const seen = new Set();
    const result = [];
