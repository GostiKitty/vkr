export function askEngineeringAssistant(question, context = {}) {
...
function findRelevantWarnings(question, warnings) {
...
function buildExplanation(question, warnings, context) {
    ...
    if (question.includes("energy") && context.energySimulation) {
        return {
            answer: `The latest energy simulation estimates a peak load of ${context.energySimulation.totals.peakDemand_W.toFixed(1)} W with ventilation energy of ${context.energySimulation.totals.ventilationEnergy_W.toFixed(1)} W.`,
            recommendations: [
