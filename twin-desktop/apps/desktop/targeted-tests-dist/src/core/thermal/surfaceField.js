import { pointToSegmentDistance, polygonArea, polygonContainsPoint, polygonCentroid, segmentLength, } from "../../entities/geometry/geom";
import { computeWallProperties } from "../../entities/material/types";
import { DEFAULT_DOOR_U_W_M2K, DEFAULT_WINDOW_U_W_M2K, } from "./wallFacadeThermal";
...
export function buildSurfaceFieldResult(input) {
    ...
    const renderGeometry = input.renderGeometry ??
    ...
    const physics = input.physics ??
        ...
        buildThermalPhysicsModel(input.model, {
    ...
    const mutablePatches = surfaceDescriptors.flatMap((surface) => buildSurfacePatches(surface, patchSizeM, allHeatSources, thermalBridges, indoorRelativeHumidity));
    const roomMeanSurfaceByRoomId = new Map();
    roomVolumes.forEach((room) => {
        const roomPatches = mutablePatches.filter((patch) => patch.roomId === room.roomId);
        if (!roomPatches.length) {
            return;
        }
        const weighted = roomPatches.reduce((acc, patch) => {
    ...
    mutablePatches.forEach((patch) => {
        ...
        const couplingC = (roomMean - patch.patchTemperatureC) * DEFAULT_ROOM_MEAN_RADIATIVE_COUPLING;
        patch.patchTemperatureC += couplingC;
        patch.cornerTemperaturesC = patch.cornerTemperaturesC.map((value) => value + couplingC);
    });
    ...
    const surfaces = surfaceDescriptors.map((surface) => {
    ...
    const roomDiagnostics = roomVolumes.map((room) => {
    ...
    const flatSurfaces = surfaces.filter((s) => s.patchIds.length > 2 && (s.maxTemperatureC - s.minTemperatureC) < 0.2);
    if (flatSurfaces.length > 0 && flatSurfaces.length >= surfaces.length * 0.5) {
        warnings.push(`Thermal field appears flat on ${flatSurfaces.length}/${surfaces.length} surfaces (range < 0.2 °C). ` +
    ...
    if (import.meta.env?.DEV) {
        ...
        surfaces.forEach((s) => {
            ...
            console.debug(`[SurfaceField] ${s.kind.padEnd(8)} patches=${String(s.patchIds.length).padStart(3)} ` +
...
export function getSurfaceFieldPatchValue(patch, mode) {
    switch (mode) {
 ...
 */
export function getSurfaceFieldValueRange(result, mode) {
...
function buildRoomAirTemperatureMap(input, renderGeometry) {
    const map = new Map();
...
function computeWallWarmthFromLevelBelow(args) {
    ...
    const belowRoom = args.roomVolumes.find((room) => room.levelId === belowLevelId && polygonContainsPoint(args.wallMidPlan, room.polygon));
    if (!belowRoom) {
...
function buildWallSurfaceDescriptors(args) {
    ...
    const vAxis = { x: 0, y: 1, z: 0 };
    const baseWallStart = { x: wall.a.x, y: levelElevationM, z: wall.a.y };
    ...
    const createDescriptorForRoom = (roomId, sideSign, boundaryType, boundaryTemperatureC) => {
        ...
        const surfaceResistanceM2K_W = boundaryType === "internal" ? DEFAULT_INTERNAL_COUPLING_SURFACE_RESISTANCE : DEFAULT_INTERNAL_SURFACE_RESISTANCE;
        const effectiveResistance = boundaryType === "internal"
            ? Math.max(args.surface.effectiveR_m2K_W + DEFAULT_INTERNAL_COUPLING_SURFACE_RESISTANCE * 2, 0.24)
            : totalResistance;
        const baseTemperatureC = boundaryType === "internal"
            ? airTemperatureC -
                ((airTemperatureC - boundaryTemperatureC) * surfaceResistanceM2K_W) /
                    Math.max(effectiveResistance, surfaceResistanceM2K_W + 0.01)
            : internalSurfaceTemperatureC(airTemperatureC, boundaryTemperatureC, surfaceResistanceM2K_W, Math.max(effectiveResistance, surfaceResistanceM2K_W + 0.01));
        const centroid = localToWorld(origin, uAxis, vAxis, wallLengthM * 0.5, Math.max(wall.height_m * 0.5, 0.1));
        ...
        args.openings.forEach((opening) => {
            const openingOrigin = localToWorld(origin, uAxis, vAxis, opening.startOffsetM, opening.sillM ?? 0);
            const openingU = Math.max(0.5, opening.uValue_W_m2K ??
            ...
            const openingBaseTemperatureC = boundaryType === "internal"
                ? airTemperatureC -
                    ((airTemperatureC - boundaryTemperatureC) * surfaceResistanceM2K_W) /
                        Math.max(openingResistance, surfaceResistanceM2K_W + 0.01)
                : internalSurfaceTemperatureC(airTemperatureC, boundaryTemperatureC, surfaceResistanceM2K_W, openingResistance);
            descriptors.push({
                ...
                centroid: localToWorld(openingOrigin, uAxis, vAxis, Math.max(0.12, opening.widthM) * 0.5, Math.max(0.12, opening.heightM) * 0.5),
    ...
    const positiveBoundaryTemperatureC = args.surface.kind === "internal"
        ? args.surface.negativeRoomId
            ? args.roomAirTemperatures.get(args.surface.negativeRoomId) ?? 21
            : args.outdoorTemperatureC
        : args.outdoorTemperatureC;
    const negativeBoundaryTemperatureC = args.surface.kind === "internal"
        ? args.surface.positiveRoomId
            ? args.roomAirTemperatures.get(args.surface.positiveRoomId) ?? 21
            : args.outdoorTemperatureC
        : args.outdoorTemperatureC;
    createDescriptorForRoom(args.surface.positiveRoomId, 1, args.surface.kind === "internal" ? "internal" : "external", positiveBoundaryTemperatureC);
    createDescriptorForRoom(args.surface.negativeRoomId, -1, args.surface.kind === "internal" ? "internal" : "external", negativeBoundaryTemperatureC);
    return descriptors;
}
function findLevelIdDirectlyBelow(levelId, levelElevationById, levelHeightById) {
...
function findLevelIdDirectlyAbove(levelId, levelElevationById, levelHeightById) {
...
function resolveInterfloorSlabForHorizontalSurface(args) {
    if (args.kind === "ceiling") {
        return args.slabs.find((slab) => slab.levelId === args.room.levelId &&
    ...
    return args.slabs.find((slab) => slab.levelId === belowLevelId &&
        slab.kind === "interfloor" &&
        polygonContainsPoint(args.centroid2D, slab.boundary));
}
function resolveFloorSlabForRoom(args) {
    const onLevel = args.slabs.filter((slab) => slab.levelId === args.room.levelId && polygonContainsPoint(args.centroid2D, slab.boundary));
...
function buildHorizontalSurfaceDescriptor(args) {
    ...
    const matchingRoof = args.roofs.find((roof) => roof.levelId === args.room.levelId && polygonContainsPoint(centroid2D, roof.boundary));
    const matchingSlab = args.kind === "floor"
    ...
    const adjacentLevelId = args.kind === "ceiling"
    ...
    const adjacentRoomAcrossSlab = adjacentLevelId
        ? args.roomVolumes.find((entry) => entry.levelId === adjacentLevelId && polygonContainsPoint(centroid2D, entry.polygon)) ?? null
        : null;
    const adjacentRoomAirTemperatureC = adjacentRoomAcrossSlab !== null
    ...
    const baseTemperatureC = boundaryType === "neutral"
        ? roomAirTemperatureC -
            ((roomAirTemperatureC - boundaryTemperatureC) * surfaceResistanceM2K_W) /
                Math.max(effectiveResistance, surfaceResistanceM2K_W + 0.01)
        : internalSurfaceTemperatureC(roomAirTemperatureC, boundaryTemperatureC, surfaceResistanceM2K_W, effectiveResistance);
    const minX = Math.min(...polygon.map((point) => point.x));
    const minY = Math.min(...polygon.map((point) => point.y));
    const maxX = Math.max(...polygon.map((point) => point.x));
    const maxY = Math.max(...polygon.map((point) => point.y));
    const elevationM = args.kind === "floor"
        ? levelElevationM + DEFAULT_OVERLAY_OFFSET_M
        : levelElevationM + levelHeightM - DEFAULT_OVERLAY_OFFSET_M;
    const normal = args.kind === "floor"
        ? { x: 0, y: 1, z: 0 }
        : { x: 0, y: -1, z: 0 };
    return {
...
function buildSurfacePatches(surface, patchSizeM, heatSources, thermalBridges, indoorRelativeHumidity) {
    ...
    for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
            ...
            const cornersWorld = cornersLocal.map((corner) => localToWorld(surface.origin, surface.uAxis, surface.vAxis, corner.x, corner.y));
            const centerSample = sampleSurfaceAtPoint(surface, centerLocal, centerWorld, heatSources, thermalBridges);
            const cornerSamples = cornersLocal.map((corner, index) => polygonContainsPoint(corner, surface.localPolygon)
                ? sampleSurfaceAtPoint(surface, corner, cornersWorld[index], heatSources, thermalBridges)
                : centerSample);
            const cornerTemperaturesC = cornerSamples.map((sample) => sample.temperatureC);
 ...
 */
function smoothSurfacePatches(patches) {
    if (patches.length <= 1) {
        return;
    }
    const ITERATIONS = 2;
    ...
    for (let iter = 0; iter < ITERATIONS; iter += 1) {
        ...
        const newCorners = new Array(patches.length);
        patches.forEach((patch, i) => {
            ...
            if (!neighborIdxs.length) {
                newCorners[i] = [...patch.cornerTemperaturesC];
                return;
            }
            ...
            const smoothed = patch.cornerTemperaturesC.map((selfT, ci) => {
                const neighborAvg = neighborIdxs.reduce((sum, idx) => sum + patches[idx].cornerTemperaturesC[ci], 0) / nCount;
...
function sampleSurfaceAtPoint(surface, localPoint, worldPoint, heatSources, thermalBridges) {
    ...
    const sourceContributions = heatSources
        ...
        if (!isSameLevel) {
            if (isHorizontalSurface) {
                ...
                if (horizDistM > INTER_LEVEL_RADIUS_M * 3.0)
                    return null;
        ...
        if (source.kind !== "pipe" && source.kind !== "floorHeating") {
            if (source.roomId && source.roomId !== surface.roomId)
                return null;
        }
        ...
        const deltaTMaxC = resolveSourceDeltaTMaxC(source);
        const falloff = source.falloffType === "gaussian"
        ...
        .filter((entry) => entry !== null && entry.deltaC > 0.01);
    const sourceDeltaC = clamp(sourceContributions.reduce((sum, entry) => sum + entry.deltaC, 0), 0, 10.5);
    ...
    const edgePenaltyC = (() => {
        if (surface.kind === "wall") {
            const verticalEdgeDistanceM = Math.min(localPoint.x, Math.max(surface.widthM - localPoint.x, 0));
            ...
            const floorJunctionPenalty = surface.warmthFromLevelBelowMaxC > 0.15
            ...
            const ceilingJunctionPenalty = 0.38 * Math.exp((-2.1 * Math.max(surface.heightM - localPoint.y, 0)) / DEFAULT_CORNER_RADIUS_M);
    ...
    const belowLevelWarmthC = surface.kind === "wall" && surface.warmthFromLevelBelowMaxC > 0
    ...
    const convectiveBlend = clamp((DEFAULT_CONVECTIVE_H_W_M2K * surface.surfaceResistanceM2K_W) /
        (1 + DEFAULT_CONVECTIVE_H_W_M2K * surface.surfaceResistanceM2K_W), 0.05, 0.16);
...
function computePatchHeatFluxWm2(surface, patchTemperatureC) {
...
function buildModelHeatSources(model, roomVolumes, levelElevationById) {
    const sources = [];
    const roomById = new Map(roomVolumes.map((room) => [room.roomId, room]));
    model.equipment.forEach((equipment) => {
        const roomId = equipment.roomId ??
        ...
        const totalPowerW = Math.max(equipment.params.nominalPowerW ??
            (equipment.type === "radiator" ? 1600 : equipment.type === "fancoil" ? 950 : 220), 0);
        if (totalPowerW <= 0) {
            return;
        }
...
function resolveSourceDeltaTMaxC(source) {
    ...
    switch (source.kind) {
        ...
        case "pipe": {
            const fluidTempC = source.metadata?.fluidTemperatureC;
            const isSupply = source.metadata?.pipeType === "heating_supply";
            const effectiveTempC = typeof fluidTempC === "number" && fluidTempC > 25
                ? fluidTempC
                : isSupply ? 65 : 55;
...
function resolveConstructionResistance(layers, fallbackResistanceM2K_W) {
 ...
 */
function horizontalDistanceToSource(point, source) {
    const flatPoint = { x: point.x, y: 0, z: point.z };
    if (source.segment) {
        const flatStart = { x: source.segment.start.x, y: 0, z: source.segment.start.z };
        const flatEnd = { x: source.segment.end.x, y: 0, z: source.segment.end.z };
        return distancePointToSegment3(flatPoint, flatStart, flatEnd);
    }
    const flatPos = { x: source.position.x, y: 0, z: source.position.z };
...
function localToWorld(origin, uAxis, vAxis, u, v) {
    return {
...
function distancePointToSegment3(point, start, end) {
    const ab = {
        x: end.x - start.x,
        y: end.y - start.y,
        z: end.z - start.z,
