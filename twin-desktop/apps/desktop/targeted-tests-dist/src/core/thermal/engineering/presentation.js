...
export function buildPresentationSummary(result) {
    const targetC = result.comfort.targetTemperatureC;
    const recommendedBand = `${formatValue(targetC - 1, 1)}–${formatValue(targetC + 1, 1)} °C`;
    const totalAreaM2 = Math.max(result.rooms.reduce((sum, room) => sum + room.areaM2, 0), 1);
    ...
    const metrics = [
        {
            ...
            unit: "°C",
            explanation: Math.abs(result.comfort.meanAirTemperatureC - targetC) <= 0.5
        ...
        },
        {
            ...
            unit: "°C",
            explanation: Math.abs(result.comfort.occupiedMeanTemperatureC - targetC) <= 0.7
...
function buildComfortStatus(result) {
...
function buildReliabilityStatus(result) {
...
function buildHeatingStatus(result) {
...
function buildHeatLossStatus(specificLossW_m2, dominantLossLabel) {
...
function buildUniformityStatus(result) {
...
function buildRecommendations(result, dominantLossLabel, heating, reliability) {
...
function pickDominantLoss(result) {
    const candidates = [
...
function getToneByDeviation(value, goodBand, warningBand) {
