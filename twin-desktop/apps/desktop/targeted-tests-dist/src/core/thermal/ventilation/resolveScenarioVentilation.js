...
function readEngineeringAirflowM3s(parameters) {
    const airflowM3H = readEngineeringParameterNumber(parameters, "airflowM3H") ?? readEngineeringParameterNumber(parameters, "flowRateM3H");
...
function sumMechanicalVentilationAirflowM3s(model) {
    const simpleDiffuserFlowM3s = model.equipment.reduce((sum, item) => sum + (item.type === "diffuser" ? Math.max(0, item.params.designAirflow_m3_s ?? 0) : 0), 0);
    const simpleAhuFlowM3s = model.equipment.reduce((sum, item) => sum + (item.type === "ahu" ? Math.max(0, item.params.designAirflow_m3_s ?? 0) : 0), 0);
    ...
    const engineeringSupplyTerminalFlowM3s = engineeringEquipment.reduce((sum, item) => sum + (item.type === "supplyDiffuser" ? readEngineeringAirflowM3s(item.parameters) : 0), 0);
    const engineeringExhaustTerminalFlowM3s = engineeringEquipment.reduce((sum, item) => sum + (item.type === "exhaustGrille" ? readEngineeringAirflowM3s(item.parameters) : 0), 0);
    const engineeringAhuFlowM3s = engineeringEquipment.reduce((sum, item) => sum + (item.type === "airHandlingUnit" ? readEngineeringAirflowM3s(item.parameters) : 0), 0);
    const engineeringSupplyPipeFlowM3s = engineeringPipes.reduce((sum, pipe) => sum + (pipe.medium === "airSupply" && pipe.flowRate != null ? Math.max(0, pipe.flowRate) / 3600 : 0), 0);
    const engineeringExhaustPipeFlowM3s = engineeringPipes.reduce((sum, pipe) => sum + (pipe.medium === "airExhaust" && pipe.flowRate != null ? Math.max(0, pipe.flowRate) / 3600 : 0), 0);
    const engineeringFlowM3s = Math.max(engineeringSupplyTerminalFlowM3s, engineeringExhaustTerminalFlowM3s, engineeringAhuFlowM3s, engineeringSupplyPipeFlowM3s, engineeringExhaustPipeFlowM3s);
...
function computeHeatRecoveryFromBuilding(model, mechanicalVentilationEnabled, ventilationAch) {
    ...
    const engineeringAhuEfficiencies = (model.engineeringSystems?.equipment ?? [])
        ...
        .map((item) => readEngineeringParameterNumber(item.parameters, "heatRecoveryEfficiency") ??
