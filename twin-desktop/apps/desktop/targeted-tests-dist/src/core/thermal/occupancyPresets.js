...
function gainsFromSp50Category(category) {
...
export function applyOccupancyPresetToConfig(draft, presetId) {
...
export function markOccupancyAsCustom(draft) {
    draft.occupancyPresetId = "custom";
}
export function isOccupancyControlledByPreset(config) {
    const id = config?.occupancyPresetId;
    return id != null && id !== "custom";
}
const VALUE_EPS = 1e-4;
function valuesMatchPreset(config, presetId) {
    const preset = getOccupancyPreset(presetId);
    const { occupancy, internalGains } = config;
    return (Math.abs(occupancy.dayFraction - preset.dayFraction) <= VALUE_EPS &&
...
export function inferOccupancyPresetFromValues(config) {
...
export function resolveOccupancyPresetSelection(config, resolved) {
...
export function occupancyPresetReferenceLabel(selection) {
    if (selection === "custom") {
        return null;
    }
