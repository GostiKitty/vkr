...
export function resolveBuildingInfiltration(building, input, context) {
    ...
    const leakage = resolveEnvelopeLeakage({
        ...
        ...(input?.pressureBased ?? {}),
    }, geometry, pressure.pressureTotalPa);
    return buildCalculatedResult({
        mode,
        geometry,
        airflowM3s: leakage.airflowM3s,
...
function resolveEnvelopeLeakage(input, geometry, explicitPressurePa) {
    ...
    const envelopeLeakage = nonNegativeOrFallback(input?.envelopeAirPermeabilityM3sM2At10Pa, DEFAULT_ENVELOPE_LEAKAGE_M3S_M2_AT_10_PA);
    ...
    const airflowM3s = (envelopeLeakage * geometry.envelopeOpaqueAreaM2 +
...
function resolvePressureBasedLeakage(input, geometry, context, density) {
    const windSpeedMps = nonNegativeOrFallback(input?.windSpeedMps, DEFAULT_WIND_SPEED_M_S);
    const windPressureCoefficient = positiveOrFallback(input?.windPressureCoefficient, DEFAULT_WIND_PRESSURE_COEFFICIENT);
...
export function summarizeInfiltrationGeometry(building, defaultLevelHeightM = DEFAULT_LEVEL_HEIGHT_M) {
...
function estimateStackHeightM(building, defaultLevelHeightM) {
...
export function estimateHeatedVolumeM3(building, defaultLevelHeightM = DEFAULT_LEVEL_HEIGHT_M) {
