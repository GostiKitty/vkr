...
import { estimateBuildingMeanMrtC, estimateMrtFromEnvelope, resolveComfortMaxC, resolveComfortMinC, resolveRelativeHumidityPercent, } from "../comfort/resolveScenarioComfort";
import { resolveScenarioEngineeringInputs, summarizeModelEngineering, } from "../engineering/resolveScenarioEngineering";
import { isOccupancyControlledByPreset } from "../occupancyPresets";
import { resolveDaySetpointScalar, resolveNightSetpointScalar, setpointScalarToOrigin, } from "../resolveScenarioSetpoints";
...
import { applyDefaultOpeningEnvelopeToModel, resolveModelDoorU_W_m2K, resolveModelShadingFactor, hasModelDoorAreaSource, hasModelWindowAreaSource, resolveModelDoorAreaM2, resolveModelWindowAreaM2, resolveModelWindowGValue, resolveModelWindowU_W_m2K, TYPICAL_PVC_WINDOW_G_VALUE, TYPICAL_WINDOW_SHADING_FACTOR, shadingEnvelopeSourceNote, windowEnvelopeSourceNote, } from "../../../shared/utils/openingThermalData";
import { buildAdjacencyGraph } from "../../graph/adjacency";
import { airflowFromACH, calculateHydronicHeatPower, calculateRequiredHydronicMassFlow, calculateRequiredHydronicVolumeFlowM3H, dewPointMagnusC, gsop, heatLossCoefficientTransmission, heatLossCoefficientTotal, heatLossCoefficientVentilation, internalSurfaceTemperatureC, reducedResistanceByHomogeneity, thermalBridgeHeatLoss, ventilationLoss, ventilationRecoveryLoss, } from "../formulas";
import { resolveBuildingInfiltration } from "../infiltration";
import { buildResolvedEnvelopeLeakageConfig, resolveScenarioEnvelopeLeakageInputs, } from "../ventilation/resolveScenarioEnvelopeLeakage";
...
import { isOpaqueConstructionKind, resolveBridgeAccountingMode, } from "../../../shared/utils/bridgeAccountingMode";
import { resolveFragmentHomogeneityCoefficient, resolveModelHomogeneityCoefficient, } from "../../../shared/utils/homogeneityFromModel";
import { calculateValidationMetrics } from "./buildingPerformanceMetrics";
import { AUTO_CALCULATED_SOURCE_LABEL, MODEL_SOURCE_LABEL, } from "../../../shared/constants/sourceDataLabels";
...
function field(key, label, value, unit, source, warnings = [], notes = []) {
    return {
        key,
        label,
        value,
...
function requirement(key, label, present, source, warnings = [], required = true) {
    return {
        key,
        label,
        present,
...
function summarizeSection(id, requirements) {
    ...
    const completionPercent = required.length > 0 ? Math.round((availableCount / required.length) * 100) : 0;
    let status = "missing";
    if (availableCount === 0) {
        status = "missing";
    }
...
function roomPolygonSignature(room) {
...
function findLevelHeightM(model, room) {
...
function inferRoomPurpose(room) {
...
function inferRoomFloorContactType(room, levelIndexById, levelCount, floorKindsByLevelId) {
...
function inferRoomRoofContactType(room, levelIndexById, levelCount, roofLevelIds) {
    const levelIndex = levelIndexById.get(room.levelId) ?? -1;
...
function resolveIndoorTemperatureC(scenarioConfig, scenario, model, thermalResult) {
...
function resolveClimateMeta(scenarioConfig, scenario, model) {
    const manual = scenarioConfig?.climate?.manual;
    const hasManualOutdoor = manual &&
    ...
    const hasModelOutdoor = sp50Climate &&
    ...
    if (hasManualOutdoor) {
        return {
            ...
            designOutdoorC: manual.outdoorDesignTemperatureC != null && Number.isFinite(manual.outdoorDesignTemperatureC)
            ...
            heatingAverageC: manual.outdoorHeatingAverageC != null && Number.isFinite(manual.outdoorHeatingAverageC)
            ...
            heatingDurationDays: manual.heatingDurationDays != null && Number.isFinite(manual.heatingDurationDays)
    ...
    if (hasModelOutdoor) {
        return {
            ...
            heatingDurationDays: sp50Climate?.heatingPeriodDurationDays ?? sp131City?.heatingPeriodDurationDays ?? null,
            source: "model",
            warnings: [],
        };
    }
...
function buildGeometrySection(model) {
    ...
    const floorKindsByLevelId = new Map();
    (geometryModel.floorSlabs ?? []).forEach((slab) => {
        const kinds = floorKindsByLevelId.get(slab.levelId) ?? new Set();
        kinds.add(slab.kind);
        floorKindsByLevelId.set(slab.levelId, kinds);
    });
    const roofLevelIds = new Set((geometryModel.roofs ?? []).map((roof) => roof.levelId));
    const rooms = geometryModel.rooms.map((room) => {
        const levelHeightM = findLevelHeightM(geometryModel, room);
        const fallbackHeightWarning = levelHeightM == null ? ["Высота помещения не задана, используется 3 м."] : [];
        const heightM = levelHeightM ?? 3;
        const areaM2 = roomAreaM2(room);
        const heated = inferRoomHeated();
        const floorContactType = inferRoomFloorContactType(room, levelIndexById, sortedLevels.length, floorKindsByLevelId);
        ...
        const heightSource = levelHeightM != null ? "model" : "fallback";
        return {
            roomId: room.id,
            roomName: room.name,
            ...
            floorContactType: field(`room:${room.id}:floor-contact`, "Контакт пола", floorContactType.value, null, floorContactType.source, floorContactType.warnings),
            roofContactType: field(`room:${room.id}:roof-contact`, "Контакт кровли", roofContactType.value, null, roofContactType.source, roofContactType.warnings),
            purpose: field(`room:${room.id}:purpose`, "Назначение помещения", purpose.value, null, purpose.source, purpose.warnings),
        };
    });
    ...
    const envelopeAreaM2 = facadeAreaM2 +
        roofAreaTotalM2 +
        (geometryModel.floorSlabs ?? [])
            .filter((slab) => slab.kind === "ground" || slab.kind === "basement")
            .reduce((sum, slab) => sum + slabAreaM2(slab), 0);
    const compactness = heatedVolumeM3 > 0 && envelopeAreaM2 > 0 ? envelopeAreaM2 / heatedVolumeM3 : null;
    ...
    const requirements = [
        ...
        requirement("geometry.heights", "Высоты помещений", rooms.every((room) => room.heightM.source !== "missing"), rooms.some((room) => room.heightM.source === "fallback") ? "fallback" : "model", rooms.some((room) => room.heightM.source === "fallback") ? ["Часть объёмов рассчитана с допущением по высоте 3 м."] : []),
        requirement("geometry.heated-rooms", "Признак отапливаемых помещений", rooms.length > 0, rooms.length > 0 ? "model" : "missing"),
    ];
...
function collectBridgeFragments(model) {
...
function envelopeFragmentKind(constructionType) {
...
function resolveOpeningUFields(model, scenarioConfig, scenario) {
...
function resolveOpeningOpticalFields(model, scenarioConfig, scenario, solarTime) {
    ...
    const windowG = modelWindowG ??
        (scenarioConfig?.materials?.windowGValue != null
            ...
            : isCanonicalDemoProjectModel(model) && hasWindows
                ...
                : null);
    const windowGSource = modelWindowG != null ? "model" : scenarioConfig?.materials?.windowGValue != null ? "user" : windowG != null ? "model" : "missing";
    const shadingFactor = modelShading ??
        (scenarioConfig?.materials?.shadingFactor != null
            ? scenario.materials?.shadingFactor ?? null
            : isCanonicalDemoProjectModel(model) && hasWindows
                ? TYPICAL_WINDOW_SHADING_FACTOR
                : null);
    const shadingFactorSource = modelShading != null
        ? scenarioConfig?.materials?.shadingFactor != null
            ? "user"
        ...
        : scenarioConfig?.materials?.shadingFactor != null
            ...
            : shadingFactor != null
                ? modelShadingResult.usesSolarTime
                    ...
                    : "model"
                : "missing";
    const shadingNotes = shadingFactorSource === "model" && scenarioConfig?.materials?.shadingFactor == null
...
function resolveConstructionRows(model, scenarioConfig, scenario, designDeltaT, thermalResult, solarTime) {
    const warnings = [];
    const rows = [];
    ...
    const materialElements = (model.thermalProtection?.envelope ?? [])
        .map((fragment) => {
    ...
    materialElements.forEach((element) => {
        const props = element.layers?.length
            ...
            : null;
        const localWarnings = [];
        const presetRuntimeU = element.metadata?.runtimeU_W_m2K != null && element.metadata.runtimeU_W_m2K > 0
            ? element.metadata.runtimeU_W_m2K
            : null;
        const scenarioOverrideU = element.kind === "window"
            ? openingU.windowU
            : element.kind === "door"
                ? openingU.doorU
                : null;
        ...
        const homogeneityForElement = userHomogeneityCoefficient ??
            (fragmentFromModel ? resolveFragmentHomogeneityCoefficient(fragmentFromModel, r0) : null) ??
            modelHomogeneity.value ??
            1;
        const homogeneitySource = userHomogeneityCoefficient != null
            ? "user"
            : fragmentFromModel &&
                ...
                : modelHomogeneity.value != null
                    ? "model"
                    : "fallback";
        const rReduced = homogeneityMode && r0 != null ? reducedResistanceByHomogeneity(r0, homogeneityForElement) : r0;
        const transmissionU = homogeneityMode && isOpaqueConstructionKind(element.kind) && rReduced != null && rReduced > 0
            ? 1 / rReduced
            : derivedU;
        if (homogeneityMode && userHomogeneityCoefficient == null && homogeneityForElement === 1 && !modelHomogeneity.hasBridgeData) {
            ...
            localWarnings.push(`r = ${homogeneityForElement.toFixed(3)} рассчитан из модели (ψ/χ или зоны неоднородности).`);
        }
        ...
        if (overrideU != null && overrideU > 0 && presetRuntimeU != null) {
            localWarnings.push(element.kind === "window"
        ...
        const rowLayers = (element.layers ?? []).map((layer, index) => {
            ...
            const resistance = thickness != null && lambda != null && lambda > 0 ? thickness / lambda : null;
            const layerWarnings = element.metadata?.sourceType === "preset" && element.metadata.sourceNote
            ...
            const materialSource = element.metadata?.sourceType === "preset"
                ? "model"
                : normMaterial != null
                    ? normMaterial.isApproximate
                        ? "fallback"
            ...
            return {
                ...
                lambda_W_mK: field(`${element.id}:layer:${index}:lambda`, "Теплопроводность λ", lambda, "Вт/(м·К)", lambda != null ? materialSource : "missing", layerWarnings),
                densityKgM3: field(`${element.id}:layer:${index}:rho`, "Плотность ρ", material?.rho_kg_m3 ?? null, "кг/м³", material ? materialSource : "missing", layerWarnings),
                heatCapacity_J_kgK: field(`${element.id}:layer:${index}:c`, "Теплоёмкость c", material?.c_J_kgK ?? null, "Дж/(кг·К)", material ? materialSource : "missing", layerWarnings),
                resistance_m2K_W: field(`${element.id}:layer:${index}:r`, "Сопротивление слоя", resistance, "м²·К/Вт", resistance != null ? "calculated" : "missing", layerWarnings),
            };
        });
        rows.push({
            id: element.id,
            ...
            reducedResistance_m2K_W: field(`${element.id}:rred`, "R_red", rReduced, "м²·К/Вт", rReduced != null
                ? homogeneityMode
                    ? homogeneitySource === "fallback"
                        ? "fallback"
                        : "calculated"
                    : "calculated"
                : "missing", localWarnings, homogeneityMode ? ["Приведённое сопротивление учитывает неоднородность конструкции."] : []),
            uValue_W_m2K: field(`${element.id}:u`, "U", transmissionU, "Вт/(м²·К)", transmissionU != null
                ? presetRuntimeU != null
                    ? "model"
                    : overrideU != null && overrideU > 0
                        ? "scenario"
                        : "calculated"
                : "missing", localWarnings, presetRuntimeU != null
                ? ["Используется U типового пресета из конструктора."]
                : overrideU != null && overrideU > 0
                    ? ["Используется U, заданный для окна/двери в исходных данных."]
                    : []),
            bridgeMode: field(`${element.id}:bridge-mode`, "Учёт мостиков холода", bridgeMode, null, bridgeModeOrigin === "user" ? "user" : "calculated", resolvedBridge.notes),
            layers: rowLayers,
    ...
    const validTransmissionElements = rows
        ...
        .map((row) => {
        const areaM2 = row.areaM2.value;
        const catalogU = row.uValue_W_m2K.value;
        const r0 = row.resistanceR0_m2K_W.value;
        const rRed = row.reducedResistance_m2K_W.value;
        const U_W_m2K = homogeneityMode &&
            typeof r0 === "number" &&
            typeof rRed === "number" &&
            r0 > 0 &&
            rRed > 0 &&
    ...
    const requirements = [
        requirement("materials.layers", "Слои конструкций", rows.some((row) => row.layers.length > 0), rows.some((row) => row.layers.length > 0) ? "model" : "missing"),
        requirement("materials.bridge-mode", "Способ учёта неоднородностей", bridgeMode !== "disabled" || bridgeModeOrigin === "user", bridgeModeOrigin === "user" ? "user" : bridgeMode === "disabled" ? "fallback" : "calculated", bridgeModeOrigin === "user" ? [] : resolvedBridge.notes),
        requirement("materials.homogeneity-or-bridges", "Данные по неоднородностям", bridgeMode === "disabled" ||
            ...
            (bridgeMode === "explicitPsiChi" && (linear.length > 0 || point.length > 0)), bridgeMode === "explicitPsiChi"
    ...
    const computedFields = [
        field("materials.bridge-mode", "Режим учёта мостиков холода", bridgeMode, null, bridgeModeOrigin === "user" ? "user" : "calculated", warnings, resolvedBridge.notes),
        field("materials.homogeneity-coefficient", "Коэффициент однородности r", homogeneityCoefficient, null, userHomogeneityCoefficient != null ? "user" : modelHomogeneity.value != null ? "model" : "missing", [], modelHomogeneity.notes),
        ...
        field("materials.shading-factor", "Shading factor", openingOptical.shadingFactor, null, openingOptical.shadingFactorSource, [], openingOptical.shadingNotes),
        ...
        field("materials.h-psi", "H_psi", modelH_psi, "Вт/К", modelH_psi != null ? (bridgeAggregates.hasLinear ? "sp50" : "calculated") : "missing"),
        field("materials.h-chi", "H_chi", modelH_chi, "Вт/К", modelH_chi != null ? (bridgeAggregates.hasPoint ? "sp50" : "calculated") : "missing"),
...
function buildClimateSection(scenarioConfig, scenario, model) {
    ...
    const designDeltaT = climate.designOutdoorC != null && indoor.value != null ? indoor.value - climate.designOutdoorC : null;
    const gsopValue = indoor.value != null && climate.heatingAverageC != null && climate.heatingDurationDays != null
        ? gsop(indoor.value, climate.heatingAverageC, climate.heatingDurationDays)
        : null;
    const requirements = [
        requirement("climate.profile", "Город / климатический профиль", Boolean(scenarioConfig?.climateCityId) || Boolean(climate.cityLabel), scenarioConfig?.climateCityId ? "sp131" : climate.cityLabel ? climate.source : "missing"),
        requirement("climate.heating-average", "Средняя температура отопительного периода", climate.heatingAverageC != null, climate.source, climate.warnings),
        requirement("climate.heating-duration", "Продолжительность отопительного периода", climate.heatingDurationDays != null, climate.source, climate.warnings),
        requirement("climate.design-outdoor", "Расчётная наружная температура", climate.designOutdoorC != null, climate.source, climate.warnings),
    ];
    const computedFields = [
...
function buildOperationSection(scenarioConfig, scenario, model, thermalResult) {
    const performance = thermalResult?.diagnostics?.buildingPerformance ?? null;
    ...
    const requirements = [
        ...
        requirement("operation.occupancy", "Занятость", true, isOccupancyControlledByPreset(scenarioConfig) ? "sp50" : scenarioConfig?.occupancy ? "scenario" : "fallback"),
        ...
        requirement("operation.outdoor-profile", "График наружной температуры сценария", scenarioConfig?.climate != null, scenarioConfig?.climate ? "scenario" : "fallback", scenarioConfig?.climate ? [] : ["Используются значения сценария по умолчанию."]),
    ];
    const computedFields = [
        ...
        field("operation.day-setpoint", "Дневная уставка", daySetpoint.value, "°C", setpointScalarToOrigin(daySetpoint.source)),
        field("operation.night-setpoint", "Ночная уставка", nightSetpoint.value, "°C", setpointScalarToOrigin(nightSetpoint.source)),
        ...
        field("operation.internal-gains-day", "Внутренние теплопоступления днём", scenario.internalGains.dayGain_W_m2, "Вт/м²", isOccupancyControlledByPreset(scenarioConfig)
            ? "sp50"
            : scenarioConfig?.internalGains?.dayGain_W_m2 != null
                ? "scenario"
                : "fallback"),
        field("operation.internal-gains-night", "Внутренние теплопоступления ночью", scenario.internalGains.nightGain_W_m2, "Вт/м²", isOccupancyControlledByPreset(scenarioConfig)
            ? "sp50"
            : scenarioConfig?.internalGains?.nightGain_W_m2 != null
                ? "scenario"
                : "fallback"),
        field("operation.occupancy-day", "Занятость днём", scenario.occupancy.dayFraction, null, isOccupancyControlledByPreset(scenarioConfig)
            ? "sp50"
            : scenarioConfig?.occupancy?.dayFraction != null
                ? "scenario"
                : "fallback"),
        field("operation.occupancy-night", "Занятость ночью", scenario.occupancy.nightFraction, null, isOccupancyControlledByPreset(scenarioConfig)
            ? "sp50"
        ...
        field("operation.degree-hours-underheat", "DH_underheat", performance?.degreeHoursUnderheat.building.value ?? null, "°C·ч", performance?.degreeHoursUnderheat.building.value != null ? "result" : "missing"),
        field("operation.degree-hours-overheat", "DH_overheat", performance?.degreeHoursOverheat.building.value ?? null, "°C·ч", performance?.degreeHoursOverheat.building.value != null ? "result" : "missing"),
...
function formatInfiltrationModeRu(mode) {
    switch (mode) {
        case "manualAch":
            return "Ручной ввод";
...
function formatAchSourceRu(source) {
    switch (source) {
        case "manual":
...
function buildAirExchangeSection(model, scenarioConfig, scenario, designDeltaT, dayOfYear) {
    const indoorTemperatureC = scenario.setpoints.day;
    const outdoorTemperatureC = designDeltaT != null ? indoorTemperatureC - designDeltaT : scenario.climate.baseC + scenario.climate.seasonalOffsetC;
    ...
    const infiltrationResult = resolveBuildingInfiltration(model, {
    ...
    const ventilationFlowM3s = totalVolumeM3 > 0 && mechanicalVentilationEnabled ? airflowFromACH(ventilationAch, totalVolumeM3) : 0;
    const infiltrationLossW = infiltrationFlowM3s != null ? infiltrationResult.heatLossW : null;
    const ventilationLossBeforeW = ventilationFlowM3s != null && designDeltaT != null
        ? ventilationLoss(ventilationFlowM3s, AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, designDeltaT)
        : null;
    const recovery = ventilationFlowM3s != null && designDeltaT != null
        ? ventilationRecoveryLoss(ventilationFlowM3s, AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, designDeltaT, heatRecoveryFactor)
        : null;
    const H_inf = infiltrationFlowM3s != null
        ? heatLossCoefficientVentilation(infiltrationFlowM3s, AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, 0)
        : null;
    const H_vent_before = ventilationFlowM3s != null
        ? heatLossCoefficientVentilation(ventilationFlowM3s, AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, 0)
        : null;
    const H_vent_after = ventilationFlowM3s != null
        ? heatLossCoefficientVentilation(ventilationFlowM3s, AIR_DENSITY_KG_M3, AIR_HEAT_CAPACITY_J_KG_K, heatRecoveryFactor)
        : null;
    const H_total = H_inf != null && H_vent_after != null ? heatLossCoefficientTotal(H_inf, H_vent_after) : null;
    const warnings = [
    ...
    const infiltrationFieldSource = infiltrationResult.diagnostics.achSource === "calculated"
        ? "calculated"
    ...
    return {
        ...
        warnings,
        computedFields: [
            field("air.heat-recovery", "КПД рекуперации", heatRecoveryFactor, null, resolvedVentilation.heatRecoveryFactor.source === "user"
                ? "scenario"
                : resolvedVentilation.heatRecoveryFactor.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.g-envelope", "Воздухопроницаемость ограждений G_air", resolvedLeakage.envelopeAirPermeabilityM3sM2At10Pa.value, "м³/(с·м²)", resolvedLeakage.envelopeAirPermeabilityM3sM2At10Pa.source === "user"
                ? "scenario"
                : resolvedLeakage.envelopeAirPermeabilityM3sM2At10Pa.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.g-window", "Воздухопроницаемость окон G_air", resolvedLeakage.windowAirPermeabilityM3sMAt10Pa.value, "м³/(с·м)", resolvedLeakage.windowAirPermeabilityM3sMAt10Pa.source === "user"
                ? "scenario"
                : resolvedLeakage.windowAirPermeabilityM3sMAt10Pa.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.g-door", "Воздухопроницаемость дверей G_air", resolvedLeakage.doorAirPermeabilityM3sMAt10Pa.value, "м³/(с·м)", resolvedLeakage.doorAirPermeabilityM3sMAt10Pa.source === "user"
                ? "scenario"
                : resolvedLeakage.doorAirPermeabilityM3sMAt10Pa.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.pressure-exponent", "Показатель степени n", resolvedLeakage.pressureExponent.value, null, resolvedLeakage.pressureExponent.explicit ? "scenario" : "calculated"),
            field("air.wind-speed", "Скорость ветра", resolvedVentilation.windSpeedMps.value, "м/с", resolvedVentilation.windSpeedMps.source === "user"
                ? "scenario"
                : resolvedVentilation.windSpeedMps.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.wind-cp", "Коэффициент давления Cp", resolvedVentilation.windPressureCoefficient.value, null, resolvedVentilation.windPressureCoefficient.source === "user"
                ? "scenario"
                : resolvedVentilation.windPressureCoefficient.source === "fallback"
                    ...
                    : resolvedVentilation.windPressureCoefficient.source === "model"
                        ? "sp50"
                        : "calculated"),
            field("air.mechanical-pressure", "ΔP мех. вентиляции", resolvedVentilation.mechanicalPressurePa.value, "Па", resolvedVentilation.mechanicalPressurePa.source === "user" ? "scenario" : "calculated"),
            field("air.total-volume", "Отапливаемый объём", totalVolumeM3 || null, "м³", totalVolumeM3 > 0 ? "calculated" : "missing"),
            field("air.envelope-opaque-area", "Площадь непрозрачного ограждения", infiltrationResult.geometry.envelopeOpaqueAreaM2 > 0 ? infiltrationResult.geometry.envelopeOpaqueAreaM2 : null, "м²", infiltrationResult.geometry.envelopeOpaqueAreaM2 > 0 ? "calculated" : "missing"),
            field("air.window-perimeter", "Периметр окон", infiltrationResult.geometry.windowPerimeterM > 0 ? infiltrationResult.geometry.windowPerimeterM : null, "м", infiltrationResult.geometry.windowPerimeterM > 0 ? "calculated" : "missing"),
            field("air.door-perimeter", "Периметр дверей", infiltrationResult.geometry.doorPerimeterM > 0 ? infiltrationResult.geometry.doorPerimeterM : null, "м", infiltrationResult.geometry.doorPerimeterM > 0 ? "calculated" : "missing"),
            field("air.stack-height", "Высота для теплового напора", resolvedVentilation.stackHeightM.value, "м", resolvedVentilation.stackHeightM.source === "user"
                ? "scenario"
                : resolvedVentilation.stackHeightM.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.infiltration-mode", "Режим инфильтрации", formatInfiltrationModeRu(scenario.ventilation.infiltrationMode ?? "manualAch"), null, scenarioConfig?.ventilation?.infiltrationMode != null ? "scenario" : "fallback"),
            field("air.infiltration-source", "Источник кратности инфильтрации", formatAchSourceRu(infiltrationResult.diagnostics.achSource), null, infiltrationFieldSource),
            field("air.infiltration-ach", "Кратность инфильтрации", infiltrationResult.calculatedACH, "1/ч", infiltrationFieldSource, infiltrationResult.assumptions),
            field("air.ventilation-ach", "Кратность механической вентиляции", ventilationAch, "1/ч", resolvedVentilation.ventilationACH.source === "user"
                ? "scenario"
                : resolvedVentilation.ventilationACH.source === "fallback"
                    ? "fallback"
                    : "calculated"),
            field("air.infiltration-flow", "Расход инфильтрации", infiltrationFlowM3s != null ? infiltrationFlowM3s * 3600 : null, "м³/ч", infiltrationFlowM3s != null ? "calculated" : "missing"),
            field("air.ventilation-flow", "Расход механической вентиляции", ventilationFlowM3s != null ? ventilationFlowM3s * 3600 : null, "м³/ч", ventilationFlowM3s != null ? "calculated" : "missing"),
            ...
            field("air.q-vent-before", "Теплопотери вентиляции до рекуперации", ventilationLossBeforeW, "Вт", ventilationLossBeforeW != null ? "calculated" : "missing"),
            field("air.q-vent-after", "Теплопотери вентиляции после рекуперации", recovery?.afterRecoveryW ?? null, "Вт", recovery ? "calculated" : "missing", warnings),
            field("air.saved-by-recovery", "Экономия за счёт рекуперации", recovery?.savedW ?? null, "Вт", recovery ? "calculated" : "missing", warnings),
            field("air.h-inf", "Коэффициент теплопотерь инфильтрации", H_inf, "Вт/К", H_inf != null ? "calculated" : "missing"),
            field("air.h-vent-before", "Коэффициент теплопотерь вентиляции до рекуперации", H_vent_before, "Вт/К", H_vent_before != null ? "calculated" : "missing"),
...
function buildHumiditySection(scenarioConfig, scenario, model, thermalResult, climateSection) {
    ...
    const surfaceTemperatureC = scenarioConfig?.comfort?.measuredSurfaceTemperatureC != null
        ? scenarioConfig.comfort.measuredSurfaceTemperatureC
        : minSurfaceFromDiagnostics;
    const dewPointC = indoor.value != null && humidity.value != null
        ? dewPointMagnusC(indoor.value, humidity.value / 100)
        : null;
    const measuredMrtC = scenarioConfig?.comfort?.measuredMrtC != null && Number.isFinite(scenarioConfig.comfort.measuredMrtC)
        ? scenarioConfig.comfort.measuredMrtC
        : null;
    const mrtFromRun = estimateBuildingMeanMrtC(thermalResult);
    const mrtFromEnvelope = indoor.value != null && outdoorDesignC != null
    ...
    const mrtSource = measuredMrtC != null ? "user" : mrtFromRun != null ? "result" : mrtFromEnvelope != null ? "calculated" : "missing";
    const operativeTemperatureC = indoor.value != null && mrtC != null ? (indoor.value + mrtC) / 2 : null;
    ...
    const surfaceProps = surfaceFragment?.layers?.length
        ? computeWallProperties(surfaceFragment.layers, undefined, { includeSp50AirFilms: true })
        : null;
    const tauSiC = surfaceTemperatureC != null
        ? surfaceTemperatureC
        : indoor.value != null && outdoorDesignC != null && surfaceProps?.rTotal_m2K_W
            ...
            : null;
    const condensationMarginC = tauSiC != null && dewPointC != null ? tauSiC - dewPointC : null;
    const condensationStatus = condensationMarginC == null ? null : condensationMarginC <= 0 ? "condensation_risk" : condensationMarginC < 2 ? "cold_surface" : "normal";
    const requirements = [
    ...
    return {
        ...
        computedFields: [
            ...
            field("humidity.comfort-min", "Минимально допустимая температура", comfortMin.value, "°C", comfortBoundsOrigin(comfortMin.source)),
            field("humidity.comfort-max", "Максимально допустимая температура", comfortMax.value, "°C", comfortBoundsOrigin(comfortMax.source)),
            field("humidity.mrt", "Средняя радиационная температура", mrtC, "°C", mrtSource, measuredMrtC == null && mrtFromEnvelope != null && mrtFromRun == null
...
function engineeringInputOrigin(source) {
    switch (source) {
        case "user":
            return "user";
        case "model":
...
function buildEngineeringSection(model, scenarioConfig, scenario, thermalResult) {
    ...
    const requiredPowerFromSimulationW = peakLoadFromSimulationKw != null && Number.isFinite(peakLoadFromSimulationKw) && peakLoadFromSimulationKw > 0
    ...
    const provisionalDeltaT = scenario.engineeringSystems?.supplyTemperatureC != null &&
        ...
        : 20;
    const provisionalRequiredMassFlowKgS = requiredPowerW != null && provisionalDeltaT > 0
        ? calculateRequiredHydronicMassFlow(requiredPowerW, provisionalDeltaT, fluidProps("water").cp)
        : null;
    const resolved = resolveScenarioEngineeringInputs(scenarioConfig, scenario, model, thermalResult, provisionalRequiredMassFlowKgS);
    ...
    const hydronicDeltaT = hydronic.deltaT != null && hydronic.deltaT > 0
        ? hydronic.deltaT
        : resolved.supplyTemperatureC.value - resolved.returnTemperatureC.value > 0
            ...
            : null;
    const requiredMassFlowKgS = requiredPowerW != null && hydronicDeltaT != null
        ? calculateRequiredHydronicMassFlow(requiredPowerW, hydronicDeltaT, fluid.cp)
        : null;
    const requiredVolumeFlowM3H = requiredPowerW != null && hydronicDeltaT != null
        ? calculateRequiredHydronicVolumeFlowM3H(requiredPowerW, hydronicDeltaT, fluid.density, fluid.cp)
        : null;
    const requiredFlowOrigin = requiredMassFlowKgS != null
    ...
    const requirements = [
        ...
        requirement("engineering.capacity", "Установленная мощность", !capacityLimited || installedCapacityW > 0, !capacityLimited
            ? "calculated"
            : resolved.installedCapacityW.explicit
                ...
                : resolved.installedCapacityW.source === "model" || resolved.installedCapacityW.source === "result"
                    ...
                    : "missing", capacityLimited && installedCapacityW <= 0
    ...
    return {
        ...
        computedFields: [
            ...
            field("engineering.heat-carrier", "Теплоноситель", resolved.fluidType, null, engineeringInputOrigin(resolved.fluidTypeSource)),
            ...
            field("engineering.pipe-count", "Количество трубных участков", pipeSegmentCount || null, null, pipeSegmentCount > 0 ? "model" : "missing"),
            field("engineering.pipe-total-length", "Суммарная длина труб", totalPipeLengthM || null, "м", totalPipeLengthM > 0 ? "model" : "missing"),
            field("engineering.pipe-diameter", "Диаметры труб", diameters.length ? (diameters.length === 1 ? String(diameters[0]) : `${diameters[0]}–${diameters[diameters.length - 1]}`) : null, "мм", diameters.length ? "model" : "missing"),
            field("engineering.pipe-insulation", "Теплоизоляция труб", pipeSegmentCount > 0 ? `${insulatedPipeCount}/${pipeSegmentCount}` : null, null, pipeSegmentCount > 0 ? "model" : "missing"),
...
function buildEcologySection(model, scenarioConfig, scenario, thermalResult) {
    ...
    const emissionFactorOrigin = resolvedEmissionFactor.source === "user"
        ? "user"
        : resolvedEmissionFactor.source === "norm"
            ? "calculated"
            : "missing";
    const energyKWh = thermalResult?.summary.totalEnergyKWh ?? null;
    const co2Kg = emissionFactor != null && energyKWh != null ? emissionFactor * energyKWh : null;
...
function buildEconomySection(model, scenarioConfig, scenario, thermalResult) {
    const periodDays = scenario.operation?.duration === "7d" ? 7 : 1;
    const periodEnergyKWh = thermalResult?.summary.totalEnergyKWh ?? null;
    const annualizedEnergyKWh = periodEnergyKWh != null && periodDays > 0 ? (periodEnergyKWh / periodDays) * 365 : null;
    ...
    const annualCostRub = annualizedEnergyKWh != null ? tariff * annualizedEnergyKWh : null;
    const economyOrigin = (field) => {
        if (field.explicit) {
            return "scenario";
        }
        switch (field.source) {
    ...
    const requirements = [
        ...
        requirement("economy.capex", "CAPEX мероприятий", resolvedEconomy.capexRub.value != null, economyOrigin(resolvedEconomy.capexRub), [], false),
        requirement("economy.discount", "Ставка дисконтирования", resolvedEconomy.discountRatePercent.value != null, economyOrigin(resolvedEconomy.discountRatePercent)),
    ];
    const warnings = [];
    const tariffUnit = resolvedTariff.heatingEnergySource === "electricity" ? "руб/кВт·ч" : "руб/кВт·ч (из руб/Гкал)";
    return {
        id: "economy",
        ...
        warnings,
        computedFields: [
            field("economy.heating-source", "Способ отопления", resolvedTariff.heatingEnergySourceLabel, null, scenario.ecology?.energySource ? "scenario" : model.pipes.length > 0 ? "model" : "fallback"),
            field("economy.tariff", "Тариф", tariff, tariffUnit, scenario.climateCityId ? "sp131" : "fallback"),
            field("economy.capex", "CAPEX", resolvedEconomy.capexRub.value, "руб", economyOrigin(resolvedEconomy.capexRub)),
            field("economy.analysis-period", "Период анализа", resolvedEconomy.analysisPeriodYears.value, "лет", economyOrigin(resolvedEconomy.analysisPeriodYears)),
            field("economy.discount", "Ставка дисконтирования", resolvedEconomy.discountRatePercent.value, "%", economyOrigin(resolvedEconomy.discountRatePercent)),
            field("economy.tariff-growth", "Рост тарифа", resolvedEconomy.annualTariffGrowthPercent.value, "%", economyOrigin(resolvedEconomy.annualTariffGrowthPercent)),
            field("economy.maintenance", "Стоимость обслуживания", resolvedEconomy.annualMaintenanceCostRub.value, "руб/год", economyOrigin(resolvedEconomy.annualMaintenanceCostRub)),
            field("economy.insulation-cost", "Стоимость утепления", resolvedEconomy.insulationCostRub.value, "руб", economyOrigin(resolvedEconomy.insulationCostRub)),
            field("economy.windows-cost", "Стоимость окон", resolvedEconomy.windowsCostRub.value, "руб", economyOrigin(resolvedEconomy.windowsCostRub)),
            field("economy.equipment-cost", "Стоимость оборудования", resolvedEconomy.equipmentCostRub.value, "руб", economyOrigin(resolvedEconomy.equipmentCostRub)),
...
function buildValidationSection(model, scenario, thermalResult) {
    const validation = scenario.validation;
    const explicitUnavailable = validation?.availabilityStatus === "unavailable" ||
        validation?.dataOrigin === "synthetic" ||
        model.meta?.validationStatus === "unavailable";
    if (explicitUnavailable) {
        const warning = validation?.note ??
        ...
        return {
            ...
            computedFields: [
                ...
                field("validation.origin", "Источник данных", validation?.dataOrigin ?? "synthetic", null, "scenario", ["Synthetic/demo sensors не выдаются за реальные измерения."]),
    ...
    const simulationStepHours = simulationTimeline.length >= 2 ? simulationTimeline[1].timeHours - simulationTimeline[0].timeHours : null;
    ...
    const alignedMeasured = parsedSeries.flatMap((entry) => {
        ...
        const nearest = simulationTimeline.reduce((best, point) => {
    ...
    const validationMetrics = roomExists && alignedMeasured.length >= 3
        ? calculateValidationMetrics(alignedMeasured.map((entry) => ({ value: entry.measured })), alignedMeasured.map((entry) => ({ value: entry.simulated })))
        : null;
    const warnings = [];
    if (!selectedRoomId) {
...
function buildReportsSection(reportInputs) {
...
export function prepareModelForSourceData(model, scenarioConfig) {
    let next = syncAndEnrichThermalProtection(model);
    next = ensureModelClimate(next, scenarioConfig);
    return next;
}
export function buildSourceDataWorkspaceReport(input) {
    const model = applyDefaultOpeningEnvelopeToModel(prepareModelForSourceData(input.model, input.scenarioConfig));
    ...
    const materials = resolveConstructionRows(model, input.scenarioConfig, scenario, climate.designDeltaT, input.thermalResult, input.solarTime);
    const operation = buildOperationSection(input.scenarioConfig, scenario, model, input.thermalResult);
    const airExchange = buildAirExchangeSection(model, input.scenarioConfig, scenario, climate.designDeltaT, input.solarTime?.dayOfYear);
    ...
    const summaryCards = Object.keys(sections).map((key) => sections[key].summary);
    const reportWarnings = Array.from(new Set(Object.keys(sections).flatMap((key) => [
