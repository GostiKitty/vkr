import { estimateWallOrientation } from "../graph/adjacency";
import { computeSolarPosition, formatSolarHour } from "./solarPosition";
export function orientationToAzimuthDeg(orientation) {
    switch (orientation) {
        case "N":
            return 0;
        case "E":
...
export function wallFacadeAzimuthDeg(wall) {
 ...
 */
export function exteriorFacadeAzimuthDeg(wall, roomCentroid) {
    ...
    const inward = normalA.x * toRoom.x + normalA.y * toRoom.y >= 0 ? normalA : { x: -normalA.x, y: -normalA.y };
 ...
 */
export function computeFacadeSolarAccessFactor(solarPosition, facadeAzimuthDeg) {
 ...
 */
export function combineArchitecturalAndSolarShading(baseShading, solarAccessFactor) {
...
export function resolveWindowFacadeAzimuthDeg(model, window) {
...
export function buildSolarShadingNote(solarTime, solarPosition) {
    return `Учтено солнечное время ${formatSolarHour(solarTime.hour)}, высота солнца ${solarPosition.altitudeDeg.toFixed(1)}°.`;
}
export function resolveSolarPositionFromTime(solarTime) {
    if (!solarTime) {
        return null;
    }
