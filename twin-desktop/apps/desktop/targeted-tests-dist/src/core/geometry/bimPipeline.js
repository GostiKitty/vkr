...
function buildPolygonSignatureForVolumeDedupe(points) {
    return points.map((point) => `${point.x.toFixed(3)}:${point.y.toFixed(3)}`).join("|");
}
function polygonPlanDiagonal(points) {
    if (points.length < 2) {
        return 1;
    }
...
function autoVolumeMatchesManualRoom(volume, room) {
 ...
 */
export function dedupeRoomVolumesForModel(roomVolumes, modelRooms, activeLevelId) {
    ...
    return [...grouped.values()].map((variants) => {
        ...
        const matchedRoom = modelRooms
            .filter((room) => room.levelId === fallback.levelId &&
 ...
 */
export function finalizeRoomVolumesForModel(roomVolumes, modelRooms, activeLevelId) {
    ...
    const groupedByRoomId = new Map();
    resolved.forEach((volume) => {
        const list = groupedByRoomId.get(volume.roomId) ?? [];
        list.push(volume);
        groupedByRoomId.set(volume.roomId, list);
    });
    return [...groupedByRoomId.values()].map((variants) => variants.sort((left, right) => right.areaM2 - left.areaM2)[0]);
}
export function buildResolvedGeometryRenderModel(model, activeLevelId = null) {
