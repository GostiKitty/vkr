...
export function isCanonicalDemoProjectModel(model) {
    ...
    return (projectSource === DEMO_PROJECT_SOURCE ||
        sourceProjectId === DEMO_PROJECT_ID ||
        (demoScenarioId != null && CANONICAL_DEMO_SCENARIO_IDS.has(demoScenarioId)));
}
export function isLegacyDemoModel(model) {
    if (!model) {
        return false;
    }
    const demoScenarioId = normalizeToken(typeof model.meta?.demoScenarioId === "string" ? model.meta.demoScenarioId : null);
