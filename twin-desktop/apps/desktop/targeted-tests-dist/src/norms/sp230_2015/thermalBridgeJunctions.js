...
const R_UT_COLUMNS = [1.5, 3.0, 6.0];
const LAMBDA_O_COLUMNS = [0.2, 0.6, 1.8];
/** Табл. Г.28 — выпуклый угол СФТК, Вт/(м·К). */
const G28_CONVEX_PSI = [
    [0.088, 0.076, 0.06],
    [0.167, 0.121, 0.082],
    [0.234, 0.15, 0.093],
];
/** Табл. Г.34 — примыкание оконного блока, dн = 20 мм, λо = 0,2, Вт/(м·К). */
const G34_WINDOW_JAMB_PSI_BY_R_UT = {
    1.5: 0.092,
    3.0: 0.092,
    6.0: 0.072,
};
...
export function psiSftkConvexCorner_W_mK(input) {
    ...
    const byLambda = LAMBDA_O_COLUMNS.map((lambdaColumn, rowIndex) => interpolate1D(rUt, R_UT_COLUMNS, G28_CONVEX_PSI[rowIndex] ?? G28_CONVEX_PSI[0]));
 ...
 */
export function psiSftkWindowJamb_W_mK(insulationResistance_m2K_W) {
    const rUt = Math.max(0.1, insulationResistance_m2K_W);
    const entries = Object.entries(G34_WINDOW_JAMB_PSI_BY_R_UT).map(([key, value]) => [Number(key), value]);
...
const G81_WALL_ROOF_R_UT1 = [1.88, 3.13, 5.0];
const G81_WALL_ROOF_R_UT2 = [1.5, 3.0, 5.0, 6.0];
const G81_WALL_ROOF_PSI = [
...
const G40_FLOOR_EDGE_R_UT1 = [1.5, 3.0, 6.0];
const G40_FLOOR_EDGE_R_UT2 = [1.88, 3.13, 5.0, 7.81];
const G40_FLOOR_EDGE_PSI = [
    [0.156, 0.135, 0.115, 0.099],
    [0.175, 0.152, 0.128, 0.107],
    [0.194, 0.168, 0.141, 0.116],
];
/** Табл. Г.103 — стропила, dдоп = 0, Вт/(м·К). */
const G103_RAFTER_D_UT = [100, 150, 200];
const G103_RAFTER_PSI = [0.044, 0.034, 0.027];
/** Табл. Г.104 — конёк/ендова, Вт/(м·К). */
const G104_RIDGE_D_UT = [100, 150, 200];
const G104_RIDGE_PSI = [0.084, 0.065, 0.053];
function interpolate2DGrid(rUt1, rUt2, rows, cols, grid) {
    let rowIndex = 0;
...
export function psiSftkWallRoofJunction_W_mK(input) {
    return interpolate2DGrid(Math.max(0.1, input.wallInsulationResistance_m2K_W), Math.max(0.1, input.roofInsulationResistance_m2K_W), G81_WALL_ROOF_R_UT1, G81_WALL_ROOF_R_UT2, G81_WALL_ROOF_PSI);
}
/**
...
export function psiFloorOnGroundEdge_W_mK(input) {
    return interpolate2DGrid(Math.max(0.1, input.wallInsulationResistance_m2K_W), Math.max(0.1, input.floorInsulationResistance_m2K_W), G40_FLOOR_EDGE_R_UT1, G40_FLOOR_EDGE_R_UT2, G40_FLOOR_EDGE_PSI);
}
/** ψ прохождения стропила через утеплитель скатной кровли (табл. Г.103, dдоп = 0). */
export function psiPitchedRoofRafter_W_mK(insulationThicknessMm) {
...
export function psiPitchedRoofRidge_W_mK(insulationThicknessMm) {
