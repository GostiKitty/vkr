import type { BuildingModel, Sp50EnvelopeFragmentInput } from "../../entities/geometry/types";
...
const OPAQUE_CONSTRUCTION_TYPES = new Set<Sp50EnvelopeFragmentInput["constructionType"]>([
  "wall",
  "roof",
  "covering",
  "atticFloor",
...
function clampHomogeneityCoefficient(value: number): number {
 ...
 */
export function homogeneityCoefficientFromLinearPointBridges(
  r0_m2K_W: number,
  areaM2: number,
  linear: ReadonlyArray<{ lengthM: number; psi_W_mK: number }>,
  point: ReadonlyArray<{ count: number; chi_W_K: number }>
): number {
  if (!(r0_m2K_W > 0) || !(areaM2 > 0)) {
    return 1;
  }
  const bridgeConductance_W_K =
    thermalBridgeLinearConductance(linear) + thermalBridgePointConductance(point);
...
function resolveFragmentR0_m2K_W(fragment: Sp50EnvelopeFragmentInput): number | null {
...
export function resolveFragmentHomogeneityCoefficient(
  fragment: Sp50EnvelopeFragmentInput,
  r0_m2K_W: number | null
): number | null {
  ...
  if (linear.length > 0 || point.length > 0) {
    return homogeneityCoefficientFromLinearPointBridges(
      r0_m2K_W,
      Math.max(0, fragment.areaM2),
      linear,
      point
    );
  }
...
export interface ResolvedModelHomogeneity {
  /** Площадно-взвешенное r по непрозрачным ограждениям модели. */
  value: number | null;
  byFragmentId: Record<string, number>;
  hasBridgeData: boolean;
  notes: string[];
}

export function resolveModelHomogeneityCoefficient(model: BuildingModel): ResolvedModelHomogeneity {
  const fragments = model.thermalProtection?.envelope ?? [];
  const byFragmentId: Record<string, number> = {};
  let weightedSum = 0;
  let totalArea = 0;
  ...
  if (fragments.length === 0) {
    ...
    notes.push(
      "r рассчитан по данным модели: для каждого ограждения R_red / R_0 с учётом ψ, χ и зон, затем усреднение по площади."
    );
  }
