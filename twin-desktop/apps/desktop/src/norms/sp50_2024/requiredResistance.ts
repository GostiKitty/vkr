...
export interface RequiredResistanceCoefficients {
  a: number;
  b: number;
  notes?: string;
}

export type RequiredResistanceTableCategory = "1.1" | "1.2" | "2" | "3";
export type RequiredResistanceTableType = "wall" | "covering" | "floor" | "window" | "lantern";

interface RequiredResistanceRow {
  values: Record<number, number>;
  coefficients?: RequiredResistanceCoefficients;
  capAt12000?: boolean;
}

type RequiredResistanceTable = Record<RequiredResistanceTableCategory, Record<RequiredResistanceTableType, RequiredResistanceRow>>;

export const REQUIRED_RESISTANCE_GSOP_POINTS = [1000, 2000, 4000, 6000, 8000, 10000, 12000] as const;

export const REQUIRED_RESISTANCE_TABLE: RequiredResistanceTable = {
  "1.1": {
    wall: {
...
const BUILDING_CATEGORY_TO_TABLE_CATEGORY: Partial<Record<Sp50BuildingCategory, RequiredResistanceTableCategory>> = {
  residential: "1.1",
  medical: "1.2",
  preschool: "1.2",
  educational: "1.2",
...
function mapConstructionType(constructionType: Sp50ConstructionType): RequiredResistanceTableType | null {
...
export function getRequiredResistanceCoefficients(
  buildingCategory: Sp50BuildingCategory,
  constructionType: Sp50ConstructionType
): RequiredResistanceCoefficients | null {
  return getRequiredResistanceRow(buildingCategory, constructionType)?.coefficients ?? null;
}

export function getRequiredResistanceRow(
  buildingCategory: Sp50BuildingCategory,
  constructionType: Sp50ConstructionType
): RequiredResistanceRow | null {
...
export function getRequiredResistanceTableCategory(
  buildingCategory: Sp50BuildingCategory
): RequiredResistanceTableCategory | null {
  return BUILDING_CATEGORY_TO_TABLE_CATEGORY[buildingCategory] ?? null;
}

export function interpolateRequiredResistance(
  values: Record<number, number>,
  gsop: number,
  capAt12000 = false
): number | null {
