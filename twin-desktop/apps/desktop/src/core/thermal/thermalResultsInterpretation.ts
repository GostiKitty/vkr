...
export function getThermalSimulationDurationHours(duration: ThermalSimulationOptions["duration"]): number {
  return duration === "7d" ? 24 * 7 : 24;
}

export function formatThermalSimulationPeriodRu(duration: ThermalSimulationOptions["duration"]): string {
 ...
 */
export function buildThermalSimulationInsightLines(
  result: ThermalSimulationResult,
  options: Pick<ThermalSimulationOptions, "duration"> & { totalHeatedAreaM2?: number }
): string[] {
  const lines: string[] = [];
  ...
  lines.push(
    `Период расчёта: ${period}. Пиковая мощность отопления — максимум по времени суммы мощностей по всем зонам (одновременная нагрузка на «систему»).`
  );
  lines.push(
    `Тепловая энергия за период — интеграл суммарной мощности отопления: ${energy.toFixed(1)} кВт·ч (пропорционально длине шага).`
  );
  lines.push(
    `Показатель дискомфорта — сумма по зонам времени, когда температура зоны ниже уставки более чем на 0,05 °C; при нескольких зонах число может превышать длительность периода.`
  );
  ...
  if (area && area > 1 && Number.isFinite(peak)) {
    ...
    if (wPerM2 > 90) {
      lines.push(
        `Пиковая удельная нагрузка около ${wPerM2.toFixed(0)} Вт/м² по отапливаемой площади — относительно высоко для учебного прототипа; имеет смысл проверить наружные ограждения, инфильтрацию и запасной наружный контур зон без фасада.`
      );
    } else if (wPerM2 < 25) {
      lines.push(
        `Пиковая удельная нагрузка около ${wPerM2.toFixed(0)} Вт/м² — низкая относительно типичных ориентиров; убедитесь, что модель геометрии и режимы соответствуют задумке сценария.`
      );
    }
  }
  ...
  if (discomfort > 0) {
    if (discomfort > getThermalSimulationDurationHours(options.duration) * 0.25) {
      lines.push(
        "Значительная доля периода с температурой ниже уставки: для прототипа это сигнал проверить уставки, притоки и потери; не трактуйте как нормативный показатель комфорта."
      );
    } else {
  ...
  const roomTemps = Object.values(result.rooms).map((r) => r.timeline[r.timeline.length - 1]?.temperatureC).filter((t) => Number.isFinite(t)) as number[];
...
export function buildMonteCarloInterpretationLines(result: ThermalMonteCarloResult, heatingThresholdKW?: number): string[] {
  const lines: string[] = [];
  ...
  if (spread > Math.max(peak.p50 * 0.35, 1e-6)) {
    lines.push(
      "Заметный разброс между P5 и P95 — результат чувствителен к неопределённости исходных данных в рамках выбранных распределений."
    );
  }
  ...
  if (heatingThresholdKW !== undefined && result.exceedanceProbability !== undefined) {
    if (result.exceedanceProbability > 0.2) {
      lines.push(
        `Высокая доля прогонов (${(result.exceedanceProbability * 100).toFixed(0)}%) с пиком выше ${heatingThresholdKW} кВт — осторожно трактуйте как инженерный сигнал по выборке, а не как готовую вероятность для проектной документации.`
      );
  ...
  lines.push(
    "Показатель «условный год (365×)» — это 365× среднесуточная энергия за смоделированный период (24 ч или 7 суток), упрощённая экстраполяция для сравнения сценариев, не нормативный годовой расход."
  );
  return lines;
}
