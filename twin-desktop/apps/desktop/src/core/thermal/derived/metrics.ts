...
import {
  freeCoolingTemperatureAtTime,
  heatLossCoefficientTotal,
  specificHeatLoadByArea,
  specificHeatLoadByVolume,
  specificHeatLoadByVolumeAndDeltaT,
  thermalTimeConstantSeconds,
} from "../formulas";
import type { HydronicAssessment } from "../engineering/types";
import type { ThermalBuildingDiagnostics, ThermalZoneDiagnostics } from "../thermalDiagnostics";
import type {
  DerivedBalanceLine,
  DerivedFreeCoolingPoint,
  DerivedMetricContour,
  DerivedMetricSource,
  DerivedMetricStatus,
  DerivedMetricValue,
  DerivedNormativeVentilationCheck,
  DerivedRcDiagnostics,
  DerivedSpecificIndicators,
  DerivedVentilationRecovery,
} from "./types";

const SECONDS_PER_HOUR = 3600;
interface BuildRcDerivedDiagnosticsInput {
...
function zoneOutdoorConductance(model: ThermalModel, zoneId: string): number {
  return model.outdoorLinks.reduce((sum, link) => (link.fromZoneId === zoneId ? sum + link.conductance_W_K : sum), 0);
}

function zoneAverageReferenceTemperature(zones: ThermalZoneDiagnostics[]): number | null {
  if (!zones.length) {
    return null;
  }
  const weighted = zones.reduce(
    (sum, zone) => {
...
function buildVentilationRecovery(
  options: ThermalSimulationOptions,
...
function buildSpecificIndicators(
  model: ThermalModel,
  building: ThermalBuildingDiagnostics,
  zones: ThermalZoneDiagnostics[]
): DerivedSpecificIndicators {
  ...
  return {
    ...
    qVolumeDeltaT_W_m3K: metric(
      ...
      "derived-only",
      false,
      ["ΔT берётся по референсному срезу RC-диагностики."],
      sharedWarnings
    ),
  };
}

function buildNormativeVentilation(model: ThermalModel): DerivedNormativeVentilationCheck {
  const providedFlowM3H =
    model.zones.reduce((sum, zone) => sum + Math.max(0, zone.ventilationACH) * Math.max(0, zone.volume_m3), 0);
  const warnings = [
...
export function buildRcDerivedDiagnostics(input: BuildRcDerivedDiagnosticsInput): DerivedRcDiagnostics {
  ...
  const totalConductance = heatLossCoefficientTotal(
    transmissionConductance,
    infiltrationConductance,
  ...
  const freeCooling: DerivedFreeCoolingPoint[] =
    buildingTauSeconds === null
      ? [1, 6, 12, 24].map((hours) => ({
          hours,
          temperatureC: metric<number | null>(
            null,
            "°C",
            "engineering-derived",
            "derived-only",
            ...
            false,
            [],
            ["Не удалось определить тепловую постоянную времени τ."]
          ),
        }))
      : [1, 6, 12, 24].map((hours) => ({
          hours,
          temperatureC: metric(
            freeCoolingTemperatureAtTime(
              zoneAverageReferenceTemperature(zones) ?? 20,
              building.referenceOutdoorC,
            ...
            "derived-only",
            false,
            ["Свободное остывание без отопления не заменяет основную RC-симуляцию."]
          ),
        }));

  return {
    transmissionHeatLossCoefficient_W_K: metric(
      transmissionConductance,
      "Вт/К",
      "solver",
      "derived-only",
      "derived-only",
      false,
      ["H_tr агрегирован по conductance внешних связей зональной RC-модели."]
    ),
    infiltrationHeatLossCoefficient_W_K: metric(
      infiltrationConductance,
      "Вт/К",
      "solver",
      "derived-only",
      "derived-only",
      false,
      ["Инфильтрация показана отдельно, чтобы не задваивать её с механической вентиляцией."]
    ),
    ventilationHeatLossCoefficient_W_K: metric(
      ventilationConductance,
      "Вт/К",
      "solver",
      "derived-only",
      "derived-only",
      false,
      ["H_ve агрегирован по effective conductance механической вентиляции зональной RC-модели с учётом рекуперации, если она задана."]
    ),
    totalHeatLossCoefficient_W_K: metric(
      totalConductance,
      "Вт/К",
      "solver",
      "derived-only",
      "derived-only",
      false,
      ["H_total = H_tr + H_inf + H_ve,eff."]
    ),
    buildingTauHours: metric(
      buildingTauSeconds === null ? null : buildingTauSeconds / SECONDS_PER_HOUR,
      "ч",
      "engineering-derived",
      "derived-only",
      "derived-only",
      ...
      totalConductance <= 0 ? ["Суммарный коэффициент теплопотерь H <= 0, τ не определена."] : []
    ),
    zoneTauHours: model.zones.map((zone) => {
      const zoneConductance = heatLossCoefficientTotal(
        zoneOutdoorConductance(model, zone.id),
        Math.max(0, zone.infiltrationConductance_W_K),
        Math.max(0, zone.ventilationConductance_W_K)
      );
      ...
      return {
        ...
        tauHours: metric(
          tauSeconds === null ? null : tauSeconds / SECONDS_PER_HOUR,
          "ч",
          "engineering-derived",
          "derived-only",
          "derived-only",
