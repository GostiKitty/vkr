...
import type { BuildingModel, Sp50EnvelopeFragmentInput, Window } from "../../../entities/geometry/types";
import { computeWallProperties } from "../../../entities/material/types";
import type { PipeNetwork } from "../../../entities/networks/types";
...
import type { ThermalModel } from "../model";
import type { ThermalSimulationOptions, ThermalSimulationResult } from "../solver";
import type { DerivedRcDiagnostics } from "./types";
import type {
  BuildBuildingPerformanceDiagnosticsOptions,
  BuildingPerformanceDiagnostics,
  BuildingPerformanceElementInput,
  DataCompletenessSectionId,
  DataCompletenessSectionSummary,
  DerivedMetricContour,
  DerivedMetricSource,
  DerivedMetricStatus,
  DerivedMetricValue,
  InputDataCompletenessAudit,
  MetricDataRequirementsMap,
  MetricRequirementInput,
  SurfaceTemperatureFactorStatus,
  TemperatureSeriesPoint,
  TwinValidationStatus,
  ValidationSeriesPoint,
} from "./types";

const DERIVED_CONTOUR: DerivedMetricContour = "derived-only";
const DERIVED_STATUS: DerivedMetricStatus = "derived-only";
const DEFAULT_COMFORT_MAX_C = 26;

function derivedMetric<T>(
  value: T,
  unit: string | null,
  source: DerivedMetricSource = "engineering-derived",
  assumptions: string[] = [],
  warnings: string[] = []
): DerivedMetricValue<T> {
  return {
    value,
    unit,
    source,
...
const DATA_COMPLETENESS_SECTION_LABELS: Record<DataCompletenessSectionId, string> = {
  geometry: "Геометрия",
  materials: "Материалы",
  climate: "Климат",
...
function makeRequirementInput(
  input: Omit<MetricRequirementInput, "warnings" | "required"> & {
    warnings?: string[];
    required?: boolean;
  }
): MetricRequirementInput {
...
function summarizeDataCompletenessSections(metrics: MetricDataRequirementsMap[]): DataCompletenessSectionSummary[] {
  return (Object.keys(DATA_COMPLETENESS_SECTION_LABELS) as DataCompletenessSectionId[]).map((sectionId) => {
    const fieldMap = new Map<string, MetricRequirementInput>();
    const ingestField = (field: MetricRequirementInput) => {
    ...
    let status: DataCompletenessSectionSummary["status"] = "needs_input";
    if (!fields.length) {
      status = "needs_input";
...
export function calculateCompactness(A_env: number, V_h: number): DerivedMetricValue<number | null> {
...
export function calculateWindowToWallRatio(
  A_win: number,
  A_facade: number
): { ratio: DerivedMetricValue<number | null>; percent: DerivedMetricValue<number | null> } {
  const warnings: string[] = [];
...
export function calculateEquivalentUValue(elements: readonly BuildingPerformanceElementInput[]): DerivedMetricValue<number | null> {
  const valid = elements.filter(
    (element) =>
      element.U_W_m2K !== null &&
      element.areaM2 !== null &&
  ...
  const weighted = valid.reduce(
    (sum, element) => ({
      numerator: sum.numerator + (element.U_W_m2K as number) * (element.areaM2 as number),
      area: sum.area + (element.areaM2 as number),
    }),
    { numerator: 0, area: 0 }
  );
  if (weighted.area <= 0) {
  ...
  if (reducedElements.length) {
    assumptions.push(
      `Элементы с R_red не смешиваются с ψ/χ: ${reducedElements.map((element) => element.label ?? element.id).join(", ")}.`
    );
  }
...
export function calculateTotalHeatLossCoefficient(input: {
  ...
  H_psi?: number | null;
  H_chi?: number | null;
}): BuildingPerformanceDiagnostics["heatLossBreakdown"] {
  const assumptions = ["H_total = H_tr + H_ve + H_ψ + H_χ"];
  const warnings: string[] = [];
  const H_tr = Number.isFinite(input.H_tr) ? Math.max(0, input.H_tr as number) : null;
  const H_ve = Number.isFinite(input.H_ve) ? Math.max(0, input.H_ve as number) : null;
  const H_psi =
    input.H_psi === undefined || input.H_psi === null
      ? 0
      : Number.isFinite(input.H_psi)
        ? Math.max(0, input.H_psi)
        : null;
  const H_chi =
    input.H_chi === undefined || input.H_chi === null
      ? 0
  ...
  const H_total =
    H_tr === null || H_ve === null || H_psi === null || H_chi === null
      ? null
...
export function calculateDegreeHoursUnderheat(
  series: readonly TemperatureSeriesPoint[],
  T_min: number,
  dtHours: number
): DerivedMetricValue<number | null> {
  if (!series.length) {
...
export function calculateDegreeHoursOverheat(
  series: readonly TemperatureSeriesPoint[],
  T_max: number,
  dtHours: number
): DerivedMetricValue<number | null> {
...
export function calculateOperativeTemperature(
  T_air: number | null,
  T_mrt: number | null,
  approximateMrt = false
): DerivedMetricValue<number | null> {
...
export function classifySurfaceTemperatureFactor(f_Rsi: number | null): SurfaceTemperatureFactorStatus | null {
...
export function calculateSurfaceTemperatureFactor(
  tau_si: number | null,
  T_out: number | null,
  T_in: number | null
): { f_Rsi: DerivedMetricValue<number | null>; status: SurfaceTemperatureFactorStatus | null } {
...
export function calculatePipeHeatLoss(
  U_pipe: number | null,
  lengthM: number | null,
  T_water: number | null,
  T_air: number | null,
...
export function calculateCO2(
  ...
  energySourceLabel: string | null = null
): BuildingPerformanceDiagnostics["co2"] {
...
export function calculateValidationMetrics(
  measuredSeries: readonly ValidationSeriesPoint[],
  simulatedSeries: readonly ValidationSeriesPoint[]
): BuildingPerformanceDiagnostics["validation"] {
  const warnings: string[] = [];
...
function emptyValidation(status: TwinValidationStatus, warnings: string[]): BuildingPerformanceDiagnostics["validation"] {
  return {
...
function alignValidationSeries(
  measuredSeries: readonly ValidationSeriesPoint[],
  simulatedSeries: readonly ValidationSeriesPoint[],
  warnings: string[]
): Array<{ measured: number; simulated: number; timeHours: number | null }> {
  if (measuredSeries.length !== simulatedSeries.length) {
    warnings.push(
      `Длины рядов различаются (measured=${measuredSeries.length}, simulated=${simulatedSeries.length}) — выполнено усечение по min(n).`
    );
  }
  const count = Math.min(measuredSeries.length, simulatedSeries.length);
  const aligned: Array<{ measured: number; simulated: number; timeHours: number | null }> = [];
...
function formatValidationPeriod(aligned: Array<{ timeHours: number | null }>): string | null {
  const times = aligned.map((point) => point.timeHours).filter((value): value is number => value !== null);
  if (!times.length) {
...
function windowAreaM2(window: Window): number {
  return Math.max(0, window.width_m) * Math.max(0, window.height_m);
}

function exteriorEnvelopeAreaM2(model: BuildingModel, adjacency: AdjacencyResult): number {
...
function facadeAreaM2(adjacency: AdjacencyResult): number {
  return adjacency.external.reduce((sum, edge) => sum + Math.max(0, edge.area_m2), 0);
}

function totalWindowAreaM2(model: BuildingModel): number {
  return model.windows.reduce((sum, window) => sum + windowAreaM2(window), 0);
}

function heatedVolumeM3(model: BuildingModel, thermalModel: ThermalModel): number {
...
function collectOutdoorLinkElements(thermalModel: ThermalModel): BuildingPerformanceElementInput[] {
  return thermalModel.outdoorLinks.flatMap((link, index) => {
    const parts: Array<{ id: string; label: string; conductance: number; areaM2: number }> = [];
    const pushPart = (suffix: string, conductance?: number) => {
...
function fragmentUValue(fragment: Sp50EnvelopeFragmentInput): number | null {
...
function collectExplicitUElements(model: BuildingModel, adjacency: AdjacencyResult): BuildingPerformanceElementInput[] {
...
function resolveThermalBridgeConductances(model: BuildingModel): { H_psi: number | null; H_chi: number | null; explicit: boolean } {
...
function polylineLengthM(path: PipeNetwork["path"]): number {
  if (path.length < 2) {
    return 0;
  }
  let length = 0;
...
function derivePipeLinearU_W_mK(pipe: PipeNetwork): { U_pipe: number | null; warnings: string[] } {
  const warnings: string[] = [];
...
function resolvePipeWaterTemperature(pipe: PipeNetwork, hydronicSupply: number | null, hydronicReturn: number | null): {
  T_water: number | null;
  warnings: string[];
} {
  const warnings: string[] = [];
...
function extractSensorValidationSeries(
  model: BuildingModel,
  thermalResult: ThermalSimulationResult,
  roomId: string | null
): { measured: ValidationSeriesPoint[]; simulated: ValidationSeriesPoint[] } {
  if (
  ...
  const sensors = model.sensors.filter(
    (sensor) => sensor.type === "temperature" && sensor.history.length > 0 && (roomId ? sensor.roomId === roomId : true)
  );
  if (!sensors.length) {
    return { measured: [], simulated: [] };
  }
...
function resolveComfortMinC(options: ThermalSimulationOptions, performanceOptions?: BuildBuildingPerformanceDiagnosticsOptions): {
  value: number;
  warnings: string[];
} {
...
function resolveComfortMaxC(performanceOptions?: BuildBuildingPerformanceDiagnosticsOptions): {
  value: number;
  warnings: string[];
} {
...
function estimateMeanRadiantTemperatureC(
  surfaces: Array<{ tau_si: number; areaM2: number }>
): { T_mrt: number | null; approximate: boolean; warnings: string[] } {
  if (!surfaces.length) {
...
function buildSurfaceTemperatureDiagnostics(
  model: BuildingModel,
  T_in: number,
  T_out: number
): BuildingPerformanceDiagnostics["surfaceTemperatureFactor"] {
  const fragments = model.thermalProtection?.envelope ?? [];
  const surfaces = fragments.map((fragment) => {
    ...
    const tau_si =
      totalResistance && totalResistance > 0
...
export function buildDataRequirementsAudit(input: {
  ...
  zoneDiagnostics?: Array<{ zoneId: string; temperatureC: number }>;
}): InputDataCompletenessAudit {
  ...
  const validEquivalentElements = explicitUElements.filter(
    (element) =>
      element.U_W_m2K !== null &&
      element.areaM2 !== null &&
      Number.isFinite(element.U_W_m2K) &&
      Number.isFinite(element.areaM2) &&
      (element.U_W_m2K as number) > 0 &&
      (element.areaM2 as number) > 0
  );
  ...
  const peakIndoorTemperature = hasZoneDiagnostics
    ? (zoneDiagnostics as Array<{ zoneId: string; temperatureC: number }>).reduce((sum, zone) => sum + zone.temperatureC, 0) /
      Math.max((zoneDiagnostics as Array<{ zoneId: string; temperatureC: number }>).length, 1)
  ...
  const measuredSeries =
    performanceOptions?.measuredSeries ?? extractSensorValidationSeries(model, thermalResult, null).measured;
  const simulatedSeries =
    performanceOptions?.simulatedSeries ?? extractSensorValidationSeries(model, thermalResult, null).simulated;
  ...
  const hydronicWarnings =
    hydronic?.usedInputs.fluidDensityKgM3 != null && hydronic.usedInputs.fluidHeatCapacityJkgK != null
  ...
  const hasPipeInsulationData = model.pipes.some(
    (pipe) =>
      pipe.insulationThickness_mm !== undefined &&
  ...
  const metrics: MetricDataRequirementsMap[] = [
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        ...
        makeRequirementInput({
          ...
          section: "geometry",
          sourceCategory:
            heatedVolumeMeta && heatedVolumeMeta > 0 ? "buildingModel" : "computedAutomatically",
          sourcePath:
            heatedVolumeMeta && heatedVolumeMeta > 0
    ...
    buildMetricRequirementsMap({
      ...
      warningIfFallback:
        !linearBridges.length || !pointBridges.length
          ? "При отсутствии явных ψ/χ текущий derived-контур принимает их равными 0 и должен показывать warning."
          : null,
      canCalculateNow:
        isPresentValue(derived.transmissionHeatLossCoefficient_W_K.value) &&
        isPresentValue(derived.infiltrationHeatLossCoefficient_W_K.value) &&
        isPresentValue(derived.ventilationHeatLossCoefficient_W_K.value),
    }),
    buildMetricRequirementsMap({
    ...
    buildMetricRequirementsMap({
      ...
      computedInputs: [
        ...
        makeRequirementInput({
          ...
          sourcePath:
            peakIndoorTemperature !== null ? "zone diagnostics / last room temperature" : "ThermalSimulationOptions.setpoints.day",
      ...
      canCalculateNow:
        hasExplicitSurfaceLayers && (peakIndoorTemperature !== null || isPresentValue(options.setpoints.day)),
    }),
    buildMetricRequirementsMap({
      metricId: "Q_pipe",
      ...
      computedInputs: [
        makeRequirementInput({
          ...
          required: false,
          warnings:
            hasPipeFluidTemperature || !hasHydronicTemperatures
              ? []
    ...
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        makeRequirementInput({
          ...
          sourcePath:
            performanceOptions?.annualEnergyKWh != null
    ...
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        makeRequirementInput({
          ...
          section: "validation",
          sourceCategory:
            performanceOptions?.measuredSeries != null ? "scenarioConfig" : measuredSeries.length ? "buildingModel" : "missingUserInput",
          sourcePath:
            performanceOptions?.measuredSeries != null
        ...
        makeRequirementInput({
          ...
          section: "validation",
          sourceCategory:
            performanceOptions?.simulatedSeries != null ? "scenarioConfig" : "diagnosticsThermalResult",
          sourcePath:
            performanceOptions?.simulatedSeries != null
    ...
    buildMetricRequirementsMap({
      ...
      canCalculateNow:
        envelopeFragments.some((fragment) => Boolean(fragment.layers?.length)) &&
    ...
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        ...
        makeRequirementInput({
          ...
          section: "climate",
          sourceCategory:
            outdoorTemperature !== null && (peakIndoorTemperature !== null || isPresentValue(options.setpoints.day))
              ? "diagnosticsThermalResult"
              : "missingUserInput",
      ...
      canCalculateNow:
        (linearBridges.length > 0 || pointBridges.length > 0) &&
    ...
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        ...
        makeRequirementInput({
          ...
          section: "operation",
          sourceCategory:
            isPresentValue(climateMeta?.indoorTemperatureC) ? "thermalProtectionSp50" : "scenarioConfig",
          sourcePath:
            isPresentValue(climateMeta?.indoorTemperatureC)
    ...
    buildMetricRequirementsMap({
      ...
      canCalculateNow:
        thermalRcModel.zones.some((zone) => zone.capacitance_J_K > 0) &&
        isPresentValue(derived.totalHeatLossCoefficient_W_K.value) &&
        (derived.totalHeatLossCoefficient_W_K.value as number) > 0,
    }),
    buildMetricRequirementsMap({
      ...
      computedInputs: [
        makeRequirementInput({
          ...
          section: "materials",
          sourceCategory:
            surfaceTempFallbackWarnings.length > 0 ? "fallbackDefault" : "computedAutomatically",
    ...
    buildMetricRequirementsMap({
      ...
      canCalculateNow:
        isPresentValue(derived.ventilationRecovery.ventilationLossBeforeRecovery_W.value) &&
    ...
    buildMetricRequirementsMap({
      ...
      requiredInputs: [
        ...
        makeRequirementInput({
          ...
          section: "engineeringNetworks",
          sourceCategory:
            hydronic?.supplyTemperatureC != null ? "buildingModel" : "missingUserInput",
        ...
        makeRequirementInput({
          ...
          section: "engineeringNetworks",
          sourceCategory:
            hydronic?.returnTemperatureC != null ? "buildingModel" : "missingUserInput",
  ...
  const generatedWarnings = Array.from(
    new Set(
...
export function buildBuildingPerformanceDiagnostics(
  model: BuildingModel,
  thermalResult: ThermalSimulationResult,
  thermalRcModel: ThermalModel,
  options: ThermalSimulationOptions,
  ...
  const H_ve =
    (derived.infiltrationHeatLossCoefficient_W_K.value ?? 0) +
  ...
  const buildingUnderheatSeries = Object.values(thermalResult.rooms).flatMap((room) =>
    room.timeline.map((point) => ({ timeHours: point.timeHours, temperatureC: point.temperatureC }))
  );
    ...
    zones: Object.values(thermalResult.rooms).map((room) => {
      ...
      const fragmentSurfaces = (model.thermalProtection?.envelope ?? [])
        ...
        .map((fragment) => {
          ...
          const totalResistance = props?.rValue ?? null;
          const tau_si =
            totalResistance && totalResistance > 0 && T_air !== null && T_out !== null
      ...
      const mrtEstimate =
        T_air !== null
          ...
          : { T_mrt: null, approximate: false, warnings: ["T_mrt недоступна."] as string[] };
  ...
  const peakT_in =
    zoneDiagnostics && zoneDiagnostics.length
  ...
  const pipeHeatLoss = {
    pipes: model.pipes.map((pipe) => {
      ...
      const connectedRoomId =
        model.equipment.find((item) => pipe.connectedEquipmentIds.includes(item.id))?.roomId ?? null;
      const T_air =
      ...
      const gain =
        connectedRoomId && Q_loss.value !== null
