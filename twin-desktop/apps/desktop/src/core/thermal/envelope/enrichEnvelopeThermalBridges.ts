...
import {
  ...
  shouldSkipEnvelopeBridgeEnrichment,
} from "../../../demo/deriveExteriorWallThermalBridges";
import {
  resolveExteriorWallIds,
  resolveThermalBridgeInputsFromLayers,
} from "./layerThermalBridgeInputs";

const VIDEO_DEMO_FRAGMENT_IDS = new Set(["video-ext-walls", "video-roof", "video-floor-ground"]);
const OPAQUE_WALL_TYPES = new Set<Sp50EnvelopeFragmentInput["constructionType"]>(["wall"]);
const OPAQUE_ROOF_TYPES = new Set<Sp50EnvelopeFragmentInput["constructionType"]>(["covering", "roof"]);
const OPAQUE_FLOOR_TYPES = new Set<Sp50EnvelopeFragmentInput["constructionType"]>([
  "floorOnGround",
  "floorOverBasement",
  "atticFloor",
]);
...
function resolveWallLayersFromModel(wall: Wall | undefined): WallLayer[] {
  if (!wall) {
    return [];
  }
...
function resolveFragmentLayers(model: BuildingModel, fragment: Sp50EnvelopeFragmentInput): WallLayer[] {
...
function footprintFromModel(model: BuildingModel): { widthM: number; depthM: number } {
...
function exteriorConvexCornerLengthM(model: BuildingModel, exteriorWallIds: ReadonlySet<string>): number {
  const endpointMap = new Map<string, Wall[]>();
...
function resolveReferenceWallLayers(
  model: BuildingModel,
...
function deriveWallFragmentThermalBridges(input: {
  ...
  const anchorCount = Math.max(
    0,
    Math.round(SP230_DISC_ANCHOR_DENSITY_PER_M2 * Math.max(0, input.wallAreaM2))
  );

  const linear: NonNullable<Sp50EnvelopeFragmentInput["heterogeneity"]>["linear"] = [];
  const cornerLengthM =
    input.cornerLengthShareM > 0
 ...
 */
export function enrichEnvelopeThermalBridges(model: BuildingModel): BuildingModel {
  ...
  const nextEnvelope = envelope.map((fragment) => {
    ...
    if (OPAQUE_WALL_TYPES.has(fragment.constructionType)) {
      ...
      const cornerShareM =
        wallId && totalExteriorLengthM > 0 && totalCornerLengthM > 0
  ...
  if (!changed) {
    ...
    if (aggregate.length === 0 && exteriorWallIds.size > 0) {
      const targetIndex = nextEnvelope.findIndex(
        (fragment) =>
          OPAQUE_WALL_TYPES.has(fragment.constructionType) &&
