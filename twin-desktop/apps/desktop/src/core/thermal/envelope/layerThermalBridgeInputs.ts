import type { Sp50OperationCondition, WallLayer } from "../../../entities/geometry/types";
...
export interface LayerThermalBridgeInputs {
  insulationResistance_m2K_W: number;
  baseLambda_W_mK: number;
  insulationMaterialIds: string[];
  baseMaterialId: string | null;
}

export function resolveLayerLambda_W_mK(
  materialId: string,
...
export function resolveLayerResistance_m2K_W(
  layer: WallLayer,
  operationCondition: Sp50OperationCondition
): number | null {
...
function isFinishLayer(layer: WallLayer, lambda_W_mK: number | null): boolean {
...
function isInsulationLayer(resistance_m2K_W: number, lambda_W_mK: number | null): boolean {
 ...
 */
export function resolveThermalBridgeInputsFromLayers(
  layers: WallLayer[],
  operationCondition: Sp50OperationCondition
): LayerThermalBridgeInputs | null {
  if (!layers.length) {
    return null;
  }
  ...
  const insulationEntries = analyzed.filter(
    (entry) => entry.resistance != null && isInsulationLayer(entry.resistance, entry.lambda)
  );
  const structuralEntries = analyzed.filter(
    (entry) =>
  ...
  let insulationResistance_m2K_W =
    insulationEntries.length > 0
  ...
  const baseCandidate =
    structuralEntries.length > 0
      ? [...structuralEntries].sort(
          (left, right) => right.layer.thickness_m - left.layer.thickness_m
        )[0]
...
export function resolveExteriorWallIds(model: BuildingModel): Set<string> {
