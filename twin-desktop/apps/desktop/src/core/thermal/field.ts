...
const roomSampleCache = new Map<string, Vec2[]>();

export interface ThermalFieldBuildOptions extends Omit<ThermalPhysicsOptions, "fixedRoomTemperaturesC"> {
  roomTemperaturesC: Record<string, number>;
  detailScale?: number;
}

export interface ThermalFieldRoom {
  roomId: string;
  levelId: string;
  polygon: Vec2[];
  centroid: Vec2;
  ...
  normalizationBiasC: number;
  samplePoints: Vec2[];
}

export type ThermalFieldSourceKind = "radiator" | "equipment" | "lighting" | "occupant" | "pipe" | "solar";

export interface ThermalFieldSource {
  id: string;
  kind: ThermalFieldSourceKind;
  levelId: string;
  roomId: string | null;
  position: Vec2;
  powerW: number;
...
export interface ThermalFieldBoundary {
  wallId: string;
  wall: Wall;
  levelId: string;
  ...
  orientation: ThermalSurfaceEstimate["orientation"];
  ...
  opaqueAreaM2: number;
  bridgeFactor: number;
  positiveCoolingC: number;
  negativeCoolingC: number;
  internalMixC: number;
  solarGainC: number;
}

export interface ThermalFieldModel {
  outdoorTemperatureC: number;
  physics: ThermalPhysicsModel;
  renderGeometry: GeometryRenderModel;
  rooms: ThermalFieldRoom[];
  roomMap: Map<string, ThermalFieldRoom>;
  sources: ThermalFieldSource[];
  sourcesByLevel: Map<string, ThermalFieldSource[]>;
  boundaries: ThermalFieldBoundary[];
  boundariesByLevel: Map<string, ThermalFieldBoundary[]>;
...
export function sampleThermalFieldAtPoint(field: ThermalFieldModel, levelId: string, point: Vec2): number {
...
export function sampleSmoothedThermalFieldAtPoint(
  field: ThermalFieldModel,
  levelId: string,
  point: Vec2,
  radiusM = DEFAULT_SMOOTHING_RADIUS_M
): number {
  const offsets = [
    { x: 0, y: 0, weight: 0.3 },
...
export interface LevelAdjacency {
  levelId: string;
  belowLevelId: string | null;
  aboveLevelId: string | null;
}

/** Sorted by `elevation_m` — used for vertical coupling in 3D floor overlays. */
export function buildLevelAdjacencyMap(levels: { id: string; elevation_m: number }[]): Map<string, LevelAdjacency> {
  const sorted = [...levels].sort((left, right) => left.elevation_m - right.elevation_m);
  const map = new Map<string, LevelAdjacency>();
 ...
 */
export function sampleSmoothedThermalFieldForFloorOverlay(
  field: ThermalFieldModel,
  levelId: string,
  point: Vec2,
...
export type InterfloorSlabFace = "top" | "bottom";
 ...
 */
export function sampleSmoothedThermalFieldForInterfloorSlab(
  field: ThermalFieldModel,
  slabLevelId: string,
  point: Vec2,
  levelAdjacency: Map<string, LevelAdjacency>,
  face: InterfloorSlabFace,
  radiusM = DEFAULT_SMOOTHING_RADIUS_M
): number | null {
...
export function sampleWallSurfaceTemperatures(
  field: ThermalFieldModel,
  wallId: string
): WallSurfaceTemperatureSample | null {
  const boundary = field.boundaryByWallId.get(wallId);
  if (!boundary) {
    return null;
  }
  const positiveRoomTemp =
    boundary.positiveRoomId !== null ? field.roomMap.get(boundary.positiveRoomId)?.baseTemperatureC ?? field.outdoorTemperatureC : field.outdoorTemperatureC;
  const negativeRoomTemp =
    boundary.negativeRoomId !== null ? field.roomMap.get(boundary.negativeRoomId)?.baseTemperatureC ?? field.outdoorTemperatureC : field.outdoorTemperatureC;
...
function computeWallSourceBoost(field: ThermalFieldModel, boundary: ThermalFieldBoundary, roomId: string | null): number {
  if (!roomId) {
    return 0;
  }
  const rawBoost = (field.sourcesByLevel.get(boundary.levelId) ?? []).reduce((sum, source) => {
    ...
    const influence =
      Math.exp(-(distanceToWall * distanceToWall) / Math.max(0.22, source.spreadM * source.spreadM * 0.72));
...
function createThermalRoom(
  room: RoomVolumeDescriptor,
  balance: ThermalRoomBalance | undefined,
  detailScale: number
): ThermalFieldRoom {
...
function buildRoomSamplePoints(room: RoomVolumeDescriptor, detailScale: number): Vec2[] {
  ...
  const samples: Vec2[] = [{ ...room.centroid }];
  const step = clamp(
    Math.sqrt(Math.max(room.areaM2, 1)) / (7.2 * normalizedScale),
    MIN_ROOM_SAMPLE_STEP_M / normalizedScale,
    MAX_ROOM_SAMPLE_STEP_M / Math.sqrt(normalizedScale)
  );
...
function readEngineeringParameterNumber(params: Record<string, unknown> | undefined, key: string): number | null {
  const value = params?.[key];
  return typeof value === "number" && Number.isFinite(value) ? value : null;
}

function readEngineeringAirflowM3s(params: Record<string, unknown> | undefined): number {
  const airflowM3H =
    readEngineeringParameterNumber(params, "airflowM3H") ??
    readEngineeringParameterNumber(params, "designAirflowM3H");
  if (airflowM3H != null) {
    return Math.max(0, airflowM3H) / 3600;
  }
  const airflowM3S =
    readEngineeringParameterNumber(params, "airflowM3S") ??
    readEngineeringParameterNumber(params, "designAirflowM3S");
  return airflowM3S != null ? Math.max(0, airflowM3S) : 0;
}

function resolveEngineeringEquipmentPowerW(params: Record<string, unknown> | undefined): number | null {
  ...
  const powerKW =
    readEngineeringParameterNumber(params, "powerKW") ??
...
function buildHeatSources(
  ...
  const sources: ThermalFieldSource[] = [];
  ...
  model.equipment.forEach((item) => {
    ...
    let decayM = 1.2;
    let spreadM = 1.8;
    let kind: ThermalFieldSourceKind = "equipment";
    ...
    switch (item.type) {
      ...
      case "sensor": {
        const nominal =
          typeof item.params.nominalPowerW === "number" && item.params.nominalPowerW > 0
  ...
  (model.engineeringSystems?.equipment ?? []).forEach((item) => {
    ...
    switch (item.type) {
      ...
      case "roofFan": {
        const electricalPowerW = resolveEngineeringEquipmentPowerW(item.parameters) ?? 1500;
        powerW = Math.max(80, electricalPowerW * 0.58);
        amplitudeC = clamp(powerW / Math.max(520, roomArea * 86), 0.08, 1.15);
        decayM = clamp(Math.sqrt(roomArea) * 0.18, 0.45, 1.75);
        spreadM = decayM * 1.38;
        break;
      }
      case "airHeater": {
      ...
      case "airHumidifier": {
        powerW = resolveEngineeringEquipmentPowerW(item.parameters) ?? 3000;
        amplitudeC = clamp((powerW / Math.max(260, roomArea * 30)) * 0.5, 0.18, 2.6);
        decayM = clamp(Math.sqrt(roomArea) * 0.2, 0.55, 2);
        spreadM = decayM * 1.4;
        break;
      }
      case "airDehumidifier": {
        powerW = resolveEngineeringEquipmentPowerW(item.parameters) ?? 2500;
        amplitudeC = clamp((powerW / Math.max(260, roomArea * 32)) * 0.44, 0.14, 2.1);
        decayM = clamp(Math.sqrt(roomArea) * 0.18, 0.5, 1.8);
        spreadM = decayM * 1.3;
        break;
      }
      case "supplyDiffuser": {
...
function buildFieldBoundaries(physics: ThermalPhysicsModel, outdoorTemperatureC: number): ThermalFieldBoundary[] {
...
function computeLocalTemperatureEffect(field: ThermalFieldModel, room: ThermalFieldRoom, point: Vec2): number {
  const rawSourceGain = (field.sourcesByLevel.get(room.levelId) ?? []).reduce((sum, source) => {
    ...
    const roomFactor =
      source.roomId === room.roomId ? 1 : source.roomId ? 0.08 : 0.18;
 ...
 */
function computeExternalCoolingAmplitude(
  roomTemperatureC: number,
  outdoorTemperatureC: number,
  boundary: Pick<
    ThermalFieldBoundary | ThermalSurfaceEstimate,
    "effectiveU_W_m2K" | "windowAreaM2" | "doorAreaM2" | "opaqueAreaM2"
  >
): number {
...
function resolveRoomAtPoint<T extends Pick<ThermalFieldRoom, "levelId" | "polygon">>(
  rooms: T[],
  levelId: string,
  point: Vec2
): T | null {
...
function groupByLevel<T extends { levelId: string }>(items: T[]): Map<string, T[]> {
...
function smoothBoundaryDecay(distance: number, decayM: number): number {
  const normalized = distance / Math.max(0.16, decayM);
  return Math.exp(-(normalized * normalized)) * 0.68 + Math.exp(-normalized) * 0.32;
}

function computePerimeterCooling(polygon: Vec2[], point: Vec2): number {
  return 0.14 * Math.exp(-distanceToPolygonEdges(polygon, point) / 1.15);
}

function distanceToPolygonEdges(polygon: Vec2[], point: Vec2): number {
  if (polygon.length < 2) {
    return 0;
  }
