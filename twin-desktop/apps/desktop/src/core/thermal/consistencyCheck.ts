...
import type { ThermalMonteCarloResult } from "../uncertainty/thermalMonteCarlo";
...
export interface GeometrySummary {
  ...
  zoneCount: number;
  heatedAreaM2: number;
  /** Computed by summing area × height per room from BuildingModel. */
  heatedVolumeM3: number;
  /** External wall area derived from OutdoorLinks area sum. Only if model available. */
  externalWallAreaM2: number | null;
  /** Reverse-engineered: totalWindowLossW / max(ΔT, 1) at reference. */
  windowAreaEquivM2: number | null;
  /** Reverse-engineered: totalDoorLossW / max(ΔT, 1) at reference. */
  doorAreaEquivM2: number | null;
}

export interface HeatLossCoefficients {
  H_tr_W_K: number | null;
  H_inf_W_K: number | null;
  H_ve_W_K: number | null;
  H_total_W_K: number | null;
  /** Derived from reference slice: totalOpaqueLossW / ΔT_ref */
  H_opaque_slice_W_K: number | null;
  H_window_slice_W_K: number | null;
  H_door_slice_W_K: number | null;
}

export interface DesignDeltaT {
  T_setpoint_day_C: number;
  T_outdoor_ref_C: number;
  deltaT_K: number;
  T_avg_zone_C: number | null;
  deltaT_avg_zone_K: number | null;
}

export interface PeakLossReconstruction {
  /** Solver result, kW */
  peakLoadKW_solver: number;
  /** H_total × ΔT_setpoint / 1000 — stationary estimate, no gains subtracted */
  Q_stat_H_setpoint_kW: number | null;
  /** H_total × ΔT_setpoint / 1000 − Q_internal / 1000 — stationary minus internal gains */
  Q_stat_minus_gains_kW: number | null;
  /** Stationary minus gains, used as best reconstruction for ΔT */
  Q_best_reconstructed_kW: number | null;
  Q_internal_gains_kW: number | null;
  differenceStatKW: number | null;
  differenceStatPercent: number | null;
  differenceWithGainsKW: number | null;
  differenceWithGainsPercent: number | null;
  /**
   * Context explaining expected discrepancy sources.
   */
  explanation: string;
}

export interface ComponentSumCheck {
  Q_opaque_W: number;
  Q_window_W: number;
  Q_door_W: number;
  ...
  Q_air_exchange_W: number;
  sumAllComponentsW: number;
  reportedTotalLossW: number;
  differenceW: number;
  differencePercent: number;
}

export interface RoomSumCheck {
  zoneCount: number;
  sumZoneLossW: number;
  buildingTotalLossW: number;
  differenceW: number;
  differencePercent: number;
}

export interface EnergyIntegrationCheck {
  timelineStepCount: number;
  /** Median timestep in seconds (may vary if last step is shorter). */
...
export interface MonteCarloCheck {
  runs: number;
  underheatingThresholdC: number;
  underheatingBelow20Count: number;
  underheatingProbabilityReported: number | null;
  underheatingProbabilityRecomputed: number | null;
  probabilityDifferenceAbs: number | null;
  peakLoad_P10_kW: number | null;
  peakLoad_P50_kW: number | null;
  peakLoad_P90_kW: number | null;
  energy_P10_kWh: number | null;
  energy_P50_kWh: number | null;
  energy_P90_kWh: number | null;
  minTemp_P10_C: number | null;
  minTemp_P50_C: number | null;
  minTemp_P90_C: number | null;
}

export interface ThermalConsistencyReport {
  generatedAt: number;
  geometry: GeometrySummary;
  heatLossCoefficients: HeatLossCoefficients;
  designDeltaT: DesignDeltaT;
  peakReconstruction: PeakLossReconstruction;
  componentSum: ComponentSumCheck;
  roomSum: RoomSumCheck;
  energyIntegration: EnergyIntegrationCheck;
  monteCarlo: MonteCarloCheck | null;
  /** PASS/WARN/FAIL per invariant */
  invariants: ConsistencyItem[];
  assumptions: string[];
  fallbacks: string[];
  /** Overall status: worst across all invariants */
...
function diffPct(actual: number, expected: number): number | null {
  if (!Number.isFinite(expected) || Math.abs(expected) < 1e-9) return null;
  return ((actual - expected) / Math.abs(expected)) * 100;
}

function heatedVolumeFromModel(model: BuildingModel): number {
...
function zoneTempAverage(result: ThermalSimulationResult): number | null {
...
function recomputeEnergyFromTimeline(result: ThermalSimulationResult): {
  recomputedKWh: number;
  stepCount: number;
  medianDtSeconds: number;
  minTotalPowerKW: number;
  maxTotalPowerKW: number;
  ...
  for (let i = 1; i < tl.length; i++) {
    ...
    if (stepPowerKW < minPowerKW) minPowerKW = stepPowerKW;
    if (stepPowerKW > maxPowerKW) maxPowerKW = stepPowerKW;
...
function mkItem(
  ...
  const differenceAbs =
    actual !== undefined && expected !== undefined ? actual - expected : undefined;
  const differencePercent =
    differenceAbs !== undefined && expected !== undefined && Math.abs(expected) > 1e-9
...
function worstStatus(items: ConsistencyItem[]): CheckStatus {
...
export function buildThermalConsistencyReport(
  ...
  assumptions.push(
    "Отапливаемый объём рассчитан как Σ(area_i × height_i) по всем комнатам BuildingModel (height из level или 3 м по умолчанию)."
  );
  ...
  assumptions.push(
    "Эквивалентные площади окон/дверей рассчитаны обратно из H_window/H_door, используя типовые U: окна 1.8, двери 1.5 Вт/(м²·К)."
  );
  ...
  const heatLossCoefficients: HeatLossCoefficients = {
    H_tr_W_K: H_tr,
    H_inf_W_K: H_inf,
    H_ve_W_K: H_ve,
    H_total_W_K: H_total,
  ...
  // 3. DESIGN ΔT
  // ------------------------------------------------------------------
  const designDeltaT: DesignDeltaT = {
  ...
  let Q_stat_H_setpoint_kW: number | null = null;
  let Q_stat_minus_gains_kW: number | null = null;
  ...
  const diffStatKW =
    Q_stat_H_setpoint_kW !== null ? peakLoadKW - Q_stat_H_setpoint_kW : null;
  const diffStatPct = diffStatKW !== null && Q_stat_H_setpoint_kW !== null
    ? diffPct(peakLoadKW, Q_stat_H_setpoint_kW)
    : null;
  const diffWithGainsKW =
    Q_stat_minus_gains_kW !== null ? peakLoadKW - Q_stat_minus_gains_kW : null;
  const diffWithGainsPct =
    diffWithGainsKW !== null && Q_stat_minus_gains_kW !== null
      ? diffPct(peakLoadKW, Q_stat_minus_gains_kW)
      : null;

  const peakReconstruction: PeakLossReconstruction = {
    ...
    explanation:
      "Стационарная оценка Q_stat = H_total × ΔT не совпадает с пиком RC по нескольким причинам: " +
  ...
  const componentSum: ComponentSumCheck = {
    Q_opaque_W: Q_opaque,
    Q_window_W: Q_window,
    Q_door_W: Q_door,
  ...
  // ------------------------------------------------------------------
  const { recomputedKWh, stepCount, medianDtSeconds, minTotalPowerKW, maxTotalPowerKW, durationHours } =
    recomputeEnergyFromTimeline(result);
  ...
  const energyIntegration: EnergyIntegrationCheck = {
  ...
  // ------------------------------------------------------------------
  const invariants: ConsistencyItem[] = [];
  ...
  if (H_tr !== null && H_inf !== null && H_ve !== null && H_total !== null) {
    ...
    invariants.push(
      mkItem(
        "INV-01",
        "H_total = H_tr + H_inf + H_ve",
        pct < 0.1 ? "PASS" : pct < 1 ? "WARN" : "FAIL",
        sum,
        H_total,
        "Вт/К",
        pct < 0.1
  ...
  // INV-02: Component loss sum ≈ totalLossW
  {
    ...
    invariants.push(
      mkItem(
        "INV-02",
        "Σ компонент ≈ totalLossW",
        pct < 0.5 ? "PASS" : pct < 2 ? "WARN" : "FAIL",
        sumAllW,
        reportedTotalLossW,
        "Вт",
        pct < 0.5
  ...
  // INV-03: Room loss sum ≈ totalLossW
  {
    ...
    invariants.push(
      mkItem(
        "INV-03",
        "Σ потерь по зонам ≈ totalLossW",
        pct < 1 ? "PASS" : pct < 5 ? "WARN" : "FAIL",
        sumZoneLossW,
        reportedTotalLossW,
        "Вт",
        pct < 1
  ...
  // INV-04: Energy integration agrees with summary
  {
    ...
    invariants.push(
      mkItem(
        "INV-04",
        "∫ Q(t)dt / 3.6M ≈ summary.totalEnergyKWh",
        pct < 0.5 ? "PASS" : pct < 2 ? "WARN" : "FAIL",
        recomputedKWh,
        reportedEnergyKWh,
        "кВт·ч",
  ...
  if (Q_best_reconstructed_kW !== null) {
    ...
    invariants.push(
      mkItem(
        "INV-05",
        "H_total × ΔT − Q_int ≈ peakLoadKW",
        pct < 10 ? "PASS" : pct < 20 ? "WARN" : "INFO",
        peakLoadKW,
        Q_best_reconstructed_kW,
        "кВт",
        pct < 10
  ...
  // INV-06: peakLoadKW consistency — max of timeline Σ matches summary
  {
    ...
    for (const point of result.timeline) {
      ...
      if (sum > timelinePeak) timelinePeak = sum;
    }
    ...
    invariants.push(
      mkItem(
        "INV-06",
        "max∑Q(t) из timeline = summary.peakLoadKW",
        pct < 0.5 ? "PASS" : pct < 2 ? "WARN" : "FAIL",
        timelinePeakKW,
  ...
  // INV-07: Room count matches zone diagnostic count
  {
    ...
    invariants.push(
      mkItem(
        "INV-07",
        "Число комнат BuildingModel = число зон в diagnostics",
        roomCount === zoneCount ? "PASS" : "WARN",
        zoneCount,
        roomCount,
        "шт",
        roomCount === zoneCount
  ...
  if (bld) {
    ...
    invariants.push(
      mkItem(
        "INV-08",
        "Остаток баланса |r| (C·dT/dt в динамическом RC)",
        "INFO",
        residual,
        0,
        "Вт",
        `|r| = ${residual.toFixed(0)} Вт = ${(relResidual * 100).toFixed(1)}% от Q_total. ` +
  ...
  if (monteCarlo) {
    ...
    invariants.push(
      mkItem(
        "INV-09",
        "P(T_min < 20°C) пересчитана = reported",
        probDiff === null ? "INFO" : probDiff < 0.01 ? "PASS" : probDiff < 0.03 ? "WARN" : "FAIL",
        monteCarlo.underheatingProbabilityRecomputed ?? undefined,
        monteCarlo.underheatingProbabilityReported ?? undefined,
        "доля",
        probDiff === null
  ...
  if (heatedAreaM2 > 0 && bld) {
    ...
    invariants.push(
      mkItem(
        "INV-10",
        "Удельная нагрузка q_A в инженерном диапазоне [10, 300] Вт/м²",
        sane ? "PASS" : qA < 5 || qA > 500 ? "FAIL" : "WARN",
        qA,
        undefined,
        "Вт/м²",
        sane
