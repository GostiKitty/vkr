export interface ThermalNode {
  ...
  internalGain?: number; // W
  infiltrationMassFlow?: number; // kg/s
  infiltrationSpecificHeat?: number; // J/(kg·K)
}
...
export interface OutdoorBoundary {
  outdoorTemp: (timeHours: number) => number;
}
...
export interface StepNodeSnapshot {
  nodeId: string;
  temperature: number;
  netPower: number;
}

export interface StepObserverPayload {
  timeHours: number;
  nodes: StepNodeSnapshot[];
}

export type StepObserver = (payload: StepObserverPayload) => void;

interface NodeCache {
  node: ThermalNode;
  adjacency: ThermalConnection[];
}

export function simulateThermalModel(
  nodes: ThermalNode[],
  ...
  const series: TemperatureSeriesPoint[] = [];
  ...
  for (let step = 0; step <= steps; step++) {
    ...
    const observerSnapshots: StepNodeSnapshot[] | undefined = stepObserver ? [] : undefined;
...
function buildAdjacency(
  nodes: ThermalNode[],
  connections: ThermalConnection[]
): Map<string, NodeCache> {
  const adjacency = new Map<string, NodeCache>();
  nodes.forEach((node) => {
