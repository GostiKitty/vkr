...
import type { BuildingModel, Sp50ConstructionType, Wall } from "../../../entities/geometry/types";
import type { ScenarioConfig, ScenarioEnvelopeLeakageConfig } from "../../../entities/workflow/workflow.store";
...
import {
  DEFAULT_DOOR_LEAKAGE_M3S_M_AT_10_PA,
  DEFAULT_ENVELOPE_LEAKAGE_M3S_M2_AT_10_PA,
  DEFAULT_PRESSURE_EXPONENT,
  DEFAULT_REFERENCE_PRESSURE_PA,
  DEFAULT_WINDOW_LEAKAGE_M3S_M_AT_10_PA,
} from "../infiltration";
import type { VentilationInputSource } from "./resolveScenarioVentilation";

const AIR_DENSITY_KG_M3 = 1.204;
const REFERENCE_PRESSURE_PA = 10;

export interface ResolvedLeakageScalar {
  value: number;
  source: VentilationInputSource;
  explicit: boolean;
}

export interface ResolvedEnvelopeLeakageInputs {
  envelopeAirPermeabilityM3sM2At10Pa: ResolvedLeakageScalar;
  windowAirPermeabilityM3sMAt10Pa: ResolvedLeakageScalar;
  doorAirPermeabilityM3sMAt10Pa: ResolvedLeakageScalar;
  pressureExponent: ResolvedLeakageScalar;
  referencePressurePa: ResolvedLeakageScalar;
}

interface OpeningLeakageGeometry {
  windowAreaM2: number;
...
function kgM2HToM3sM2At10Pa(kgM2H: number, densityKgM3 = AIR_DENSITY_KG_M3): number {
  return Math.max(0, kgM2H) / densityKgM3 / 3600;
}

function kgM2HToM3sMAt10Pa(kgM2H: number, areaM2: number, perimeterM: number, densityKgM3 = AIR_DENSITY_KG_M3): number {
...
function summarizeOpeningLeakageGeometry(building: BuildingModel): OpeningLeakageGeometry {
...
function sumWallAirResistanceM2hPaKg(wall: Wall): number | null {
...
function permeabilityM3sM2FromWallLayers(wall: Wall): number | null {
...
function pickDominantExternalWall(building: BuildingModel): Wall | null {
  const adjacency = buildAdjacencyGraph(building);
  const externalWallIds = new Set(adjacency.external.map((edge) => edge.wallId));
...
function permeabilityM3sM2FromNorm(constructionType: Sp50ConstructionType): number | null {
  const normKgM2H = getNormalizedAirPermeability(constructionType);
  return normKgM2H != null ? kgM2HToM3sM2At10Pa(normKgM2H) : null;
}

function computeEnvelopePermeabilityFromModel(model: BuildingModel): { value: number; source: VentilationInputSource } | null {
  ...
  const envelopeFragment = model.thermalProtection?.envelope?.find(
    (fragment) => fragment.constructionType === "wall" || fragment.constructionType === "roof"
  );
...
function computeWindowPermeabilityFromModel(model: BuildingModel, openings: OpeningLeakageGeometry): { value: number; source: VentilationInputSource } | null {
...
function computeDoorPermeabilityFromModel(model: BuildingModel, openings: OpeningLeakageGeometry): { value: number; source: VentilationInputSource } | null {
...
export function resolveVentilationScalar(
  raw: number | null | undefined,
  computed: { value: number; source: VentilationInputSource } | null,
  fallback: number
): ResolvedLeakageScalar {
...
export function buildResolvedEnvelopeLeakageConfig(
  resolved: ResolvedEnvelopeLeakageInputs
): ScenarioEnvelopeLeakageConfig {
  return {
...
export function resolveScenarioEnvelopeLeakageInputs(
  scenarioConfig: ScenarioConfig | null | undefined,
  scenario: ScenarioConfig,
  model: BuildingModel
): ResolvedEnvelopeLeakageInputs {
  ...
  return {
    envelopeAirPermeabilityM3sM2At10Pa: resolveVentilationScalar(
      raw?.envelopeAirPermeabilityM3sM2At10Pa,
      envelopeComputed,
      DEFAULT_ENVELOPE_LEAKAGE_M3S_M2_AT_10_PA
    ),
    windowAirPermeabilityM3sMAt10Pa: resolveVentilationScalar(
      raw?.windowAirPermeabilityM3sMAt10Pa,
      windowComputed,
      DEFAULT_WINDOW_LEAKAGE_M3S_M_AT_10_PA
    ),
    doorAirPermeabilityM3sMAt10Pa: resolveVentilationScalar(
      raw?.doorAirPermeabilityM3sMAt10Pa,
      doorComputed,
      DEFAULT_DOOR_LEAKAGE_M3S_M_AT_10_PA
    ),
    ...
    referencePressurePa: resolveVentilationScalar(
      raw?.referencePressurePa,
      { value: DEFAULT_REFERENCE_PRESSURE_PA, source: "model" },
      DEFAULT_REFERENCE_PRESSURE_PA
    ),
  };
}
