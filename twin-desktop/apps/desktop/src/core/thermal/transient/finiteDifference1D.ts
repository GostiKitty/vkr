...
export interface FiniteDifference1DOptions {
  scheme?: "explicit";
  innerSurfaceLimit_C?: number;
}

export function solveTransient1DExplicit(
  ...
  const warnings: string[] = ["Использована узловая аппроксимация многослойной конструкции."];
  ...
  const temperatures: number[][] = [initialTemperature];
  const innerSurfaceTemperature: number[] = [];
  const outerSurfaceTemperature: number[] = [];
  const innerBoundarySeries: number[] = [];
  const outerBoundarySeries: number[] = [];
  ...
  for (let step = 0; step < time.length; step += 1) {
    ...
    if (
      !isTrustworthyTemperature(boundary.innerSurfaceTemperature_C) ||
    ...
    for (let index = 0; index < current.length; index += 1) {
      ...
      const leftFluxIntoNode_W_m2 =
        index === 0 ? boundary.innerFluxIntoWall_W_m2 : computeInterfaceFlux(nodes[index - 1], current[index - 1], node, current[index]);
      const rightFluxOutOfNode_W_m2 =
      ...
      const deltaT =
        (scenario.timeStep_s / Math.max(volumetricHeatCapacity * node.dx_m, 1e-9)) *
    ...
    if (!isTemperatureSeriesTrustworthy(next, current)) {
      ...
      return buildInvalidTransientResult({
        ...
        warnings,
        invalidReason:
          `Нестационарный расчет признан неустойчивым по численному контролю.${suggestion}`,
        nodes,
        initialTemperature,
        time: time.slice(0, step + 1),
        temperatures,
...
export function getTransientFrame(result: TransientCalculationResult, timeIndex: number): TransientFrame {
...
function discretizeLayers(layers: TransientLayer[]): DiscretizedNode[] {
  const nodes: DiscretizedNode[] = [];
  let cursor = 0;
...
function buildTimeAxis(duration_s: number, timeStep_s: number): number[] {
...
function buildInitialTemperature(nodes: DiscretizedNode[], scenario: TransientScenario): number[] {
...
function interpolateProfile(profile: Array<{ x_m: number; temperature_C: number }>, x_m: number): number {
...
function evaluateExplicitStability(nodes: DiscretizedNode[], timeStep_s: number): {
  stable: boolean;
  maxR: number;
  suggestedTimeStep_s: number;
} {
  let maxR = 0;
...
function computeInterfaceFlux(left: DiscretizedNode, leftTemperature_C: number, right: DiscretizedNode, rightTemperature_C: number): number {
  const resistance =
    0.5 * left.dx_m / Math.max(left.lambda_W_mK, 1e-9) + 0.5 * right.dx_m / Math.max(right.lambda_W_mK, 1e-9);
  return (leftTemperature_C - rightTemperature_C) / Math.max(resistance, 1e-9);
}

function evaluateBoundaryState(
  nodes: DiscretizedNode[],
  temperature: number[],
  scenario: TransientScenario,
  time_s: number,
  warnings: string[]
): {
  innerFluxIntoWall_W_m2: number;
  outerFluxIntoWall_W_m2: number;
  innerSurfaceTemperature_C: number;
  outerSurfaceTemperature_C: number;
  innerBoundaryReference_C: number;
  outerBoundaryReference_C: number;
} {
...
function resolveBoundary(
  node: DiscretizedNode,
  nodeTemperature_C: number,
  boundary: TransientBoundaryCondition,
  time_s: number,
  side: "inner" | "outer",
  warnings: string[]
): {
  fluxIntoWall_W_m2: number;
...
function evaluateBoundaryValue(value: number | ((time_s: number) => number) | undefined, time_s: number, fallback: number): number {
...
function evaluateInternalHeatSource(scenario: TransientScenario, x_m: number, time_s: number): number {
...
function validateLayers(layers: TransientLayer[]): string[] {
...
function validateNodes(nodes: DiscretizedNode[]): string[] {
  const warnings: string[] = [];
...
function isTrustworthyTemperature(value: number): boolean {
  return Number.isFinite(value) && Math.abs(value) <= MAX_REASONABLE_TEMPERATURE_C;
}

function isTemperatureSeriesTrustworthy(next: number[], previous?: number[]): boolean {
...
function buildInvalidTransientResult(input: {
  ...
  stableState?: { maxR: number; suggestedTimeStep_s: number };
  innerBoundarySeries?: number[];
  outerBoundarySeries?: number[];
}): TransientCalculationResult {
  const nodes = input.nodes?.map((node) => node.x_m) ?? [];
  const temperatureRow =
    input.temperatures?.[input.temperatures.length - 1] ??
