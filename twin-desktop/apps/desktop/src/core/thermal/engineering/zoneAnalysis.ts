...
import type { ThermalPhysicsModel } from "../physics";
import { getOrientationWallLabel, getRoomDisplayName } from "./display";
import type { EngineeringFieldCell, EngineeringFieldResult, EngineeringZoneInsight } from "./types";

interface CellMeta {
  cell: EngineeringFieldCell;
  roomAreaM2: number;
  distanceToWallM: number;
  distanceToExternalWallM: number;
  distanceToWindowM: number;
  distanceToHeatingM: number;
  nearWall: boolean;
  nearWindow: boolean;
  nearHeating: boolean;
  nearCorner: boolean;
  nearestOrientation: Orientation | null;
...
export function filterFieldCells(field: EngineeringFieldResult | null): EngineeringFieldCell[] {
  if (!field) {
    return [];
  }
  return field.cells.filter(
    (cell) =>
      Number.isFinite(cell.temperatureC) &&
...
export function computeAverageFieldTemperature(cells: EngineeringFieldCell[]): number {
  if (!cells.length) {
    return 0;
  }
...
export function clusterFieldCells(cells: EngineeringFieldCell[], distanceThresholdM: number): EngineeringFieldCell[][] {
  const clusters: EngineeringFieldCell[][] = [];
  ...
  for (let index = 0; index < cells.length; index += 1) {
    ...
    const queue = [index];
    const cluster: EngineeringFieldCell[] = [seed];
...
export function buildZoneInsights(
  ...
  const heatingPoints = model.equipment.filter(
    (item) => item.levelId === field.levelId && item.state !== "off" && HEATING_TYPES.has(item.type)
  );
  ...
  const coldClusters = summarizeClusters(
    metas.filter((meta) => meta.cell.temperatureC <= coldThresholdC && (meta.nearWindow || meta.nearWall || meta.nearCorner)),
    distanceThresholdM
  )
    .sort((left, right) => left.temperatureC - right.temperatureC)
    .slice(0, 2);

  const hotClusters = summarizeClusters(
    metas.filter((meta) => meta.cell.temperatureC >= hotThresholdC && meta.nearHeating),
    distanceThresholdM
  )
    .sort((left, right) => right.temperatureC - left.temperatureC)
    .slice(0, 2);

  const wallCluster = summarizeClusters(
    metas.filter((meta) => meta.nearWall && !meta.nearWindow),
    distanceThresholdM
  )
    .sort((left, right) => left.temperatureC - right.temperatureC)[0];

  const windowCluster = summarizeClusters(
    metas.filter((meta) => meta.nearWindow),
    distanceThresholdM
  )
    .sort((left, right) => left.temperatureC - right.temperatureC)[0];

  const heatingCluster = summarizeClusters(
    metas.filter((meta) => meta.nearHeating),
    distanceThresholdM
  )
  ...
  const occupiedCluster =
    summarizeClusters(occupiedCandidates, distanceThresholdM)
      .sort(
        (left, right) =>
          Math.abs(left.temperatureC - meanTemperatureC) - Math.abs(right.temperatureC - meanTemperatureC)
      )[0] ?? summarizeClusters(metas.filter((meta) => meta.cell.roomId === dominantRoom?.roomId), distanceThresholdM)[0];

  const insights: EngineeringZoneInsight[] = [];
...
function buildCellMeta(
  cell: EngineeringFieldCell,
  rooms: ThermalPhysicsModel["renderGeometry"]["roomVolumes"],
  surfaces: ThermalPhysicsModel["surfaces"],
  heatingPoints: BuildingModel["equipment"],
  cellSizeM: number
): CellMeta {
  const room = rooms.find((entry) => entry.roomId === cell.roomId) ?? null;
  const roomSegments = room ? polygonSegments(room.polygon) : [];
  const externalSurfaces = surfaces.filter(
    (surface) => surface.positiveRoomId === cell.roomId || surface.negativeRoomId === cell.roomId
  );
...
function summarizeClusters(metas: CellMeta[], distanceThresholdM: number): Cluster[] {
  if (!metas.length) {
    return [];
  }
  const clusters = clusterFieldCells(
    metas.map((meta) => meta.cell),
    distanceThresholdM
  );
...
function buildInsight(
  ...
  cluster: Cluster,
  category: EngineeringZoneInsight["category"],
  index: number
): EngineeringZoneInsight {
...
function resolveSeverity(category: EngineeringZoneInsight["category"], deltaFromAverageC: number): EngineeringZoneInsight["severity"] {
...
function resolveCornerLabel(point: Vec2, polygon: Vec2[]): string | null {
...
function polygonSegments(polygon: Vec2[]): Array<{ a: Vec2; b: Vec2 }> {
