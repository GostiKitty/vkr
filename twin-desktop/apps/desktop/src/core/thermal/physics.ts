...
type ThermalNetworkContext = {
  ...
  roomSupplyAir: Map<string, RoomMechanicalAirState>;
...
function readEngineeringAirflowM3s(parameters: Record<string, unknown> | undefined): number {
  const airflowM3H =
...
function readEngineeringPipeAirflowM3s(pipe: Pick<EngineeringPipe, "flowRate">): number {
  return typeof pipe.flowRate === "number" && Number.isFinite(pipe.flowRate) ? Math.max(0, pipe.flowRate) / 3600 : 0;
}

function getConnectedEngineeringAirPipes(
  pipesByEquipmentId: Map<string, EngineeringPipe[]>,
  equipmentId: string,
  medium: "airSupply" | "airExhaust"
): EngineeringPipe[] {
  return (pipesByEquipmentId.get(equipmentId) ?? []).filter((pipe) => pipe.medium === medium);
}

function resolveEngineeringAirTerminalAirflowM3s(
  equipment: EngineeringEquipment,
  medium: "airSupply" | "airExhaust",
  pipesByEquipmentId: Map<string, EngineeringPipe[]>,
  equipmentById: Map<string, EngineeringEquipment>
): number {
...
function resolveEngineeringSupplyTemperatureC(
  equipment: EngineeringEquipment,
  pipesByEquipmentId: Map<string, EngineeringPipe[]>,
  equipmentById: Map<string, EngineeringEquipment>,
  fallbackC: number
  ...
  const pipeTemperature = connectedPipes.find(
    (pipe): pipe is EngineeringPipe & { temperature: number } => typeof pipe.temperature === "number" && Number.isFinite(pipe.temperature)
  )?.temperature;
  ...
  if (equipment.type === "supplyDiffuser") {
    for (const pipe of connectedPipes) {
      ...
      const otherTemperature =
        readEngineeringParameterNumber(otherEquipment.parameters, "supplyTemperatureC") ??
  ...
  const ownTemperature =
    readEngineeringParameterNumber(equipment.parameters, "supplyTemperatureC") ??
...
function accumulateRoomMechanicalAir(
  roomSupplyAir: Map<string, RoomMechanicalAirState>,
  roomId: string,
  input: {
    supplyAirflow_m3_s?: number;
...
function getRoomMechanicalAirflowM3s(state: RoomMechanicalAirState | null | undefined): number {
  if (!state) {
    return 0;
  }
...
function buildThermalNetworkContext(
  ...
  const roomSupplyAir = new Map<string, RoomMechanicalAirState>();
  ...
  const engineeringAirPipes = (model.engineeringSystems?.pipes ?? []).filter(
    (pipe) => pipe.medium === "airSupply" || pipe.medium === "airExhaust"
  );
  const engineeringAirPipesByEquipmentId = new Map<string, EngineeringPipe[]>();
  ...
  engineeringEquipment.forEach((item) => {
    ...
    if (item.type === "supplyDiffuser") {
      const airflow_m3_s = resolveEngineeringAirTerminalAirflowM3s(
        item,
        "airSupply",
        engineeringAirPipesByEquipmentId,
      ...
      accumulateRoomMechanicalAir(roomSupplyAir, room.id, {
        supplyAirflow_m3_s: airflow_m3_s,
        supplyTemperatureC: resolveEngineeringSupplyTemperatureC(
          item,
          engineeringAirPipesByEquipmentId,
          engineeringEquipmentById,
          options.supplyAirTemperatureC ?? options.outdoorTemperatureC
        ),
      });
      return;
    }
