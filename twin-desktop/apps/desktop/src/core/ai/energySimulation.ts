import type { EnergySimulationInput, EnergySimulationResult, EnergyZoneSimulation } from "./types";

const VENTILATION_HEAT_FACTOR_W_PER_M3_H_K = 0.33;
const COOLING_SHARE = 0.62;

export function runEnergySimulation(
  input: EnergySimulationInput | EnergySimulationInput[]
  ...
  const averageInsulation =
    zoneCount > 0 ? zones.reduce((sum, zone) => sum + zone.insulationCoefficient_W_m2K, 0) / zoneCount : 0;
  const designDelta =
    zoneCount > 0 ? zones.reduce((sum, zone) => sum + zone.temperatureDifference_C, 0) / zoneCount : 0;
...
function simulateZone(input: EnergySimulationInput, index: number): EnergyZoneSimulation {
