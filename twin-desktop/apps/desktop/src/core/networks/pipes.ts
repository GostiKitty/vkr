...
import type { ConnectedComponent, PipeNetworkSummary } from "./types";
import { polylineLength } from "./utils";

export function summarizePipes(pipes: PipeNetwork[]): PipeNetworkSummary {
  if (!pipes.length) {
    return {
      totalLength_m: 0,
      branchCount: 0,
  ...
  const weightedTemp =
    pipes.reduce((sum, pipe) => sum + pipe.fluidTemperatureC * Math.max(pipe.flowRate_kg_s, 0.001), 0) /
...
export function buildPipeConnectivity(pipes: PipeNetwork[]): ConnectedComponent[] {
