import type { DuctNetwork } from "../../entities/networks/types";
import type { ConnectedComponent, DuctNetworkSummary } from "./types";
import { polylineLength } from "./utils";

export function summarizeDucts(ducts: DuctNetwork[]): DuctNetworkSummary {
  if (!ducts.length) {
    return {
      totalLength_m: 0,
      branchCount: 0,
  ...
  const averageAirVelocity_m_s =
    ducts.reduce((sum, duct) => sum + Math.max(0, duct.airVelocity_m_s), 0) / Math.max(ducts.length, 1);
...
export function buildDuctConnectivity(ducts: DuctNetwork[]): ConnectedComponent[] {
