...
export function runEconomicAssessment(report: Sp50ComplianceReport, scenario: EconomicScenario): EconomicAssessmentResult {
  ...
  if (scenario.heatingEnergySource === "gas") {
    ...
    if (penaltyPct > 0) {
      warnings.push(
        `Источник тепла — газовый котёл. При наличии договора с минимальным объёмом выборки экономия от снижения потребления уменьшается на штраф за недобор (~${penaltyPct} % от сэкономленной суммы). Колонка «Эффект. экономия» учитывает этот штраф.`
      );
    }
  ...
  if (scenario.heatingEnergySource === "heat") {
    ...
    if (minFraction > 0.05) {
      warnings.push(
        `Для зданий без приборов учёта тепловой энергии часть оплаты (~${Math.round(minFraction * 100)} %) не зависит от фактического потребления. Эффективная экономия рассчитана с учётом этого ограничения.`
      );
    }
  }
  ...
  const packageSavedCO2_raw = packageMeasures.reduce<number | null>((acc, entry) => {
    if (entry.savedCO2_tCO2_year === null) return acc;
  ...
  const specificHeatConsumptionAfter_kWh_m2 =
    afterEnergy !== null && heatedArea != null && heatedArea > 0
      ? afterEnergy / heatedArea
      : null;
  const sp50EnergyNorm_kWh_m2 = Number.isFinite(qNorm) ? (qNorm ?? null) : null;
  const energyClassBefore =
    specificHeatConsumption_kWh_m2 !== null && sp50EnergyNorm_kWh_m2 !== null
      ? classifyEnergyClass(specificHeatConsumption_kWh_m2, sp50EnergyNorm_kWh_m2)
      : null;
  const energyClassAfter =
    specificHeatConsumptionAfter_kWh_m2 !== null && sp50EnergyNorm_kWh_m2 !== null
      ? classifyEnergyClass(specificHeatConsumptionAfter_kWh_m2, sp50EnergyNorm_kWh_m2)
      : null;
  const sp50EnergyComplies =
    specificHeatConsumption_kWh_m2 !== null && sp50EnergyNorm_kWh_m2 !== null
 ...
 */
function classifyEnergyClass(qActual_kWh_m2: number, qNorm_kWh_m2: number): string {
  if (qNorm_kWh_m2 <= 0) return "—";
  const deviation = ((qActual_kWh_m2 - qNorm_kWh_m2) / qNorm_kWh_m2) * 100;
  if (deviation < -60) return "А++";
  if (deviation < -45) return "А+";
  if (deviation < -30) return "А";
  if (deviation < -10) return "В";
  if (deviation < 5)   return "С";
  if (deviation < 25)  return "D";
  if (deviation < 50)  return "E";
  if (deviation < 75)  return "F";
  return "G";
}
 ...
 */
function calculateContractPenalty(
  annualSaving_RUB: number,
  savedEnergy_kWh_year: number,
  baseEnergy_kWh: number | null,
  scenario: EconomicScenario
): number {
  if (annualSaving_RUB <= 0) return 0;
...
function calculateCO2Saving(savedEnergy_kWh_year: number, scenario: EconomicScenario): number | null {
  const factor =
    scenario.co2EmissionFactor_kgPerKWh ??
    DEFAULT_CO2_EMISSION_FACTORS_KG_PER_KWH[scenario.heatingEnergySource] ??
    null;
  if (!factor || !Number.isFinite(savedEnergy_kWh_year) || savedEnergy_kWh_year <= 0) return null;
 ...
 */
function resolveResidualValuePercent(lifetimeYears: number | undefined, analysisPeriod_years: number): number {
  if (!lifetimeYears || lifetimeYears <= analysisPeriod_years) return 0;
