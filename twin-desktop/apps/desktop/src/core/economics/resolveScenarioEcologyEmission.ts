...
import {
  CO2_EMISSION_FACTOR_NORM_NOTES,
  defaultCo2EmissionFactorFromEcologyEnergySource,
  defaultCo2EmissionFactorKgPerKWh,
  ecologyEnergySourceToHeatingSource,
} from "./defaultCo2EmissionFactors";

export type EcologyEmissionFactorSource = "user" | "norm" | "missing";

export interface ResolvedEcologyEmissionFactor {
  value: number | null;
  source: EcologyEmissionFactorSource;
  explicit: boolean;
  heatingSource: HeatingEnergySource | null;
  normNote: string | null;
}

export function ecologyEmissionFactorIsExplicit(
  ecology: ScenarioConfig["ecology"] | null | undefined
): boolean {
  const value = ecology?.emissionFactorKgPerKWh;
  return value != null && Number.isFinite(value);
}

export function resolveScenarioEcologyEmissionFactor(
  scenarioConfig: ScenarioConfig | null | undefined
): ResolvedEcologyEmissionFactor {
...
export function syncEcologyEmissionFactorOnEnergySourceChange(
  ecology: NonNullable<ScenarioConfig["ecology"]>,
  previousEnergySource: string | null | undefined,
  nextEnergySource: string | null | undefined
): void {
  ...
  const keepUserOverride =
    isExplicit && (previousNorm == null || current == null || current !== previousNorm);
  if (keepUserOverride) {
    return;
  }
