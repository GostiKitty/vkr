...
export interface FilletResult {
  /** Эффективный радиус после ограничения доступной длиной рёбер (м). */
  radius: number;
  /** Вынос точки касания вдоль каждого ребра от вершины (м). */
  trim: number;
  /** Точка касания на первом ребре (направление dirA). */
  tangentA: Vec2;
  /** Точка касания на втором ребре (направление dirB). */
  tangentB: Vec2;
  /** Центр дуги скругления. */
  center: Vec2;
  /** Угол дуги (рад) — внешний угол вершины, π − внутренний. */
  turnAngle: number;
  /** Угол точки tangentA относительно центра (рад). */
  startAngle: number;
  /** Знаковый размах дуги от tangentA к tangentB вокруг центра (рад). */
  signedSweep: number;
}
 ...
 */
export function filletFromDirections(
  corner: Vec2,
  dirA: Vec2,
  dirB: Vec2,
  radius: number,
  maxTrim = Number.POSITIVE_INFINITY
): FilletResult | null {
  if (!(radius > 0)) {
    return null;
  }
 ...
 */
export function roundedContourPoints(
  polygon: Vec2[],
  radiusForIndex: (index: number) => number,
  segmentsPerArc = 10
): Vec2[] {
