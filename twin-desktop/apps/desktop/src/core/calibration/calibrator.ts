...
export function calibrateParameters(config: CalibrationConfig): CalibrationResult {
...
function sampleParameters(parameters: CalibrationParameter[], rng: DeterministicRng): Record<string, number> {
...
function validatePrediction(prediction: number[], expectedLength: number): void {
...
function computeRmse(prediction: number[], observations: number[]): number {
...
function computeMape(prediction: number[], observations: number[]): number {
  const eps = 1e-6;
