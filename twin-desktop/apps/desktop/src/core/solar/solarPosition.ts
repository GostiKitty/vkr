...
export interface SolarPosition {
  /** Высота солнца над горизонтом, °. Отрицательная → солнце за горизонтом. */
  altitudeDeg: number;
  /** Азимут от севера по часовой стрелке, ° [0..360). */
  azimuthDeg: number;
  /** true, если солнце выше горизонта. */
  isAboveHorizon: boolean;
  /**
   * Нормированный вектор положения источника света в системе THREE.js.
   * Используется для THREE.DirectionalLight.position.set(lightX, lightY, lightZ).
   */
  lightX: number;
  lightY: number;
  lightZ: number;
...
export function formatSolarHour(hourDecimal: number): string {
