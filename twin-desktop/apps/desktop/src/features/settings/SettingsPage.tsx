...
export function SettingsPage() {
  ...
  return (
    <section className="space-y-6 rounded-3xl border border-slate-200 bg-white/70 p-6 shadow-sm">
      ...
      <div className="space-y-4 rounded-3xl border border-slate-100 bg-white/80 p-5">
        <label className="flex flex-col gap-2 text-sm font-medium text-slate-600">
          ...
          <input
            ...
            className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-base text-slate-900 shadow-inner focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-200"
          />
        </label>
        <div className="flex flex-wrap gap-3">
          ...
          <button
            ...
            onClick={handleTest}
            disabled={testing}
            className="rounded-full border border-emerald-300 px-5 py-2 text-sm font-semibold text-emerald-700 hover:border-emerald-500 disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        {testResult === "error" && (
          <p className="text-sm text-red-600">Не удалось подключиться. Проверьте адрес, порт или состояние сервера.</p>
        )}
      </div>
