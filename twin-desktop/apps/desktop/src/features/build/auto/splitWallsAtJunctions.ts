...
function isEndpoint(point: Vec2, endpoint: Vec2, tolerance = JUNCTION_TOLERANCE): boolean {
  return segmentLength(point, endpoint) <= tolerance;
}

function pointOnInterior(point: Vec2, start: Vec2, end: Vec2): boolean {
...
function splitWallAtPoints(wall: Wall, points: Vec2[]): Wall[] {
  const ordered = uniquePoints(points, JUNCTION_TOLERANCE).sort(
    (left, right) => distanceAlong(wall.a, left) - distanceAlong(wall.a, right)
  );
  if (!ordered.length) {
    return [wall];
  }
 ...
 */
export function splitWallsAtJunctions(walls: Wall[]): Wall[] {
