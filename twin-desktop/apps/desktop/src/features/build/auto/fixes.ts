...
export function autoCloseRooms(rooms: Room[], updateRoom: (id: string, patch: Partial<Room>) => void): number {
  let patched = 0;
  rooms.forEach((room) => {
...
export function mergeColinearWalls(walls: Wall[], setWalls: (walls: Wall[]) => void): number {
  let changes = 0;
...
export function removeTinySegments(
  model: BuildingModel,
...
const canMergeWalls = (a: Wall, b: Wall): boolean => {
  ...
  return intervalsTouchOrOverlap(
    projection.start,
    projection.end,
    otherProjection.start,
    otherProjection.end,
    WALL_MERGE_TOLERANCE
  );
};

const haveCompatibleWallProps = (a: Wall, b: Wall): boolean =>
  Math.abs(a.thickness_m - b.thickness_m) <= 0.02 &&
...
const areColinear = (a: Wall, b: Wall): boolean => {
...
const mergeWalls = (a: Wall, b: Wall): Wall => {
  const projection = projectWall(a);
  const otherProjection = projectWallOnAxis(b, projection.axis);
...
const projectWallOnAxis = (wall: Wall, axis: Vec2) => ({
  start: Math.min(dot(axis, wall.a), dot(axis, wall.b)),
  end: Math.max(dot(axis, wall.a), dot(axis, wall.b)),
});

const intervalsTouchOrOverlap = (aStart: number, aEnd: number, bStart: number, bEnd: number, tolerance: number): boolean =>
  Math.min(aEnd, bEnd) >= Math.max(aStart, bStart) - tolerance;

const normalize = (vector: Vec2): Vec2 | null => {
...
const scale = (vector: Vec2, factor: number): Vec2 => ({ x: vector.x * factor, y: vector.y * factor });

const dot = (left: Vec2, right: Vec2): number => left.x * right.x + left.y * right.y;
