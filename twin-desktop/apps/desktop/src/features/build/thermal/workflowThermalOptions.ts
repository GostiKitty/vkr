...
import {
  buildResolvedPressureBasedConfig,
  resolveScenarioVentilationInputs,
} from "../../../core/thermal/ventilation/resolveScenarioVentilation";
import type { BuildingModel } from "../../../entities/geometry/types";
import { resolveScenarioConfig, type ScenarioConfig } from "../../../entities/workflow/workflow.store";
import { DEFAULT_THERMAL_OPTIONS } from "./defaultThermalOptions";

export function cloneThermalOptions(options: ThermalSimulationOptions): ThermalSimulationOptions {
  return {
    ...options,
...
export function buildThermalOptionsFromWorkflow(
  scenario: ScenarioConfig | null,
  base: ThermalSimulationOptions = DEFAULT_THERMAL_OPTIONS,
  model: BuildingModel | null = null,
  ...
  const resolvedLeakage =
    model != null ? resolveScenarioEnvelopeLeakageInputs(scenario, resolved, model) : null;
  const resolvedVentilation =
    model != null ? resolveScenarioVentilationInputs(scenario, resolved, model, options) : null;
  ...
  next.heatRecoveryFactor =
    resolvedVentilation?.heatRecoveryFactor.value ?? resolved.ventilation.heatRecoveryFactor ?? undefined;
  next.performanceOptions = buildPerformanceOptionsFromScenario(scenario, resolved, model);
