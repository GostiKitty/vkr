...
function buildModelThermalKey(model: BuildingModel): string {
...
function buildFrameKey(frame: ThermalTimelinePoint | null): string {
  if (!frame) {
    return "preview";
  }
...
export function usePreparedThermalAnalysis(
  model: BuildingModel,
  fieldOptions: Omit<ThermalFieldBuildOptions, "roomTemperaturesC">,
  frame: ThermalTimelinePoint | null,
  enabled: boolean
) {
  const [result, setResult] = useState<PreparedThermalAnalysis | null>(null);
  const [pending, setPending] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [cacheSize, setCacheSize] = useState(0);
  const workerRef = useRef<Worker | null>(null);
  const requestIdRef = useRef(0);
  const cacheRef = useRef(new Map<string, PreparedThermalAnalysis>());
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent<ThermalWorkerResponse>) => {
    ...
    const payload: ThermalWorkerRequest = {
      requestId,
      model,
      fieldOptions,
      frame,
