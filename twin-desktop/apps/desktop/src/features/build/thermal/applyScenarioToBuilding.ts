...
export function applyScenarioToBuilding(model: BuildingModel, scenario: ScenarioConfig | null): BuildingModel {
  if (!scenario) {
    return model;
  }
  ...
  const climate = {
    ...
    outdoorHeatingPeriodAverageC:
      sp131?.outdoorHeatingPeriodAverageC ?? existing.climate?.outdoorHeatingPeriodAverageC,
    heatingPeriodDurationDays:
      sp131?.heatingPeriodDurationDays ?? existing.climate?.heatingPeriodDurationDays,
    outdoorDesignTemperatureC:
      sp131?.outdoorDesignTemperatureC ?? existing.climate?.outdoorDesignTemperatureC,
  };
  const withOptical =
    scenario.materials?.windowGValue != null || scenario.materials?.shadingFactor != null
