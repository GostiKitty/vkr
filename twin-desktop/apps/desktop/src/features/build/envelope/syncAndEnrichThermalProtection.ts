...
export function syncAndEnrichThermalProtection(model: BuildingModel): BuildingModel {
  const hasEnvelopeGeometry =
    model.walls.length > 0 || (model.roofs?.length ?? 0) > 0 || (model.floorSlabs?.length ?? 0) > 0;
