export interface StablePreviewBounds {
...
export interface StableFitCameraFrame {
...
export interface StableGridLayout {
  size: number;
  divisions: number;
  position: { x: number; y: number; z: number };
}
...
export function normalizeStablePreviewBounds(
  center: { x: number; y: number; z: number },
  size: { x: number; y: number; z: number },
  options?: {
    minPlanDimM?: number;
...
export function calculateFitCameraForBounds(
  bounds: StablePreviewBounds,
  aspectRatio: number,
  fovDeg: number,
  fitPadding = 1.55
): StableFitCameraFrame {
...
export function calculateTopViewCameraForBounds(
  bounds: StablePreviewBounds,
  aspectRatio: number,
  fovDeg: number,
  fitPadding = 1.45
  ...
  const safeAspect = Number.isFinite(aspectRatio) ? Math.max(aspectRatio, 0.5) : 1;
  const halfFovRad = (Math.max(fovDeg, 10) * Math.PI) / 360;
  const planDistance = Math.max(
    bounds.size.x / Math.max(2 * Math.tan(halfFovRad) * safeAspect, 0.1),
    bounds.size.z / Math.max(2 * Math.tan(halfFovRad), 0.1),
    bounds.maxDim * 0.6
  ) * fitPadding;
  return {
    target: bounds.center,
...
export function calculateGridLayoutForBounds(bounds: StablePreviewBounds): StableGridLayout {
