...
import type { Equipment, EquipmentType, PipeSystemType } from "../../../entities/networks/types";
...
export interface Build3DStablePreviewHandle {
...
interface Build3DStablePreviewProps {
  model: BuildingModel;
  tool: BuildTool;
...
export interface StablePreviewVisibilityStats {
  roomCount: number;
  wallCount: number;
  openingCount: number;
  roofCount: number;
...
function createBox3(): THREE.Box3 {
...
function applyCameraFrame(camera: THREE.PerspectiveCamera, controls: OrbitControls, frame: { position: { x: number; y: number; z: number }; target: { x: number; y: number; z: number }; near: number; far: number }) {
...
function buildSimpleEquipment(item: Equipment, levelElevation: number): THREE.Object3D {
...
function getStableFitRoot(shellRoot: THREE.Group | null, contentRoot: THREE.Group | null): THREE.Object3D | null {
...
export function getStablePreviewVisibilityStats(model: BuildingModel, activeLevelId: string | null): StablePreviewVisibilityStats {
  ...
  const matchLevel = <T extends { levelId: string }>(items: T[]) => (activeLevelId ? items.filter((item) => item.levelId === activeLevelId) : items);
  return {
...
export function resolveStablePreviewLevelId(model: BuildingModel, activeLevelId: string | null): string | null {
  if (!activeLevelId) {
    return null;
  }
...
function buildOpeningPoint(wallsById: Map<string, Wall>, opening: BuildingModel["doors"][number] | BuildingModel["windows"][number]) {
...
export const Build3DStablePreview = React.forwardRef<Build3DStablePreviewHandle, Build3DStablePreviewProps>(
  ({ model, activeLevelId, selection, viewer }, forwardedRef) => {
    ...
    const currentSelectionRef = useRef<Selection | null>(selection);
    ...
    const logStableFit = (...args: unknown[]) => {
      if (DEBUG_STABLE_3D_FIT) {
        console.info("[stable-3d-fit]", ...args);
      }
    };

    const fitToShell = (reason: "initial" | "model-change" | "manual-focus" | "reset" | "top-view", options?: { topView?: boolean; updateHome?: boolean }) => {
    ...
    const scheduleInitialFit = (reason: "initial" | "model-change") => {
    ...
    useEffect(() => {
      ...
      gridMaterials.forEach((material) => {
        (material as THREE.Material & { transparent?: boolean; opacity?: number }).transparent = true;
        (material as THREE.Material & { transparent?: boolean; opacity?: number }).opacity = 0.38;
      });
      scene.add(grid);
      ...
      if (viewer.showWalls) {
        renderGeometry.walls.forEach(({ wall }) => {
          ...
          addBoxBetween(
            ...
            wall.thickness_m,
            wall.height_m,
            levelElevation + wall.height_m / 2,
            visibleSelection?.kind === "wall" && visibleSelection.id === wall.id ? HIGHLIGHT_MATERIAL : wallMaterial
          );
        });
      }
      ...
      if (viewer.showOpenings) {
        model.doors.forEach((door) => {
          ...
          const mesh = new THREE.Mesh(
            new THREE.BoxGeometry(door.width_m, door.height_m, Math.max(data.wall.thickness_m * 0.55, 0.08)),
            visibleSelection?.kind === "door" && visibleSelection.id === door.id ? HIGHLIGHT_MATERIAL : OPENING_MATERIAL
          );
        ...
        model.windows.forEach((windowItem) => {
          ...
          const mesh = new THREE.Mesh(
            new THREE.BoxGeometry(windowItem.width_m, windowItem.height_m, Math.max(data.wall.thickness_m * 0.4, 0.06)),
            visibleSelection?.kind === "window" && visibleSelection.id === windowItem.id ? HIGHLIGHT_MATERIAL : OPENING_MATERIAL
          );
          mesh.rotation.y = data.angle;
    ...
    return <canvas ref={canvasRef} className="block h-full w-full touch-none overscroll-contain" />;
  }
);
