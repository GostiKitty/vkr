...
import {
  getSurfaceFieldPatchValue,
  getSurfaceFieldValueRange,
  SURFACE_FIELD_MODE_LABELS,
} from "../../../core/thermal/surfaceField";
import { thermalColor } from "./thermalColormap";
import type { BuildSceneHoverInfo } from "./sceneContracts";

const SURFACE_KIND_LABELS: Record<SurfaceFieldPatch["surfaceKind"], string> = {
...
export interface BuildSurfaceFieldOverlayOptions {
  ...
  xRay?: boolean;
  /** Surface patch opacity override (0–1). Defaults to 0.52. */
  overlayOpacity?: number;
}

export function buildSurfaceFieldOverlayGroup(
  result: SurfaceFieldResult,
  options: BuildSurfaceFieldOverlayOptions
): THREE.Group {
  ...
  if (options.showSurfaceField && range) {
    const patchesBySurfaceId = new Map<string, SurfaceFieldPatch[]>();
    ...
    patchesBySurfaceId.forEach((patches, surfaceId) => {
      ...
      const colors: number[] = [];
      const patchIdsByCellIndex: string[] = [];
  ...
  if (options.showHeatSources) {
    ...
    result.heatSources.forEach((source) => {
      const color = twoStopColor(
        clamp(source.totalPowerW / 1800, 0, 1),
        0xf59e0b,
        0xdc2626,
      );
      if (source.segment) {
      ...
      const mesh = new THREE.Mesh(
        new THREE.SphereGeometry(source.kind === "radiator" ? 0.09 : 0.06, 14, 14),
...
export function buildSurfaceFieldHoverInfo(args: {
  ...
  if (category === "surface-heat-source" || category === "surface-thermal-bridge") {
    return {
      title: (userData.title as string | undefined) ?? "Диагностика поверхности",
      subtitle: undefined,
      details: (userData.details as Array<{ label: string; value: string }> | undefined) ?? [],
      screenX: args.screenX,
      screenY: args.screenY,
    };
  }
...
function buildCornerValuesForMode(
  ...
  if (mode === "condensationRisk") {
    return patch.cornerTemperaturesC.map((value) => clamp((-((value - patch.patchTemperatureC) + patch.condensationMarginC) + 1.5) / 4, 0, 1)) as [
      number,
      number,
      number,
      number,
...
function twoStopColor(ratio: number, cold: number, hot: number): THREE.Color {
...
function formatModeValue(mode: SurfaceFieldRenderMode, value: number) {
  switch (mode) {
