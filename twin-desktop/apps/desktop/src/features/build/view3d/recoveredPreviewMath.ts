...
export interface RecoveredPreviewBounds {
  minX: number;
  maxX: number;
  minY: number;
  maxY: number;
...
export interface RecoveredPreviewCameraFrame {
...
export function createEmptyRecoveredBounds(): RecoveredPreviewBounds {
  return {
...
export function extendRecoveredBounds(bounds: RecoveredPreviewBounds, x: number, y: number, z: number) {
...
export function extendRecoveredBoundsWithPolygon(
...
export function finalizeRecoveredBounds(bounds: RecoveredPreviewBounds): RecoveredPreviewBounds {
  if (bounds.empty) {
    return bounds;
  }
  const rawSize = {
...
export function calculateRecoveredCameraFrame(
  bounds: RecoveredPreviewBounds,
  mode: "focus" | "top" = "focus"
): RecoveredPreviewCameraFrame {
  if (bounds.empty) {
    return {
...
export function calculateRecoveredGrid(bounds: RecoveredPreviewBounds) {
  const maxDim = Math.max(bounds.size.x, bounds.size.y, bounds.size.z, MIN_DIMENSION_M);
  return {
    size: maxDim * 2,
