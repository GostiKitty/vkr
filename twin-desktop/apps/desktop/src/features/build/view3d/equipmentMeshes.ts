...
import type { Equipment, EquipmentType, SensorDevice } from "../../../entities/networks/types";
...
const createMaterial = (color: number, profile: { roughness: number; metalness: number }, selected: boolean) =>
...
const addFeet = (group: THREE.Group, xOffset: number, zOffset: number, material: THREE.Material, y = 0.05) => {
...
export const getEquipmentBaseY = (type: EquipmentType | string, levelHeight: number): number => {
  if (type === "diffuser") {
...
const createRadiatorMesh = (selected: boolean): THREE.Group => {
...
const createBoilerMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createPumpMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createAhuMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createDiffuserMesh = (selected: boolean, levelHeight: number): THREE.Group => {
  const group = new THREE.Group();
  const shell = createMaterial(0xf8fafc, PAINT, selected);
...
const createFanCoilMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createHeatExchangerMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
  const shell = createMaterial(0xe2e8f0, PAINT, selected);
...
const createElevatorMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createExpansionTankMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createDirtSeparatorMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
const createGenericEquipmentMesh = (selected: boolean): THREE.Group => {
  const group = new THREE.Group();
...
export const createEquipmentVisual = (item: Equipment, options: VisualOptions): THREE.Group => {
...
export const createSensorVisual = (sensor: SensorDevice, options: VisualOptions): THREE.Group => {
  const group = new THREE.Group();
...
export const getEquipmentWorldConnectionPoint = (
  item: Equipment,
...
export const getSensorWorldPosition = (sensor: SensorDevice, levelElevation: number, levelHeight: number): THREE.Vector3 =>
  new THREE.Vector3(sensor.position.x, levelElevation + Math.max(1.58, levelHeight - 0.62), sensor.position.y);
