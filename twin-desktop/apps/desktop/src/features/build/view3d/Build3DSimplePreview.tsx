...
import {
  buildSimple3DModel,
  type Simple3DModel,
  type SimpleOpeningModel,
  type SimpleTemperatureSurface,
} from "./buildSimple3DModel";
import { calculateSimplePreviewCamera, calculateSimplePreviewGrid } from "./simplePreviewMath";

export interface Build3DSimplePreviewHandle {
...
interface Build3DSimplePreviewProps {
  model: BuildingModel;
...
function addCylinderBetween(root: THREE.Object3D, start: THREE.Vector3, end: THREE.Vector3, radius: number, material: THREE.Material) {
...
function getSelectionMaterial(selection: Selection | null, kind: Exclude<NonNullable<Selection>["kind"], "loop">, id: string, material: THREE.Material) {
...
function createTemperatureMaterial(surface: SimpleTemperatureSurface, summary: Simple3DModel["temperatureSummary"]) {
...
function createOpeningMesh(opening: SimpleOpeningModel, selection: Selection | null) {
  const baseMaterial =
    opening.type === "window"
...
export const Build3DSimplePreview = React.forwardRef<Build3DSimplePreviewHandle, Build3DSimplePreviewProps>(
  (
    {
      model,
      activeLevelId,
    ...
    const homeModeRef = useRef<"iso" | "top">("iso");
    ...
    const simpleModel = useMemo(
      () =>
        buildSimple3DModel(model, activeLevelId, {
          showNetworks,
          showEquipment,
          showTemperature,
          showWallTemperature,
    ...
    const applyCameraMode = (mode: "iso" | "top", setHome = true) => {
      ...
      (camera as THREE.PerspectiveCamera & { near: number; far: number }).near = frame.near;
      (camera as THREE.PerspectiveCamera & { near: number; far: number }).far = frame.far;
      ...
      simpleModel.simpleTemperatureSurfaces.forEach((surface) => {
        ...
        if (surface.sourceType === "wall" && surface.wall) {
          addBoxBetween(
            ...
            surface.wall.start,
            surface.wall.end,
            Math.max(Math.min(surface.wall.thickness_m * 0.16, 0.07), 0.03),
            Math.max(surface.wall.height_m - 0.05, 0.4),
            surface.wall.elevation_m + surface.wall.height_m * 0.5,
            material
          );
          return;
        }
    ...
    return (
      <div className="relative h-full w-full overflow-hidden touch-none">
        ...
        {simpleModel.temperatureSummary && simpleModel.simpleTemperatureSurfaces.length > 0 ? (
          <div className="pointer-events-none absolute bottom-3 left-3 z-10">
            <div className="ui-overlay max-w-[15rem] px-3 py-2.5">
              <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">Температура</p>
        ...
        {showTemperatureWarning ? (
          <div className="pointer-events-none absolute bottom-3 left-3 z-10">
            <div className="ui-overlay max-w-[16rem] px-3 py-2.5 text-xs text-slate-600">Нет температурных данных.</div>
          </div>
        ) : null}
      </div>
    );
