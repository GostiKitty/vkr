...
import type { Sp50ComplianceReport, Sp50ConstructionCheck } from "../../../core/thermal/sp50/types";
...
import type { ReportMeta } from "./reportMetaPersistence";
import { DEFAULT_REPORT_META } from "./reportMetaPersistence";

export interface SpReportPdfTable {
  title: string;
  headers: string[];
  rows: string[][];
}

export interface SpReportPdfSection {
  number?: string;
  title: string;
  paragraphs: string[];
  bullets?: string[];
  tables?: SpReportPdfTable[];
}

export interface SpReportPdfData {
  title: string;
  ...
  sections: SpReportPdfSection[];
  appendixSections: SpReportPdfSection[];
  limitations: string[];
}

export interface BuildExpertiseReportInput {
...
interface BuildSpReportPdfDataInput {
  ...
  transientResult?: TransientCalculationResult | null;
  transientWarnings?: string[];
  engineeringConclusion?: string | null;
  thermalResult?: ThermalSimulationResult | null;
  meta?: Partial<ReportMeta>;
}
...
export function buildExpertiseThermalReportData(input: BuildExpertiseReportInput): SpReportPdfData {
  const generatedAt = input.calculationTimestampIso ?? new Date().toISOString();
  const projectName = sanitizeVisibleText(
    input.projectName ?? input.model?.meta?.name ?? input.model?.meta?.sourceProjectName,
    "Текущий проект"
  );
  ...
  const sections: SpReportPdfSection[] = [
    ...
    buildConclusionsSection(
      report,
      input.thermalResult ?? null,
      input.engineering?.presentation?.summaryLines?.[0] ?? null
    ),
    buildMissingDataSection(report),
  ];

  const appendixSections: SpReportPdfSection[] = [
  ...
  return {
    ...
    documentSubtitle:
      "Расчётный документ сформирован в структуре текстовой части проектной документации и предназначен для представления расчётно-пояснительных материалов по тепловой защите здания.",
    filename,
    generatedAt,
    projectName,
    meta,
...
export function buildSpReportPdfData(input: BuildSpReportPdfDataInput): SpReportPdfData {
...
export function exportSpReportToPdf(data: SpReportPdfData): boolean {
...
export function buildSpReportHtml(data: SpReportPdfData): string {
  ...
  const cityLine = escapeHtml(
    sanitizeVisibleText(meta.documentCity, meta.buildingAddress ? (meta.buildingAddress.split(",")[0] ?? "") : "")
  );
...
function buildIntroductionSection(projectName: string, meta: ReportMeta): SpReportPdfSection {
...
function buildNormativeBasisSection(): SpReportPdfSection {
  return {
    number: "4",
...
function buildSourceDataSection(
  report: Sp50ComplianceReport,
  projectName: string,
  generatedAt: string,
  scenarioLabel: string | null | undefined,
  climateBaseLabel: string | null | undefined
): SpReportPdfSection {
...
function buildBuildingSummarySection(report: Sp50ComplianceReport, projectName: string): SpReportPdfSection {
  const source = report.sourceData;
...
function buildClimateOperationSection(
  report: Sp50ComplianceReport,
  scenarioLabel: string | null | undefined,
  climateBaseLabel: string | null | undefined
): SpReportPdfSection {
...
function buildMethodologySection(): SpReportPdfSection {
  return {
    number: "6",
...
function buildConstructionsSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildResistanceCheckSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildKobSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildTemperatureSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildAirPermeabilitySection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildMoistureSection(report: Sp50ComplianceReport): SpReportPdfSection {
  const moisture = report.moistureProtection;
  const statusText =
    moisture.status === "calculated"
...
function buildFloorSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildThermalInertiaSection(
  report: Sp50ComplianceReport,
  transientResult: TransientCalculationResult | null | undefined,
  transientWarnings: string[] | undefined
): SpReportPdfSection {
  ...
  if (transientResult?.valid && transientResult.stable) {
    paragraphs.push(
      `Дополнительно выполнен нестационарный 1D-расчёт конструкции: t_si,min = ${safeTemperatureText(transientResult.minInnerSurfaceTemperature)}, t_si,max = ${safeTemperatureText(transientResult.maxInnerSurfaceTemperature)}.`
    );
...
function buildEnergySection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildConclusionsSection(
  report: Sp50ComplianceReport,
  thermalResult: ThermalSimulationResult | null,
  engineeringHeadline: string | null
): SpReportPdfSection {
  const diagnostics = buildConstructionDiagnostics(report);
...
function buildMissingDataSection(report: Sp50ComplianceReport): SpReportPdfSection {
...
function buildEnergyPassportAppendix(
  report: Sp50ComplianceReport,
  meta: ReportMeta,
  projectName: string,
  generatedAt: string
...
function buildEnvelopeAppendixSection(report: Sp50ComplianceReport): SpReportPdfSection {
  const diagnostics = buildConstructionDiagnostics(report);
...
function buildRcAppendixSection(
  report: Sp50ComplianceReport,
...
function renderSectionHtml(section: SpReportPdfSection, options: RenderSectionOptions): string {
...
function renderTableHtml(table: SpReportPdfTable, tableCounter: { value: number }): string {
...
function buildTableOfContentsHtml(sections: SpReportPdfSection[]): string {
  return sections
    .map((section, index) => {
...
function buildDocumentDesignation(meta: ReportMeta): string {
...
function resolveStageShortLabel(stage: string): string {
...
function buildGostStampHtml(input: GostStampInput): string {
...
function downloadReportHtml(html: string, filename: string) {
  const blob = new Blob([html], { type: "text/html;charset=utf-8" });
...
function hasSectionContent(section: SpReportPdfSection): boolean {
  return Boolean(section.paragraphs.length || section.bullets?.length || section.tables?.length);
}

function resolveOverallStatusLabel(report: Sp50ComplianceReport): string {
  const statuses = [
...
function aggregateConstructionStatus(report: Sp50ComplianceReport): "pass" | "fail" | "insufficient_data" {
...
function resolveMargin(entry: Sp50ConstructionCheck): number | null {
  ...
  if (Number.isFinite(entry.actualResistance_m2K_W) && Number.isFinite(entry.requiredResistance_m2K_W)) {
    return (entry.actualResistance_m2K_W as number) - (entry.requiredResistance_m2K_W as number);
  }
  return null;
}

function collectConstructionGroupLabels(constructions: Sp50ConstructionCheck[]): string[] {
  return Array.from(
    new Set(
      constructions
...
function buildConstructionDiagnostics(report: Sp50ComplianceReport) {
  const source = report.sourceData;
  return [...report.constructions]
    .map((entry) => {
      ...
      const suspiciousSurfaceTemperature = hasSuspiciousSurfaceTemperature(
        entry,
        source.indoorTemperatureC,
        source.outdoorDesignTemperatureC
      );
...
function buildConstructionGroupSummary(report: Sp50ComplianceReport) {
  const diagnostics = buildConstructionDiagnostics(report);
  const groups = new Map<
    string,
    {
      ...
      count: number;
      area: number;
      areaForActual: number;
      areaForRequired: number;
      areaForU: number;
  ...
  diagnostics.forEach((item) => {
    ...
    if (Number.isFinite(item.entry.actualResistance_m2K_W)) {
      bucket.actualResistance += (item.entry.actualResistance_m2K_W as number) * area;
      bucket.areaForActual += area;
    }
    if (Number.isFinite(item.entry.requiredResistance_m2K_W)) {
      bucket.requiredResistance += (item.entry.requiredResistance_m2K_W as number) * area;
      bucket.areaForRequired += area;
    }
    if (Number.isFinite(item.entry.heatTransferCoefficient_W_m2K)) {
      bucket.uValue += (item.entry.heatTransferCoefficient_W_m2K as number) * area;
...
function buildResistanceConclusionText(
  report: Sp50ComplianceReport,
  diagnostics: ReturnType<typeof buildConstructionDiagnostics>
) {
...
function buildEnergyConclusionText(report: Sp50ComplianceReport) {
...
function buildDynamicConclusionText(
  report: Sp50ComplianceReport,
  thermalResult: ThermalSimulationResult | null
) {
  if (!thermalResult) {
...
function buildMissingDataConclusionText(missingData: string[]) {
...
function collectHumanizedMessages(report: Sp50ComplianceReport) {
...
function hasVentilationPlaceholderWarnings(warnings: string[]) {
  const text = warnings.join(" ").toLowerCase();
  return (
    text.includes("l_vent") ||
    text.includes("lvent") ||
    text.includes("g_inf") ||
    text.includes("ginf") ||
...
function humanizeReportMessage(message: string | null | undefined) {
...
function resolveConstructionGroupKey(type: string) {
  switch (type) {
    case "covering":
    case "roof":
      return "roof";
...
function resolveConstructionTypeLabel(type: string) {
  switch (type) {
    case "wall":
...
function hasClassificationConflict(entry: Sp50ConstructionCheck) {
  const openingType =
    entry.constructionType === "window" ||
...
function hasSuspiciousSurfaceTemperature(
  entry: Sp50ConstructionCheck,
  indoorTemperature: number | null,
  outdoorTemperature: number | null
) {
  if (
    !Number.isFinite(entry.internalSurfaceTemperatureC) ||
    !Number.isFinite(indoorTemperature) ||
    !Number.isFinite(outdoorTemperature)
  ) {
    return false;
  }
  const tSi = entry.internalSurfaceTemperatureC as number;
  const indoor = indoorTemperature as number;
  const outdoor = outdoorTemperature as number;
...
function mapConstructionStatus(status: "pass" | "fail" | "insufficient_data") {
  switch (status) {
    case "pass":
...
function mapZoneDiagnosticStatus(status: string) {
  switch (status) {
    case "risk":
      return "риск";
    case "attention":
...
function isDynamicComfortAcceptable(result: ThermalSimulationResult) {
...
function buildSpConclusion(report: Sp50ComplianceReport): string {
...
function sumAreaByType(constructions: Sp50ConstructionCheck[], types: string[]): number {
...
function formatBuildingCategory(value: string | null | undefined): string {
  switch (value) {
    case "residential":
      return "Жилое";
    case "public":
...
function safeNumber(value: number | null | undefined): number {
  if (!Number.isFinite(value) || Math.abs(value as number) > SAFE_NUMBER_LIMIT) {
    return 0;
  }
  return value as number;
}

function isSafeTemperature(value: number | null | undefined): boolean {
  return Number.isFinite(value) && Math.abs(value as number) <= SAFE_TEMPERATURE_LIMIT_C;
}
...
function safeArea(value: number | null | undefined): string {
  return Number.isFinite(value) ? formatArea(value as number) : NO_DATA_TEXT;
}

function safeVolume(value: number | null | undefined): string {
  return Number.isFinite(value) ? formatVolume(value as number) : NO_DATA_TEXT;
}

function safeInteger(value: number | null | undefined): string {
  return Number.isFinite(value) ? String(Math.round(value as number)) : NO_DATA_TEXT;
}

function safeDays(value: number | null | undefined): string {
  return Number.isFinite(value) ? `${Math.round(value as number)}` : NO_DATA_TEXT;
}

function safeTemperatureText(value: number | null | undefined, unit = ""): string {
  if (!isSafeTemperature(value)) {
    return NO_DATA_TEXT;
  }
  const formatted = formatNumber(value as number, { maximumFractionDigits: 1 });
  return unit ? `${formatted} ${unit}` : formatted;
}

function safeNumberText(value: number | null | undefined, unit: string): string {
  if (!Number.isFinite(value) || Math.abs(value as number) > SAFE_NUMBER_LIMIT) {
    return NO_DATA_TEXT;
  }
  const formatted = formatNumber(value as number, { maximumFractionDigits: 2 });
  return unit ? `${formatted} ${unit}` : formatted;
}
...
function formatSp50StatusText(value: string): string {
  switch (value) {
    case "pass":
      return "соответствует";
    case "fail":
...
function sanitizeParagraph(value: unknown): string {
