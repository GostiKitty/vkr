...
import type { DuctNetwork, PipeNetwork } from "../../../entities/networks/types";
...
export function validateEngineeringModel(model: BuildingModel, snapshot: SmartModelSnapshot): BuildIssue[] {
  const issues: BuildIssue[] = [];
  const heatingModel = buildHeatingModelSnapshot(model);
  const networksById = new Map<string, PipeNetwork | DuctNetwork>([
    ...model.pipes.map((pipe) => [pipe.id, pipe] as const),
    ...model.ducts.map((duct) => [duct.id, duct] as const),
  ]);
