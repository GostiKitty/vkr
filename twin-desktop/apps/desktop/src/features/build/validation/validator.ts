...
import type { BuildIssue } from "./issues";
import { anchorToOffset } from "../utils/openingMath";

const MIN_WALL_LENGTH = 0.15;

export function validateModel(model: BuildingModel): BuildIssue[] {
  ...
  const openingsByWall = new Map<string, { start: number; end: number; id: string }[]>();
...
const detectOverlappingRooms = (rooms: Room[], issues: BuildIssue[]) => {
...
const segmentsShareEndpoint = (a: Wall, b: Wall): boolean => {
  const equals = (p: Vec2, q: Vec2) => Math.hypot(p.x - q.x, p.y - q.y) < 1e-3;
  return equals(a.a, b.a) || equals(a.a, b.b) || equals(a.b, b.a) || equals(a.b, b.b);
};

const wallSegmentsOverlap = (a: Wall, b: Wall): boolean => {
...
const areColinearWalls = (a: Wall, b: Wall): boolean => {
  const axis = normalize({ x: a.b.x - a.a.x, y: a.b.y - a.a.y });
  if (!axis) {
    return false;
  }
...
const isSelfIntersecting = (points: Vec2[]): boolean => {
  if (points.length < 4) {
    return false;
  }
