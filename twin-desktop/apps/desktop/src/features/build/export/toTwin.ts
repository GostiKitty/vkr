...
export interface TwinExportOptions {
  projectName?: string;
  defaultHeight?: number;
  projectId?: string | null;
}

export function buildModelToTwin(model: BuildingModel, options: TwinExportOptions = {}): Twin {
  ...
  const spaces: Space[] = model.rooms.map((room) => {
  ...
  const projectName =
    options.projectName ||
    (model.meta && typeof model.meta["name"] === "string" ? (model.meta["name"] as string) : "Ручной проект");
...
const estimateDefaultHeight = (levels: Level[]): number => {
