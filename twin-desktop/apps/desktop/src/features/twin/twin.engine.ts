...
import {
  estimateExteriorEnvelopeAreaM2,
  estimateSharedWallAreaM2,
  twinInternalConductanceWPerK,
  twinNodeCapacitanceJPerK,
  twinOutdoorConductanceWPerK,
} from "../../core/thermal/twinGraphHeuristics";
...
export function buildSpaceInstances(spaces: Space[]): SpaceInstance[] {
  ...
  const levelMap = new Map<string, Space[]>();
  ...
  for (const levelKey of sortedLevelKeys) {
    ...
    for (const space of levelSpaces) {
      ...
      if (h > maxFloorHeight) maxFloorHeight = h;
    }
...
export function buildSpaceInstancesFromModel(model: BuildingModel): SpaceInstance[] {
...
function extractFloorNumber(label: string): number | null {
  const match = label.match(/-?\d+/);
  return match ? parseInt(match[0], 10) : null;
}

function compareFloorLabels(a: string, b: string): number {
