...
import type { MetricInfoDefinition } from "../reports/resultsMetricInfo";

function formulaInfoFromRegistry(
  primaryId: string,
  options?: {
    relatedIds?: string[];
    calculatedIn?: string;
...
): MetricInfoDefinition | undefined {
...
const SOURCE_FIELD_FORMULA_INFO: Record<string, MetricInfoDefinition> = {
...
const SUFFIX_FORMULA_IDS: Array<{ test: (key: string) => boolean; primaryId: string; relatedIds?: string[] }> = [
...
export function getSourceFieldFormulaInfo(fieldKey: string): MetricInfoDefinition | undefined {
