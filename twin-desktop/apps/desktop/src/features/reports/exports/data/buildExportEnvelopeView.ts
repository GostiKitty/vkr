...
import type { Sp50ConstructionCheck } from "../../../../core/thermal/sp50/types";
import type {
  BuildingModel,
  Door,
  FloorSlab,
...
export type ExportEnvelopeCategory =
  | "external-wall"
  | "window"
  | "external-door"
  | "roof"
...
export type ExportEnvelopeClassification =
  | "included"
  | "excluded-internal"
  | "requires-review";

export interface ExportEnvelopeElement {
  key: string;
  modelId: string;
  designation: string;
  name: string;
  ...
  requiredResistance: number | null;
  normalizedResistance: number | null;
  reducedResistance: number | null;
  uValue: number | null;
  nt: number | null;
  contribution: number | null;
  status: string;
  classification: ExportEnvelopeClassification;
  classificationNote: string;
  includeInEnvelope: boolean;
  category: ExportEnvelopeCategory;
  internalSurfaceTemperature: string;
}

export interface ExportEnvelopeGroup {
  key: string;
  typeLabel: string;
  count: number;
  areaM2: number;
...
export interface ExportEnvelopeView {
  includedElements: ExportEnvelopeElement[];
  appendixElements: ExportEnvelopeElement[];
  groupedElements: ExportEnvelopeGroup[];
  criticalElements: ExportEnvelopeElement[];
  warnings: string[];
  kobActual: number | null;
  kobNorm: number | null;
  kobStatus: string;
  kOverall: number | null;
}
...
export function buildExportEnvelopeView(input: {
  ...
  const externalWallOrientation = new Map(
    adjacency.external.map((edge) => [edge.wallId, edge.orientation])
  );
  ...
  const rawElements = input.constructions.map((construction) =>
  ...
  const totalContribution = includedElements.reduce(
    (sum, entry) => sum + (Number.isFinite(entry.contribution) ? entry.contribution ?? 0 : 0),
    0
  );
  const totalArea = includedElements.reduce(
    (sum, entry) => sum + (Number.isFinite(entry.areaM2) ? entry.areaM2 ?? 0 : 0),
    0
  );
  const kobActual =
    input.heatedVolumeM3 && input.heatedVolumeM3 > 0 && totalContribution > 0
...
function classifyConstruction(input: {
  ...
  roofById: Map<string, Roof>;
  slabById: Map<string, FloorSlab>;
  externalWallIds: Set<string>;
  internalWallIds: Set<string>;
  externalWallOrientation: Map<string, string>;
  outdoorDesignTemperatureC: number | null;
}): ExportEnvelopeElement {
  ...
  const isExternalDoorMissingNorm =
    categoryResult.category === "external-door" &&
    categoryResult.classification === "included" &&
    normalizedResistance === null;

  let status =
    categoryResult.classification === "requires-review"
  ...
  return {
    ...
    category: categoryResult.category,
    internalSurfaceTemperature: formatInternalSurfaceTemperature(
      input.construction.internalSurfaceTemperatureC,
      input.outdoorDesignTemperatureC
    ),
  };
}

function resolveConstructionSource(construction: Sp50ConstructionCheck): {
  kind: "wall" | "window" | "door" | "roof" | "slab" | "other";
  modelId: string | null;
...
function classifyBySource(input: {
  construction: Sp50ConstructionCheck;
  source: ReturnType<typeof resolveConstructionSource>;
  externalWallIds: Set<string>;
  internalWallIds: Set<string>;
  slabById: Map<string, FloorSlab>;
}): {
...
function classifyOpening(
  ...
  externalWallIds: Set<string>,
  internalWallIds: Set<string>
): { category: ExportEnvelopeCategory; classification: ExportEnvelopeClassification; note: string } {
...
function assignDesignations(elements: ExportEnvelopeElement[]): ExportEnvelopeElement[] {
...
function groupEnvelopeElements(elements: ExportEnvelopeElement[]): ExportEnvelopeGroup[] {
  const groups = new Map<string, ExportEnvelopeGroup>();
  for (const element of elements) {
    ...
    entry.count += 1;
    entry.areaM2 += area;
    entry.weightedRequiredResistance = mergeWeighted(
      entry.weightedRequiredResistance,
      previousArea,
      element.normalizedResistance,
      area
    );
    entry.weightedActualResistance = mergeWeighted(
      entry.weightedActualResistance,
      previousArea,
      element.actualResistance,
      area
    );
    entry.weightedUValue = mergeWeighted(
      entry.weightedUValue,
      previousArea,
      element.uValue,
      area
    );
  ...
  return Array.from(groups.values()).sort(
    (left, right) => categoryOrder(left.key) - categoryOrder(right.key)
  );
}

function collectWarnings(elements: ExportEnvelopeElement[]): string[] {
  const seen = new Set<string>();
  const result: string[] = [];
  elements.forEach((element) => {
    if (
      element.classification !== "included" &&
    ...
    if (
      element.internalSurfaceTemperature === "требует проверки расчётной модели" &&
...
function buildProjectElementName(input: {
  ...
  const facade =
    input.orientation && /^[NESWА-Я]$/i.test(input.orientation)
...
function resolveLevelLabel(model: BuildingModel, levelId: string | null): string {
  if (!levelId) {
...
function enrichSourceWithModelData(
  source: ReturnType<typeof resolveConstructionSource>,
  maps: {
    wallById: Map<string, Wall>;
    doorById: Map<string, Door>;
...
): ReturnType<typeof resolveConstructionSource> {
...
function designationPrefix(category: ExportEnvelopeCategory): string {
  switch (category) {
    case "external-wall":
      return "НС";
    case "window":
...
function categoryTypeLabel(category: ExportEnvelopeCategory): string {
  switch (category) {
    case "external-wall":
...
function mapConstructionStatus(
...
function formatInternalSurfaceTemperature(
  value: number | null,
  outdoorDesignTemperatureC: number | null
): string {
  ...
  if (
    Number.isFinite(outdoorDesignTemperatureC) &&
