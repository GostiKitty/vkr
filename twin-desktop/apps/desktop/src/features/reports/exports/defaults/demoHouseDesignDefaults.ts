...
import type { BuildReportBaseDataInput } from "../data/buildReportBaseData";

/**
 * Источник конкретного значения в выгрузке.
 *
 *  - "model"        — поле взято из BuildingModel;
 *  - "scenario"     — поле задано в ScenarioConfig (шаг «Сценарий»);
 *  - "calculated"   — поле получено расчётом (RC/СП50/энергетика);
 *  - "user-meta"    — поле задано пользователем в форме метаданных отчёта;
 *  - "demo-default" — поле подставлено профилем демо-допущений;
 *  - "not-provided" — поле не задано и не подставлено.
 */
...
/**
 * Описание одного применённого проектного допущения.
 * Эти записи в HTML выводятся в блоке «Принятые проектные допущения»
 * и помечаются сноской `*` в основной таблице.
 */
export interface AssumptionEntry {
  key: string;
  label: string;
  /** Отформатированное значение для отображения (например, «50 %», «нормальный»). */
  value: string;
  source: AssumptionSource;
}

/**
 * Профиль проектных допущений для демонстрационного дома.
 *
 * Все поля строго типизированы, чтобы случайно не подставить лишнее.
 */
export interface DemoHouseDesignDefaults {
  ...
  mechanicalVentilationAch: number;
  heatRecoveryEfficiencyPercent: number;
  calculationScenario: string;
  designOccupancyMode: string;
  ventilationNote: string;
  solarGainStatus: string;
  actualValuesStatus: string;
  /** Метаданные документа для красивого штампа/титула — заполняются только если пусто. */
  projectCode: string;
  objectName: string;
  organization: string;
  customerOrg: string;
 ...
 */
export const DEMO_HOUSE_DESIGN_DEFAULTS: DemoHouseDesignDefaults = {
  ...
  ventilationNote:
    "Параметры воздухообмена приняты по демонстрационному проектному сценарию и требуют уточнения на стадии проектирования.",
 ...
 */
export const DEMO_DEFAULT_FOOTNOTE =
  "* Значение принято по проектному допущению для демонстрационного расчёта и требует уточнения.";
 ...
 */
function isMeaningfulNumber(value: number | null | undefined): boolean {
 ...
 */
export function applyDemoDesignDefaults(
  input: BuildReportBaseDataInput,
  defaults: DemoHouseDesignDefaults = DEMO_HOUSE_DESIGN_DEFAULTS
): { input: BuildReportBaseDataInput; appliedAssumptions: AssumptionEntry[] } {
  const appliedAssumptions: AssumptionEntry[] = [];
...
function applyDefaultsToModel(
  model: BuildingModel,
  defaults: DemoHouseDesignDefaults,
  applied: AssumptionEntry[]
): BuildingModel {
  ...
  const metaName = typeof nextMeta["name"] === "string" ? (nextMeta["name"] as string) : null;
  const metaSourceName =
    typeof nextMeta["sourceProjectName"] === "string"
      ? (nextMeta["sourceProjectName"] as string)
      : null;
  ...
  const signatoryDefaults: Array<{ key: string; value: string }> = [
  ...
  const appliedSignatories: string[] = [];
  for (const item of signatoryDefaults) {
    if (!isMeaningfulText(nextMeta[item.key] as string | null | undefined)) {
...
function applyDefaultsToScenario(
...
function applyDefaultsToReportMeta(
...
function mapHumidityZone(label: string): "normal" | "wet" | "dry" {
  const v = label.trim().toLowerCase();
  if (v.startsWith("сух")) return "dry";
  if (v.startsWith("влаж")) return "wet";
  return "normal";
}

function mapMoistureMode(label: string): "normal" | "wet" | "dry" {
  const v = label.trim().toLowerCase();
  if (v.startsWith("сух")) return "dry";
  if (v.startsWith("влаж")) return "wet";
  return "normal";
...
function mapOperationCondition(label: string): "A" | "B" {
  const v = label.trim().toUpperCase();
  if (v.startsWith("А") || v.startsWith("A")) return "A";
  return "B";
}
 ...
 */
export const SOLAR_GAIN_METHODOLOGY_ASSUMPTION: AssumptionEntry = {
  key: "energy.solarGain.methodology",
  label: "Солнечные теплопоступления",
  value:
    "методика требует уточнения; значение не использовано как расчётное.",
  source: "demo-default",
};
