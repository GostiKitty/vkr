...
import type { BuildingModel, Level, Room, Sp50ClimateData } from "../../entities/geometry/types";
...
import { THERMAL_UNCERTAINTY_DEFINITIONS, type DistributionSummary, type ThermalMonteCarloResult } from "../../core/uncertainty/thermalMonteCarlo";
...
const DOCUMENT_BASIS =
  "Данные цифровой модели здания, результаты теплотехнического расчета и доступные расчетно-диагностические сводки приложения.";
const REGULATORY_REFERENCE_TEXT =
  "Структура документа ориентирована на логику состава проектной документации, установленную Постановлением Правительства РФ № 87, и используется только как справочная привязка для расчетного паспорта.";

export interface PassportMetricRow {
  label: string;
  value: number | string | null;
  unit?: string;
}

export interface PassportRoomRow {
  id: string;
  roomName: string;
  levelName: string;
  areaM2: number | null;
...
export interface PassportMaterialRow {
  id: string;
  material: string;
...
export interface PassportConstructionRow {
  id: string;
  construction: string;
  material: string;
...
export interface CalculationPassportData {
  ...
  rooms: PassportRoomRow[];
  materials: PassportMaterialRow[];
  constructions: PassportConstructionRow[];
  climate: {
    city: string | null;
  ...
  operation: {
    daySetpointC: number | null;
    nightSetpointC: number | null;
    infiltrationACH: number | null;
    ventilationACH: number | null;
    heatRecoveryFactor: number | null;
    internalGainDay_W_m2: number | null;
    internalGainNight_W_m2: number | null;
    occupancyDayFraction: number | null;
    occupancyNightFraction: number | null;
    mechanicalVentilationEnabled: boolean | null;
  };
  adjacency: {
  ...
  thermalResults: {
    ...
    minRoomTemperatureC: number | null;
    maxRoomTemperatureC: number | null;
    referenceOutdoorTemperatureC: number | null;
    referenceTimeHours: number | null;
    totalOpaqueLossW: number | null;
    totalWindowLossW: number | null;
    totalDoorLossW: number | null;
    totalInfiltrationLossW: number | null;
    totalVentilationLossW: number | null;
    ...
    balanceStatusNote: string | null;
    modelWarnings: string[];
  };
  monteCarlo: {
    available: boolean;
    runs: number | null;
    meanPeakLoadKW: number | null;
    minPeakLoadKW: number | null;
    maxPeakLoadKW: number | null;
    p5PeakLoadKW: number | null;
    p95PeakLoadKW: number | null;
    exceedanceProbabilityPct: number | null;
  ...
  summaryMetricsTable: PassportMetricRow[];
  resultMetricsTable: PassportMetricRow[];
  monteCarloMetricsTable: PassportMetricRow[];
  warnings: string[];
  recommendations: string[];
  problemZones: string[];
  insightLines: string[];
  finalConclusion: string;
  regulatoryReferenceText: string;
  limitations: string[];
}
...
export function buildCalculationPassportData(input: {
...
}): CalculationPassportData {
  const { model, projectId, scenarioConfig, thermalResult, monteCarloResult } = input;
  const levelById = new Map<string, Level>(model.levels.map((level) => [level.id, level]));
  ...
  const climate = {
    city: stringOrNull(
      firstDisplayText([sp50Report?.sourceData.city, climateInput?.city], "", { allowInternalId: false })
    ),
    ...
    outdoorDesignTemperatureC: finiteOrNull(
      sp50Report?.sourceData.outdoorDesignTemperatureC ?? climateInput?.outdoorDesignTemperatureC
    ),
    outdoorHeatingAverageC: finiteOrNull(
      sp50Report?.sourceData.outdoorHeatingPeriodAverageC ?? climateInput?.outdoorHeatingPeriodAverageC
    ),
    heatingDurationDays: finiteOrNull(
      sp50Report?.sourceData.heatingPeriodDurationDays ?? climateInput?.heatingPeriodDurationDays
    ),
  ...
  const operation = {
    ...
    mechanicalVentilationEnabled:
      typeof scenarioConfig?.ventilation.mechanicalVentilationEnabled === "boolean"
        ? scenarioConfig.ventilation.mechanicalVentilationEnabled
        : null,
  };
  const thermalDiagnostics = thermalResult?.diagnostics;
  const referenceTemperatures: number[] = (thermalDiagnostics?.zones ?? [])
  ...
  const problemZones = uniqueNonEmpty(
    thermalDiagnostics?.zones
  ...
  const thermalResults = {
    ...
    averageRoomTemperatureC:
      referenceTemperatures.length > 0
  ...
  const monteCarlo = {
    ...
    exceedanceProbabilityPct:
      monteCarloResult && Number.isFinite(monteCarloResult.exceedanceProbability)
        ? (monteCarloResult.exceedanceProbability ?? 0) * 100
        : null,
    climateFactors: THERMAL_UNCERTAINTY_DEFINITIONS.filter((item) => item.id === "outdoorBiasC").map(
      (item) => item.label
    ),
    operationalFactors: THERMAL_UNCERTAINTY_DEFINITIONS.filter((item) => item.id !== "outdoorBiasC").map(
      (item) => item.label
    ),
  ...
  const summaryMetricsTable: PassportMetricRow[] = [
  ...
  const resultMetricsTable: PassportMetricRow[] = [
  ...
  const monteCarloMetricsTable: PassportMetricRow[] = [
...
function buildRoomRows(rooms: Room[], levelById: Map<string, Level>): PassportRoomRow[] {
...
function resolveTotalArea(model: BuildingModel, roomRows: PassportRoomRow[]): number | null {
  return finiteOrNull(
...
function resolveTotalVolume(model: BuildingModel, roomRows: PassportRoomRow[]): number | null {
  return finiteOrNull(
    model.thermalProtection?.heatedVolumeM3 ??
      roomRows.reduce((sum, room) => sum + (room.volumeM3 ?? 0), 0)
  );
}

function buildClimateInput(
  ...
  return {
    ...
    outdoorHeatingPeriodAverageC:
      finiteOrNull(climateMeta?.outdoorHeatingPeriodAverageC ?? cityClimate?.outdoorHeatingPeriodAverageC) ?? undefined,
    heatingPeriodDurationDays:
      finiteOrNull(climateMeta?.heatingPeriodDurationDays ?? cityClimate?.heatingPeriodDurationDays) ?? undefined,
    outdoorDesignTemperatureC:
      finiteOrNull(climateMeta?.outdoorDesignTemperatureC ?? cityClimate?.outdoorDesignTemperatureC) ?? undefined,
...
function buildSp50Report(
  model: BuildingModel,
  climateInput: Partial<Sp50ClimateData> | undefined,
  scenarioConfig: ScenarioConfig | null
  ...
  try {
    return runSP50Compliance(model, climateInput, {
      ...
      defaultOutdoorTemperatureC:
        climateInput?.outdoorDesignTemperatureC ?? scenarioConfig?.climate.baseC ?? -20,
    });
  } catch {
    return null;
  }
...
function buildConstructionRows(sp50Report: Sp50ComplianceReport | null): PassportConstructionRow[] {
  if (!sp50Report) {
    return [];
  }
...
function buildMaterialRows(sp50Report: Sp50ComplianceReport | null, model: BuildingModel): PassportMaterialRow[] {
  const map = new Map<string, PassportMaterialRow>();
...
function collectFallbackLayers(model: BuildingModel): Array<{ materialId: string; thickness_m: number }> {
  return [
...
function resolveProjectName(model: BuildingModel, projectId: string | null): string {
  const meta = model.meta ?? {};
  const modelName = firstDisplayText(
    [meta["name"], meta["projectName"], meta["title"], meta["project"], meta["label"]],
    "",
...
function resolveModelVersion(model: BuildingModel): string | null {
  const meta = model.meta ?? {};
  const version = firstDisplayText(
    [meta["modelVersion"], meta["version"], meta["revision"], meta["schemaVersion"]],
    "",
...
function estimateEnvelopeCount(model: BuildingModel): number {
  return (
    model.walls.length +
...
function maxThermalLoss(thermalResults: CalculationPassportData["thermalResults"]): number | null {
  const values = [
...
function distributionMin(summary: DistributionSummary | null | undefined): number | null {
  const value = summary?.cdf[0]?.value;
  return finiteOrNull(value);
}

function distributionMax(summary: DistributionSummary | null | undefined): number | null {
  const cdf = summary?.cdf ?? [];
  const value = cdf.length ? cdf[cdf.length - 1]?.value : null;
  return finiteOrNull(value);
}

function inferDuration(result: ThermalSimulationResult): "24h" | "7d" {
