...
export function snapToGrid(vec: Vec2, gridStep: number): Vec2 {
  if (gridStep <= 0) {
    return { ...vec };
  }
  return {
...
export function polygonContainsPoint(point: Vec2, polygon: Vec2[]): boolean {
  let inside = false;
...
export function simplifyPolygon(points: Vec2[], tolerance = 1e-3): Vec2[] {
  if (points.length <= 3) {
    return points.slice();
  }
...
export function validateRoomPolygon(points: Vec2[]): RoomValidationResult {
  if (points.length < 3) {
...
function isClockwiseConsistent(points: Vec2[]): boolean {
  return polygonArea(points) < 0;
}

export function isPolygonSelfIntersecting(points: Vec2[]): boolean {
  if (points.length < 4) {
    return false;
  }
  for (let i = 0; i < points.length; i++) {
...
export function segmentsIntersect(a1: Vec2, a2: Vec2, b1: Vec2, b2: Vec2, tolerance = EPS): boolean {
  const det = (p: Vec2, q: Vec2, r: Vec2) => (q.x - p.x) * (r.y - p.y) - (q.y - p.y) * (r.x - p.x);
  const onSegment = (p: Vec2, q: Vec2, r: Vec2) =>
    Math.min(p.x, r.x) - tolerance <= q.x &&
  ...
  if (Math.abs(d1) <= tolerance && onSegment(a1, b1, a2)) return true;
  if (Math.abs(d2) <= tolerance && onSegment(a1, b2, a2)) return true;
  if (Math.abs(d3) <= tolerance && onSegment(b1, a1, b2)) return true;
  if (Math.abs(d4) <= tolerance && onSegment(b1, a2, b2)) return true;
  return false;
}
...
export function orthogonalSnap(target: Vec2, origin: Vec2): Vec2 {
