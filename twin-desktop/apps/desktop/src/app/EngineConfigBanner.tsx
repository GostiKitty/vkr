  ...
  return (
    <div className="ui-engine-banner border-b">
      <div className="mx-auto flex max-w-[min(100%,96rem)] flex-col gap-3 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between sm:px-6 xl:px-8">
        <div>
          <p className="text-base font-semibold">{isUnconfigured ? "Сервер не настроен" : "Сервер недоступен"}</p>
        ...
        <button type="button" onClick={() => navigate("/settings")} className="ui-btn-secondary shrink-0 px-4 py-2 text-sm">
          Открыть настройки
        </button>
      </div>
    </div>
