import { createEmptyBuildingModel, type BuildingModel, type Sp50EnvelopeFragmentInput, type Wall, type WallLayer } from "../entities/geometry/types";
import type { Equipment, PipeNetwork, SensorDevice } from "../entities/networks/types";
...
export function buildVideoDemoThermalResult(model: BuildingModel = videoDemoHouse): ThermalSimulationResult {
  ...
  const peakLoadW = timeline.reduce(
    (peak, frame) =>
      Math.max(
        peak,
        Object.values(frame.rooms).reduce((sum, payload) => sum + payload.heatingPowerW, 0)
      ),
    0
  );
    ...
    (
      timeline
        ...
        .reduce(
          (sum, frame) =>
            sum +
            Object.values(frame.rooms).reduce((roomsSum, payload) => roomsSum + payload.heatingPowerW, 0),
          0
        ) /
      1000
    ).toFixed(2)
