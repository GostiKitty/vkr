...
import type { ThermalSimulationResult, ThermalTimelinePoint } from "../core/thermal/solver";
...
const VIDEO_DEMO_ROOM_SETPOINTS: Record<string, number> = {
...
const VIDEO_DEMO_ROOM_HEATING_W: Record<string, number> = {
...
const WINDOW_LAYERS: WallLayer[] = [{ materialId: "window_block", thickness_m: 0.04 }];
...
function createWall(
  ...
  a: { x: number; y: number },
  b: { x: number; y: number },
  options: { thickness_m: number; layers: WallLayer[] }
): Wall {
  return {
    id,
    levelId,
...
function buildThermalProtection(): BuildingModel["thermalProtection"] {
...
export function buildVideoDemoThermalResult(model: BuildingModel = videoDemoHouse): ThermalSimulationResult {
  ...
  const timeline: ThermalTimelinePoint[] = HOURS.map((timeHours) => {
    ...
    const rooms = Object.fromEntries(
      roomEntries.map(({ room, baseTemperature, baseHeating, setpoint, phaseOffset }) => {
  ...
  const roomResults = Object.fromEntries(
  ...
  const peakLoadW = timeline.reduce(
    (peak, frame) => Math.max(peak, Object.values(frame.rooms).reduce((sum, payload) => sum + payload.heatingPowerW, 0)),
    0
  );
  const totalEnergyKWh = Number(
    (
      timeline
        .slice(0, -1)
        .reduce(
          (sum, frame) => sum + Object.values(frame.rooms).reduce((roomsSum, payload) => roomsSum + payload.heatingPowerW, 0),
          0
        ) / 1000
    ).toFixed(2)
  );
