import {
  ...
  type BuildingModel,
  type Sp50EnvelopeFragmentInput,
  type Wall,
  type WallLayer,
...
import {
  ...
  TYPICAL_INSULATED_DOOR_U_W_M2K,
  TYPICAL_PVC_WINDOW_G_VALUE,
  TYPICAL_PVC_WINDOW_U_W_M2K,
...
const FOOTPRINT = { width: 14, depth: 10 } as const;
...
export const VIDEO_DEMO_ROOM_TEMPERATURES: Record<string, number> = {
...
const WINDOW_LAYERS: WallLayer[] = [{ materialId: "pvc_double_glazed_unit_equivalent", thickness_m: 0.07 }];
const DEMO_WINDOW_PRESET_ID = resolveDefaultPresetId("window");
const DEMO_DOOR_PRESET_ID = resolveDefaultPresetId("door");
const DOOR_LAYERS: WallLayer[] = [
...
function createWall(
  ...
  a: { x: number; y: number },
  b: { x: number; y: number },
  options: { thickness_m: number; layers: WallLayer[]; wallAssemblyId?: string }
): Wall {
  return {
    id,
    levelId,
...
function buildWallMap(walls: Wall[]) {
  return new Map(walls.map((wall) => [wall.id, wall]));
}

function buildWallAnchor(wall: Wall, offset_m: number) {
...
function buildLevelWalls(levelId: string, prefix: string): Wall[] {
  const ext = {
...
function buildLevelRooms(levelId: string): BuildingModel["rooms"] {
...
function roomAreaM2(room: BuildingModel["rooms"][number]) {
  return Math.abs(polygonArea(room.polygon));
}

function wallAreaM2(wall: Wall) {
  return Math.max(0, segmentLength(wall.a, wall.b) * Math.max(0, wall.height_m));
}

function slabAreaM2(slab: NonNullable<BuildingModel["floorSlabs"]>[number]) {
  return Math.abs(polygonArea(slab.boundary));
}

function roofAreaM2(roof: NonNullable<BuildingModel["roofs"]>[number]) {
...
function openingAreaM2(opening: { width_m: number; height_m: number }) {
  return Math.max(0, opening.width_m) * Math.max(0, opening.height_m);
}

function buildRoomOverrides(): NonNullable<ScenarioConfig["geometry"]>["roomOverrides"] {
  return {
...
export function buildVideoDemoScenarioConfig(): ScenarioConfig {
  return {
    climate: {
      baseC: -10,
      amplitudeC: 4,
...
function buildVideoDemoThermalOptions(
  scenarioConfig: ScenarioConfig = buildVideoDemoScenarioConfig()
...
function buildOpenings(levelId: string, prefix: string, walls: Wall[]) {
...
function buildPipeDefaults(
  id: string,
  levelId: string,
  path: Array<{ x: number; y: number }>,
  type: PipeNetwork["type"],
...
function buildHeatingNetworks(): PipeNetwork[] {
  ...
  return [
    ...
    buildPipeDefaults(
      ...
      "heating_supply",
      70,
      ["video-pump", "video-heat-exchanger"]
    ),
    buildPipeDefaults(
      "video-pipe-riser-return",
      LEVEL_2_ID,
      [
        { x: 12.6, y: 6.8 },
        { x: 12.6, y: 6.8 },
      ...
      "heating_return",
      48,
      ["video-expansion-tank", "video-pump"]
    ),
  ];
}

function buildVentilationNetworks(): DuctNetwork[] {
  const mainDuctPath = [
    { x: 11.4, y: 8.0 },
    { x: 11.4, y: 5.2 },
    { x: 7.0, y: 5.2 },
...
function buildVideoDemoModelInternal(): BuildingModel {
  ...
  const preparedModel: BuildingModel = {
    ...model,
    levels: [
...
export function buildVideoDemoHouseModel(): BuildingModel {
  return cloneDeep(buildVideoDemoModelInternal());
}

export const videoDemoHouse: BuildingModel = buildVideoDemoHouseModel();

export function buildVideoDemoThermalResult(
  model: BuildingModel = videoDemoHouse,
  scenarioConfig: ScenarioConfig = buildVideoDemoScenarioConfig()
