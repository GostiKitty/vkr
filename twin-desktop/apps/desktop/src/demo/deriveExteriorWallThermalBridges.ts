...
import {
  resolveLayerLambda_W_mK,
  resolveLayerResistance_m2K_W,
  resolveThermalBridgeInputsFromLayers,
} from "../core/thermal/envelope/layerThermalBridgeInputs";
import {
  thermalBridgeLinearConductance,
  thermalBridgePointConductance,
} from "../core/thermal/formulas";
import {
  SP230_DISC_ANCHOR_CHI_W_K,
  SP230_DISC_ANCHOR_DENSITY_PER_M2,
  SP230_THERMAL_BRIDGE_SOURCE,
  psiFloorOnGroundEdge_W_mK,
  psiPitchedRoofRafter_W_mK,
  psiPitchedRoofRidge_W_mK,
  psiSftkConvexCorner_W_mK,
  psiSftkWallRoofJunction_W_mK,
  psiSftkWindowJamb_W_mK,
} from "../norms/sp230_2015/thermalBridgeJunctions";

export const VIDEO_DEMO_FOOTPRINT = { width: 14, depth: 10 } as const;

const EXTERIOR_WALL_ASSEMBLY_ID = "video-exterior-wall";

function openingPerimeterM(opening: { width_m: number; height_m: number }): number {
  return 2 * (Math.max(0, opening.width_m) + Math.max(0, opening.height_m));
}

function isHostedOnExteriorWall(model: BuildingModel, wallId: string | undefined): boolean {
  if (!wallId) {
    return false;
  }
...
function layerResistance_m2K_W(
  layers: WallLayer[],
  materialId: string,
...
function insulationResistance_m2K_W(
  layers: WallLayer[],
  operationCondition: Sp50OperationCondition
): number | null {
...
export function aggregateEnvelopeBridgeConductances(model: BuildingModel): {
  H_psi: number | null;
  H_chi: number | null;
  hasLinear: boolean;
  hasPoint: boolean;
...
export function envelopeFragmentHasLinearOrPointBridges(fragment: Sp50EnvelopeFragmentInput): boolean {
  const heterogeneity = fragment.heterogeneity;
  if (!heterogeneity) {
    return false;
  }
  return (
    (heterogeneity.linear?.some((entry) => entry.lengthM > 0 && entry.psi_W_mK !== 0) ?? false) ||
    (heterogeneity.point?.some((entry) => entry.count > 0 && entry.chi_W_K !== 0) ?? false)
  );
}

export function isUserSuppliedBridgeHeterogeneity(fragment: Sp50EnvelopeFragmentInput): boolean {
  const heterogeneity = fragment.heterogeneity;
  if (!heterogeneity) {
    return false;
  }
  ...
  return (
    (heterogeneity.linear?.length === 1 && heterogeneity.linear[0]?.label === userLabel) ||
    (heterogeneity.point?.length === 1 && heterogeneity.point[0]?.label === userLabel)
  );
}

export function envelopeFragmentHasThermalBridgeData(fragment: Sp50EnvelopeFragmentInput): boolean {
  const heterogeneity = fragment.heterogeneity;
  if (!heterogeneity) {
    return false;
  }
...
export function shouldSkipEnvelopeBridgeEnrichment(fragment: Sp50EnvelopeFragmentInput): boolean {
 ...
 */
export function deriveVideoDemoExteriorWallThermalBridges(
  model: BuildingModel,
  input: {
    layers: WallLayer[];
    operationCondition: Sp50OperationCondition;
...
): NonNullable<Sp50EnvelopeFragmentInput["heterogeneity"]> {
  ...
  const anchorCount = Math.max(
    0,
    Math.round(SP230_DISC_ANCHOR_DENSITY_PER_M2 * Math.max(0, input.grossWallAreaM2))
  );

  const linear: NonNullable<Sp50EnvelopeFragmentInput["heterogeneity"]>["linear"] = [];
  ...
  const point: NonNullable<Sp50EnvelopeFragmentInput["heterogeneity"]>["point"] =
    anchorCount > 0
      ? [
          {
            count: anchorCount,
...
function footprintPerimeterM(widthM: number, depthM: number): number {
  return 2 * (Math.max(0, widthM) + Math.max(0, depthM));
}

function pitchedRafterLengthM(depthM: number, risePerMeter: number): number {
 ...
 */
export function enrichVideoDemoEnvelopeThermalBridges(model: BuildingModel): BuildingModel {
  ...
  const exteriorWallAreaM2 =
    envelope.find((fragment) => fragment.id === "video-ext-walls")?.areaM2 ??
    model.walls
      .filter((wall) => wall.wallAssemblyId === "video-exterior-wall")
      .reduce(
        (sum, wall) => sum + Math.max(0, Math.hypot(wall.b.x - wall.a.x, wall.b.y - wall.a.y) * wall.height_m),
        0
      );
  const risePerMeter = model.roofs?.[0]?.slope?.risePerMeter ?? 0.16;
  const wallLayers =
    envelope.find((entry) => entry.id === "video-ext-walls")?.layers ??
